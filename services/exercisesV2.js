...
module.exports = class ExercisesServiceV2 extends Schmervice.Service {
      ...
      if (course_id !== null && course_id !== undefined) {
        ...
        if (team_id) {
          const [
            ...
            coursePer,
            // eslint-disable-next-line
          ] = await progressTrackingService.getProgressTrackPercentagec4ca(team_id, course_id);
          ...
          const [
            ...
            coursePer,
            // eslint-disable-next-line
          ] = await progressTrackingService.getProgressTrackPercentage(userId, course_id);
          coursePercentage = coursePer;
        }
        let existsCourse;
        let completedCourse;
        ...
        const modules = moduleCourseDetails?.data?.attributes?.modules;

        if (modules !== null && modules !== undefined && modules.data.length > 0) {
          const module_id = moduleCourseDetails?.data?.attributes?.modules?.data[0]?.id;
          if (module_id !== null && module_id !== undefined) {
            let sum = 0;
            let count = 0;

            const moduleDetails = await strapi.findOne('modules', module_id, {
              populate: ['courses'],
            });
            const moduleCoursePercentDetails = moduleDetails?.data?.attributes?.courses?.data;
            ...
            if (moduleCoursePercentDetails !== null && moduleCoursePercentDetails !== undefined) {
              ...
              if (existsModule !== null && existsModule !== undefined && existsModule.length > 0) {
                if (team_id) {
                  await ModuleCompletionV2.query()
              ...
              if (pathway_id !== null && pathway_id !== undefined) {
                const pathwayDetails = await strapi.findOne('pathways', pathway_id, {
                  populate: ['modules'],
                });

                const pathwayModule = pathwayDetails?.data?.attributes?.modules.data;

                if (
                  pathwayModule !== null &&
                  pathwayModule !== undefined &&
                  pathwayModule.length > 0
                ) {
                  ...
                  if (
                    existspathway !== null &&
                    existspathway !== undefined &&
                    existspathway.length > 0
                  ) {
                    if (team_id) {
                      await PathwayCompletionV2.query()
                        .update(completedPathway)
                        .where('team_id', team_id)
          ...
          const pathway_id = courseDetails?.data?.attributes?.pathway?.data.id;
  ...
  async insertLearningTrackData() {
    ...
    try {
      const lrningTrkData = await LearningTrackStatusOutcome.query().select(
        'user_id',
        'exercise_id'
      );
