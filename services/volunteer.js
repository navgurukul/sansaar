...
const { keys } = require('../routes/volunteers');
...
module.exports = class UserRoleService extends Schmervice.Service {
    ...
    try {
      ...
      let res = filtered_res.map((v) => {
        var pathway = {
          1: 'Python',
          2: 'Spoken English',
          7: 'Amazon Coding Bootcamp'
        };
        const pathways = v.pathway_id.filter((id) => pathway.hasOwnProperty(id)).map((id) => pathway[id]);
  ...
  async CheckNumberRegisteredOrNot(userId, contact) {
    try {
      ...
      if (contact) {
        let checkPhoneNo = await User.query().where({ contact: contact });
        if (checkPhoneNo.length > 0 && checkPhoneNo !== null && checkPhoneNo !== undefined) {
          checkUserContactExits = await User.query().where({ contact: contact }).andWhere({ id: userId })
          if (checkUserContactExits.length > 0 && checkUserContactExits !== null && checkUserContactExits !== undefined) {
