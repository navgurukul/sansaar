...
module.exports = class CoursesService extends Schmervice.Service {
  ...
  async getOLdCourseByNameWithJsonType(courseName) {
    const { Courses } = this.server.models();
    let courses;
    try {
      courses = await Courses.query().where('name', courseName).where('course_type', 'json');
      return [null, courses];
    } catch (err) {
      return [errorHandler(err), null];
    }
  ...
  async getCourseByIdWithJsonType(courseId) {
    const { Courses } = this.server.models();
    let courses;
    try {
      courses = await Courses.query().findById(courseId).where('course_type', 'json');
      return [null, courses];
    } catch (err) {
      return [errorHandler(err), null];
    }
