  ...
  async parseExerciseContent(filePath) {
    ...
    if (parentExer) {
      _.map(parsedContent, (tokens) => {
        if (tokenTypes.indexOf(tokens.type) < 0) {
          tokenTypes.push(tokens.type);
        }
        console.log(tokens);
        // console.log(tokenTypes);
        if (tokens.type === 'space') {
          modifiedContent = modifiedContent + tokens.raw.substr(1); // Always omit first \n ;
        }
        // hr
        if (tokens.type === 'hr') {
          modifiedContent = modifiedContent + tokens.raw; // Always omit first \n ;
        }
        // table has innerTokens but has more than one tokens due to which it is parsed more than once
        if (tokens.type === 'table') {
          let tableHeaderContent = '|';
          let tableDivider = '|';

          _.map(tokens.header, (tableHeader) => {
            keyNumber += 1;
