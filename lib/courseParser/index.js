...
class CoursesSeeder {
  ...
  static getCourseFolderName(flagSingle, course) {
    // getting folder name
    if (flagSingle) {
      try {
        fs.statSync(path.resolve(`curriculum/${course}`)); // stat
        return course;
      } catch (e) {
        this.showErrorAndExit(
          `The specified courseFolderName ${course} doesn't exist in curriculum.`
        );
        return { error: true, message: "Course Doesn't Exist" };
      }
    }
    // eslint-disable-next-line
    console.log('Specify correct flag');
    return { error: true, message: 'Specify Correct Flag' };
  }
}
const runParser = async ({ flagSingle, ifUpdateDB, course }) => {
  // if (!module.parent) {
  process.umask(0);
  const courseFolderName = CoursesSeeder.getCourseFolderName(flagSingle, course);
  if (courseFolderName.error) {
    return courseFolderName;
  }
  const seeder = new CoursesSeeder(flagSingle, courseFolderName);
  const res = await seeder.init(ifUpdateDB);
  if (res) {
    return { success: true, message: `Successfully seeded ${courseFolderName}` };
  }
