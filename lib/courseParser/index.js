    ...
    if (fs.existsSync(path)) {
      ...
      _.map(items, (item) => {
        ...
        if (split[1]) {
          ...
          if (logo.indexOf('```ngMeta') < 0 || split[0].indexOf('```') < 0)
  ...
  async parseIntoJSON(filePath, tokens) {
    ...
    _.map(tokens, (token) => {
      ...
      if (token.type === 'paragraph' && token.text.match(/\[.+\]\(.+\)/i)) {
        ...
        token.type === 'code' &&
        ['python', 'html', 'js', 'css', 'javascript', 'json', 'bash', 'solution', 'sh', ''].indexOf(
          token.lang
        ) > -1
      ) {
        token.text = token.text.replace(/`/g, '');
        ...
        // eslint-disable-next-line
        token.text = token.text.replace(/    /g, '&emsp;');
        // eslint-disable-next-line
        if (token.lang === 'python' || token.lang === 'javascript') {
          courseLang = token.lang;
          ...
          DataJson.push(jsonContent);
          jsonContent = {};
        } else if (token.lang === 'solution') {
          if (courseLang === 'python' || courseLang === 'javascript') {
            jsonContent.component = 'code';
            jsonContent.type = courseLang;
            jsonContent.title = token.lang;
            jsonContent.value = `${token.text.trim()}`;
            DataJson.push(jsonContent);
            jsonContent = {};
          } else {
        ...
        token.type === 'code' &&
        (token.lang === 'trytyping' || token.lang === 'practicetyping')
      ) {
        token.text = token.text.replace(/\n/g, '');
        token.text = token.text.replace(/,/g, '');
        token.text = token.text.replace(/ /g, '');
        ...
        actionData.label = 'Practice Typing';
        actionData.data = {};
        const typing_content = token.text.split('');
        actionData.data.content = [...new Set(typing_content)];
        ...
        DataJson.push(jsonContent);
        jsonContent = {};
      } else if (token.type === 'code' && token.lang === 'wordstyping') {
        token.text = token.text.replace(/,/g, '');
        token.text = token.text.replace(/ /g, '');
        jsonContent.component = 'banner';
        jsonContent.title = bannerTitle;
        ...
        const typing_content = token.text.split('\n');
        actionData.data.content = [...new Set(typing_content)];
        actionData.data.code = 'wordstyping';
        actionData.variant = 'primary';
        actionData.data = JSON.stringify(actionData.data);
        jsonContent.actions.push(actionData);
        DataJson.push(jsonContent);
        ...
        _.map(token.items, (listItems) => {
          ...
          if (listItems.text.match(/.*(\[.+\])(\((http).+\)).*/g)) {
            ...
            _.map(listItems.tokens, (innerListItemToken) => {
              if (
                innerListItemToken.type === 'code' &&
                ['python', 'html', 'css', 'js', 'javascript', 'json', 'bash', 'sh'].indexOf(
                  innerListItemToken.lang
                ) > -1
              ) {
                ...
                innerListItemToken.text = innerListItemToken.text.replace(/`/g, '');
                // eslint-disable-next-line
                innerListItemToken.text = innerListItemToken.text.replace(/    /g, '&emsp;');
                if (
                  innerListItemToken.lang === 'python' ||
                  innerListItemToken.lang === 'javascript'
                ) {
                  courseLang = innerListItemToken.lang;
                  jsonContent.component = 'code';
                  ...
                  DataJson.push(jsonContent);
                  jsonContent = {};
                } else if (innerListItemToken.lang === 'solution') {
                  if (courseLang === 'python' || courseLang === 'javascript') {
                    jsonContent.component = 'code';
                    jsonContent.type = courseLang;
                    jsonContent.title = innerListItemToken.lang;
                    jsonContent.value = `${innerListItemToken.text.trim()}`;
                    DataJson.push(jsonContent);
                    jsonContent = {};
                  } else {
                    jsonContent.component = 'code';
                ...
                if (innerListItemToken.type !== 'space') {
                  if (innerListItemToken.items !== undefined) {
                      ...
                      _.map(innerListItemToken.items[0].tokens, (innerListItemTokenCode) => {
                        if (
                          ...
                          [
                            ...
                            'bash',
                            'sh',
                          ].indexOf(innerListItemTokenCode.lang) > -1
                        ) {
                          ...
                          if (
                            innerListItemTokenCode.lang === 'python' ||
                            innerListItemTokenCode.lang === 'javascript'
                          ) {
                            courseLang = innerListItemTokenCode.lang;
                            jsonContent.component = 'code';
                            ...
                            DataJson.push(jsonContent);
                            jsonContent = {};
                          } else if (innerListItemTokenCode.lang === 'solution') {
                            if (courseLang === 'python' || courseLang === 'javascript') {
                              jsonContent.component = 'code';
                              jsonContent.type = courseLang;
                              jsonContent.title = innerListItemTokenCode.lang;
                              jsonContent.value = `${innerListItemTokenCode.text.trim()}`;
                              DataJson.push(jsonContent);
                              jsonContent = {};
                            } else {
                              jsonContent.component = 'code';
            ...
            if (_.findIndex(listItems.tokens, { type: 'list' }) > -1) {
              ...
              const heading = listText.trim().split(' ')[0];
              if (heading.startsWith('#')) {
                listText = `<h${heading.length}>${listText
                  .replace(heading, '')
                  .replace(/<br>/g, '')}</h${heading.length}>`;
              }
              const text = listText;
              let repText = converter.makeHtml(text);
              repText = repText.replace(/<p>/g, '');
