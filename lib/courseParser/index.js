  ...
  async parseExerciseDetails(file, folder, courseId) {
    ...
    if (file.childExercises.length) {
      ...
      if (fs.existsSync(path)) {
        ...
        const embedYTLinks = dataInString.match(/@\[(youtube)\]\(\w+\)/gi);
        const splittedByYTLinks = dataInString.split(/@\[(youtube)\]\(\w+\)/gi);
        let contentByParts = [];
        let name;
        let beginKeyOn = 0;
        for (const md of splittedByYTLinks) {
          if (md !== 'youtube') {
            const tokens = marked.lexer(md);
            _.map(tokens, (exer) => {
              if (exer.lang === 'ngMeta') {
                exercise.name = exer.text
                  .replace('name: ', '')
                  .replace('\nsubmission_type: url', '');
              }
            });
            const { beginKeyFrom, ...contents } = JSON.parse(
            ...
            contentByParts.push({
              ...
              value: dataInString
                .match(/@\[(youtube)\]\(\w+\)/i)
                .slice(
                  dataInString.match(/@\[(youtube)\]\(\w+\)/i).indexOf('(') + 1,
                  dataInString.match(/@\[(youtube)\]\(\w+\)/i).indexOf(')')
                ),
            });
          }
        }
        ...
        // splittedByYTLinks.forEach(async (md, ind) => {
        //   // console.log('Iteration', ind);
        //   // console.log('Key', beginKeyOn);
        //   if (md !== 'youtube') {
        //     const tokens = marked.lexer(md);
        //     _.map(tokens, (exer) => {
        //       if (exer.lang === 'ngMeta') {
        ...
        //       }
        //     });
        //     console.log('Before parsing', ind);
        //     await this.parseMarkdownContent(path, tokens, beginKeyOn);
        //     console.log('After parsing', ind);

        //     // const { beginKeyFrom, ...contents } = JSON.parse(
        //     //   await this.parseMarkdownContent(path, tokens, beginKeyOn)
        //     // )[0];
        //     // beginKeyOn = beginKeyFrom + 1;
        //     // console.log('value set', beginKeyOn);
        //   }
        ...
        // });
        // name = file.fileName.replace('.md', '');
        // const contents = await this.parseMarkdownContent(path, tokens);
        // exercise.name = exercise.name ? exercise.name : name;
        // exercise.sequence_num = baseSequenceNum;
        // exercise.course_id = courseId;
        // exercise.content = contents;
        // exercise.slug = path
        //   .split(`${this.courseDir}/`)
        //   .slice(-1)[0]
        //   .replace('/', '__')
        ...
        /**
         * MMMMMYYYMMMYYM STRT_YTBX = 5, END_YTBX = 7
         * 1) Search DataInString for first youtube link - call the start index of youtube link as [Start_YTBX]. And end as [End_YTBX]
         * 2)  [0, Start_YTBX-1]. Call it  MDX
         * 3) Send  MDX to your high level MD parser. (marked.lexer)
         * 4) Store last prop_key_number.
         * 5) Add [Start_YTBX, End_YTBX]  to exercise
         * 6) set DataInString = [ End_YTBX, dataInStringEndIdx] . Call Step1
         */
      }
    }
  ...
  async parseMarkdownContent(filePath, tokens, beginKeyOn) {
    ...
    _.map(tokens, (token) => {
      ...
      if (
        ...
        _.find(token.tokens, { type: 'link' }) &&
        (token.raw.indexOf('[youtube]') > -1 || token.raw.indexOf('[Youtube]') > -1)
      ) {
        youtubeLinks = [];
        youtubeLinks.push({
          type: 'youtube',
          value: _.find(token.tokens, { type: 'link' }).href.trim(),
        });
      }
    ...
    if (youtubeLinks.length > 0) {
      exercise.push(youtubeLinks);
    }
    exercise.push({ type: 'markdown', value: modifiedContent, beginKeyFrom: keyNumber });
    await this.writeFiles({
      exrRelPath,
      exerciseName,
      modifiedContent,
