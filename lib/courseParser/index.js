...
class CoursesSeeder {
  ...
  async parseExerciseDetails(file, folder, courseId) {
    ...
    if (file.childExercises.length) {
      _.map(file.childExercises, (childExercisesFile) => {
        ...
        promises.push(this.parseChildExercise(path, baseSequenceNum, courseId, childExercisesFile));
      });
    } else {
      const Path = basePath.replace(/\s/g, '');
      return this.commonfunc({ Path, baseSequenceNum, courseId, file });
    }
  ...
  // eslint-disable-next-line
  async parseChildExercise(basepath, baseSequenceNum, courseId, file) {
    const Path = basepath.replace(/\s/g, '');
    return this.commonfunc({ Path, baseSequenceNum, courseId, file });
  }

  async commonfunc({ Path, baseSequenceNum, courseId, file }) {
    const exercise = {};
    if (fs.existsSync(Path)) {
      const data = fs.readFileSync(Path);
      ...
      exercise.slug = Path.split(`${this.courseDir}/`)
        .slice(-1)[0]
        .replace('/', '__')
        .replace('.md', '');
