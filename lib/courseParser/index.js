...
class CoursesSeeder {
  ...
  async parseCourseDir(folder, updateCourseNames) {
    ...
    const folderName = path.split('/').slice(6, 7).join('/');
    const fileRelPath = path.split('/').slice(7).join('/');
    ...
    if (fs.existsSync(path)) {
      ...
      if (courseDetails) {
         ...
         * ## CHANGES
         */
        const allJsonData = data.toString();
        if (this.ifCreateJSON) {
  ...
  async parseExerciseDetails(file, folder, courseId) {
    ...
    if (file.childExercises.length) {
      ...
      if (fs.existsSync(path)) {
        ...
        const boldItalic = dataForJson.match(
          /(\*\*\*)([a-z0-9A-Z]|[@&'.,:;(\\s)<>#]|\n|\r|\t|\s)+(\*\*\*)/gm
        );
        _.map(boldItalic, (value) => {
          let replacedText = value.split('***');
          replacedText = `<b><i>${replacedText[1]}</i></b>`;
          dataForJson = dataForJson.replace(value, replacedText);
        });
        const bold = dataForJson.match(
          /(\*\*)([a-z0-9A-Z]|[@&'.,:;(\\s)<>#]|\n|\r|\t|\s)+(\*\*)/gm
        );
        _.map(bold, (value) => {
          let replacedText = value.split('**');
          replacedText = `<b>${replacedText[1]}</b>`;
          dataForJson = dataForJson.replace(value, replacedText);
        });
        const Italic = dataForJson.match(/(\*)([a-z0-9A-Z]|[@&'.,:;(\\s)<>#]|\n|\r|\t|\s)+(\*)/gm);
        _.map(Italic, (value) => {
          let replacedText = value.split('*');
          replacedText = `<i>${replacedText[1]}</i>`;
          dataForJson = dataForJson.replace(value, replacedText);
        });
        const hightlight = dataForJson.match(/(`)[^`]+(`)|((```)[^`]+(```))/gm);
        _.map(hightlight, (value) => {
          if (value.startsWith('```')) {
            // console.log(value)
          } else {
            let replacedText = value.split('`');
            replacedText = `<span style="background-color: #f3f402;">${replacedText[1]}</span>`;
            dataForJson = dataForJson.replace(value, replacedText);
          }
        });
        ...
        for (const md of splittedMD) {
          if (splitBy.indexOf(md) > -1) {
            ...
            if (this.ifCreateJSON) {
              const parsedContentJson = JSON.parse(
                await this.parsedJsonContent(path, dataInStringTokens)
              );
  ...
  async parseChildExercise(path, courseId, childExercisesFile) {
    ...
    if (fs.existsSync(childExercisesPath)) {
      ...
      const Italic = dataForJson.match(
        /(\*)([a-z0-9A-Z]|[@&'.,:;(\\s)<>#]|\n|\r|\t|\s|[^\*])+(\*)/gm
      );
      _.map(Italic, (value) => {
        let replacedText = value.split('*');
  ...
  async parseAndUploadImage(imageDir) {
    ...
    for (const img of imageDir) {
      ...
      const partPath = this.cloudPath.split('/').slice(0).join('/');
      const data = partPath.split('/');
      const imgPath = data.slice(data.length - 2).join('/');
      const relPath = imgPath.split('/').slice(0, 1).join('/');
      const PathForImage = `${this.courseFolderName}/${relPath}`;
      ...
      if (fs.existsSync(img)) {
        ...
        if (this.ifCreateJSON) {
          this.createDirsJson(PathForImage);
          const subDir = PathForImage.split('/').slice(1).join('/');
          if (fs.existsSync(`${this.courseDir}/${this.courseFolderName}_json/${subDir}`)) {
            fs.copyFileSync(
              `${this.courseDir}/${this.courseFolderName}/${imgPath}`,
              `${this.courseDir}/${this.courseFolderName}_json/${imgPath}`
            );
          }
        }
      } else {
  ...
  async parseMarkdownContent(filePath, tokens, beginKeyOn) {
    ...
    _.map(tokens, (token) => {
      ...
      // tokens with inner tokens that doesn't contain an image or link
        ...
        const tokenText = token.raw.replace(regExPattern, (matched) => {
          if (!matched.startsWith('`')) {
            return `\`${matched}\``;
          }
          return matched;
        ...
        const paragraphText = token.raw.replace(regExPattern, (matched) => {
          if (!matched.startsWith('`')) {
            return `\`${matched}\``;
          }
          return matched;
        ...
        keyProp = `${exerciseName}_key${keyNumber}`;
        modifiedContent += `${keyProp}\n`;
        keyPropMapping[keyProp] = `${paragraphText}\n`;
      } else {
        // console.log(token);
      }
    });
  ...
  async parsedJsonContent(filePath, tokens) {
    // regex pattern to find all keywords and enclose them within tilde
    const regExPattern = new RegExp(
      `(\`){0,1}(${this.programmingKeywords.join('|')})([s|es]{0,1})(\`){0,1}`,
      'gi'
    ...
    _.map(tokens, (token) => {
      ...
      if (token.type === 'paragraph' && token.text.match(/\[.+\]\(.+\)/i)) {
        _.forEach(token.tokens, (innerToken) => {
          if (innerToken.type === 'link') {
            if (innerToken.text === 'youtube') {
              jsonContent.component = innerToken.text;
              jsonContent.value = `${innerToken.href}<br>`;
              DataJson.push(jsonContent);
              jsonContent = {};
            } else {
              innerToken.text = innerToken.text.replace(/\n/g, '<br>');
              jsonContent.component = 'link';
              jsonContent.value = `${innerToken.text.trim()}<br>`;
              jsonContent.href = `${innerToken.href}<br>`;
              DataJson.push(jsonContent);
              jsonContent = {};
            }
          } else {
            ...
            if (innerToken.type === 'image') {
              ...
              jsonContent.value = innerToken.href;
              jsonContent.alt = innerToken.text;
              DataJson.push(jsonContent);
              jsonContent = {};
            } else {
              jsonContent.component = 'text';
              jsonContent.value = innerToken.text;
              DataJson.push(jsonContent);
              jsonContent = {};
            }
          }
        ...
        jsonContent.title = '';
        jsonContent.value = `${token.text}<br>`;
        DataJson.push(jsonContent);
        jsonContent = {};
      } else if (
        token.type === 'space' ||
      ...
      // hr
        ...
        _.map(token.tokens, (innerToken) => {
          if (innerToken.type !== 'space') {
            _.map(innerToken.tokens, (blockquoteAtomicToken) => {
              blockquoteAtomicToken.text = blockquoteAtomicToken.text.replace(/\n/g, '<br>');
              if (blockquoteAtomicToken.type === 'link') {
                jsonContent.component = 'blockquote';
                jsonContent.value = `${blockquoteAtomicToken.text}`;
                jsonContent.href = blockquoteAtomicToken.href;
                DataJson.push(jsonContent);
                jsonContent = {};
              } else if (
                blockquoteAtomicToken.type !== 'link' &&
      ...
      // table has innerTokens but has more than one tokens due to which it is parsed more than once
        ...
        jsonContent.value = `${token.text.trim()}<br>`;
        DataJson.push(jsonContent);
        jsonContent = {};
      }
      // tokens with inner tokens that doesn't contain an image or link
        ...
        const tokenText = token.raw.replace(regExPattern, (matched) => {
          if (!matched.startsWith('`')) {
            return `<span class='highlight-this'>${matched}</span>`;
          }
          let replacedText = matched.split('`');
          replacedText = `<span style="background-color: #f3f402;">${matched[1]}</span>`;
          return replacedText;
        });
        jsonContent.component = 'text';
        jsonContent.value = `${tokenText}<br>`;
        DataJson.push(jsonContent);
        jsonContent = {};
      } else if (token.type === 'list') {
        _.map(token.items, (listItems) => {
          ...
          if (listItems.text.match(/.*(\[.+\])(\((http).+\)).*/g)) {
            ...
            if (listType === '-') {
              jsonContent.component = 'text';
              jsonContent.value = `${listItems.text}<br>`;
              jsonContent.decoration = {};
              jsonContent.decoration.type = 'bullet';
              DataJson.push(jsonContent);
              jsonContent = {};
              ...
              jsonContent.decoration.value = listType;
              DataJson.push(jsonContent);
              jsonContent = {};
            }
          } else if (listItems.raw.indexOf('```') > -1) {
            _.map(listItems.tokens, (innerListItemToken) => {
              if (
                ...
                jsonContent.title = '';
                jsonContent.value = `${innerListItemToken.text}<br>`;
                DataJson.push(jsonContent);
                jsonContent = {};
              } else {
                const listType = `${listItems.raw.trim().split(' ')[0]}`;
            ...
            if (_.findIndex(listItems.tokens, { type: 'list' }) > 0) {
              _.map(listItems.tokens, (innerListItemToken) => {
                if (innerListItemToken.type === 'text') {
                  jsonContent.component = 'text';
                  jsonContent.variant = 3;
                  jsonContent.value = `${innerListItemToken.text.trim()}<br>`;
                  DataJson.push(jsonContent);
                  jsonContent = {};
                } else {
                  _.map(innerListItemToken.items, (listAtomicToken) => {
                    if (_.findIndex(listAtomicToken.tokens, { type: 'list' }) > 0) {
                      _.map(listAtomicToken.tokens, (innerListItemTokens) => {
                        if (innerListItemTokens.type === 'text') {
                          ...
                          jsonContent.component = 'text';
                          jsonContent.variant = 3;
                          jsonContent.value = `${innerListItemTokens.text.trim()}<br>`;
                          DataJson.push(jsonContent);
                          jsonContent = {};
                        } else {
                          _.map(innerListItemTokens.items, (listAtomicTokenType) => {
                            ...
                            if (listAtomicTokenTypeDec === '-') {
                              jsonContent.component = 'text';
                              jsonContent.value = `${listAtomicTokenType.text}`;
                              jsonContent.decoration = {};
                              jsonContent.decoration.type = 'bullet';
                              DataJson.push(jsonContent);
                              jsonContent = {};
                              ...
                              jsonContent.component = 'text';
                              jsonContent.value = `${listAtomicTokenType.text}<br>`;
                              jsonContent.decoration = {};
                              jsonContent.decoration.type = 'number';
                              jsonContent.decoration.value = listAtomicTokenTypeDec;
                              DataJson.push(jsonContent);
                              jsonContent = {};
                            }
                          });
                      ...
                      if (innerListItemtype === '-') {
                        jsonContent.component = 'text';
                        jsonContent.value = `${listAtomicToken.text}`;
                        jsonContent.decoration = {};
                        jsonContent.decoration.type = 'bullet';
                        DataJson.push(jsonContent);
                        jsonContent = {};
                        ...
                        jsonContent.component = 'text';
                        jsonContent.value = `${listAtomicToken.text}<br>`;
                        jsonContent.decoration = {};
                        jsonContent.decoration.type = 'number';
                        jsonContent.decoration.value = innerListItemtype;
                        DataJson.push(jsonContent);
                        jsonContent = {};
                      }
                    }
              ...
              if (listItemstype === '-') {
                ...
                jsonContent.decoration = {};
                jsonContent.decoration.type = 'number';
                jsonContent.decoration.value = listItemstype;
                DataJson.push(jsonContent);
                jsonContent = {};
              }
            }
        ...
        let paragraphText = token.raw.replace(regExPattern, (matched) => {
          if (!matched.startsWith('`')) {
            return `<span class='highlight-this'>${matched}</span>`;
          }
          let replacedText = matched.split('`');
        ...
        jsonContent.component = 'text';
        jsonContent.value = `${paragraphText}<br>`;
        DataJson.push(jsonContent);
        jsonContent = {};
      } else {
        // console.log(token);
  ...
  async writeFilesJson({ fileRelPath, name, allJsonData }) {
    if (!name) {
      if (fileRelPath.indexOf('/') > -1) {
        ...
        const newFileName = fileRelPath.split('/')[1];
        if (fs.existsSync(`${this.courseDir}/${this.courseFolderName}_json/${subDir}`)) {
          fs.writeFileSync(
            path.resolve(
              `${this.courseDir}/${this.courseFolderName}_json/${subDir}/${newFileName}`
            ),
            allJsonData
          );
        }
        ...
        fs.writeFileSync(
          path.resolve(`${this.courseDir}/${this.courseFolderName}_json/${fileRelPath}`),
          allJsonData
        );
      }
    } else if (name) {
