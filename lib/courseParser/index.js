  ...
  async parseExerciseDetails(file, folder, courseId) {
    ...
    if (file.childExercises.length) {
      ...
      if (fs.existsSync(path)) {
        ...
        let splitBy = [];
        if (dataInString.match(/(@|!)\[(youtube|.+)\]\(.+\)/gi)) {
          splitBy = dataInString.match(/(@|!)\[(youtube|.+)\]\(.+\)/gi);
        }
        const splittedMD = [];
        ...
        for (const md of splittedMD) {
          if (splitBy.indexOf(md) > -1) {
            const imageLink = md.slice(md.indexOf('(') + 1, md.indexOf(')'));
            const imageText = md.slice(md.indexOf('[') + 1, md.indexOf(']'));
            if (md.startsWith('![') && imageLink.startsWith('http')) {
              contentByParts.push(JSON.stringify({ type: 'image', value: { url: imageLink, text: imageText } }));
              ...
              contentByParts.push(JSON.stringify({ type: 'image', value: { url: awsS3Path, text: imageText} }));
            } else if (md.indexOf('[youtube]') > -1) {
  ...
  async parseMarkdownContent(filePath, tokens, beginKeyOn) {
    ...
    _.map(tokens, (token) => {
      ...
      if (token.type === 'table') {
        ...
        formingTableContent = `\n ${formingTableContent + tableHeaderContent}\n`;
