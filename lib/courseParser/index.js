  ...
  async parseExerciseDetails(file, folder, courseId) {
    ...
    if (file.childExercises.length) {
      ...
      if (fs.existsSync(path)) {
        ...
        const codeSeparator = dataInString.match(
          /(```python|```solution|```javascript|```trytyping|```practicetyping)[^```]+(```)(\n)*/gis
        );
        // youtube data inserting into splitBy
        const img_youtubeSeparator = dataInString.match(/(@|!)\[.+\]\(.+\)/gi);
        const separators = _.concat(codeSeparator, img_youtubeSeparator);
        if (separators) {
          splitBy = separators;
        }
        ...
        for (const md of splittedMD) {
          if (splitBy.indexOf(md) > -1) {
            ...
            if (md.startsWith('![') && imageLink.startsWith('http')) {
              ...
              const getLang = md.split('\n')[0].split('```').pop();
              const contentArr = md.split('\n');
              const getContent = contentArr.slice(1, contentArr.length - 1).join('\n');
              contentByParts.push(
                JSON.stringify({
                  type: getLang,
                  value: { code: getContent, testCases: [] },
                })
              );
      ...
      // tokens with inner tokens that doesn't contain an image or link
        ...
        _.map(token.items, (listItems) => {
          ...
          if (listItems.text.match(/.*(\[.+\])(\((http).+\)).*/g)) {
            const linkText = listItems.text
              .substr(listItems.text.indexOf('[') + 1, listItems.text.indexOf(']') - 1)
