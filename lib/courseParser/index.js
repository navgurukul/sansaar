...
class CoursesSeeder {
  ...
  async parseExerciseDetails(file, folder, courseId) {
    ...
    if (file.childExercises.length) {
      ...
      if (fs.existsSync(path)) {
        ...
        const separators =
          dataInString.match(/(@|!)\[.+\]\(.+\)/gi) ||
          dataInString.match(/```(solution)\n.+\n```/gi);
        if (separators) {
          splitBy = separators;
        }
        ...
        for (const md of splittedMD) {
          if (splitBy.indexOf(md) > -1) {
            ...
            } else if (md.startsWith('```solution')) {
              contentByParts.push(
                JSON.stringify({
                  type: 'solution',
                  value: md.split('```solution')[1].split('```')[0].trim(),
                })
              );
  ...
  async parseChildExercise(path, courseId, childExercisesFile) {
    ...
    if (fs.existsSync(childExercisesPath)) {
      ...
      const separators =
        dataInString.match(/(@|!)\[.+\]\(.+\)/gi) || dataInString.match(/```(solution)\n.+\n```/gi);
      if (separators) {
        splitBy = separators;
      }
      const splittedMD = [];
      ...
      // tokens with inner tokens that doesn't contain an image or link
        ...
        _.map(token.items, (listItems) => {
          ...
          if (listItems.text.match(/.*(\[.+\])(\((http).+\)).*/g)) {
            const frontExtraText = listItems.text.slice(0, listItems.text.indexOf('['));
            const LastExtraText = listItems.text.slice(listItems.text.indexOf(')') + 1);

            const linkText = listItems.text.slice(
              listItems.text.indexOf('[') + 1,
              listItems.text.indexOf(']')
            );

            const hrefLink = listItems.text.slice(
              listItems.text.indexOf('('),
              listItems.text.indexOf(')') + 1
            );
            if (frontExtraText !== '') {
              if (LastExtraText !== '') {
                modifiedContent += keyProp;
                keyPropMapping[keyProp] = `${listItems.raw.substr(0, 1)} ${frontExtraText}`;
                keyNumber += 1;
                keyProp = `${exerciseName}_key${keyNumber}`;
                modifiedContent += `[${keyProp}]${hrefLink}`;
                keyPropMapping[keyProp] = `${linkText}`;
                keyNumber += 1;
                keyProp = `${exerciseName}_key${keyNumber}`;
                modifiedContent += `${keyProp}\n`;
                keyPropMapping[keyProp] = `${LastExtraText}`;
              } else {
                modifiedContent += keyProp;
                keyPropMapping[keyProp] = `${listItems.raw.substr(0, 1)} ${frontExtraText}`;
                keyNumber += 1;
