...
class CoursesSeeder {
    ...
    if (file.childExercises.length) {
      ...
      if (fs.existsSync(path)) {
        ...
        if (path.match(/.json/g)) {
          ...
          allContentByParts = JSON.stringify(parsedDataInString);
        }
        if (path.match(/.md/g)) {
          let splitBy = [];
  ...
  async parseIntoJSON(filePath, tokens) {
    ...
    const exercise = [];
    if (tokens[0].raw.indexOf('type: typing') > -1) {
      const dictJsonContent = {};
      _.map(tokens, (token) => {
        ...
        if (token.type === 'heading') {
          ...
          repText = repText.replace(/<\/em>/g, '</i>');
          const jsonData = {};
          jsonData.code = token.lang;
          jsonData.contant = repText.split('');
          dictJsonContent.data = jsonData;
        }
        dictJsonContent.url = `https://merakilearn.org/typing`;
        dictJsonContent.label = '';
        jsonContent.action = [];
        jsonContent.action.push(dictJsonContent);
      });
      DataJson.push(jsonContent);
    } else if (tokens[0].raw.indexOf('type: typing') < 0) {
      _.map(tokens, (token) => {
        const converter = new showdown.Converter();
        if (token.type === 'paragraph' && token.text.match(/\[.+\]\(.+\)/i)) {
          ...
          _.map(token.items, (listItems) => {
            ...
            if (listItems.text.match(/.*(\[.+\])(\((http).+\)).*/g)) {
              ...
              if (listType === '-') {
                ...
                jsonContent.decoration.value = parseInt(listType);
                DataJson.push(jsonContent);
                jsonContent = {};
              }
            } else if (listItems.raw.indexOf('```') > -1) {
              _.map(listItems.tokens, (innerListItemToken) => {
                if (
                  ...
                  console.log(innerListItemToken);
              ...
              if (_.findIndex(listItems.tokens, { type: 'list' }) > -1) {
                    ...
                    _.map(innerListItemToken.items, (listAtomicToken) => {
                      if (_.findIndex(listAtomicToken.tokens, { type: 'list' }) > -1) {
                          ...
                          if (innerListItemTokens.type === 'text') {
                            ...
                            _.map(innerListItemTokens.items, (listAtomicTokenType) => {
                              ...
                              if (listAtomicTokenTypeDec === '-') {
                                ...
                                jsonContent.decoration = {};
                                jsonContent.decoration.type = 'number';
                                jsonContent.decoration.value = parseInt(listAtomicTokenTypeDec);
                                DataJson.push(jsonContent);
                                jsonContent = {};
                              }
                            });
                        ...
                        if (innerListItemtype === '-') {
                          ...
                          jsonContent.decoration = {};
                          jsonContent.decoration.type = 'number';
                          jsonContent.decoration.value = parseInt(innerListItemtype);
                          DataJson.push(jsonContent);
                          jsonContent = {};
                        }
                      }
                ...
                if (listItemstype === '-') {
                  ...
                  jsonContent.decoration = {};
                  jsonContent.decoration.type = 'number';
                  jsonContent.decoration.value = parseInt(listItemstype);
                  DataJson.push(jsonContent);
                  jsonContent = {};
                }
              }
