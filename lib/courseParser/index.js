...
class CoursesSeeder {
  ...
  async parseCourseDir(folder, updateCourseNames) {
    ...
    const fileName = path.split('/').slice(7).join('/');
    const A = await this.createDirsJson(folderName);
    console.log(A);
    ...
    if (fs.existsSync(path)) {
      ...
      if (courseDetails) {
        ...
        if (this.ifCreateJSON) {
          this.writeFilesWithoutJson(fileName, data.toString());
        }
  ...
  async parseAndUploadImage(imageDir) {
    ...
    for (const img of imageDir) {
      ...
      if (fs.existsSync(img)) {
        ...
        /**
         * ## CHANGES
         * Copy files instead of creating buffer
         */
        if (this.ifCreateJSON) {
          this.writeFilesWithoutJson(imgPath, this.imageBuffer);
        }
      } else {
                ...
                if (innerListItemToken.type === 'text') {
                  ...
                  _.map(innerListItemToken.items, (listAtomicToken) => {
                    ...
                    if (innerListItemtype === '-') {
                      ...
                      jsonContent.decoration = {};
                      jsonContent.decoration.type = 'number';
                      jsonContent.decoration.value = listAtomicToken.text;
                      DataJson.push(jsonContent);
                      jsonContent = {};
                    }
                  });
   ...
   */
  async writeFilesWithoutJson(fileName, JsonData) {
    /**
     * ## CHANGES
     * 1. Merge these 2 func into 1
     * 2. Use FS to determine whether it's a folder or not
     * 3. First check if folder is there, then write files.
     * */
    if (fileName.indexOf('/') > -1) {
      const subDir = fileName.split('/')[0];
      const newFileName = fileName.split('/')[1];
      fs.writeFileSync(
        path.resolve(`${this.courseDir}/${this.courseFolderName}_json/${subDir}/${newFileName}`),
        JsonData
      );
    } else {
      fs.writeFileSync(
