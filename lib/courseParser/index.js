...
class CoursesSeeder {
      ...
      if (fs.existsSync(path)) {
        ...
        if (path.match(/.json/g)) {
          ...
          if (parsedContent.length > 0) {
            ...
            onlyMarkdownAndJson += JSON.stringify(contents.value, null, 2);
          ...
          for (const md of splittedMD) {
            if (splitBy.indexOf(md) > -1) {
              ...
              if (parsedContent.length > 0) {
                ...
                beginKeyOn = beginKeyFrom;
                contentByParts.push(JSON.stringify(contents));
                onlyMarkdownAndJson += contents.value;
                allJSONKeys = { ...allJSONKeys, ...jsonKeys };
              }
            }
          }
  ...
  async parseIntoModifiedContent(filePath, jsonContent, beginKeyOn) {
    ...
    _.map(jsonContent, (jsonData) => {
      if (
        ...
        let modifiedContent = '';
        const token = marked.lexer(jsonData.value);
        if (token[0] !== undefined) {
          _.map(token[0].tokens, (tokenText) => {
            if (tokenText.type === 'text') {
              keyNumber += 1;
              keyProp = `${exerciseName}_key${keyNumber}`;
              modifiedContent += `${keyProp}`;
              keyPropMapping[keyProp] = `${tokenText.text}`;
            }
            if (tokenText.type === 'html') {
              modifiedContent += `${tokenText.text}`;
            }
          });
        }
        ...
        _.map(jsonData.value, (table) => {
          const headers = marked.lexer(table.header);
          let modifiedContent = '';
          _.map(headers[0].tokens, (tokenText) => {
            if (tokenText.type === 'text') {
              keyNumber += 1;
              keyProp = `${exerciseName}_key${keyNumber}`;
              modifiedContent += `${keyProp}`;
          ...
          _.map(table.items, (tokenText) => {
            const item = marked.lexer(tokenText);
            let modifiedContentItems = '';
            _.map(item[0].tokens, (itemsToken) => {
              if (itemsToken.type === 'text') {
                keyNumber += 1;
                keyProp = `${exerciseName}_key${keyNumber}`;
                modifiedContentItems += `${keyProp}`;
                keyPropMapping[keyProp] = `${itemsToken.text}`;
              }
              if (itemsToken.type === 'html') {
                modifiedContentItems += `${itemsToken.text}`;
              }
            });
        ...
        let modifiedContent = '';
        const token = marked.lexer(jsonData.alt);
        if (token[0] !== undefined) {
          _.map(token[0].tokens, (tokenText) => {
            if (tokenText.type === 'text') {
              keyNumber += 1;
  ...
  async writeFiles({ fileRelPath, name, onlyMarkdownAndJson, allJSONKeys }) {
    const fileType = fileRelPath.split('.');
    const mdOrjson = fileType[fileType.length - 1].trim();
    const courseName = this.courseDetails[0].name;
    if (fileRelPath.indexOf('/') > -1) {
      const subDir = fileRelPath.split('/')[0];
      fs.writeFileSync(
        path.resolve(
          `${this.courseDir}/${this.courseFolderName}/PARSED_CONTENT/MODIFIED_FILES/${subDir}/${courseName}_${name}.${mdOrjson}`
        ),
        onlyMarkdownAndJson
      );
      ...
      fs.writeFileSync(
        path.resolve(
          `${this.courseDir}/${this.courseFolderName}/PARSED_CONTENT/MODIFIED_FILES/${courseName}_${name}.${mdOrjson}`
        ),
        onlyMarkdownAndJson
      );
