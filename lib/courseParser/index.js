...
class CoursesSeeder {
  ...
  async init() {
    ...
    // console.log(CoursesSeeder.updateDatabase())
  ...
  async parseCourseDir(folder, updateCourseNames) {
    ...
    if (fs.existsSync(path)) {
      ...
      // getting course details from parseCourseDetails
      ...
      if (courseDetails) {
        ...
        _.map(items, (item, outerIndex) => {
          ...
          _.map(exerciseFiles, (file, index) => {
            ...
            if (file.type === 'text') {
              // checking file is mdfile or not
  ...
  async parseChildExercise(path, courseId, childExercisesFile) {
    // console.log(path, courseId, childExercisesFile)
    const exercise = {};
