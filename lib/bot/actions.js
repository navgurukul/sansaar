...
const createARoom = async (roomAlias) => {
  ...
  console.log(`${roomAlias} ROOM ALIAS`);
  const roomIndexNo = roomAlias.substring(2);
  const createdRoom = await axiosAuth.post(`/_matrix/client/r0/createRoom`, {
    initial_state: [],
    visibility: 'public',
    preset: 'public_chat',
...
const joinARoom = async (client, sender, roomIdToJoin, roomAlias) => {
