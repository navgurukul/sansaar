...
class CoursesSeeder {
  constructor(parseSingleCourse, courseName) {
  ...
  static getCourseName() {
    if (process.argv.indexOf('--parseSingleCourse') > -1) {
      const courseName = process.argv[process.argv.indexOf('--parseSingleCourse') + 1];
      if (!courseName) {
...
if (!module.parent) {
  ...
  const seeder = new CoursesSeeder(parseSingleCourse, courseName);
