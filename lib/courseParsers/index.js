  ...
  async parseMarkdownContent(filePath, tokens) {
    ...
    _.map(tokens, (token) => {
      ...
      if (token.type === 'code' && token.lang === 'ngMeta') {
        ...
        modifiedContent = `${modifiedContent}${token.lang}\n${keyProp}\n` + `` + `\n`;
        keyPropMapping = `${keyProp}=${token.text.trim()}\n`;
        // console.log(modifiedContent);
      }

      if (token.type === 'space') {
        modifiedContent += token.raw.substr(1); // Always omit first \n ;
      }

      if (token.type === 'heading') {
        keyNumber += 1;
        keyProp = `${exerciseName}_key${keyNumber}`;
        modifiedContent += `${token.raw.trim().split(' ')[0]} ${keyProp}\n\n`;
        keyPropMapping += `${keyProp}=${token.text.trim()}\n`;
      }
      // hr
      if (token.type === 'hr') {
      ...
      if (token.type !== 'table' && token.token) {
        _.map(token.token, (innerToken) => {
          // eslint-disable-next-line no-console
          console.log(token);
          // space
          ...
          // headings (headings have inner tokens so it never goes to the else part)
            ...
            keyNumber += 1;
            keyProp = `${exerciseName}_key${keyNumber}`;
            modifiedContent += `${tokens.raw.split(' ')[0]} ${keyProp}\n`;
          ...
          // normal text
            ...
            modifiedContent += innerToken.raw.trim();
          }
        });
      }
      // list
