...
class CoursesSeeder {
  ...
  async parseMarkdownContent(filePath, tokens) {
    ...
    _.map(tokens, (token) => {
      ...
      if (token.type !== 'table' && token.tokens) {
        _.map(token.tokens, (innerToken) => {
          // space
          if (innerToken.type === 'text' && innerToken.text === '\n') {
            modifiedContent += innerToken.text;
          }
          // paragraph - links
          ...
          // headings (headings have inner tokens so it never goes to the else part)
          else if (token.type === 'heading') {
            keyNumber += 1;
            keyProp = `${exerciseName}_key${keyNumber}`;
            modifiedContent += `${token.raw.split(' ')[0]} ${keyProp}\n`;
            keyPropMapping += `${keyProp}=${token.text}\n`;
          }
          ...
          // normal text
            ...
            keyProp = `${exerciseName}_key${keyNumber}`;
            modifiedContent += `${keyProp}`;
            keyPropMapping += `${keyProp}=${innerToken.raw}\n`;
          }
          // codespan and escape
          else if (innerToken.type === 'codespan' || innerToken.type === 'escape') {
            modifiedContent += `${innerToken.raw}`;
          }
          // paragraphs in inner token
          else if (innerToken.type === 'paragraph') {
            keyNumber += 1;
            keyProp = `${exerciseName}_key${keyNumber}`;
            modifiedContent += `${keyProp}`;
            keyPropMapping += `${keyProp}=${innerToken.text}\n`;
          } else {
  ...
  async writeFiles({ exrRelPath, exerciseName, modifiedContent, keyPropMapping }) {
    ...
    if (exrRelPath.indexOf('/') > -1) {
      ...
      fs.writeFileSync(
        path.resolve(
          `${this.courseDir}/${this.courseFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${subDir}/${courseName}_${exerciseName}_en.properties`
        ),
        keyPropMapping
      );
    } else {
      ...
      fs.writeFileSync(
        path.resolve(
          `${this.courseDir}/${this.courseFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_${exerciseName}_en.properties`
        ),
        keyPropMapping
      );
    }
    ...
    if (process.argv.indexOf('--parseSingleCourse') > -1) {
      ...
      try {
        ...
        this.showErrorAndExit(
          `The specified courseFolderName ${courseFolderName} is doesn't exist in curriculum.`
        );
      }
    }
    // eslint-disable-next-line
