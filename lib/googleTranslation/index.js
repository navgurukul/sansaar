  ...
  async main() {
    ...
    _.map(this.courseFiles, async (property) => {
      ...
      for (const index of _.keys(fileData)) {
        ...
        let spanStr;
        if (
          splitedData.includes('<span style="background-color: #f3f402;">') &&
          splitedData.includes('</span>')
        ) {
          spanStr = splitedData.split('span');
          _.map(spanStr, (str, index) => {
            if (index % 2 === 1) {
              if (str.slice(-2) === '</') {
                let encoded = base64encode(str.slice(36, -2));
                spanStr[index] = ' style="background-color: #f3f402;">' + encoded + '</';
              }
            }
          });
        ...
        if (
          ...
          _.map(spanStr, (str, index) => {
            if (index % 2 === 1) {
              if (str.slice(-2) === '</') {
                let decoded = base64decode(str.slice(36, -2));
                spanStr[index] = ' style="background-color: #f3f402;">' + decoded + '</';
              }
            }
          });
          splitedData = spanStr.join('span');
