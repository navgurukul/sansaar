...
module.exports = class ExerciseCompletion extends ModelBase {
  ...
  async $afterInsert(queryContext) {
    ...
    const checkAll = await checkCourseCompletionEligibility(transaction, user_id, exercise_id);
    if (checkAll.status) {
      const courseInserted = await insertIntoCourse(transaction, user_id, checkAll.courseId);
    }
  }
  ...
  async $afterDelete(queryContext) {
    ...
    const checkAll = await checkCourseCompletionEligibility(transaction, user_id, exercise_id);
    if (!checkAll.status) {
      const courseDeleted = await deleteFromCourse(transaction, user_id, checkAll.courseId);
    }
  }
};
