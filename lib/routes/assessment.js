...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [err, assessment] = await assessmentService.markAssessmentCompleteBYSlug(details);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [err, assessment] = await assessmentService.DeleteAssessmentResultByUserOrTeam(
          user_team
        );
        if (err) {
          logger.error(JSON.stringify(err));
