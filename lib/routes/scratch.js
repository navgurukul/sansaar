...
module.exports = [
  {
    ...
    handler: async (request, h) => {
      ...
      const [err, data] = await scratchService.tempCretentialsForAndroid();
      if (err||data==null) {
...
},
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { scratchService } = request.services();
        const [errCheck,chek_projectId] = await scratchService.check()
        ...
        return { userId: data.userId_scratch, url: data.url, scratch_url: data.scratch_url };
      },
    },
  },
  ...
  {
    method: 'GET',
    path: '/scratch/FileUploadS3/{userId_scratch}',
    options: {
      description: 'Get chat user details',
      tags: ['api'],
      validate: {
        params: Joi.object({
          userId_scratch: Joi.number().integer().required(),
        }),
      },
      handler: async (request, h) => {
        const { scratchService } = request.services();
        // logger.info('Get chat user details');
        const [err, data] = await scratchService.getScratchFile(request.params.userId_scratch);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { scratchService } = request.services();
        const { userId_scratch } = request.params;
