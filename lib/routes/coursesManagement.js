...
module.exports = [
  ...
  {
    method: 'POST',
    path: '/courses/{courseId}/cp',
    options: {
      description: 'copy assets from v1 to v2 and fix link',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        scope: getRouteScope('admin'),
      ...
      handler: async (request, h) => {
        ...
        if (coursesDetails !== undefined) {
          ...
          const addCourse = await coursesServiceV2.addParsedContent(coursesDetails);
        }
        logger.info('copied all assets to curriculum_new');
        return 'addCourse';
      },
    },
  },
