...
module.exports = [
  ...
  {
    method: 'PUT',
    path: '/courses/{courseId}/changeHighlightColor',
    options: {
      description: 'Change hightlights colour, remove words colour and shadow from exercises',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        scope: getRouteScope(['admin', 'partner', 'volunteer']),
      ...
      handler: async (request, h) => {
        ...
        const { courseId } = request.params;
        const [errorWhileFetchingDetails, courseDetails] = await coursesServiceV2.getCourseById(
          courseId
        );
        if (errorWhileFetchingDetails) {
          return h.response(errorWhileFetchingDetails).code(errorWhileFetchingDetails.code);
        ...
        if (courseDetails !== undefined) {
          const [err, courseUpdated] = await coursesServiceV2.updateHighlightColour(
            courseId,
            courseDetails
          );
          if (err){
        ...
        // logger.info('Get all courses');
        return 'updated hightlights colour';
      },
    },
  },
];
