...
module.exports = [
    ...
    options: {
      ...
      validate: {
        payload: Joi.object({
          ...
          lang_available: Courses.field('lang_available'),
        }),
      },
      handler: async (request, h) => {
        const { coursesService } = request.services();
  ...
  {
    method: 'PUT',
    path: '/courses-QH2hh8Ntynz5fyTv/{name}',
    options: {
      ...
      validate: {
        payload: Joi.object({
          ...
          lang_available: Joi.string(),
        }),
      },
      handler: async (request) => {
        const { coursesService } = request.services();
        return coursesService.patchCourse(request.payload, request.params.name);
      },
    },
  },
  {
