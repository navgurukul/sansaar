...
module.exports = [
  ...
  {
    method: 'PUT',
    path: '/courses/{courseId}/changeImagesLink',
    options: {
      description: 'update link of exercises',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        scope: getRouteScope('admin'),
      ...
      handler: async (request, h) => {
        const { coursesService, coursesServiceV2 } = request.services();
        const { courseId } = request.params;
        const [errorWhileFetchingDetails, getCourseDetails] = await coursesServiceV2.getCourseById(
          courseId,
          request.payload
        );
        ...
        if (getCourseDetails !== undefined) {
          const [err, courses] = await coursesService.getOLdCourseByNameWithJsonType(
          ...
          const [errorWhileUpdatelink, updatelink] = await coursesServiceV2.updateImagesLinkCourses(
            courses[0],
            getCourseDetails
          );
          if (errorWhileUpdatelink) {
            logger.error(JSON.stringify(errorWhileUpdatelink));
            return h.response(errorWhileUpdatelink).code(errorWhileUpdatelink.code);
          }
        }
        logger.info('Get all courses');
        return 'updated images link';
      },
    },
  },
  ...
  {
    method: 'POST',
    path: '/courses/{courseId}/fixImages',
    options: {
      description: 'copy assets from v1 to v2 and fix image url',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        scope: getRouteScope('admin'),
      ...
      handler: async (request, h) => {
        ...
        if (coursesDetails !== undefined) {
          ...
          // eslint-disable-next-line
          const [errorWhileCopying, copyingAssets] = await coursesServiceV2.copyAssetsToV2(
            courses[0],
            coursesDetails
          );
          if (errorWhileCopying) {
            logger.error(JSON.stringify(errorWhileCopying));
            return h.response(errorWhileCopying).code(errorWhileCopying.code);
          }
          // eslint-disable-next-line no-unused-vars
          const urlFixed = await coursesServiceV2.fixImageUrl(coursesDetails);
        }
        logger.info('copied all assets to curriculum_new url updated');
        return 'images added and url updated';
      },
    },
  },
];
