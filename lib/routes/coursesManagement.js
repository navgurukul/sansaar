...
module.exports = [
  ...
  {
    method: 'POST',
    path: '/courses/{courseId}/exercises/{exerciseId}/assessment',
    options: {
      ...
      handler: async (request, h) => {
        ...
        const assessmentData = { ...request.payload, course_id: courseId, exercise_id: exerciseId };
        const [err, assessment] = await exercisesServiceV2.addAssessment(assessmentData);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
