        ...
        options: {
            ...
            validate: {
                payload: Joi.object({
                    ...
                    programming_languages: Joi.array(),
        ...
        options: {
            ...
            validate: {
                query: Joi.object({
                    ...
                    course_url: Joi.string().regex(/^(https?|http):\/\/\S+$/),
                    course_description: Joi.string(),
                    course_category: Joi.string(),
                    developers_id: Joi.number().required(),
                }),
            },
            handler: async (request, h) => {
                try {
                    const { developerLearning } = request.services();
                    const data = await developerLearning.createResources(request.query);
      ...
      handler: async (request, h) => {
        try {
            ...
            if (array != null && array != undefined && array.length == 0) {
                let lang = recommendations.interests
                for (let rec of lang){
                    const array2 = await developerLearning.courseLinks(rec)
                    if (array2.length != 0){
                        return h.response(array2).code(200);
                    }
                }
                let lang2 = recommendations.skills
                for (let rec of lang2){
                    const array2 = await developerLearning.courseLinks(rec)
                    if (array2.length != 0){
                        return h.response(array2).code(200);
                    }
...

    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                try {
                    const { developerLearning } = request.services();
                    const data = await developerLearning.messageAi(request.query.message);
                    logger.info('Get course resources');
                    return h.response(data).code(200);
                } catch (error) {
                    logger.error(error);
