...
module.exports = [
    {
        ...
        options: {
          ...
          handler: async (request, h) => {
            ...
            let [err,res] = await youtubeBroadCastService.createLiveBroadcast( request.payload.classes);
            if (err){
    ...
    },
    {
      ...
      options: {
        ...
        handler: async (request, h) => {
          const {  youtubeBroadCastService } = request.services();
          // let {id,email} = request.auth.credentials;
          let [err,res]= await youtubeBroadCastService.updateLiveBroadcast(request.payload.classes);
          if (err){
            return h.response(err).code(err.code);
          }
          return h.response(res).code(200);
    ...
    },
    {
      ...
      options: {
        ...
        handler: async (request, h) => {
          const {  youtubeBroadCastService } = request.services();
          // let {id,email} = request.auth.credentials;
          let {recurring_id,class_id} = request.query;
          let [err,res]= await youtubeBroadCastService.deleteBroadcast(recurring_id, class_id);
          if (err){
            return h.response(err).code(err.code);
          }
          return h.response(res).code(200);
