      ...
      handler: async (request, h) => {
        ...
        if (appVersion >= androidVersions.latest) {
          ...
          for (let exercise in course[0].exercises) {
            ...
            const exerciseContent = JSON.parse(JSON.parse(course[0].exercises[exercise].content));
            if (exerciseContent.value.value !== undefined) {
              course[0].exercises[exercise].content = exerciseContent.value.value;
            } else {
              course[0].exercises[exercise].content = exerciseContent.value;
            }
            sequence += 1;
