          ...
          for (let exercise in course[0].exercises) {
            ...
            if (Assessment.length > 0) {
              ...
              for (const singleAssessment in Assessment) {
                course[0].exercises.push(Assessment[singleAssessment]);
                sequence += 1;
              }
            }
            ...
            const [
              ...
              getSingleClasses,
            ] = await classesService.getClassByExercise(course[0].exercises[exercise].id);
            if (errorInsingleClasses) {
              logger.error(JSON.stringify(errInFetching));
            }
            const getAllClasses = [...getSingleClasses, ...getRecurringClasses];

            if (getAllClasses !== null && getAllClasses.length > 0) {
              // eslint-disable-next-line
