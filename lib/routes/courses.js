...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        if (appVersion >= androidVersions.latest) {
          if (flag) {
            const courseV2 = await redisClient.get(
              `courseV2_${language}_${courseId}_${user.partner_id}`
        ...
        let courseV1;
        if (flag) {
          courseV1 = await redisClient.get(`courseV1_${language}:${courseId}_${user.partner_id}`);
        }
        if (courseV1) {
        ...
        if (flag) {
          redisClient.set(
            `courseV1_${language}_${courseId}_${user.partner_id}`,
            JSON.stringify(course)
          );
        }
        return { course };
