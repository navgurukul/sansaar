...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const appVersion = request.headers['version-code'] ? request.headers['version-code'] : null;
        if (appVersion >= androidVersions.latest) {
          const courseV2 = await redisClient.get(`courseV2_${language}:${courseId}`); // getting course data from cache
          if (courseV2) {
        ...
        const courseV1 = await redisClient.get(`courseV1_${language}:${courseId}`);
        if (courseV1) {
