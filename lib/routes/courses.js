...
module.exports = [
      ...
      handler: async (request, h) => {
        ...
        if (appVersion >= androidVersions.latest) {
          ...
          if (!courses || !Array.isArray(courses)) {
            logger.error('Courses data is not an array or is null');
  ...
  {
    method: 'GET',
    path: '/courses/{course_id}/prequiz',
    options: {
      description: 'Get pre-quiz questions of MCDigital-pathway ⓜ',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
      },
      ...
      handler: async (request, h) => {
        ...
        const { coursesServiceV2 } = request.services();
        const [err, course] = await coursesServiceV2.getcourseByprequiz(
          course_id,
        );
        if (err) {
          logger.error(JSON.stringify(err));
        ...
        logger.info('Successfully retrieved course and pre-quiz data');
        return { course };
      },
    },
  },
