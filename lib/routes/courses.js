    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        if (appVersion >= androidVersions.latest) {
          ...
          const course = await displayService.courseV2Slug(courseWithexercise);
          let recurringId;
          // eslint-disable-next-line
          for (let exercise in course.course_content) {
            course.course_content[exercise].course_name = course.name;
            course.course_content[exercise].sequence_num = sequence;
            course.course_content[exercise].content_type = 'exercise';

            const [
              errorInsingleClasses,
              getSingleClasses,
              // eslint-disable-next-line
            ] = await classesService.getClassBySlug(course.course_content[exercise].slug_id);
            if (errorInsingleClasses) {
              logger.error(JSON.stringify(errorInsingleClasses));
            }
            if (getSingleClasses != null && getSingleClasses.length > 0 && request.auth.credentials.id) {
              ...
              for (let singleClass in getSingleClasses) {
                ...
                course.course_content[exercise].content.push(singleClassBanner);
              }
            }
            if (appVersion > androidVersions.latest) {
              // eslint-disable-next-line
              const Assessment = await assessmentService.findAssessmentByCourseDetailsV2ForSlug(
                course.course_content[exercise],
                courseId,
                course.name,
                sequence,
                request.auth.credentials
              ...
              if (Assessment.length > 0) {
                // eslint-disable-next-line
                for (const singleAssessment in Assessment) {
                  course.course_content.push(Assessment[singleAssessment]);
                  sequence += 1;
                }
              }
            }
            ...
            if (enrolledClasses.length > 0 && request.auth.credentials.id) {
              ...
              for (const e in enrolledClasses) {
                if (
                  enrolledClasses[e].course_id === courseId &&
                  enrolledClasses[e].slug_id === course.course_content[exercise].slug_id
                ) {
                  // eslint-disable-next-line
                  const classesForOldversion = await displayService.filterClassDetails(
                    enrolledClasses[e],
                  ...
                  sequence += 1;
                  course.course_content.push(classesForOldversion);
                }
              }
            }
            if (request.auth.credentials.id) {
              if (recurringId === undefined || recurringId === null) {
                if (getPathwayId !== null) {
                  if (userEnrolled) {
                    ...
                    // eslint-disable-next-line
                    const classDetails = await classesService.getcohortClassByExercise(
                      course.course_content[exercise].id,
                      request.auth.credentials
                    );
                    if (classDetails.length > 0) {
                      recurringId = classDetails[0].recurring_id;
            ...
            if (recurringId !== undefined && request.auth.credentials.id) {
              const [
                ...
                recurringId,
                course.course_content[exercise].slug_id,
                classesRegisteredByUser
              );
              if (errInFetching) {
                logger.error(JSON.stringify(errInFetching));
              ...
              if (getRecurringClasses !== null && getRecurringClasses.length > 0) {
                ...
                let classesIds = [];
                course.course_content.forEach((dic) => {
                  if (dic && 'title' in dic && 'start_time' in dic && 'end_time' in dic) {
                    classesIds.push(dic.id);
                  }
                });
                ...
                for (let singleClass in singleClassWithFacilitator) {
                  ...
                  sequence += 1;
                  if (!classesIds.includes(classes.id)) {
                    course.course_content.push(classes);
                  }
                }
              }
            }
          ...
          course.course_content.sort(function (a, b) {
            const sequenceNumA = a ? parseFloat(a.sequence_num) : 0;
            const sequenceNumB = b ? parseFloat(b.sequence_num) : 0;
            
            return sequenceNumA - sequenceNumB;
          });
          

          logger.info('Get complete list of exercises in the course');
          const PythonCourseIds = [87, 89, 17, 8, 10, 11, 88, 86, 21].includes(courseId);
          if (PythonCourseIds && course.course_content && course.course_content.length > 0) {
            course.course_content[course.course_content.length - 1].project_id = 890907;
            return { course };
          }
