...
module.exports = [
  ...
  },
  {
    ...
    path: '/partner/{userId}',
    options: {
      description: 'Remove an user from the partner_id,Space_id,Group_id, ',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        // scope: getRouteScope(['partner', 'partner_edit']),
      ...
      handler: async (request, h) => {
        ...
        let removeAnPartnerUser;
        const [err, deleted] = await partnerService.removeGrpPrtSpc(userId);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
  ...
  //   path: '/partner/{userId}',
  //   options: {
  //     description: 'Remove an user Partner_id,group_id,space_id',
  //     tags: ['api'],
  //     auth: {
  //       strategy: 'jwt',
  //       // scope: getRouteScope(['partner', 'partner_edit']),
  ...
  //       let removeAnPartnerUser;
  //       const [err, deleted] = await partnerService.removeGrpPrtSpc(request.query.userId);
  //       if (err) {
  //         logger.error(JSON.stringify(err));
  //         return h.response(err).code(err.code);
  //       }
  ...
  //         removeAnPartnerUser = {
  //           status: `success`,
  //           message: `Yaay! ${userId} PrtGrpSpc_id removed successfully!`,
  //         };
  //       }
  //       logger.info(`Remove an user from the partner`);
  //       return removeAnPartnerUser;
