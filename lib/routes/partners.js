...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const partnerUser = await partnerService.arrayCall(request.payload, partner_id, space_id, group_id);
        // if (spaceID == null) {
        //   return {
        //     error: true,
...
//partners create ====>>>
  ...
  },
  {
    ...
    handler: async (request, h) => {
      const { partnerService } = request.services();
      try {
        const {file, group_id, partner_id, space_id} = request.payload;
        if (!file) {
        ...
        if (typeOfFile[1] == 'csv'){
          data =  await partnerService.extractDataFromCsv(filePath, partner_id, space_id, group_id);
        }else if(typeOfFile[1]== 'xlsx'){
          data =  await partnerService.extractDataFromXlsx(filePath,partner_id, space_id, group_id);
        }else{
