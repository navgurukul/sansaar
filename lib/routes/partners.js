...
//partners create ====>>>
    ...
    options: {
      ...
      handler: async (request, h) => {
        try {
          ...
          const [error,results] = await partnerService.readTheFile(csvFilePath,request.params.batch_id);
  ...
  {
    method: 'GET',
    path: '/partners/students/{batch_id}',
    options: {
      ...
      handler: async (request, h) => {
        try{
            const { partnerService } = request.services();
            const [err, Data] = await partnerService.getStudentsBySpaceid(request.params.batch_id);
            if (err) {
              logger.error(JSON.stringify(err));
              return h.response(err).code(err.code);
            }
    ...
    options: {
      ...
      handler: async (request, h) => {
        try {
          ...
          console.log('runing');
          const [error,results] = await partnerService.csvDataUpload(csvFilePath,partner_id, space_id);
