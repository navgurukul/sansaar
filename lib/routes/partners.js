...
// /partners/create >>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  {
    method: 'PUT',
    path: '/partners/newspace/{user_id}/update',
    options: {
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        const [errorInData, Data] = await partnerService.updateParterSpace(request.params.user_id,request.payload);
        if (errorInData) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        const [errorInData, Data] = await partnerService.getAllParterSpace();
        if (errorInData) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
  ...
  {
    method: 'DELETE',
    path: '/partners/newspace/del/{user_id}',
    options: {
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        const [errorInData, Data] = await partnerService.deleteParterSpace(request.params.user_id);
        if (errorInData) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        try {
          const { partnerService } = request.services();
          const [error, data] = await partnerService.createPartnerNewSpace(request.payload);
          if (error != null) {
            return { "Error": error }
          }
          return data
