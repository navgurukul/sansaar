...
//partners create ====>>>
    ...
    handler: async (request, h) => {
      ...
      try {
        ...
        if(data[0].error == true){
          ...
          let [err, fileCrpt] = await partnerService.returnCsvFile(errorData)
          if (err){
            logger.error(JSON.stringify(err));
            return h.response(err).code(err.code);
          }
          return h.file(fileCrpt[0],fileCrpt[1]);
        // if(data[0].error == true){
        //   let {err,data} = returnCsvFile(data)
        //   if (err) {
        //     logger.error(JSON.stringify(err));
        //     return h.response(err).code(err.code);
        //   }
  ...
  {
    method: 'PUT',
    path: '/partners/group/{group_id}/link',
    options: {
      description: 'Create a bitly link for Meraki android app and Web app for a partner.',
      tags: ['api'],
      // auth: {
      ...
      handler: async (request) => {
        const { partnerService } = request.services();
        let {partner_id, group_id} = request.params
        logger.info(`Create a bitly link for Meraki android app and Web app for a partner`);
        return partnerService.groupSpecificLinkAuto(
          request.headers.platform,
          group_id
        );
      }
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        try {
          const data = await partnerService.updateUsersPartnerSpaceGroupIds(request.query, email);
          logger.info(` Updating users PartnerId, SpaceId, GroupId`);
          return data;
        } catch (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
