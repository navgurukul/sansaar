...
module.exports = [
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const partnerUser = await partnerService.arrayCall(request.payload, group_id);
        // if (spaceID == null) {
        //   return {
        //     error: true,
  ...
  {
    method: 'DELETE',
    path: '/partners/{partner_id}/userIn',
    options: {
...
//partners create ====>>>
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        try{
          const { partnerService } = request.services();
          const [err, Data] = await partnerService.getPartnerSpaceByPartnerID(request.params.partner_id);
          if (err) {
            logger.error(JSON.stringify(err));
            return h.response(err).code(err.code);
          }
  ...
  //   method: 'POST',
  //   path: '/partners/uploadstudents/{group_id}',
  //   options: {
  //     payload: {
  //       output: 'file',
  //       multipart: true,
  //     },
  //     description: 'bulk data uploading the students data to the database with the help of group_id',
  //     notes: 'file-upload',
  //     tags: ['api'],
  //     // auth: {
  //     //   strategy: 'jwt',
  ...
  //         const [error,results] = await partnerService.readTheFile(csvFile,request.params.group_id);
  ...
  //         // please put state partner group csv file inside statePartnerGroupsData folder
  //         // const csvFilePath = file.hapi.filename;
  //         // let {partner_id, space_id} = request.query
  //         const [error,results] = await partnerService.csvDataUpload(csvFile);
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        try {
          const { partnerService } = request.services();
          const [err, data] = await partnerService.createGroup(request.query.space_id,request.payload);
          if (err != null) {
            logger.error(JSON.stringify(err));
            return h.response(err).code(err.code);
          }
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        try{
          const { partnerService } = request.services();
          let update_data = request.payload
          const [err, Data] = await partnerService.updateGroup(request.params.group_id, update_data);
          if (err) {
            logger.error(JSON.stringify(err));
            return h.response(err).code(err.code);
          }
  ...
  },
  {
    ...
    handler: async (request, h) => {
      const { partnerService } = request.services();
      try {
        const {file, group_id} = request.payload;
        if (!file) {
        ...
        if (typeOfFile[1] == 'csv'){
          data =  await partnerService.extractDataFromCsv(filePath,group_id);
        }else if(typeOfFile[1]== 'xlsx'){
          data =  await partnerService.extractDataFromXlsx(filePath,group_id);
        }else{
          return h.response({ message:'you trying to uploade the wrong formate file. try only google sheet, csv or xl'}).code(403);
        }
        if(data[0].error == true){
          ...
          return h.file(csvFilePath, { mode: 'attachment', filename: csvFileName });
        }else{
          return data;
        }
      } catch (error) {
  ...
  },
  {
    ...
    options: {
      description: 'get the batched data by the group_id',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
      },
