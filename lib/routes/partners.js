...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        //   };
        // }
        if(partnerUser[0].error || partnerUser[0].Error){
          let err = partnerUser[0];
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
        try {
...
//partners create ====>>>
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request,h) => {
        const { partnerService } = request.services();
        let {partner_id,space_id, group_id} = request.query
        logger.info(`Create a bitly link for Meraki android app and Web app for a partner`);
        let [err ,outPut] = await partnerService.groupSpecificLinkAuto(
          partner_id,
          space_id,
          group_id
        );
