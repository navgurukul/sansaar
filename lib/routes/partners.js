...
// /partners/create >>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  {
    method: 'PUT',
    path: '/partners/{partner_id}/update',
    options: {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        if (deleted == 1) {
          removeAnPartnerUser = {
            status: `success`,
            message: `Yep, tata bye bye!!! userid is ${partner_id} removed successfully!`,
          };
          return removeAnPartnerUser;
          // eslint-disable-next-line
        } else {
  ...
  {
    method: 'PUT',
    path: '/partners/space/{space_id}/update',
    options: {
      description: 'update the exist partner newspace by id.',
      tags: ['api'],
      // auth: {
      //   strategy: 'jwt',
      //   // scope: getRouteScope(['admin','partner_edit']),
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        const [errorInData, Data] = await partnerService.updateParterSpace(request.params.space_id,request.payload);
        if (errorInData) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
  ...
  },
  {
    ...
    options: {
      description: 'get the all partner new space  data',
      tags: ['api'],
      validate: {
        query: Joi.object({
          partner_id: Joi.number().integer(),
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        const [errorInData, Data] = await partnerService.getAllParterSpace(request.query.partner_id);
        if (errorInData) {
  ...
  {
    method: 'DELETE',
    path: '/partners/del/{space_id}',
    options: {
      description: 'Delete the partner new space',
      tags: ['api'],
      // auth: {
      //   strategy: 'jwt',
      //   // scope: getRouteScope(['partner', 'partner_edit']),
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        const [errorInData, Data] = await partnerService.deleteParterSpace(request.params.space_id);
        if (errorInData) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
  ...
  {
    //partner can create the newspace with the p
    method: 'POST',
    path: '/partners/newspace/create',
    options: {
      description: 'Create the new newspace.',
      tags: ['api'],
      // auth: {
      //   strategy: 'jwt'
      // },
    ...
    options: {
      ...
      handler: async (request, h) => {
        try {
          ...
          const [error,results] = await partnerService.readTheFile(csvFilePath,request.params.partner_id);
          if (error != null){
