...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        console.log(partnerService,"routes3")
        console.log(displayService,"routers4")
        const [err, data] = await partnerService.getAllPartner(request.query);
        console.log(data,"routers1")
        console.log(err,"routers2")
        if (err) {
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const partnerUser = await partnerService.getPartnerUser(request.payload, partnerId);
        if (partnerId == null) {
          return {
            error: true,
  ...
  
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        const [err, deleted] = await partnerService.removeAndUserFromPartner(request.params.user_id);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
