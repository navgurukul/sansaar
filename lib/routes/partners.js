    ...
    handler: async (request, h) => {
      ...
      try {
        ...
        if (data.length != 0) {
          ...
          if (fileCrpt[0].status) {
            const obj = {
              Succes: fileCrpt.length,
              Failed: 0
            }
            return h.response(obj).header('X-Object', JSON.stringify(obj));
          }
          const response = h.file(fileCrpt[0], {
            mode: 'attachment',
            filename: fileCrpt[1].filename
          });
          response.header('Content-Disposition', `attachment; filename=${fileCrpt[1].filename}`);
          response.header('X-Object', JSON.stringify(fileCrpt[2]));
          return response;
        }
      } catch (error) {
