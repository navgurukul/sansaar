...
module.exports = [
  ...
  {
    method: 'GET',
    path: '/partners/{partnerGroupId}/district',
    options: {
      description: 'Get data for the partner group',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        scope: getRouteScope('partner'),
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        const { partnerGroupId } = request.params;
        const [err, partnerGroupData] = await partnerService.getPartnerGroupData(partnerGroupId);
...
// Mock API for state partner group data.
  {
    method: 'GET',
    path: '/partners/mock/{partnerGroupId}/district',
    options: {
      description: 'Mock API for state partner group data',
      tags: ['api'],
      // auth: {
      //   strategy: 'jwt',
      //   scope: getRouteScope('partner'),
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        logger.info(`Mock API for state partner group data`);
        return {
          total_number_of_districts: 3,
          total_number_of_schools: 4,
          total_number_of_students: 800,
          partner_groups_data: [
            {
              partner_group_id: 23,
              partner_group_name: 'ambala_S',
              partners: [
                {
                  partner_id: 82,
                  partner_name: 'GSMM School',
                  users_count: 200,
                },
              ],
            },
            {
              partner_group_id: 22,
              partner_group_name: 'Faridabad_S',
              partners: [
                {
                  partner_id: 92,
                  partner_name: 'GSMM School',
                ...
                {
                  partner_id: 99,
                  partner_name: 'Afghanistan School',
                  users_count: 200,
                },
              ],
            },
            {
              partner_group_id: 24,
              partner_group_name: 'Bhiwani_S',
              partners: [
                {
                  partner_id: 94,
                  partner_name: 'Shri Sai Kendra',
                  users_count: 200,
                },
              ],
            },
