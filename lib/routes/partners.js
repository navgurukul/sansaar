...
module.exports = [
  ...
  {
    method: 'GET',
    path: '/partners/userProgress/{pathway_id}',
    options: {
      description: 'get the batched data by the group_id',
      tags: ['api'],
      auth: {
      ...
      validate: {
        ...
        query: Joi.object({
          user_ids: Joi.array().items(Joi.number().integer().greater(0)),
        }),
      },
      handler: async (request, h) => {
        ...
        try {
          const { partnerService } = request.services();
          const [err, Data] = await partnerService.GetUserProgress(user_ids, pathway_id);
          if (err) {
            logger.error(JSON.stringify(err));
            return h.response(err).code(err.code);
          }
          logger.info('Get a users progress for pathway');
          return Data;
        } catch (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
