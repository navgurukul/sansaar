    ...
    handler: async (request, h) => {
      ...
      try {
        ...
        if (data.length > 1){
          ...
          const csvData = [columns.join(','), ...rows].join('\n');
          const csvFileName = 'output.csv';
          await fs.promises.writeFile(csvFileName, csvData);
          const csvFilePath = Path.resolve(csvFileName);
          logger.error(JSON.stringify(data[0]));
          return h.file(csvFilePath, { mode: 'attachment', filename: csvFileName }).code(403);
          ...
          logger.error(JSON.stringify(data[0]));
          return h.file(csvFilePath, { mode: 'attachment', filename: csvFileName }).code(403);
          // return h.response(data[0]).code(data[0].code);
        }
      } catch (error) {
