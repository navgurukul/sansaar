...
module.exports = [
  ...
  {
    method: 'POST',
    path: '/partners/{batch_id}/addstudent',
    options: {
      description: 'Add the student  to the  batch by batch_id.',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
      ...
      handler: async (request, h) => {
        const { partnerService } = request.services();
        const spaceID = request.params.batch_id;
...
//partners create ====>>>
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        try {
          const { partnerService } = request.services();
          const [err, data] = await partnerService.create(request.payload);
          if (err) {
            logger.error(JSON.stringify(err));
            return h.response(err).code(err.code);
          }
...

  {
    ...
    method: 'POST',
    path: '/partners/uploadstudents/{batch_id}',
    options: {
      payload: {
        output: 'file',
        multipart: true,
      },
      description: 'bulk data uploading the students data to the database with the help of batch_id',
      notes: 'file-upload',
      tags: ['api'],
      // auth: {
      //   strategy: 'jwt',
      ...
      handler: async (request, h) => {
        ...
        try {
          ...
          const [error,results] = await partnerService.readTheFile(csvFile,request.params.batch_id);
...

  {
    ...
    options: {
      description: 'Removing the student from the partner space by the email',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        // scope: getRouteScope(['partner', 'partner_edit']),
...

  {
    ...
    options: {
      description: 'update the exist partner group  by group_id',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        // scope: getRouteScope(['admin','partner_edit']),
...

  {
    ...
    path: '/partners/group/{group_id}',
    options: {
      description: 'Delete the partner space by group_id',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        // scope: getRouteScope(['partner', 'partner_edit']),
