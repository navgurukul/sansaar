...
module.exports = [
  ...
  },
  {
    ...
    options: {
      description: 'Get exercises by exerciseId',
      tags: ['api'],
      auth: { strategy: 'jwt' },
      validate: {
        params: Joi.object({
      ...
      handler: async (request, h) => {
        const { exercisesServiceV2 } = request.services();
        const { exerciseId } = request.params;
        const [err, exercise] = await exercisesServiceV2.getExerciseByExerciseId(exerciseId);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
        logger.info('Get exercises by their exerciseId');
        return exercise;
      },
    },
  },
