    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        if (appVersion >= androidVersions.latest) {
          const [err, marked] = await exercisesService.markExerciseCompleteV2(
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const appVersion = request.headers['version-code'] ? request.headers['version-code'] : null;
        if (appVersion >= androidVersions.latest) {
          const getId = await exercisesService.getIdForRemovalV2(
            request.auth.credentials.id,
            request.params.exerciseId
          );
          if (getId) {
            const deleted = async (txn) => {
              return exercisesService.removeExerciseCompleteV2(
                getId[0].id,
                request.auth.credentials.id,
                request.params.exerciseId,
                txn
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const appVersion = request.headers['version-code'] ? request.headers['version-code'] : null;
        if (appVersion >= androidVersions.latest) {
          const [err, fetchMarked] = await exercisesService.getExerciseCompleteV2(
            request.auth.credentials.id
          );
          if (err) {
            logger.error(JSON.stringify(err));
