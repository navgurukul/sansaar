...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [error, addCourse] = await courseEditorService.postcreateCourses();
        if (error) {
    ...
    options: {
      ...
      validate: {
        payload: Joi.array().items(
          Joi.object({
            ...
            lang_available: Joi.array().items(Joi.string()).optional(),
          })
        ),
        options: { allowUnknown: true },
      },
      handler: async (request, h) => {
        ...
        const addCourse = async () => {
          return request.payload.map((el) => courseEditorService.createNewCourseEditor(el));
        };
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        // request.payload.name = details[0].name;
