...
module.exports = [
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { courseEditorService } = request.services();
        const { courseId } = request.params;
        const [err, courseUpdated] = await courseEditorService.updateCourseEditor(
          courseId,
          request.payload
        );
        if (err) {
...

  {
    ...
    path: '/courseEditor/exercise{exerciseId}',
    options: {
      description: 'Delete the exercise by Id',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
      },
      ...
      handler: async (request, h) => {
        ...
        const [err, deleted] = await courseEditorService.deleteExerciseById(exerciseId);
        if (deleted){
          return deleted
        }
        if (err) {
        ...
        logger.info('Delete exercises using exercise ID');
        return 'Exercise data is not present';
      },
    },
  },
  {
    method: 'POST',
    path: '/courseEditor/{courseId}/exercise',
    options: {
      description: 'Create exercises of the courseEditor',
      tags: ['api'],
      validate: {
        params: Joi.object({
          courseId: Joi.number().integer(),
      ...
      handler: async (request, h) => {
       ...
       const {name,content} = request.payload
        const [
          errorWhileFetchingCoursesDetails,
          coursesDetails,
        ] = await coursesServiceV2.getCourseById(request.params.courseId);
        console.log(coursesDetails,'0000000')
        if (errorWhileFetchingCoursesDetails) {
          logger.error(JSON.stringify(errorWhileFetchingCoursesDetails));
          return h
            .response(errorWhileFetchingCoursesDetails)
        ...
        const [errInUpdating, exercises] = await courseEditorService.addSingleExercise(
          ...
          const insertdata = await courseEditorService.insertExerciseById(courseId,name,content,version)
        return exercises;
      },
    },
  },
