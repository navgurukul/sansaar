...
module.exports = [
  {
    ...
    options: {
      ...
      // },
      handler: async () => {
        const filenames = fs.readdirSync('curriculum_v2');
        const courses = [];
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const fileDir = `curriculum_v2/${courseName}/${details[0].version}/PARSED_CONTENT`;
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { courseEditorService } = request.services();
        if (!fs.existsSync(`curriculum_v2/${request.params.courseName}`)) {
          return {
            error: true,
        ...
        const folderName = `curriculum_v2/${request.params.courseName}/v${version}`;
        ...
        const [errInUpdating, exercises] = await courseEditorService.updateSingleExercises(
          ...
          request.payload,
          `curriculum_v2/${request.params.courseName}/v${details[0].version.split('v')[1]}`
        );
        if (errInUpdating) {
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { courseEditorService, exercisesServiceV2 } = request.services();
        if (!fs.existsSync(`curriculum_v2/${request.params.courseName}`)) {
          return {
            error: true,
            message: 'course name is wrong',
        ...
        const folderName = `curriculum_v2/${request.params.courseName}/${version}`;
        await courseEditorService.copyFolder(
          folderName,
          request.params.courseName,
          details[0].version,
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const fileDir = `curriculum_v2/${courseName}/v${version}/PARSED_CONTENT`;
        const filenames = fs.readdirSync(fileDir);
        const courses = [];
        filenames.forEach((file) => {
          courses.push(file);
