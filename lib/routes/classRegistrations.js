    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        if (request.headers['register-to-all'] && classDetails.recurring_id) {
          ...
          // check if max limit for enrollment is being reached
          const [err, reg] = await classesService.getRegistrationsByClassId(classDetails.id);
          if (err) {
            logger.error('could not fetch class registrations!');
          }
          if (reg[0].count >= classDetails.max_enrolment) {
            throw Boom.badRequest('Enrollment limit exceeded!!');
          }
