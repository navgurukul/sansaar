...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [err, classById] = await classesService.mergedClasses(classId, mergeClassId, h);
        if (err) {
          // eslint-disable-next-line
          logger.error(`classId- ${classId} ` + JSON.stringify(err));
          return h.response(err).code(err.code);
...

  {
    ...
    options: {
      description: 'Gets a list of all upcoming and ongoing batches.',
      tags: ['api'],
      validate: {
        query: Joi.object({
      ...
      handler: async (request, h) => {
        ...
        const [err, batches] = await classesService.getAllPartnerBatch(page, limit, pathwayId, typeOfClass);
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const userId = 20230// request.auth.credentials.id;
        const scope = 'admin' //request.auth.credentials
        const [err, classes] = await classesService.getAllBatchClasses(recruiterId);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
        const data = await displayService.upcomingClassesWithEnrolledKey(classes, userId, scope);
        logger.info('Gets a list of all upcoming batches');
        return data
      },
    },
  },
  {
    ...
    path: '/classes/today',
    options: {
      description: "this will give today classes",
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        scope: getRouteScope(['admin']),
      ...
      handler: async (request, h) => {
        const { classesService } = request.services();
        const [err, classes] = await classesService.mergeWarToday();
        if (err) {
          return h.response(err).code(err.code);
        }   
