...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [err, classes] = await classesService.getClassesForXXXTime(duration);
        if (err) {
  ...
  //       if (userEnrolled && classDetails.recurring_id) {
  //         const [
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const userEnrolled = classesRegisteredByUser.message !== 'enrolled';
        let calendarDetails;
        if (request.query['register-all'] && !userEnrolled && classDetails.recurring_id) {
          /* eslint-enable */
          return h
            .response({
              error: 'true',
        ...
        if (request.query['register-all'] && userEnrolled && classDetails.recurring_id) {
          ...
          if (err) {
            ...
            logger.error(
              // eslint-disable-next-line
              `line no- 238, Calendar event error, id- ${request.auth.credentials.id}: ` +
                JSON.stringify(err)
            );
          }
          if (user[0].email !== data[0].facilitator.email && user[0].chat_id) {
