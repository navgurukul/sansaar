...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [err, classes] = await classesService.getClassesForWeek(userId);
        if (err) {
  ...
  // register with parent calender_id
  {
    method: 'POST',
    path: '/classes/{classId}/register',
    options: {
      ...
      handler: async (request, h) => {
        ...
        // patch attendees in the calendar event
        /* eslint-disable */
        for (const cd of calendarDetails) {
          // removing start_time & end_time because it is stored in UTC in DB and will update the calendar event time
          if (!cd.enrolled) {
            payload.push({ user_id: userId, class_id: cd.id, registered_at: new Date() });
          }
        }
        // outside of the loop
        const emailList = [];
  ...
  // unregister with parent calender_id
  {
    method: 'DELETE',
    path: '/classes/{classId}/unregister',
    options: {
