...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        logger.info(
          `id- ${request.auth.credentials.id}: Gets a list of all classes a particular user registered to ( Meant for dashboard admin )`
        );
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        if (request.headers['register-to-all'] && classDetails.recurring_id) {
          ...
          await chatService.sendInitialMessageToStudent(
            data[0].parent_class.cohort_room_id,
            user[0].id
          );
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { classesService, displayService, chatService, calendarService } = request.services();
        const { classId } = request.params;
