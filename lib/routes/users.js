...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        let [err, user] = await userService.createUser(request.payload.lang);
        if (err) {
        ...
        // Create matrix credentials and chat room asynchronously if lang is sent as request body
        if (request.payload.lang) {
          displayService.createMatrixCredentials(user.name, user.id);
          return { user, token };
        }
        // Create Matrix credentials if user doesn't send lang in request body and send initial message
        ...
        await chatService.sendInitialMessage(room_id);
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        if (request.payload.id === undefined && user.chat_id === null) {
          // Create matrix credentials and chat room asynchronously if lang is sent as request body
          if (request.payload.lang) {
            displayService.createMatrixCredentials(userObj.user.name, userObj.user.id);
