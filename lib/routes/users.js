...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const partnerGroupIds = await userService.getPartnerGroupIds(userObj.user);
        userObj.user.partner_group_ids = partnerGroupIds;
  ...
  {
    method: 'POST',
    path: '/users/enrolment',
    options: {
      description: 'check the student is already enrolled in the cohort class or not',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
      },
      ...
      handler: async (request, h) => {
        const { classesService, userService } = request.services();
        const [err, user] = await userService.getUserByEmail(request.query.email);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
        let FinalResult = 'Not Enrolled';
        let result = true;
        const [
          errWhileFetchingByUserId,
          classesRegisteredByUser,
        ] = await classesService.getRecurringClassesIdByUserId(user[0].id);
        if (classesRegisteredByUser === null) {
          return FinalResult;
        }
        if (errWhileFetchingByUserId) {
          /* eslint-disable */
          logger.error(JSON.stringify(errWhileFetchingByUserId));
          return h.response(errWhileFetchingByUserId).code(errWhileFetchingByUserId.code);
        }
        let i = 0;
        while (i < classesRegisteredByUser.length) {
          const [errWhileFetching, classDetails] = await classesService.getClassById(
          ...
          if (classDetails.recurring_id) {
            const [
              ...
              allRecurringClasses,
            ] = await classesService.getClassesIdByRecurringId(classDetails.recurring_id);
            ...
            _.map(allRecurringClasses, (ClassRegisterdId) => {
              if (classesRegisteredByUser.indexOf(ClassRegisterdId) > -1) {
                classesRegisteredByUser.splice(
                  classesRegisteredByUser.indexOf(ClassRegisterdId),
                  1
                );
              } else {
                result = false;
            ...
            if (result === true) {
              const [
                errWhileFetchingClassDetails,
                singleClassDetails,
              ] = await classesService.getClassById(
                allRecurringClasses[allRecurringClasses.length - 1]
              );
              if (errWhileFetchingClassDetails) {
                /* eslint-disable */
                logger.error(JSON.stringify(errWhileFetchingClassDetails));
                return h
                  .response(errWhileFetchingClassDetails)
                  .code(errWhileFetchingClassDetails.code);
              }
              if (singleClassDetails.pathway_id!=null && singleClassDetails.pathway_id === request.query.pathway_id) {
                ...
                if (lastDate > currDate) {
                  ...
                  FinalResult = `Enrolled but finished`;
                }
              }
            }
          }
