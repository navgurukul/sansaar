  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [errWhileFetchingPathways, dataPathways] = await pathwayServiceV2.find();
        const userEnrolledInBatches = [];
        let volunteerEnrolled=[];
        if (dataPathways !== null && dataPathways !== undefined) {
          // eslint-disable-next-line
          for (const singlePathway of dataPathways) {
            const pathwaysEnrolledDetails = {};
            // eslint-disable-next-line
            const isEnrolledInPathways = await classesService.getIfStudentEnrolled(
              token.decoded.id,
              singlePathway.id
            );
            pathwaysEnrolledDetails.pathwaysId = singlePathway.id;
            pathwaysEnrolledDetails.isEnrolled = false;
            if (isEnrolledInPathways.message === 'enrolled') {
              pathwaysEnrolledDetails.isEnrolled = true;
            }
            userEnrolledInBatches.push(pathwaysEnrolledDetails);
          }
          user.UserEnrolledInBatches = userEnrolledInBatches;
          const volunteerUserId=await userRoleService.volunteerUser(user.id)
          const VolunteerEnrolledPathways_id = {};
          let check=true;
          for(user_id of volunteerUserId){
            let pathways_id=user_id.pathway_id
            if(user_id.user_id !=null && user_id.user_id!=undefined){
              check=false;
               VolunteerEnrolledPathways_id.pathwaysId=pathways_id
            }
          }
          if(check){
            VolunteerEnrolledPathways_id.pathwaysId=null
          }
          volunteerEnrolled.push(VolunteerEnrolledPathways_id)
          user.VolunteerEnrolled=volunteerEnrolled;
        }

        // if (dataPathways !== null && dataPathways !== undefined){
          //   for(let volunteerDetails of dataPathways){
            //     const volunteerAll=await userRoleService.getAllVolunteers(
              //       user
              //     )
              //     console.log(volunteerAll)
              //   }
              // }
              // eslint-disable-next-line
        if (errWhileFetchingPathways) {
          logger.error(JSON.stringify(errWhileFetchingPathways));
          return h.response(errWhileFetchingPathways).code(errWhileFetchingPathways.code);
        }
