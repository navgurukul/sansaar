...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        // console.log(await h.context.transaction());
  ...
  {
    method: 'PUT',
    path: '/users/updateContact/{userId}',
    options: {
      description: 'Edit a user contact details',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
      },
      ...
      handler: async (request, h) => {
        const { userService } = request.services();
        const [err, userUpdate] = await userService.updateContactUserById(
        ...
        // return h.response({ data: userUpdate, msg: 'successfully updated' });
        return userUpdate;
      },
    },
  },
