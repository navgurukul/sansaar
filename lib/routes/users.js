...
module.exports = [
  ...
  },
  {
    ...
    path: '/users/random',
    options: {
      description: 'Get 10 random users with non-null contact information.',
      tags: ['api'],
      handler: async (request, h) => {
        const { userService } = request.services();
        try {
          const users = await userService.getRandomUsersWithContact();
          return h.response({ users }).code(200);
        } catch (error) {
          request.log(['error'], 'Error fetching random users:', error);
