...
//     path: '/users',
//     options: {
//         description: 'Export users with contact information to a CSV file',
//         tags: ['api'],
//         handler: async (request, h) => {
          
//             try {
//                 const { userService } = request.services();
//                 await userService.exportUsersToCSVAndJsonFile(); 
//                 return h.response('User data has been successfully exported to CSV.').code(200);
//             } catch (error) {
//                 return h.response('Failed  users to CSV.').code(500);
//             }
//         },
//     },
// },
  ...
  options: {
    ...
    handler: async (request, h) => {
      try {
        ...
        const { userService } = request.services();
        const [err, data] = await userService.filterByNameOrEmail(request.query);
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        if (userObj === false) {
          return h.response({ error: true, message: "Invalid Token" }).code(400)
        }
