...
module.exports = [
  ...
  },
  {
    ...
    path: '/offline/course',
    options: {
      description: 'Stroing data in local storage.',
      tags: ['api'],
      // auth: {
      //   strategy: 'jwt',
      // },
      handler: async (request, h) => {
        const { userService } = request.services();
        let path = `${__dirname}/../../lib/helpers/classesInfo/offlineCourse.json`;
        const [error, sentData] = await userService.uploadHtmlCssCourseData(path);
        if (error) {
          logger.error(JSON.stringify(error));
          return h.response(error).code(error.code);
        }
        return h.response(sentData).code(200);
      },
    },
  },
];
