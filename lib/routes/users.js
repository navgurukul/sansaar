...
module.exports = [
  ...
  {
    method: 'POST',
    path: '/users/login/meraki_student',
    options: {
      ...
      handler: async (request, h) => {
        const { userService, } = request.services();
        const login_id = request.payload.student_id;
        const password = request.payload.password;
        const [err, student] = await userService.loginMerakiStudent(login_id, password);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
        if(student){
          ...
          logger.info('Login with meraki student credentials successfully');
