...
module.exports = [
  ...
  },
  {
    ...
    path: '/users/me',
    options: {
      description: 'Update the details of current user and we are using for the auto assign partner id',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
      },
    ...
    options: {
      ...
      validate: {
        query: Joi.object({
          ...
          pathway_code: Joi.number().integer().required()
        }),
      },
      handler: async (request, h) => {
        const { userService, displayService,coursesServiceV2 ,pathwayServiceV2 } = request.services();
        console.log('pathway_code', request.query.pathway_code);
        const [errpathwayIDBy, pathwayCourses] = await pathwayServiceV2.pathwayIDBycoursesExercisesAssessmentsIds(
