  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        let tokens;
        // even if by mistake if front-end calls the API more than once, just return the token res
        // eslint-disable-next-line
        const [errInFetchingToken, res] = await userService.getUserTokens(user_id);
        if (res) {
          return res;
        }
