...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      description:
        'Generate JWT for sansaar, using google idToken.',
      tags: ['api'],
      validate: {
        payload: Joi.object({
          idToken: Joi.string().required(),
  ...
  // User routes.js file   <<<<<<<<>>>>>>>> new code 
  {
      method: 'POST',
      path: '/login',
      options: {
        tags: ['api'],
        auth: false, // disable authentication for login page
        validate: {
          payload: Joi.object({
      ...
      handler: async (request, h) => {
        let authToken
        const cookieData = request.state.userCookie || {}; // read cookie data or fallback to an empty object
        const { userService } = request.services();
        const { name, email } = request.payload;
        console.log("name",name)
        const user = await userService.findUserByNameAndEmail(name, email);
        console.log('thsi>>>>>>>>>>>>>>>>>>>>>',user)
        if (!user) {
          return h.response('Invalid credentials').code(401);
        }
        const cookieOptions = {
          ttl: 24 * 60 * 60 * 1000, // Cookie expires in 1 day
          encoding: 'none', // Cookie data is not encoded
          isSecure: true, // Cookie will only be sent over HTTPS
          isHttpOnly: true, // Cookie cannot be accessed via client-side JavaScript
          clearInvalid: false, // Do not clear invalid cookies
          strictHeader: true // Prevent duplicate cookies from being set in the same header
        };
        ...
        if (Object.keys(cookieData).length === 0) {
          ...
          authToken = await userService.createUsersJWT(usersData);
          // console.log(user, 'MMMMMMMMMMMM');
          h.state('userCookie', authToken);
          return 'set the cookies !!!'
        } else {
          let decodedAuth = await userService.verifyGoogleIdToken(cookieData)
          return decodedAuth;
        }
      },
    },
