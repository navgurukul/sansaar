...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request) => {
        ...
        const [err, newUser] = await userService.createUser();
        if (err) {
          return err;
        }
        ...
        const user = { ...newUser, ...chatCredentials };
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request) => {
        const { userService, displayService } = request.services();
        const { artifacts: token } = request.auth;
        const [err, user] = await userService.updateById(token.decoded.id, request.payload.name);
        if (err) {
          return err;
        }
        await displayService.updateMatrixProfile(user);
