...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { displayService, progressTrackingService, pathwayServiceV2 } = request.services();
        const { pathwayId } = request.params;
        const appVersion = request.headers['version-code'] ? request.headers['version-code'] : null;
        ...
        if (appVersion >= androidVersions.latest) {
          ...
          //   return h.response(errInCompleted).code(errInCompleted.code);
          // }
          // if (completedPathway.total_completed_portion === 100) {
          //   pathwayCourses.shouldShowCertificate = true;
          // } else {
          //   pathwayCourses.shouldShowCertificate = false;
          // }
