...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        let partner_id = request.auth.credentials.partner_id;
        // pathways order
        ...
        const [errPartner,partner] = await partnerService.getPartnerThrowUserID(partner_id);
        if (errPartner) {
        ...
        if(partner[0].name != 'Amazon Coding Bootcamp'){
          let updated_pathway = pathways.filter(pathway => pathway.code !== 'ACB');
          return { pathways: updated_pathway };
        }
