  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const { pathwayId } = request.params;
        const appVersion = request.headers['version-code'] || null;
        const statusCodeMap = {
          'Scratch JR': 'true',
          'Google Form': 'false',
          'MS Word': 'false',
        ...
        const fetchLanguages = (languages, langAvailable, pathwayLang) => {
          _.map(langAvailable, (lang) => {
            const matchingLanguage = languages.find((langObj) => langObj.code === lang);
            if (
              matchingLanguage &&
              !_.find(pathwayLang, { code: matchingLanguage.code, label: matchingLanguage.label })
            ) {
              pathwayLang.push(matchingLanguage);
            }
          });
        };

        const processCourses = async (courses, request, pathwayLang) => {
          for (const p of courses) {
            p.completed_portion = 0;
            let markPreQuiz = null;
            ...
            if (request.auth.credentials) {
              ...
              const [errInPercentage, percentage] = await progressTrackingService.getProgressTrackPercentage(
                request.auth.credentials.id,
                p.id
              );
              if (errInPercentage) {
                logger.error(JSON.stringify(errInPercentage));
                throw h.response(errInPercentage).code(errInPercentage.code);
              }
            ...
            fetchLanguages(languages, p.lang_available, pathwayLang);
        ...
        try {
          if (appVersion >= androidVersions.latest) {
            const [errInPathway, pathwayCourses] = await displayService.getPathwayCoursesV2(pathwayId);
            if (errInPathway) return h.response(errInPathway).code(errInPathway.code);

            const pathwayLang = [];
            await processCourses(pathwayCourses.courses, request, pathwayLang);

            pathwayCourses.lang_available = pathwayLang;
          ...
          _.map(pathwayCourses.courses, (c) => {
            ...
            c.lang_available = c.lang_available || ['en'];
          ...
          _.map(pathwayCourses.courses, (p) => {
            fetchLanguages(
              Object.keys(lang_key).map((code) => ({ code, label: lang_key[code] })),
              p.lang_available,
              pathwayLang
            );
          });
          ...
          return h.response(error).code(error.code || 500);
        }
      },
    },
  },
