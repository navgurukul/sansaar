...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        let pathway = await displayService.pathway(pathways);
        _.map(pathway, (p) => {
          const pathway_lang = [];
        ...
        if (request.headers.course_type === 'json') {
          pathway = {
            pathways: _.filter(pathway, (p) => p.course_type === request.headers.course_type),
          };
        }
        if (request.headers.platform !== 'web') {
          pathway = { pathways: _.filter(pathway, (e) => e.code !== 'JVSCPT') };
        }
