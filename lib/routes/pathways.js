...
module.exports = [
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { pathwayServiceV2, progressTrackingService } = request.services();
        const allCourses = []; // all courses id as course_id with or without completed_portion
        const [err, pathway] = await pathwayServiceV2.findById(request.params.pathwayId, true); // true = get pathway with its courses
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
        ...
        let avg = count > 0 ? Math.floor((sum / (count * 100)) * 100) : 0;
