...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        // pathways order
        const pathways_order = ['PRGPYT', 'SPKENG', 'TYPGRU', 'JSRPIT', 'PRCRSE', 'SHCEL', 'ACB'];
        ...
        if (partner_id) {
          ...
          if(partner[0].name === 'amazon coding bootcamp'){
            return { pathways:pathways }
          }else{
            const pathwaysNoAmz = pathways.filter(pathway => pathway.code !== 'ACB');
            return { pathways:pathwaysNoAmz }
          }
        }
        const pathwaysNoAmz = pathways.filter(pathway => pathway.code !== 'ACB');
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        if (appVersion >= androidVersions.latest) {
          ...
          const pathwayCourses = await displayService.pathwayV2(pathway);
          // eslint-disable-next-line
          for (const p of pathwayCourses[0].courses) {
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { pathwayServiceV2, displayService, progressTrackingService } = request.services();
        const allCourses = []; // all courses id as course_id with or withour completed_portion
        const [err, pathway] = await pathwayServiceV2.findById(request.params.pathwayId);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
        const pathwayData = await displayService.pathwayV2(pathway);
        let sum = 0;
        let count = 0;
        // eslint-disable-next-line
        for (const course of pathwayData[0].courses) {
          const courseId = {}; // single course id as course_id with completed_portion
          if (request.auth.credentials !== null) {
            const [
              errInPercentage,
              percentage,
            ...
            courseId.completed_portion = percentage;
            sum += percentage;
            count += 1;
        ...
        if(avg === 100){
          const [errInCompleted, completed] = await pathwayServiceV2.getPathwayComplete(
            request.auth.credentials.id,request.params.pathwayId
          );
          if (errInCompleted) {
            logger.error(JSON.stringify(errInCompleted));
            return h.response(errInCompleted).code(errInCompleted.code);
          }
          return { pathway: allCourses, total_completed_portion: avg ,complete_at: completed }
        }
        logger.info('Get a single pathway');
        return { pathway: allCourses, total_completed_portion: avg ,complete_at : null};
      },
    },
  },
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { displayService, progressTrackingService } = request.services();
        const { pathwayId } = request.params;
        const appVersion = request.headers['version-code'] ? request.headers['version-code'] : null;
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [err, pathwaysData] = await pathwayServiceV2.find();
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
        // eslint-disable-next-line
        for (const pathway in pathwaysData) {
          // eslint-disable-next-line
          const [errPathwaysTopic, pathwaysTopic] = await pathwayServiceV2.findPathwaysOngoingTopic(
            userId,
            pathwaysData[pathway].id
          );
          if (errPathwaysTopic) {
            logger.error(JSON.stringify(err));
            return h.response(err).code(err.code);
          }
          if (pathwaysTopic !== null && pathwaysTopic !== undefined && pathwaysTopic.length > 0) {
            const topic = {};
            topic.exercise_id = pathwaysTopic[0].exercise_id;
            topic.assessment_id = pathwaysTopic[0].assessment_id;
            topic.course_id = pathwaysTopic[0].course_id;
            topic.pathway_id = pathwaysTopic[0].pathway_id;
            pathwaysOngoingTopic.push(topic);
          }
        }
        return pathwaysOngoingTopic;
      },
    },
  },

  {
    ...
    path: '/pathways/{pathwayId}/courses',
    options: {
      description: 'Add courses in a pathway',
      tags: ['api'],
      auth: {
        strategy: 'jwt',
        scope: getRouteScope('admin'),
      ...
      handler: async (request, h) => {
        ...
        if (appVersion >= androidVersions.latest) {
          const updateAndFetch = async () => {
            const [err, updatedPathway] = await pathwayServiceV2.insertPathwayCourses(
              pathwayId,
              courseIds
            );
            if (err) {
          ...
          logger.info(`id- ${request.auth.credentials.id} Add courses in a pathway`);
          return pathwayCourses;
        }

        const updateAndFetch = async () => {
          const [err, updatedPathway] = await pathwayService.insertPathwayCourses(
            pathwayId,
            courseIds
          );
          if (err) {
  ...
  {
    method: 'GET',
    path: '/pathways/class/{pathway_id}',
    options: {
      ...
      handler: async (request, h) => {
        const { pathwayServiceV2 } = request.services();
        const pathway_id = request.params.pathway_id;
        const space_id = request.auth.credentials.space_id;
        const [err, classesData] = await pathwayServiceV2.gettingUpcomingClasses(pathway_id, space_id);
