...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [err, data] = await (useV2 ? pathwayServiceV2 : pathwayService).find(true); // true = get pathways with their courses
        if (err) {
        ...
        if (partner_id) {
          ...
          if (partner[0].name.toLowerCase() !== 'amazon coding bootcamp') {
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const appVersion = request.headers['version-code'] ? request.headers['version-code'] : null;
        // pathways order
        const pathways_order = ['PRGPYT', 'SPKENG', 'TYPGRU', 'JSRPIT', 'PRCRSE', 'SHCEL', 'TCBPI'];
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { pathwayServiceV2, progressTrackingService } = request.services();
      ...
      handler: async (request) => {
        ...
        if (data.message === 'enrolled') {
          ...
          if (upcomingRecurringClasses !== null && upcomingRecurringClasses.length > 0) {
            ...
            allClasses.some(item => {if(item.id != upcomingRecurringClasses[0].id){
        ...
        if (doubtClass !== undefined && doubtClass !== null && doubtClass.length > 0) {
          // checking the classes or dublication classes becaus we want only one class
          allClasses.some(item => {if(item.id != doubtClass[0].id){
        ...
        if (data.message === 'enrolled' || data.message === 'enrolled_but_finished') {
          ...
          if (classes !== null && classes.length > 0) {
            ...
            if (parentClasses !== undefined && parentClasses !== null && parentClasses.length > 0) {
              ...
              // checking the classes or dublication classes becaus we want only one class
              allClasses.some(item => {if(item.id != classes[0].id){
      ...
      handler: async (request, h) => {
        ...
        for (const pathway in pathwaysData) {
          ...
          if (pathwaysTopic !== null && pathwaysTopic !== undefined && pathwaysTopic.length > 0) {
            ...
            topic.pathway_name = pathwaysTopic.name;
            topic.logo = pathwaysTopic.logo;
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { pathwayServiceV2 } = request.services();
        let [err,dataOfpathway] = await pathwayServiceV2.pathwaysNames()
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
