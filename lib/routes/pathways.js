...
module.exports = [
      ...
      handler: async (request, h) => {
        ...
        for (const course of pathway.courses) {
          ...
          if (request.auth.credentials !== null) {
            ...
            if(courseId.completed_portion > 100){
              courseId.completed_portion = 100;
            }
            sum += percentage;
            count += 1;
...

  {
    ...
    handler: async (request, h) => {
      ...
      const [err, pathway] = await pathwayServiceV2.getC4CAPathway()
      if (err) {
        logger.error(JSON.stringify(err));
        return h.response(err).code(err.code);
      }
  ...
  {
    method: 'GET',
    path: '/pathways/{pathwayId}/c4ca/modules',
    options: {
    ...
    handler: async (request, h) => {
      ...
      const [errInpathwayModule, pathwayModule] = await pathwayServiceV2.getModuleC4CA(pathwayId,user_id)
      if (errInpathwayModule) {
        logger.error(JSON.stringify(errInpathwayModule));
        return h.response(errInpathwayModule).code(errInpathwayModule.code);
      }
