...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        let [err, data] = await (useV2 ? pathwayServiceV2.find(true) : pathwayService.find()); // true = get pathways with their courses

        // If V2 fails or returns null, fallback to V1
        if ((err || !data || !Array.isArray(data)) && useV2) {
          logger.warn('PathwayServiceV2 failed or returned null, falling back to V1');
          [err, data] = await pathwayService.find();
        }
        ...
        // Add null check to prevent "pathway is not iterable" error
        if (!pathway || !Array.isArray(pathway)) {
          logger.error('Pathways data is not an array or is null');
          return h.response({ error: 'Failed to fetch pathways' }).code(500);
        }
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        // Try V2 first, fallback to V1 if it fails
        let [err, data] = await (useV2 ? pathwayServiceV2 : pathwayService).find();
        ...
        // Final null check before forEach
        if (!data || !Array.isArray(data)) {
          logger.error('Pathways data is not an array or is null in dropdown endpoint after fallback');
          return h.response({ error: 'Failed to fetch pathways' }).code(500);
        }
        ...
        if (appVersion >= androidVersions.latest) {
          ...
          for (const p of pathwayCourses.courses) {
            ...
            if (request.auth.credentials !== null) {
              ...
              // for now removing the key completed_portion from the courses API, because there is no use of this key, and this is confirmed by Roshini & Aman
              // const [
              //   errInPercentage,
              //   percentage,
              //   // eslint-disable-next-line
            ...
            if (
              (employeeType === 'teacher' ||
                employeeType === 'mentor_teacher' ||
                employeeType === 'school_inspector') &&
              pathwayCourses.code === "TCBPI2"
            ) {
              // All courses are mandatory for these roles
              p.isMandatory = 'true';
            } else if (
              (employeeType === 'clerical_staff' ||
                employeeType === 'principal') &&
              pathwayCourses.code === "TCBPI2"
            ) {
              // Only "Scratch JR" is not mandatory for these roles
              p.isMandatory = (p.name === 'Scratch JR') ? 'false' : 'true';
            } else {
              // For everyone else, nothing is mandatory
              p.isMandatory = 'false';
            }
