...
module.exports = [
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const partner_id = request.auth.credentials?.partner_id || null;
        const pathwaysNoAmz = pathways.filter(pathway => pathway.code !== 'ACB');
        let showAmazonBootcamp = false;
        ...
        if (partner_id) {
          ...
          if(partner[0].name === 'Amazon Coding Bootcamp'){
            showAmazonBootcamp = true;
          }
        }
        logger.info('List of all Pathways');
        return { pathways: showAmazonBootcamp ? pathway : pathwaysNoAmz };
      },
    },
  },
  {
