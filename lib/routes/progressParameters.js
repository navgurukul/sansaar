...
module.exports = [
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        logger.info(
          `id- ${request.auth.credentials.id} Updated Learning track status successfully!`
        );
  ...
  {
    method: 'GET',
    path: '/progressTracking/learningTrackStatus/{courseId}',
    options: {
      description: 'Status of all learning track.',
      tags: ['api'],
      auth: { strategy: 'jwt' },
      validate: {
        params: Joi.object({
      ...
      handler: async (request, h) => {
        ...
        const [err, data] = await progressTrackingService.getLearningTrackStatus(userId, courseId);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
  ...
  {
    method: 'GET',
    path: '/courseProgressTracking/{courseId}',
    options: {
      description: 'Status of all learning track.',
      tags: ['api'],
      auth: { strategy: 'jwt' },
      ...
      handler: async (request, h) => {
        ...
        const { courseId } = request.params;
        const userId = request.auth.credentials.id;
        const [err, data] = await progressTrackingService.getProgressTrackingStatus(
          userId,
          courseId
        );
        if (err) {
