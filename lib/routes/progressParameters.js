    ...
    options: {
      ...
      validate: {
        payload: Joi.object({
          ...
          course_index: Joi.number().integer(),
        }),
      },
      handler: async (request, h) => {
        ...
        /* eslint-disable */
        const user_id = (payload.user_id = request.auth.credentials.id);
  ...
  },
  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        const [err, data] = await progressTrackingService.getLearningTrackStatus(userId);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
