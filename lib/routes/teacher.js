...
module.exports = [
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        const { teacherService, pathwayServiceV2, displayService } = request.services();
        const { page, limit } = request.query;

        try {
          const [errInPathway, checkPathwayID] = await displayService.getPathwayCoursesV2(
            request.params.pathway_id
          );

          if (errInPathway) return h.response(errInPathway).code(errInPathway.code);
          if (checkPathwayID === null || checkPathwayID === undefined || checkPathwayID.code !== "TCBPI2") {
