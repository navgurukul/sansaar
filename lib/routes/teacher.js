...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        try {
          const teacher = await teacherService.getUserByEmail(email);
          if (teacher.error) {
            return h.response({ error: teacher.error }).code(400);
          }
          const courses = await teacherService.getCoursesByEmployeeType(teacher.employee_type);
          if (courses.error) {
            return h.response({ error: courses.error }).code(400);
          }
