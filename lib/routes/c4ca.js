...
module.exports = [
    ...
    {
        method: 'GET',
        path: '/c4ca/{facilitator_id}/teacher',
        options: {
          description: 'get the teachers details by the facilitator_id',
          tags: ['api'],
        //   auth: {
        //     strategy: 'jwt',
        ...
        //   },
          ...
          handler: async (request, h) => {
            try {
              const { c4caService } = request.services();
              const { facilitator_id } = request.params;
              const [err, Data] = await c4caService.getTeachersDataBy(facilitator_id);
              if (err) {
                logger.error(JSON.stringify(err));
                return h.response(err).code(err.code);
              }
