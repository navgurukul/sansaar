...
module.exports = [
    ...
    {
        method: 'GET',
        path: '/c4ca/teams/byDistrict/{district}',
        options: {
            description: 'Get facilitators data by partner id',
            tags: ['api'],
            auth: {
            ...
            handler: async (request, h) => {
                const { c4caService } = request.services();
                console.log(request.query.district,'district');
                const [err, resp] = await c4caService.getTeamsByDistrict(request.query.district);
                if (err) {
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null, err.message);
                }
