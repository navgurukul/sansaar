...

    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                ...
                const [err, resp] = await c4caService.uploadProjectTopic(request.payload.project_topic, team_id, project_title, project_summary, is_submitted, module_id,request.payload.projectTopic_file_name);
                if (err) {
                    logger.error(JSON.stringify(err));
