...
module.exports = [
        ...
        options: {
            ...
            validate: {
                payload: Joi.object({
                    ...
                    profile_url: Joi.any().meta({ swaggerType: 'file' }).description('file'),
        ...
        options: {
            ...
            validate: {
                payload: Joi.object({
                    ...
                    project_topic: Joi.any().meta({ swaggerType: 'file' }).description('file').required(),
                }),
            },
            handler: async (request, h) => {
                const { c4caService } = request.services();
                ...
                const [err, resp] = await c4caService.uploadProjectTopic(request.payload.project_topic, team_id, project_title, project_summary);
        ...
        options: {
            ...
            validate: {
                payload: Joi.object({
                    ...
                    project_file_url: Joi.any().meta({ swaggerType: 'file' }).description('file').required(),
                }),
            },
            handler: async (request, h) => {
                const { c4caService } = request.services();
                const team_id = request.auth.credentials.team_id;
                //    uplaod project_uploadFile in s3
                const [err, resp] = await c4caService.uploadProjectSubmit(request.payload.project_file_url, team_id, request.payload.project_link);
                if (err) {
                    logger.error(JSON.stringify(err));
                    return h.response(err).code(err.code);
                }
    ...
    {
        method:'GET',
        path:'/c4ca/projectTopic_count',
        options:{
            description:'get project topic count',
            tags:['api'],
            auth:{
                strategy:'jwt',
            },
            handler:async(request,h)=>{
                const {c4caService}=request.services();
                const [err,resp]=await c4caService.getProjectTopicCount();
                if(err){
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null,err.message);
                }
    ...
    {
        method:'GET',
        path:'/c4ca/projectSubmit_count',
        options:{
            description:'get project submit count',
            tags:['api'],
            auth:{
                strategy:'jwt',
            },
            handler:async(request,h)=>{
                const {c4caService}=request.services();
                const [err,resp]=await c4caService.getProjectSubmitCount();
                if(err){
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null,err.message);
                }
