...
module.exports = [
    {
        ...
        options: {
            payload: {
                ...
                maxBytes: 1048576, // Adjust the maximum file size as needed
              },
            ...
            handler: async (request, h) => {
                ...
                const [err, resp] = await c4caService.setTeacherProfile(request.payload, user_id,profileImage);
                if (err) {
    ...
    },
    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                const { c4caService } = request.services();
                const user_id = request.auth.credentials.id;
                const [err, resp] = await c4caService.addStudentsToTeam(request.payload, user_id,request.payload.team_id);
                if (err) {
                    logger.error(JSON.stringify(err));
                    return h.response(err).code(err.code);
                }
...

    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                ...
                const token = await c4caService.createToken({ id: resp.id, login_id: resp.login_id });
                return token;
            },
        },
    },
...

{
    ...
    options: {
      ...
      handler: async (request, h) => {
        ...
        try {
          const resp = await c4caService.uploadProject(payload, request.server); // Pass 'request.server'
