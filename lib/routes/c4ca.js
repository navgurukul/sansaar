...
module.exports = [
        ...
        options: {
            ...
            validate: {
                payload: Joi.object({
                    ...
                    facilitator_id: Joi.number().integer().optional(),
                    // profile_link: Joi.string().optional(),
                })
            },
            handler: async (request, h) => {
                ...
                // const { partner_id } = request.payload.credentials; // activate this line when partner url get fix
                const partner_id = 112; // and remove this line 
...

    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                ...
                return await c4caService.responseWrapper(null, "you are not a valid teacher");
            },
        },
    },
...

    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                ...
                return await c4caService.responseWrapper(null, 'login id or password incorrect')
            },
        },
    },
        ...
        options: {
            ...
            validate: {
                payload: Joi.object({
                    ...
                    teacher_invite_link: Joi.string().required(),
            ...
            handler: async (request, h) => {
                const { c4caService } = request.services();
                const [err, resp] = await c4caService.createFacilitator(request.payload);
                if (err) {
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null, err.message);
                }
...

    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                const { c4caService } = request.services();
                const [err, resp] = await c4caService.getTeamsByDistrictOrSchool(request.query.district, request.query.school, request.query.page, request.query.limit);
                if (err) {
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null, err.message);
                }
...

    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                ...
                const project_title = request.payload.project_title;
                const project_summary = request.payload.project_summary;
                const is_submitted = request.payload.is_submitted;
                //    uplaod project_uploadFile in s3
                const [err, resp] = await c4caService.uploadProjectTopic(request.payload.project_topic, team_id, project_title, project_summary,is_submitted);
                if (err) {
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null, err.message);
                }
...

    {
        ...
        options: {
            ...
            handler: async (request, h) => {
                ...
                const team_id = request.auth.credentials.team_id;
                //    uplaod project_uploadFile in s3
                const [err, resp] = await c4caService.uploadProjectSubmit(request.payload.project_file_url, team_id, request.payload.project_link,request.payload.is_submitted);
                if (err) {
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null, err.message);
                }
...

    {
        ...
        options:{
            ...
            handler:async(request,h)=>{
                const {c4caService}=request.services();
                const team_id=request.auth.credentials.team_id;
                const [err,resp]=await c4caService.getProjectTopicByTeamId(team_id);
                if(err){
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null,err.message);
                }
...

    {
        ...
        options:{
            ...
            handler:async(request,h)=>{
                const {c4caService}=request.services();
                const team_id=request.auth.credentials.team_id;
                const [err,resp]=await c4caService.getProjectSubmitByTeamId(team_id);
                if(err){
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null,err.message);
                }
    ...
    {
        method:'POST',
        path:'/c4ca/projectTopic/unlockedDate',
        options:{
            ...
            handler:async(request,h)=>{
                const {c4caService}=request.services();
                const team_id=request.auth.credentials.team_id;
                const [err,resp]=await c4caService.shareProjectTopicTime(team_id);
                if(err){
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null,err.message);
                }
    ...
    {
        method:'POST',
        path:'/c4ca/projectSubmit/unlockedDate',
        options:{
            ...
            handler:async(request,h)=>{
                const {c4caService}=request.services();
                const team_id=request.auth.credentials.team_id;
                const [err,resp]=await c4caService.shareProjectSubmitTime(team_id);
                if(err){
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null,err.message);
                }
    ...
    },
    {
        ...
        options:{
            ...
            handler:async(request,h)=>{
                ...
                return await c4caService.responseWrapper(response_,"success");
            }
        }
    },
