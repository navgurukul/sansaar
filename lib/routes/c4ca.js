...
module.exports = [
    ...
    {
        method: 'GET',
        path: '/c4ca/teacher/getAll',
        options: {
            description: 'Get all teachers data by district and school',
            tags: ['api'],
            auth: {
                strategy: 'jwt',
            },
            ...
            handler: async (request, h) => {
                const { c4caService } = request.services();
                const [err, resp] = await c4caService.getTeachersByDistrictAndSchool(request.query.district, request.query.school);
                if (err) {
                    logger.error(JSON.stringify(err));
                    return c4caService.responseWrapper(null, err.message);
                }
