      ...
      validate: {
        payload: Joi.object({
          contact: Joi.string()
            ...
            .required(),
          pathway_id: Joi.array().items(Joi.number().integer().valid(1, 2)),
...

  {
    ...
    options: {
      ...
      handler: async (request, h) => {
        // request.auth.credentials.id
        const { userRoleService } = request.services();
        const [err, store] = await userRoleService.CheckNumberRegisteredOrNot(request.auth.credentials.id);
        if (err) {
          logger.error(JSON.stringify(err));
          return h.response(err).code(err.code);
        }
