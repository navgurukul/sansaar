      ...
      _.map(exerciseFiles, (file, index) => {
        ...
        if(file.type === 'text') {
          if (file.text.match(/.md/g)) {
            topic['fileName'] = file.text;
            topic['childExercise'] = [];
            this.exerciseFiles.push(topic);
          }
        } else if (file.type === 'list') {
          topic['fileName'] = exerciseFiles[index - 1].text;
          topic['childExercise'] = [];
          _.map(file.items, (childExerciseFile) => {
            topic['childExercise'].push(childExerciseFile.text)
          });
          this.exerciseFiles.push(topic)
        }
      })
  ...
  async parseChildExercise(file, childExerciseFile) {
    ...
    _.map(tokens, (exerciseContent) => {
      if (!exerciseContent.raw.match(/```ngMeta/g)) {
        content = content + exerciseContent.raw
      }
      if (exerciseContent.lang === 'ngMeta') {
        exercise['name'] = exerciseContent.text.replace('name: ', '').replace('\nsubmission_type: url', '');
      }
    });
    exercise['course_id'] = this.courseId
    exercise['content'] = content;
