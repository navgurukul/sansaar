...
function options(dynamic) {
  ...
  const mappedOptions = dynamic.map((option) => {
    ...
    if (!optionValue || optionValue.length === 0) {
      // console.error('Invalid optionValue:', optionValue);
      return null; // or handle the error in a way that makes sense for your application
    }
    const firstBlock = optionValue[0].blocks && optionValue[0].blocks[0];
    if (!firstBlock || !firstBlock.data || !firstBlock.data.text) {
      // console.error('Invalid block structure:', optionValue);
      return null; // or handle the error
    }
    if (optionValue[0].type === 'image') {
      const imageUrl = optionValue[0].data.file && optionValue[0].data.file.url;
      if (!imageUrl) {
        // console.error('Invalid image URL structure:', optionValue);
        return null; // or handle the error
      }
      return {
        id: option.number,
      ...
      const text = firstBlock.data.text.trim();
      return {
        id: option.number,
        type: 'text',
      ...
      // when the option has both text and image at the same time
      const text = optionValue.blocks[0].data.text.trim();
      const imageUrl = optionValue.blocks[1].data.file && optionValue.blocks[1].data.file.url;
      if (!imageUrl) {
        // console.error('Invalid image URL structure:', optionValue);
        return null; // or handle the error
      }
...
function optionsV2(option) {
  ...
  const mappedOptionsV2 = option.map((opt) => {
    ...
    if (optionValue[0].type === 'image') {
      const imageUrl = optionValue[0].image.url;
      return {
        id: opt.number,
        option_type: 'image',
        value: `${imageUrl}`,
