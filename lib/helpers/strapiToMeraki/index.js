...
const strapiToMerakiConverter = (blockData) => {
  ...
  for (const comp of blockData) {
    switch (comp.type) {
      ...
      case 'embed':
        //for supported youtube URL --> a= 'https://www.youtube.com/watch?v=iuk77TjvfmE', b='https://youtu.be/iuk77TjvfmE'
        translatedData.push({
          component: 'youtube',
          value: comp.data.source.includes('https://www.youtube.com')
            ? a.split('/watch?v=')[1].split('&t=')[0]
            : comp.data.sourcesplit('youtu.be/')[1],
        });
        break;
