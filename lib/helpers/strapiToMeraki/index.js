...
function extractTextFromPTag(element) {
  ...
  if (!isNested) {
    // Check if the 'p' tag contains only text nodes
    const containsOnlyText = element.children().length === 0 && element.contents().length > 0 && element.contents().filter((index, el) => el.type === 'text').length > 0;
