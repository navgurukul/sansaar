...
const strapiToMerakiConverter = (htmlContent) => {
  ...
  $('h2, h4, p, pre, ol, ul, strong, em, img, div[data-youtube-video], iframe, table').each((index, element) => {
    ...
    switch (elementType) {
      ...
      case 'p':
        const nestedLists = $(element).find('ol, ul');
        if (nestedLists.length) {
          nestedLists.each((index, list) => {
            translatedData.push(...extractListItems(list)); // Use spread syntax to flatten array
          });
        } else {
          const hasNestedTags = $(element).children().not('br').length > 0;
          if (!hasNestedTags) {
