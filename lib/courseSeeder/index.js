...
class CoursesSeeder {
  ...
  async init() {
    ...
    if (this.addUpdateSingleCourse) { // addUpdate single course
  ...
  async parseExerciseDetails(file, folder, courseId) {
    ...
    if (file.childExercise.length) {
      ...
      if (fs.existsSync(path)) {
        ...
        exercise.slug = path.split(this.courseDir+"/").slice(-1)[0].replace('/', '__').replace('.md', '');
        exercise.github_link = `https://github.com/navgurukul/newton/tree/master/${path.split(this.courseDir+"/").slice(-1)[0]}`
  ...
  async parseChildExercise(path, courseId, childExerciseFile) {
    ...
    if (fs.existsSync(childExercisePath)) {
      ...
      exercise.course_id = courseId;
      exercise.content = content;
      exercise.slug = childExercisePath.split( this.courseDir+"/" ).slice(-1)[0].replace('/', '__').replace('.md', '');
      exercise.github_link = `https://github.com/navgurukul/newton/tree/master/${childExercisePath.split(this.courseDir+"/").slice(-1)[0]}`
      if (exercise.name.length <= 100) {
        return exercise;
      }
      return null;
  ...
  async addCourse() {
    ...
    _.map(this.courseDetails, (course) => {
      if (course) {
        if (!course.id) {
          delete course.id;
          promises.push(axios.post(`http://localhost:5000/courses`, course))
        }
      }
    });
    ...
    _.map(this.exercises, (exercises) => {
      _.map(exercises.exerciseDetails, (exercise) => {
        if (!exercise.childExercise && exercise.course_id) {
          promises.push(axios.post('http://localhost:5000/exercises', { exercise }))
        } else {
          if (exercise.childExercise) {
            const childExercise = exercise.childExercise.filter(x => x);
            promises.push(axios.post('http://localhost:5000/exercises', { childExercise }))
          }
        }
      })
    });
  ...
  async getCourseId(name) {
    const res = await axios.get(`http://localhost:5000/courses/name`, { params: { name: name } })
  ...
  static getCourseName() {
    if (process.argv.indexOf('--addUpdateSingleCourse') > -1) {
      ...
      try {
        ...
        this.showErrorAndExit(`The specified courseName ${courseName} is doesn't exist in curriculum.`);
      }
    }
    return false;
  }
...
if (!module.parent) {
  ...
  let seeder = new CoursesSeeder(addUpdateSingleCourse, courseName);
  seeder.init()
    .then((res) => {
      if (res) {
