...
class CoursesSeeder {
  ...
  async parseExerciseDetails(file, folder, courseId) {
    ...
    if (file.childExercise.length) {
      ...
      if (fs.existsSync(path)) {
        ...
        exercise.github_link = `https://github.com/navgurukul/newton/tree/master/${path.replace(
          '../../curriculum/',
          ''
        )}`;
  ...
  async parseChildExercise(path, courseId, childExerciseFile) {
    ...
    if (fs.existsSync(childExercisePath)) {
      ...
      exercise.slug = childExercisePath
        ...
        .replace('/', '__')
        .replace('.md', '');
      exercise.github_link = `https://github.com/navgurukul/newton/tree/master/${childExercisePath.replace(
        '../../curriculum/',
        ''
      )}`;
      if (exercise.name.length <= 100) {
  ...
  async addCourse() {
    ...
    _.map(this.courseDetails, (course) => {
      if (course) {
        if (!course.id) {
          delete course.id;
          promises.push(coursesService.createNewCourse(course, ''));
    ...
    _.map(this.exercises, (exercises) => {
      _.map(exercises.exerciseDetails, (exercise) => {
        if (!exercise.childExercise && exercise.course_id) {
          promises.push(ExercisesService.upsertExercise({ exercise }, ''));
          ...
          promises.push(ExercisesService.upsertExercise({ childExercise }));
  ...
  async getCourseId(name) {
    console.log(name);
    const res = await coursesService.findByCourseName(name);
    // const res = await axios.get(`http://localhost:5000/course/name`, { params: { name } });
