...
class CoursesSeeder {
    ...
    _.map(this.courseDetails, (course) => {
      if (course) {
        if (!course.id) {
          delete course.id;
          promises.push(
            axios.post(`${CONFIG.seeder.seedUrl}/${CONFIG.seeder.courseSeedApi}`, course)
          );
        }
      }
    });
    ...
    _.map(this.exercises, (exercises) => {
      _.map(exercises.exerciseDetails, (exercise) => {
        if (!exercise.childExercise && exercise.course_id) {
          promises.push(axios.post(`${CONFIG.seeder.seedUrl}/exercises`, { exercise }));
        } else if (exercise.childExercise) {
          const childExercise = exercise.childExercise.filter((x) => x);
          promises.push(axios.post(`${CONFIG.seeder.seedUrl}/exercises`, { childExercise }));
        }
      });
    });
    await Promise.all(promises);
  ...
  async getCourseId(name) {
    const res = await axios.get(`${CONFIG.seeder.seedUrl}/courses/name`, { params: { name } });
