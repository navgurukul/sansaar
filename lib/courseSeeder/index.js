...
class CoursesSeeder {
  ...
  async init() {
    if (this.addUpdateSingleCourse) {
      ...
      const promises = [];
      // TODO get the list below using an API call
      // TODO const updateCourseNames = ["Date Types", "English Movies"];
      _.map(this.allCourseFolder, (folder) => {
        // const course = parseCourseDir(folder, updateCourseNames);
        // if (course) {
        //  promise wala statement
        // }
        promises.push(this.parseCourseDir(folder)); 
        //TODO send these to parseCourseDir, updateCourseNames));
        //TODO only insert into promise if the return of parseCourseDir isn't
        // false
      });
  ...
  async parseCourseDir(folder, updateCourseNames) {
    ...
    if (fs.existsSync(path)) {
      ...
      if (courseDetails) {
        ...
        // TODO if (updateCourseNames.length>0) {
        // if (updateCourseNames.indexof(courseDetails.name)==-1) {
        //    return false;
        //  }
        // }
        //
          ...
          _.map(exerciseFiles, (file, index) => {
            ...
            if (file.type === 'text') {
              ...
              _.map(file.items, (childExercisesFile, innerIndex) => {
                allExerciseFile.childExercises.push({fileName: childExercisesFile.text, sequenceNum: outerIndex*100+innerIndex});
              });
              exerciseFile.push(allExerciseFile);
            }
          });
