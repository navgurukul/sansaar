...
class CoursesSeeder {
  ...
  async parseCourseDir(folder, updateCourseNames) {
    ...
    console.log('@@@@@@@!!!!!!!!!!$$$$$$$%%%%%%%%%%$$$$$!!!!!!!@@@@@\n\n');
    // console.log(path, "pathhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh\n\n");
    ...
    if (fs.existsSync(path)) {
      ...
      if (courseDetails) {
        ...
        // console.log(items[0]["tokens"][1]["items"][0]["tokens"][0], "item\n\n\n\n\n\n\n\n" )
        _.map(items, (item, outerIndex) => {
          const exerciseFiles = item.tokens;
          // console.log(item["tokens"][1]["items"][0]["tokens"][0]["tokens"], "exerciseFiles\n\n")
          // console.log(exerciseFiles, "exerciseFilesexerciseFiles\n\n\n");
          _.map(exerciseFiles, (file, index) => {
            // console.log(file, "fileeeeeeeeeee\n\n")
            const allExerciseFile = {};
            if (file.type === 'text') {
              // console.log(file.text.match(/.md/g), "file.text.match(/.md/gggggggg)\n\n");
              if (file.text.match(/.md/g)) {
                ...
                // console.log(exerciseFile.push(allExerciseFile), "exerciseFile.push(allExerciseFile)\n\n")
              }
            } else if (file.type === 'list') {
              // console.log(file.items[0]["tokens"][0], "fileeeeeeeeeee if type = list\n\n\n\n\n")
              allExerciseFile.fileName = exerciseFiles[index - 1].text.trim();
              allExerciseFile.childExercises = [];
              _.map(file.items, (childExercisesFile, innerIndex) => {
                // console.log(childExercisesFile, "childExercisesFilechildExercisesFile\n\n\n\n");
        ...
        console.log(exerciseDetails, 'exerciseDetailssssssssss\n\n');

        const modifiedKeys = exerciseDetails[0];
        const mdProperties = exerciseDetails[1];

        const clientreqDefault = 'en.properties';
        const clientreq = 'hi.properties';

        const langFilter = mdProperties['childExercises'].filter((x) =>
          x.name.includes(clientreqDefault)
        );
        console.log(langFilter, 'langffff\n\n');
        const parseProperties = JSON.parse(langFilter[0]['content'])[0].value.split('\n');
        const parsePropertiesTitle = modifiedKeys['childExercises'][0]['name'];
        var finalKeys = JSON.parse(modifiedKeys['childExercises'][0]['content'])[0].value;

        const ngMetaStr = '```ngMeta\n' + parsePropertiesTitle + '\n```\n';
        // console.log(finalKeys, "parseProperties\n\n");
        var finalKeys = ngMetaStr + finalKeys;
        for (var keyAndValue of parseProperties) {
          const [key, value] = keyAndValue.split('=');
          // console.log([key]);
          // console.log([value, "value"])
          // console.log(keyAndValue, "keyAndValue\n\n");
          var finalKeys = finalKeys.replace(key, value);
        }
        console.log(finalKeys, 'finalKeys\n\n');

        course.exerciseDetails = exerciseDetails;
        this.exercises.push(course);
        // console.log(this.exercises[0]['exerciseDetails'][1], 'this.exercises\n\n');
      }
    } else {
  ...
  async parseCourseDetails(path) {
    if (fs.existsSync(path)) {
      const data = fs.readFileSync(path);
      const tokens = marked.lexer(data.toString());
      //   console.log(tokens, "tokenssssssssssss\n\n");
      const courseDetails = {};
      ...
      //   console.log(courseId, "courseIddddddddddddddddd");
  ...
  async parseExerciseDetails(file, folder, courseId) {
    // console.log(file, "file", folder, "folder", courseId, "courseId\n\n");
    ...
    if (file.childExercises.length) {
      ...
      if (fs.existsSync(path)) {
        ...
        const tokens = marked.lexer(data.toString());
        // console.log(tokens, "tokenssssssssssPTRRRRRRRRRRR\n\n");
        _.map(tokens, (exer) => {
          if (exer.lang === 'ngMeta') {
    ...
    // const childExerKeys = await JSON.parse(childExercises[0]["content"])[0].value
    // console.log(childExercises, "childExercises$$$$$$$$$$\n\n\n\n\n\n\n");
    return { childExercises };
  }

  // eslint-disable-next-line
  async parseChildExercise(path, courseId, childExercisesFile) {
    // console.log(path, "path", courseId, "courseId", childExercisesFile, "childExercisesFileeeeee\n\n");
    const exercise = {};
    ...
    if (fs.existsSync(childExercisesPath)) {
      ...
      const tokens = marked.lexer(data.toString());
      // console.log(tokens, "tokens ChildExerciseeeeee\n\n");
