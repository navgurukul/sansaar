exports.up = async (knex) => {
  await knex.schema.createTable('main.pathway_tracking_form_structure', (table) => {
    ...
    table.integer('parameter_id').unsigned().references('id').inTable('main.progress_parameters').notNullable();
    table.integer('question_id').unsigned().references('id').inTable('main.progress_questions').notNullable();
  ...
  await knex.schema.createTable('main.pathway_tracking_request_details', (table) => {
    ...
    table.integer('mentor_id').unsigned().references('id').inTable('main.users').notNullable();
    table.integer('mentee_id').unsigned().references('id').inTable('main.users').notNullable();
    table.integer('request_id').unsigned().references('id').inTable('main.pathway_tracking_request').notNullable();
    table.datetime('created_at').notNullable();
  });
...
exports.down = async (knex) => {
  await knex.schema.dropTable('main.pathway_tracking_form_structure')
