...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
        ...
        async (CourseCompletionV2Model, ExerciseModel, CourseModel) => {
          try {
            if (availableCourses !== undefined) {
              const currentFolderName = `curriculum/${availableCourses.name}_${courseId}`;
  ...
  async createNewCourse(details, txn) {
    ...
    if (!fs.existsSync(`curriculum/${courseFolderName}`)) {
      fs.mkdirSync(`curriculum/${courseFolderName}`);
    }
    return createCourse;
  }
    ...
    try {
      if (courseDetails.name !== undefined) {
        ...
        if (availableCourses !== undefined) {
          ...
          if (fs.existsSync(`curriculum/${currentFolderName}/PARSED_CONTENT/MODIFIED_FILES`)) {
            const files = fs.readdirSync(
              `curriculum/${currentFolderName}/PARSED_CONTENT/MODIFIED_FILES`
            );
            _.map(files, (file) => {
              const newFileName = file.replace(currentFolderName, newFolderName);
              fs.renameSync(
                `curriculum/${currentFolderName}/PARSED_CONTENT/MODIFIED_FILES/${file}`,
                `curriculum/${currentFolderName}/PARSED_CONTENT/MODIFIED_FILES/${newFileName}`
              );
            });
          }
          if (fs.existsSync(`curriculum/${currentFolderName}/PARSED_CONTENT/PROPERTIES_FILES`)) {
            const files = fs.readdirSync(
              `curriculum/${currentFolderName}/PARSED_CONTENT/PROPERTIES_FILES`
            );
            _.map(files, (file) => {
              const newFileName = file.replace(currentFolderName, newFolderName);
              fs.renameSync(
                `curriculum/${currentFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${file}`,
                `curriculum/${currentFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${newFileName}`
              );
            });
          }
          if (fs.existsSync(`curriculum/${currentFolderName}`)) {
            fs.renameSync(`curriculum/${currentFolderName}`, `curriculum/${newFolderName}`);
          }
        }
      }
