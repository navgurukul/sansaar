...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
  ...
  async getClassByExercise(exercise_id) {
    ...
    const classes = await Classes.query().where('exercise_id', exercise_id);
    return classes;
  }
  ...
  async createCourses(v1Courses) {
    ...
    for (const course of v1Courses) {
      const courseSingle = _.pick(course, ['id', 'name', 'logo', 'short_description']);
      courseSingle.lang_available = 'en';
      if (courseSingle.lang_available) {
        courseSingle.lang_available = val(courseSingle.lang_available).asArray().castTo('text[]');
      }
      // eslint-disable-next-line
      await CoursesV2.query().insert(courseSingle);

      const courseFolderName = `${courseSingle.name}_${courseSingle.id}`;
      if (!fs.existsSync(`curriculum_new/${courseFolderName}`)) {
        fs.mkdirSync(`curriculum_new/${courseFolderName}`);
      }
      // eslint-disable-next-line
      const courseExercises = await Exercises.query()
        .where('course_id', courseSingle.id)
        .orderBy('sequence_num');
      if (courseExercises !== []) {
        // eslint-disable-next-line
        for (const singleExercise of courseExercises) {
          const exerciseData = {};
          if (singleExercise.parent_exercise_id == null) {
            exerciseData.name = singleExercise.name;
          } else {
            let newName = singleExercise.slug.split('__');
            newName = newName[newName.length - 1].split('/').join('_').split('_');
            newName = newName.slice(0, newName.length - 1).join('_');
            exerciseData.name = newName;
          }
          let filePath = singleExercise.slug.split('__').join('/');
          const fileRealPath = filePath.split('_').pop();
          filePath = filePath.split('_');
          filePath = `${filePath.slice(0, filePath.length - 1).join('_')}.${fileRealPath}`;
          if (fs.existsSync(`curriculum/${filePath}`)) {
            const dataInString = fs.readFileSync(`curriculum/${filePath}`);
            exerciseData.id = singleExercise.id;
            exerciseData.description = `information about ${exerciseData.name}`;
            exerciseData.course_id = courseSingle.id;
