  ...
  async getCourseExercise(course_id, lang = 'en', txn) {
    ...
    try {
      ...
      _.map(courseExercises[0].exercisesV2, (exercise) => {
        ...
        const modifiedFile = `curriculum_new/${courseExercises[0].name}_${courseExercises[0].id}/PARSED_CONTENT/MODIFIED_FILES/${courseName}`;
        const propertiesFiles = `curriculum_new/${courseExercises[0].name}_${courseExercises[0].id}/PARSED_CONTENT/PROPERTIES_FILES/${courseName}`;
        if (fs.existsSync(modifiedFile)) {
          const modifiedDataInString = JSON.parse(fs.readFileSync(modifiedFile));
          const propertiesDataInString = JSON.parse(fs.readFileSync(propertiesFiles));
