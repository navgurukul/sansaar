  ...
  async getCourseExercisesBySlug(course_id, lang) {
    ...
    try {
      ...
      if (courseSlugs && courseSlugs.data) {
        for (const slug of courseSlugs.data) {
          if (slug.attributes.type === 'exercise') {
            ...
            // console.log(assessmentData,"gettinggg");
      ...
      const course = {
        ...
        android_logo: data.attributes.android_logo && data.attributes.android_logo.data ? data.attributes.android_logo.data.attributes.url : null,
        short_description: data.attributes.short_description,
        lang_available: data.attributes.lang_available,
        course_content: content.flat(),

      };
      console.log("zxcvbnmmmmmmm",short_description)
      return [null, course];
    } catch (error) {
