...
const merdBaseURL = 'http://merd-strapi.merakilearn.org/api';
...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
  ...
  async getCourseExercise(course_id, lang = 'en', txn) {
    ...
    try {
      const res = await axios.get(`${merdBaseURL}/${tableName}/${course_id}?populate=*`);
      ...
      //                 )
      //                 .join('<span border-radius: 2px; padding: 2px">');
      //             }
      //           }
      //         });
      ...
      const exercisesV2 = await courseData.attributes.exercises.data.map((ele) => {
        return {
          id: ele.id,
          name: ele.attributes.name,
          description: ele.attributes.description,
          course_id: res.data.data.id,
          content: strapiToMerakiConverter(JSON.parse(ele.attributes.content).blocks),
