...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
  ...
  async createNewCourse(details, txn) {
    ...
    const courseFolderName=`${details.name.replace(' ', '_')}_${createCourse.id}`
    ...
    try {
      if(courseDetails.name!==undefined){
        ...
        if(availableCourses!==undefined){
          const currentFolderName=`${availableCourses.name.replace(' ', '_')}_${courseId}`
          const newFolderName=`${courseDetails.name.replace(' ', '_')}_${courseId}`
