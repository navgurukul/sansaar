...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
  ...
  async migrateExerciserDataWithSlug() {
    const { ExerciseCompletionV2, exercisessHistory } = this.server.models();

    try {
      let finalData;
      let exercises = await ExerciseCompletionV2.query().limit(50000);

      for (let i of exercises) {
        try {
          let exercise_id = i.exercise_id;
          ...
          if (Object.keys(checkDataExists).length > 0) {
            ...
            finalData = {
              ...
              user_id: i.user_id,
              team_id: i.team_id,
              created_at: i.complete_at,
              updated_at: null
            };
            // Inserting the data of finalData into exercisessHistory table
            let insertedData = await exercisessHistory.query().insert(finalData);
      ...
      if (courseSlugs && courseSlugs.data) {
        for (const slug of courseSlugs.data) {
          if (slug.attributes.type === 'exercise') {
            const exerciseData = await this.getSlugsExercises(slug.id, lang);
            const exercisesContent = { data: exerciseData.flat() };
  
            const exercises = exercisesContent.data.map((exercise) => ({
              type: slug.attributes.type,
              name: exercise.attributes.name,
              description: exercise.attributes.description,
              course_id,
              slug_id: exercise.slug_id,
              exercise: strapiToMerakiConverter(exercise.attributes.content),
            }));
            ...
            if (assessmentData && assessmentData[0] !== null && assessmentData[0] !== undefined) {
              assessmentData.course_id = course_id;
              assessmentData.slug_id = slug.id;
