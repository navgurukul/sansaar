...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
  ...
  async getCourseExercise(course_id, lang = 'en', txn) {
    ...
    try {
      ...
      _.map(courseExercises[0].exercisesV2, (exercise) => {
        ...
        const modifiedFile = `${baseDir}/MODIFIED_FILES/${courseName}`;
  ...
  async findAssessmentByCourseDetails(exerciseDetail, courseId, courseName, sequence, lang = 'en') {
    ...
    if (fs.existsSync(`${currentFolderName}/MODIFIED_FILES`)) {
      const files = fs.readdirSync(`${currentFolderName}/MODIFIED_FILES`);
      // eslint-disable-next-line
      for (const file of files) {
        const fileName = file.replace('.json', '');
        // eslint-disable-next-line
        const data = await this.getExerciseAssessment(currentFolderName, fileName, lang);
        const newExercise = {};
