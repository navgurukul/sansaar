...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
  ...
  async getRecommendedCourses() {
    try {
      const allCoursesData = await strapi.find('courses', {
        populate: ['logo'],
      });
  
      const courses = allCoursesData.map((courseData) => {
        return {
          id: courseData.id,
          name: courseData.attributes.name,
          logo: courseData.attributes.logo && courseData.attributes.logo.data ? courseData.attributes.logo.data.attributes.url : null,
          short_description: courseData.attributes.short_description,
          lang_available: courseData.attributes.lang_available,
        };
      });
      ...
      // Get two recommended courses
      const recommendedCourses = shuffled.slice(0, 2);
  ...
  async deleteCourseById(courseId) {
    const { CoursesV2, CourseCompletionV3, ExercisesV2 } = this.server.models();
    try {
      ...
      const deleteCourse = await transaction(
        ...
        ExercisesV2,
        CoursesV2,
        async (CourseCompletionV3Model, ExerciseModel, CourseModel) => {
          try {
            ...
            await CourseCompletionV3Model.query().delete().where('course_id', courseId);
