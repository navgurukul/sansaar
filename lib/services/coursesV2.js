...
async function loadPropertiesFiles() {
  try {
    // Get all the .json files inside the PROPERTIES_FILES folder
    const propertiesFiles = await glob('**/PROPERTIES_FILES/**/*.json');

    // Map through each file and read its contents
    _.map(propertiesFiles, (fileName) => {
      // Read and parse the JSON file
      const fileContents = fs2.readFileSync(fileName, 'utf-8');
      allProperties[fileName] = JSON.parse(fileContents);
    });

    console.log('Properties loaded:', allProperties);
  } catch (err) {
    console.error('Error loading properties files:', err);
  }
}
