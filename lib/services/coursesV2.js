...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
  ...
  async getCourseExercisesBySlug(course_id, lang = 'en', txn) {
    ...
    try {
      ...
      const courseSlugs = data.attributes.slugs;
      let exercisesSlugs = [];
      let assessmentsSlugs = [];

      if (courseSlugs && courseSlugs.data) {
        courseSlugs.data.forEach((slug) => {
          const slugObject = slug.id;
          if (slug.attributes.type === 'exercise') {
            exercisesSlugs.push(slugObject);
          } else if (slug.attributes.type === 'assessment') {
            assessmentsSlugs.push(slugObject);
          }
        });
      }

      const exerciseShortByLang = exercisesSlugs.map((id) => this.fetchSlugsExercises(id, lang));
      const exerciseDataArray = await Promise.all(exerciseShortByLang);
      const flattenedExerciseDataArray = exerciseDataArray.flat();
      const exercisesContent = { data: flattenedExerciseDataArray }
      ...
      const assessmentsArray = assessmentsSlugs.map((id) => assessmentService.fetchSlugsAssessments(id, lang));
      const assessmentsDataArray = await Promise.all(assessmentsArray);

      const flattenedAssessmentDataArray = assessmentsDataArray.flat();
      console.log(flattenedAssessmentDataArray);
      // const exercisesContent = { data: flattenedExerciseDataArray }
  ...
  async fetchSlugsExercises(id, lang) {
    try {
      const slugsExercises = await strapi.findOne('slugs', id, {
        populate: ['exercises']
      });

      const exercisedata = slugsExercises.data.attributes.exercises;
