...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
  ...
  async updateHighlightColour(courseId, courseDetails) {
    const { CoursesV2 } = this.server.models();
    const { ExercisesV2 } = this.server.models();
    const { exercisesServiceV2 } = this.server.services();
    try {
      const courseExercisesV2 = await ExercisesV2.query()
        .where('course_id', courseId)
        .orderBy('sequence_num');
      if (courseExercisesV2.length > 0) {
        if (courseDetails.name !== undefined) {
          const coursePath = `curriculum_new/${courseDetails.name}_${courseId}`;
          if (fs.existsSync(coursePath)) {
            const redDir = fs.readdirSync(coursePath);
            for (const singleExercise of redDir) {
              if (singleExercise.includes('.json')) {
                const path = `${coursePath}/${singleExercise}`;
                const file = JSON.parse(fs.readFileSync(path, 'UTF-8'));
                for (const content of file) {
                  if (content.value.includes('span')) {
                    let correctData = content.value;
                    correctData = correctData.replaceAll(
                      'box-shadow: 2px 2px 2px rgba(22, 9, 1, 0.6);',
                      ''
                    );
