...
module.exports = class CoursesServiceV2 extends Schmervice.Service {
  ...
  async getCourseExercisesBySlug(course_id, lang = 'en', txn) {
    try {
      ...
      const exerciseShortByLang = exercisesSlugs.map((id) => this.fetchDataFromStrapi(id, lang));
  ...
  async fetchDataFromStrapi(id, lang) {
    const slug_id = id;
    try {
      const slugWithExercise = await strapi.findOne('slugs', id, {
        populate: ['exercises']
      });

      const exercisedata = slugWithExercise.data.attributes.exercises;
