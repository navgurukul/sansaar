...
module.exports = class EnglishAiService extends Schmervice.Service {
  ...
  async addEnglishAiUserReading(data) {
    ...
    try {
      ...
      for (const line of userLines) {
        ...
        if (!englishAiContent) {
          logger.info('English AI content not found for this content ID')
          results.push({ message: 'English AI content not found for this content ID' });
          continue;
        }
        ...
        const existingRecord = await EnglishAiUsersVoiceRecognise.query()
          .where('user_id', data.user_id)
          .andWhere('english_ai_content_id', data.english_ai_content_id);
        ...
        if (existingRecord.length === 0) {
          ...
          results.push(addUserReading);
        } else {
          // Record exists, so update it
          const updateResult = await this.updateEnglishAiUserReadingByUserID({
            ...data,
            users_reading: line,
            wrong_words_pronounced: unmatchedWords.join(' '),
          });
          results.push(updateResult.message);
        }
      }
      return [results[0]];
    } catch (err) {
  ...
  async updateEnglishAiUserReadingByUserID(userData, txn) {
    const { EnglishAiUsersVoiceRecognise } = this.server.models();

    try {
      // Check if a record with the specified user_id and english_ai_content_id exists
      const existingRecord = await EnglishAiUsersVoiceRecognise.query()
        .where('user_id', userData.user_id)
        .andWhere('english_ai_content_id', userData.english_ai_content_id)
      if (!existingRecord) {
        return { message: 'Record not found' };
      }

      await EnglishAiUsersVoiceRecognise.query(txn)
        .patch(userData)
        .where('user_id', userData.user_id)
        .andWhere('english_ai_content_id', userData.english_ai_content_id);
  ...
  async getEnglishAiUserReadingbyUserId(userData) {
    const { EnglishAiUsersVoiceRecognise } = this.server.models();
    try {
      const englishAiCont = await EnglishAiUsersVoiceRecognise.query().select('wrong_words_pronounced')
        .where('user_id', userData.user_id).andWhere('english_ai_content_id', userData.english_ai_content_id)
        .orderBy('id','desc');
      console.log(englishAiCont[0]);
      return [englishAiCont[0]];
    } catch (err) {
      logger.error(JSON.stringify(err));
      return [errorHandler(err), null];
    }
