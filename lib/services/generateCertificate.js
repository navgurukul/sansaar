...
module.exports = class GenerateCertificateService extends Schmervice.Service {
  ...
  async generateCertificate(user_id, Name, Course, weekDuration, Year,pathwayId) {
    ...
    var content = fs.readFileSync(
      ...
      return [{error:true,
        message:"this pathway_id does not valid for python",code:403
      }]
    }
  ...
  async TeacherCertificate(user_id, teacher_name,school_name) {
    var rows = {
     Name: teacher_name,
     School: school_name,
   };
   ...
   const read = await Certificate.query().where({ user_id });
   if (read.length === 1) {
     return [null,{URL:read[0].url,
    Status:"Already created certificate on this user_id"}];
   }
   fs.writeFileSync(filePath, fileData);
   // read certificate from /lib/helpers/assets/pdf
   fileData = fs.readFileSync(filePath);
