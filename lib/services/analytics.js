...
module.exports = class Analytics extends Schmervice.Service {
    async pushNow(data){
        ...
        try {
            const viewPage = await ViewPage.query().insert(data);
    ...
    async getDataUser(id){
        const { UserHack } = this.server.models();
        try {
            const u = await UserHack.query().throwIfNotFound().where("id",id).withGraphFetched("sessionsData").withGraphFetched("viewPageData").withGraphFetched("eventsData")
            return [null, u[0]];
        } catch (error) {
            logger.error(JSON.stringify(error));
            return [errorHandler(error), null];
    ...
    async getSessionDataByID(id ){
        const { Session } = this.server.models();
        try {
            const u = await Session.query().throwIfNotFound().where("id",id);
            return [null, u[0]];
        } catch (error) {
            logger.error(JSON.stringify(error));
            return [errorHandler(error), null];
