...
module.exports = class batchesService extends Schmervice.Service {
  async getAllPartnerBatch(
    page,
    _limit,
    pathwayId,
    type_,
    user_id,
    dateIST = UTCToISTConverter(new Date())
  ) {
    ...
    try {
        ...
        const totalData = await Promise.all(
          recurringIds.map(async ({ recurring_id }) => {
            ...
            if (firstClass.volunteer_id !== null) {
              const VolunteerData = await Volunteer.query()
                .select()
            ...
            const lastClass = await Classes.query()
              ...
              .first();
            const { start_time } = firstClass;
            const enroled = await ClassRegistrations.query()
              .select()
            ...
            if (firstClass.recurring_id) {
              ...
              [red, partnerIDs] = await classesService.getPartnerSpecificClass(firstClass.id, null);
            }
      ...
      const newData = await Classes.query()
        .select(
          'classes.id',
          'classes.title',
          'classes.type',
      ...
      const dataTotal = await Promise.all(
        newData.map(async (data) => {
          ...
          const [red, partnerIDs] = await classesService.getPartnerSpecificClass(data.id, null);
          return { ...data, partner_id: partnerIDs, enroled: enroled.length > 0 };
        })
      );
      logger.info(
        'Gets a list of all upcoming, ongoing batches and doubt_classes with pagination. dataTotal:',
        dataTotal
      );
  ...
  async getAllBatchClasses(reccuringId) {
    ...
    try {
      ...
      const endGame = await Promise.all(
        classes.map(async (data) => {
          const [red, partnerIDs] = await classesService.getPartnerSpecificClass(
            null,
            data.recurring_id
          );
  ...
  async userEnrolmentInBatch(userId) {
    try {
      ...
      const [err, pathway_names] = await pathwayServiceV2.pathwaysNames();
      if (err) {
        return [err, null];
      }
      const classes = await Classes.query()
      ...
      classes.filter((item) => {
        ...
        if (!recurring_ids.includes(item.recurring_id) && registration.length > 0) {
          const { classes_to_courses } = item;
          const pathwayId_ =
            classes_to_courses.pathway_v1 ||
            classes_to_courses.pathway_v2 ||
            classes_to_courses.pathway_v3;
          const pathway = pathway_names.filter((item) => item.id == pathwayId_);
          let pathway_name;
          ...
          if (pathway.length > 0) {
            ...
            flattenedResponse.push({
              ...
              course_id:
                classes_to_courses.course_v1 ||
                classes_to_courses.course_v2 ||
                classes_to_courses.course_v3,
              exercise_id:
                classes_to_courses.exercise_v1 ||
                classes_to_courses.exercise_v2 ||
                classes_to_courses.exercise_v3,
              enrolled: true,
              pathway_name,
