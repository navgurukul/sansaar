...
module.exports = class CareerService extends Schmervice.Service {
  ...
  async getTeacherData(user_id) {
    ...
    const { CareerTeachers, CareerRoles } = this.server.models();
    try {
      const roles = await CareerRoles.query().where('user_id', user_id);

      const teacherRole = roles.find((role) => role.role === 'teacher');
      if (teacherRole) {
