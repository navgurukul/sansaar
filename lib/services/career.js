...
const { AIDCX_PATHWAY_ID } = process.env;
...
module.exports = class CareerService extends Schmervice.Service {
  ...
  async setTeacherProfile(profile_obj, data, user_id, cluster_manager_id) {
    ...
    try {
      ...
      const teacherWithEmail = await CareerTeachers.query().where('email', data.email);
      if (teacherWithEmail.length > 0) {
        return [{ message: 'Email already exists', code: 403 }, null];
      }
      const teacherWithPhone = await CareerTeachers.query().where('phone_number', data.phone_number);
      if (teacherWithPhone.length > 0) {
        return [{ message: 'Phone already exists', code: 403 }, null];
      }
  ...
  async createTeam(data, user_id) {
    ...
    try {
      ...
      const checkTeamName = await CareerTeams.query().where('name', data.team_name).first();
      if (checkTeamName) {
        return [{ message: 'team name already exists', code: 403 }, null];
      }
      const finalData = {
        name: data.team_name,
  ...
  async getTeamByTeamId(team_id) {
    ...
    try {
      const team = await CareerTeams.query()
        ...
        // .withGraphFetched('teams_relationship');
      if (!team || team.length <= 0) {
        return [{ message: 'team_id not found' }, null];
      }
      const member = await CareerStudents.query().where('career_team_id', team_id);
      const responese = {
        team_id: team.id,
        team_name: team.name,
        ...
        teacher_name: team.teacher_relationship.name,
        teacher_email: team.teacher_relationship.email,
        teacher_phone: team.teacher_relationship.phone,
        school: team.school,
        state: team.state,
    ...
    try {
      ...
      if (data.team_size || data.team_members) {
        if (data.team_size && data.team_members) {
          ...
          deleteStudent = await CareerStudents.query().where('career_team_id', team_id).del();
      ...
      member = await CareerStudents.query().where('career_team_id', team_id);
  ...
  async getTeachersByCM(manager_id, page, limit = 10) {
    const {
      CareerTeachers,
      CareerTeams,
      CareerStudents,
    ...
    try {
      ...
      for (const data of teachers) {
        const teamsDetails = await CareerTeams.query().where('career_teacher_id', data.id);
        const teamsIds = teamsDetails.map((team) => team.id);
        const studentsDetails = await CareerStudents.query().whereIn('career_team_id', teamsIds);
