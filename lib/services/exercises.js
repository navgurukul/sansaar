...
module.exports = class ExercisesService extends Schmervice.Service {
  ...
  async upsertExercise(details, txn) {
    ...
    if (childExercise) {
      if (childExercise.length) {
        return this.addUpdateChildExercise(childExercise);
      }
    }

    if (exercise) {
      const ifExerciseExist = await Exercise.query(txn).where('slug', exercise.slug);
      if (ifExerciseExist.length) {
        return Exercise.query(txn)
          .update(exercise)
          .where('course_id', exercise.course_id)
          .andWhere('slug', exercise.slug);
      }
      return Exercise.query(txn).insert(exercise);
    }
    return true;
  }
  ...
  async addUpdateChildExercise(childExercise, txn) {
    ...
    const ifChildExerciseExist = await Exercise.query(txn).where('slug', childExercise[0].slug);
    ...
    _.map(updatedChildExercise, (exercise) => {
      promises.push(
        Exercise.query().where('course_id', exercise.course_id).andWhere('slug', exercise.slug)
      );
    });
