...
module.exports = class ExercisesService extends Schmervice.Service {
  ...
  async upsertChildExercises(childExercise, txn) {
    ...
    if (dbExercises.length > childExercise.length) {
      _.map(childExercise, (mdFileSlug) => {
        slugArr.push(mdFileSlug.slug);
      });
      const deletedExercise = dbExercises.filter((x) => !slugArr.includes(x.slug));
      _.map(deletedExercise, async (item) => {
        await Exercises.query(txn).where('slug', item.slug).delete();
      });
    }
