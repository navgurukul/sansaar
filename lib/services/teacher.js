...
module.exports = class teacherService extends Schmervice.Service {
  ...
  async DataLoaderSheetOBJ(usersInfo,usersProgress){ 
    try{
      let { Certificate } = this.server.models()
      let outcomes = [];
      ...
      for (let i = 0; i < usersInfo.length; i++ ){
        if (usersInfo[i].user_id == usersProgress[i].user_id){
          ...
          let Module_completion = Math.min(Math.max(parseInt(usersProgress[i].overallProgress ), 0), 100)
          
          outcomes.push({
            Zone: usersInfo[i].zone,
            Teacher_name: usersInfo[i].teacher_name,
