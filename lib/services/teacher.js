...
module.exports = class teacherService extends Schmervice.Service {
  ...
  async  getSheetUpdatedColumnData() {
    return new Promise((resolve, reject) => {
      const filePath1 = '1359_mcdigital_teachers_progress.csv'; // Ensure the file path is correct
      const filePath2 = '1359_last_week_users_login.csv'; // Ensure the file path is correct
  
      if (!fs.existsSync(filePath1)) {
        return reject([{ error: true, message: 'CSV file not found', code: 404 }, null]);
      }
      ...
      Promise.all([readCSVFile(filePath1), readCSVFile(filePath2)])
        .then(([file1Data, file2Data]) => {
          console.log('file1Data:', file1Data); // Debug statement
          console.log('file2Data:', file2Data); // Debug statement
          ...
          if (!sheetUpdatedOnDate) {
            return resolve([{ error: true, message: 'Sheet Updated On column not found in the first file', code: 404 }, null]);
          }
          ...
          if (!lastLoginMessageDate) {
            return resolve([{ error: true, message: 'No data found in the second file', code: 404 }, null]);
          }
          ...
          if (daysDifference <= 7) {
            ...
            message = 'No new login in this week';
          ...
          resolve([null, {
            "allUsersDetailUpdatedOn": sheetUpdatedOnDate.toISOString().split('T')[0],
            // message,
            // "Last Login Message Date": lastLoginMessageDate,
            "lastWeekUsersLoginDetailsUpdatedOn": messageDate.toISOString().split('T')[0],
            "at":"2:30 AM"

          }]);
        })
        .catch((err) => reject([{ error: true, message: err.message, code: 500 }, null]));
    });
  }
  };