...
module.exports = class teacherService extends Schmervice.Service {
  ...
  async createTeacherCapacityBuilding(data) {
    ...
    try {
      ...
      if (newData.length == 0) {
        if (data.employee_type === 'principal' || data.employee_type === 'clerical_staff') {
        ...
        return [{ error: true, message: `user details is already exits email ${data.email}`, code: 403 }, null];
      }
    } catch (err) {
  ...
  async getCoursesByEmployeeType(employee_Type) {
    try {
      ...
      } else if (['teacher', 'mentor_teacher', 'school_inspector'].includes(employee_Type)) {
        return {
