...
module.exports = class teacherService extends Schmervice.Service {
  ...
  async filterOrEmail(searchQuery) {
    const { TeacherCapacityBuilding } = this.server.models();
    try {
      let users;
      if (searchQuery.search) {
        users = await TeacherCapacityBuilding.query()
          .where('teacher_name', 'ilike', `%${searchQuery.search}%`)
          .orWhere('email', 'ilike', `%${searchQuery.search}%`);
  
        console.log('SQL Query:', TeacherCapacityBuilding.query()
          .where('teacher_name', 'ilike', `%${searchQuery.search}%`)
          .orWhere('email', 'ilike', `%${searchQuery.search}%`).toKnexQuery().toString());
      } else {
        users = await TeacherCapacityBuilding.query();
      }
      console.log('Users found:', users,"userfound");  // Log the retrieved users
      return [null, users];
    } catch (error) {
  ...
  async DataLoaderSheetOBJNew(user, pathwayCourses, total_assessmentIds) {
      ...
      console.log(pathwayCourses,"user2")
      console.log(total_assessmentIds,"ss3")
    try {
