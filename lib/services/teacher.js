...
module.exports = class teacherService extends Schmervice.Service {
  ...
  async TeacherCapacityBuildingTotalUsersIDNew(page, limit) {
    let { TeacherCapacityBuilding } = this.server.models();
    try {
      let TCBData = await TeacherCapacityBuilding.query()
        .select('user_id', 'zone', 'school_name', 'teacher_name', 'school_id', 'teacher_id', 'class_of_teacher', 'phone_number')
        .page(page, limit).withGraphFetched('userTCB');

      // make flatten the data for for email and delete created_at
      for (let data of TCBData.results) {
        data.email = data.userTCB.email;
        delete data.userTCB;
      }
