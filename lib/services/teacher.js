  ...
  async DataLoaderSheetOBJNew(user, pathwayCourses, total_assessmentIds) {
    try {
      ...
      const fetchAssessmentStats = async (userId, totalIds) => {
        ...
        const attempted = totalAttempts.length > 0 ? parseInt(totalAttempts[0].count) : 0;
        const correct = totalCorrect.length > 0 ? parseInt(totalCorrect[0].count) : 0;
      ...
      const fetchCertificateStatus = async (userId) => {
        const cert = await Certificate.query()
          .where('user_id', userId)
          .andWhere('pathway_code', 'TCBPI');
        return cert.length > 0 ? "Yes" : "No";
      };

      // Main execution logic
      finalData["Module_completion"] = await fetchPathwayProgress(user.user_id);

      for (const course of pathwayCourses) {
        const courseProgress = await fetchCourseProgress(user.user_id, course);
        finalData[courseProgress.courseName] = courseProgress.coursePercentage;
        finalData[`${courseProgress.courseName}_completion_date`] = courseProgress.completionDate;
        finalData['complete_status'] = courseProgress.completeStatus;
      }
