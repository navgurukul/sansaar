...
module.exports = class teacherService extends Schmervice.Service {
  ...
  async DataLoaderSheetOBJ(usersInfo,usersProgress){ 
    try{
      let { Certificate, User } = this.server.models()
      let outcomes = [];
      ...
      for (let i = 0; i < usersInfo.length; i++ ){
        if (usersInfo[i].user_id == usersProgress[i].user_id){
          ...
          for (let course of usersProgress[i].Courses){
            let course_name = `${course.name.replace(' ','_') }_completion`
            coursePrg[course_name] = course.courseProgressBar
            coursePrg[`${course.name.replace(' ','_') }_MCQ_score`] = course.mcqs.correctAnswers
            // eslint-disable-next-line no-unused-expressions
            correctAnswers += course.mcqs.correctAnswers;
          }
          // eslint-disable-next-line no-await-in-loop, no-unused-vars
          let userD2 = await User.query().where('id',usersInfo[i].user_id)
          // eslint-disable-next-line no-await-in-loop
          let cert = await Certificate.query().where('user_id',usersInfo[i].user_id).andWhere('pathway_code','TCBPI')
          
          logger.info(`data 106 line: ${JSON.stringify(userD2)}`)
          outcomes.push({
            Zone: usersInfo[i].zone,
            Teacher_name: usersInfo[i].userD2[0].name,
            Teacher_ID:usersInfo[i].teacher_id,
            School_Name: usersInfo[i].school_name,
            School_ID: usersInfo[i].school_id, 
            Gmail_ID:usersInfo[i].userD2[0].email,
