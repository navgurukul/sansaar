...
module.exports = class DisplayService extends Schmervice.Service {
  ...
  pathway = async (pathway, course_type = undefined) => {
    ...
    let data = await Pathways.query()
      .withGraphFetched('[pathwayCourses.courses, milestones]')
      .modifyGraph('pathwayCourses', (builder) => {
        builder.orderBy('id');
      });

    let pathways = data.map((p) => {
      const res = {
        ...p,
        courses: p.pathwayCourses.map((pc) => pc.courses).flat(),
      };
      delete res.pathwayCourses;
      return res;
    });
  ...
  filterClassDetails = async (singleClass, sequence) => {
    const exerciseDetails = _.pick(singleClass, ['id', 'title', 'description',
        'course_id', 'exercise_id', 'lang', 'type', 'meet_link', 'facilitator']);
    const contentEx = [];
    const newExercise = {};
    contentEx.push(exerciseDetails);
    newExercise.sequence_num = sequence;
    newExercise.content = contentEx;
    newExercise.content_type = 'class_topic';
    return newExercise;
  };
