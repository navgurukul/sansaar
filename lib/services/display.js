...
module.exports = class DisplayService extends Schmervice.Service {
  ...
  /* The above code defines a function called `getPathwayCoursesV2` that takes a `pathwayId` parameter.
 The function uses the `strapi` library to fetch data from a database table called `pathways` with
 the specified `pathwayId`. The function populates the `courses` and `logo` fields of the `pathways`
 table and returns an object with the `id`, `name`, `logo`, `short_description`, and
 `lang_available` fields of each course in the pathway. If the function encounters an error, it
  ...
  upcomingClassesWithEnrolledKey = async (classes, userId, scope) => {
    ...
    for (let class__ of modClass){
      ...
      if (mergeClass.length){
        ...
        let className = await Classes.query().select('title').where("id" , class__.id);
        let mergeClassName = await Classes.query().select('title').where("id" , mergeClassId);
        class__.merge_class = `Merge by the ${mergeClassName[0].title} and ${className[0].title}`;
      };
    };
    return modClass;
  };
