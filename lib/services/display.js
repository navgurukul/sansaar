...
module.exports = class DisplayService extends Schmervice.Service {
  ...
  pathway = async (pathway) => {
    ...
    const pathways = await Pathway.fetchGraph(pathway, { milestones: true, courses: true });
  ...
  allCoursesWithEnrolled = async (courses) => {
    ...
    const enrolledCourseList = await CourseEnrollment.fetchGraph(courses.enrolCourses, 'courses');
  ...
  getDefaultCourses = async (pathway) => {
    const { Pathway } = this.server.models();
    return Pathway.fetchGraph(pathway, 'courses');
  };

  pathwayCourses = async (pathwayId) => {
    const { Pathway } = this.server.models();
    return (
      Pathway.query()
