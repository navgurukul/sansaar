...
module.exports = class DisplayService extends Schmervice.Service {
  ...
  getPathwayCoursesV2 = async (pathwayId) => {
    ...
    try {
      ...
      pathway.courses = data.attributes.courses.data.map((course) => ({
        ...
        logo:
          course.attributes.logo && course.attributes.logo.data
      ...
      // console.log(pathway.courses,'this is pathway wala data...........------......-----.....-----');
      ...
      // console.log(pathway,'this is all data for pathway.........');
      return [null, pathway];
    } catch (err) {
      return [errorHandler(err), null];
    }
  ...
  transform = (input) => {
    try {
      ...
      let output = {
        ...
        publishedAt: input.attributes.publishedAt,
        type: input.attributes.type,
        video_link: input.attributes.video_link,
        logo: input.attributes.logo.data ? input.attributes.logo.data.attributes.url : null,
        modules: [],
      };
      input.attributes.modules.data.forEach((module) => {
        let moduleObj = {
          id: module.id,
          name: module.attributes.name,
          createdAt: module.attributes.createdAt,
          updatedAt: module.attributes.updatedAt,
          publishedAt: module.attributes.publishedAt,
          courses: [],
        };
        module.attributes.courses.data.forEach((course) => {
          let logo = course.attributes.logo.data ? course.attributes.logo.data.attributes.url : null;
          let courseObj = {
            id: course.id,
            name: course.attributes.name,
            short_description: course.attributes.short_description,
            ...
            logo: course.attributes.logo.data ? course.attributes.logo.data.attributes.url : null,
          };
  ...
  getPathwayC4CA = async (pathwayId) => {
    let pathway;
    try {
      const { data } = await strapi.findOne('pathways', pathwayId, {
  ...
  upcomingClassesWithEnrolledKey = async (classes, userId, scope) => {
    ...
    for (let class__ of modClass) {
      ...
      if (mergeClass.length) {
        ...
        let mergeClassName = await Classes.query()
          .select('title', 'start_time')
          .where('id', mergeClassId);
        class__.merge_class = `${mergeClassName[0].title}'s class on ${new Date(
          mergeClassName[0].start_time
        )
          .toUTCString()
          .slice(4, 17)} has been merged with this class`;
      }
    }
    return modClass;
  };
