...
module.exports = class DisplayService extends Schmervice.Service {
  ...
  markPreQuizComplete = async (userId, courseId) => {
    const { assessmentsHistory } = this.server.models();
    try {
      const { data } = await strapi.findOne('courses', courseId, {
        populate: ['prequizs.slug'],
      });
      let preQuizData = data.attributes.prequizs.data;
      let slug_ids = [];
      for (let slug_id of preQuizData) {
        slug_ids.push(slug_id.attributes.slug.data.id);
      }

      let checkPreQuiz = await assessmentsHistory.query().where('user_id', userId).whereIn('slug_id', slug_ids);
      if (checkPreQuiz.length === 0 || checkPreQuiz === undefined || checkPreQuiz === null) {
        return "false";
      }
      else {
        return "true";
