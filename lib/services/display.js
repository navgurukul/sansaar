...
module.exports = class DisplayService extends Schmervice.Service {
  ...
  updateMatrixProfile = async (user) => {
    const { chatService } = this.server.services();
    const { name, chat_id, chat_password } = user;
    const matrixAcc = await chatService.userChatLogin(chat_id, chat_password);
    await chatService.userUpdateProfile(name, chat_id, matrixAcc.access_token);
    return 'Success';
  };
  ...
  getClasses = async (classes) => {
    ...
    regClasses.forEach((ele) => {
      ...
      if (ele.facilitator[0]) regClass.facilitator = ele.facilitator[0];
