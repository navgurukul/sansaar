  ...
  upcomingClassesWithEnrolledKey = async (classes, userId, scope) => {
    ...
    for (let class__ of modClass){
      ...
      if (mergeClass.length){
        ...
        let mergeClassName = await Classes.query().select('title','start_time').where("id" , mergeClassId);
        class__.merge_class = `${mergeClassName[0].title}'s class on ${new Date(mergeClassName[0].start_time).toUTCString().slice(4, 17)} has been merged with this class`;    
      };
    };
    return modClass;
  };
