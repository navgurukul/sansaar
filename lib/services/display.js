...
module.exports = class DisplayService extends Schmervice.Service {
  ...
  getPathwayCoursesV2 = async (pathwayId) => {
    let pathway;
    try {
      const path = await strapi.findOne(`pathways`, pathwayId, {
        populate: ['courses'],
      });
      pathway = {
        id: path.data.id,
        code: path.data.attributes.code,
        name: path.data.attributes.name,
        logo: path.data.attributes.logo,
        description: path.data.attributes.description,
        courses: path.data.attributes.courses.data.map((c) => {
          return {
            id: c.id,
            name: c.attributes.name,
            logo: c.attributes.logo,
            short_description: c.attributes.short_description,
            lang_available: c.attributes.lang_available,
          };
        }),
      };
      return [null, pathway];
  ...
  upcomingClassesWithEnrolledKey = async (classes, userId, scope) => {
    ...
    for (let cls of modClass){
      let mergeClass = await MergedClasses.query().where('class_id',cls.id);
      if (mergeClass.length){
        let mergeClassId = mergeClass[0].merged_class_id
        let className = await Classes.query().select('sub_title').where("id" , cls.id)
        let mergeClassName = await Classes.query().select('sub_title').where("id" , mergeClassId)
        let new_class_name = `Merge by the ${mergeClassName[0].sub_title} and ${className[0].sub_title}`;
        console.log(new_class_name);
        cls.sub_title = new_class_name
        console.log(cls);
      }
    }
    return modClass;
