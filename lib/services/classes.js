...
module.exports = class classesService extends Schmervice.Service {
  ...
  async getEnrolledClasseByRecurringId(recurringId) {
    const { Classes, Courses, CoursesV2 } = this.server.models();
    const dateIST = UTCToISTConverter(new Date());
    ...
    try {
      ...
      if (classes.length > 0 && classes[0].course_version === 'v1') {
        const oldCourse = await Courses.query().where('id', classes[0].course_id);
        const v1Tov2Course = await CoursesV2.query().where('name', oldCourse[0].name);
        classes[0].course_id = v1Tov2Course[0].id;
        classes[0].pathway_id = classes[0].pathway_id == 39 ? 1 : classes[0].pathway_id;
      }
      return [null, classes];
    } catch (err) {
      return [errorHandler(err), null];
