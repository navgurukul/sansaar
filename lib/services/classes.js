...
module.exports = class classesService extends Schmervice.Service {
  ...
  async registerToClassById(registrationDetails) {
    ...
    const concernedClass = await ClassRegistrations.query().findOne({
      user_id,
      class_id,
    });
    // eslint-disable-next-line
    if (checkIfClassValid.facilitator_id == user_id) {
      // user_id is in string
    ...
    } else if (concernedClass !== undefined) {
