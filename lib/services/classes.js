...
module.exports = class classesService extends Schmervice.Service {
  ...
  async getAllClassesMailByTime() {
    ...
    const classessMailData = await ClassesMail.query().whereNot('status', 'success');
  ...
  async updateClassMailById(id, dict) {
    const { ClassesMail } = this.server.models();
    const updateClassMail = await ClassesMail.query().update(dict).where('id', id);
  ...
  async createRecurringEvent(recurringPayload) {
    console.log(recurringPayload, 'recurringPayload\n\n');
    ...
    try {
      ...
      console.log(recurringClassEntry, 'recurringClassEntry\n\n');
  ...
  async sendMailtoVolunteers(classCount, facilitator_name, facilitator_email, class_id) {
    ...
    console.log(
      ...
      facilitator_email,
      class_id,
      'sendMailtoVolunteers parameters log\n\n'
    );
    if (classCount === 1) {
  ...
  async createClass(newClassData) {
    ...
    let classCount;
    try {
      console.log('try ke under hai\n\n');
      classCount = await Classes.query().where(
        'facilitator_email',
        newClassData[0].facilitator_email
      );
      ...
      for (var i in newClassData) {
        await this.sendMailtoVolunteers(
          ...
          newClassData[0].facilitator_name,
          newClassData[0].facilitator_email,
          newClass[i].id
        );
      }
      console.log(newClass, 'newClass return value');
      return [null, newClass];
    } catch (err) {
      console.log(err, 'err aa rha hai bhai');
  ...
  async getClassById(classId) {
    ...
    try {
      ...
      console.log(classes, 'classesGetById');
      return [null, classes];
    } catch (err) {
      return [errorHandler(err), null];
    }
