...
module.exports = class classesService extends Schmervice.Service {
  ...
  async getClassById(classId) {
    ...
    try {
      ...
      if (classes.recurring_id === null) {
        let partnerSpecificTableData = await PartnerSpecificBatches.query().where(
          'class_id',
          classes.id
        );
        if (
      ...
      if (classes.recurring_id !== null && classes.recurring_id !== undefined) {
        let partnerSpecificBatchesData = await PartnerSpecificBatches.query().where(
          'recurring_id',
  ...
  async mergeWarToday(query) {
    ...
    try {
      ...
      let filter_classes = classes.filter((item) => {
        if (item.PartnerSpecificBatches != null && item.PartnerSpecificBatches.pathway_id == 7) {
          item.pathway_id = item.PartnerSpecificBatches && item.PartnerSpecificBatches.pathway_id;
