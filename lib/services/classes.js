...
module.exports = class classesService extends Schmervice.Service {
  ...
  async createClass(newClassData, role = null) {
    const { Classes, UserRole, PathwayCoursesV2 } = this.server.models();
    let newClass;
    // eslint-disable-next-line
    for (let singleClassData in newClassData) {
      // eslint-disable-next-line
      const pathwaysClass = await PathwayCoursesV2.query()
        .select('pathway_id')
        .where('course_id', newClassData[singleClassData].course_id);
      newClassData[singleClassData].pathway_id = pathwaysClass[0].pathway_id;
    }
    /**
