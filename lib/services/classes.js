...
module.exports = class classesService extends Schmervice.Service {
  async getAllClasses(startDate) {
    ...
    try {
      ...
      for (let i in classes) {
        if (classes[i].pathway_id !== null && classes[i].pathway_id !== undefined) {
          // eslint-disable-next-line
          const pathway = await pathwayServiceV2.findPathwayById(classes[i].pathway_id);
          if (pathway.code === 'PRPGYT') {
            const subTitle = classes[i].title.split('$');
            classes[i].title = classes[i].title.split('$').slice(0, -1).join('$');
            classes[i].sub_title = subTitle[subTitle.length - 1];
          }
        }
      }
      return [null, classes];
  ...
  async getClassById(classId) {
    ...
    try {
      ...
      if (classes.pathway_id !== null && classes.pathway_id !== undefined) {
        const pathway = await pathwayServiceV2.findPathwayById(classes.pathway_id);
        if (pathway.code === 'PRPGYT') {
          const subTitle = classes.title.split('$');
          classes.title = classes.title.split('$').slice(0, -1).join('$');
          classes.sub_title = subTitle[subTitle.length - 1];
        }
      }
      return [null, classes];
    } catch (err) {
