...
module.exports = class classesService extends Schmervice.Service {
      ...
      const deleteClassesAndClearReg = await transaction(
        ...
        async (
          ...
          try {
            ...
            await PartnerSpecificBatchesModel.query(txn).delete().where('recurring_id', recurringId);
    ...
    try {
      ...
      if (scope.indexOf('admin') > -1) {
        ...
        if (group_id !== null && group_id !== undefined) {
          classRecurringIds = await PartnerSpecificBatches.query().where('partner_id', partner_id).andWhere('group_id', group_id)
    ...
    try {
      ...
      if (recurringClassEntry !== null && recurringClassEntry !== undefined && recurringClassEntry.length > 0) {
        ...
        if (recurringClassEntry[0].space_id){
          usersDataForEnrollment = await User.query().where('space_id', recurringClassEntry[0].space_id);

        } else {
          usersDataForEnrollment = await User.query().where('group_id', recurringClassEntry[0].group_id);
        }
  ...
  async getClassesByPathwaysId(duration, pathwayId, scope, partner_id, space_id, group_id) {
    const { Classes, ClassesToCourses, PartnerSpecificBatches } = this.server.models();
    const dateIST = UTCToISTConverter(new Date());
    let classes;
    try {
  ...
  async mergeWarToday(dateString) {
    const { Classes } = this.server.models();
    try {
      // Create a Date object from the provided dateString
      const currentDate = new Date(dateString);
      
      // Set the time to the present time
      const now = new Date();
      currentDate.setHours(now.getHours() + 5);
      currentDate.setMinutes(now.getMinutes() + 30);
      ...
      const classes = await Classes.query()
        ...
        .andWhere('start_time', '<', midnightDate)
        .andWhere('type','batch');
