...
module.exports = class classesService extends Schmervice.Service {
  ...
  async createClass(newClassData, role = null) {
    const { Classes, UserRole, PathwaysV2 } = this.server.models();
    let newClass;

    try {
      // setting default max_enrolment to 10 students for spoken english classes
      if (newClassData[0].pathway_id) {
        const pathwayName = (
          await PathwaysV2.query().findById(newClassData[0].pathway_id).select('name')
        ).name;
        if (pathwayName.length && pathwayName == 'Spoken English') {
          newClassData.forEach((class_) => (class_.max_enrolment = class_.max_enrolment || 10));
        }
      }
    } catch (err) {
      console.log(err);
