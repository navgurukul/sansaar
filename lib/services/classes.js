...
module.exports = class classesService extends Schmervice.Service {
  ...
  async getAllClassesByPathwaysId(duration) {
    const { Classes, ClassesToCourses, PartnerSpecificBatches } = this.server.models();
    const dateIST = UTCToISTConverter(new Date());
    try {
      let classDetails = {
      ...
      const doubt_class = await Classes.query()
      .where('end_time', '>', dateIST)
      .andWhere('end_time', '<', duration)
      .andWhere('type','doubt_class')
      .orderBy('start_time')
      const batch = await Classes.query()
      .where('end_time', '>', dateIST)
      .andWhere('end_time', '<', duration)
      .andWhere('type','batch')
      .orderBy('start_time')
      classDetails.batch = batch
      classDetails.class = doubt_class
      return [null,classDetails];
    } catch (err) {
      return [errorHandler(err), null];
    }
