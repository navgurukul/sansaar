...
module.exports = class classesService extends Schmervice.Service {
  ...
  async getAllClassesByPathwaysId(duration,pathwayId) {
    const { Classes, ClassesToCourses, PartnerSpecificBatches } = this.server.models();
    const dateIST = UTCToISTConverter(new Date());
    try {
      let classDetails = {
      ...
      const doubtClasses = await ClassesToCourses.query()
      .where('pathway_v2', pathwayId)
      .select('*')
      .joinRelated('class')
      .whereIn('class.type', ['doubt_class'])
      .orderBy('class_id');

      const batchClasses = await ClassesToCourses.query()
      .where('pathway_v2', pathwayId)
      .select('*')
      .joinRelated('class')
      .whereIn('class.type', ['batch'])
      .orderBy('class_id'); 

      classDetails.class = doubtClasses;
      classDetails.batch = batchClasses;
      return [null,classDetails];
    } catch (err) {
      return [errorHandler(err), null];
    }
