...
module.exports = class classesService extends Schmervice.Service {
  ...
  async updateGRegistrationStatusById(classId) {
    const { ClassRegistrations } = this.server.models();
    try {
  ...
  // get class for two hours
  async getClassesForTwoHours() {
    const { Classes } = this.server.models();
    let classes;
    const dateIST = UTCToISTConverter(new Date());
    const classForTwoHours = UTCToISTConverter(
      new Date(new Date().setMinutes(new Date().getMinutes() + 120))
    );
    try {
      classes = await Classes.query()
        .where('end_time', '>', dateIST)
        .andWhere('start_time', '<', classForTwoHours)
        .orderBy('start_time')
        .withGraphFetched({ registrations: true, facilitator: true, parent_class: true })
        // Another way to do this could be only fetch graph by matching user id
  ...
  async removeRegistrationById(classIds, userId, txn) {
    ...
    try {
      ...
      if (result) {
        ...
        /* eslint-disable */
        return [null, { success: true, google_registration_status: true }];
      } else {
        await ClassRegistrations.query(txn)
          .delete()
          .whereIn('class_id', classIds)
          .andWhere('user_id', userId);
        return [null, { success: true, google_registration_status: false }];
      }
    } catch (err) {
      return [errorHandler(err), null];
    }
  ...
  async getcohortClassByExercise(exercise_id) {
    ...
    const classes = await Classes.query().whereIn('id', ids).whereNotNull('recurring_id');
    return classes;
  }
  ...
  async getClassesByPathwaysId(pathwayId, scope, partner_id) {
    const { Classes, ClassesToCourses } = this.server.models();
    const dateIST = UTCToISTConverter(new Date());
    let classes;
    try {
