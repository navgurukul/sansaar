...
module.exports = class classesService extends Schmervice.Service {
  ...
  async getAllClassesAfterDate(startDate, scope, partner_id) {
    ...
    try {
      ...
      for (let data of classes) {
        let partnerDetails = await PartnerSpecificBatches.query().skipUndefined().where('recurring_id', data.recurring_id);
        if (partnerDetails.length > 0 && partnerDetails !== null && partnerDetails !== undefined) {
          if (data.recurring_id === partnerDetails[0].recurring_id || !data.recurring_id) {
            data['partner_id'] = partnerDetails[0].partner_id;
            data['group_id'] = partnerDetails[0].group_id;
            data['space_id'] = partnerDetails[0].space_id;
            classesAllData.push(data);
          }
        }
      }
      classes = classesAllData;
      return [null, classes];
    } catch (err) {
      return [errorHandler(err), null];
    }
  ...
  async getClassById(classId) {
    ...
    try {
      ...
      delete classes.partner_id;
      delete classes.space_id;
      delete classes.classes_to_courses;

      if (classes.recurring_id === null) {
        let partnerSpecificTableData = await PartnerSpecificBatches.query().where('class_id', classes.id);
        if (partnerSpecificTableData !== null && partnerSpecificTableData !== undefined && partnerSpecificTableData.length > 0) {
      ...
      if (classes.recurring_id !== null && classes.recurring_id !== undefined) {
        let partnerSpecificBatchesData = await PartnerSpecificBatches.query().where('recurring_id', classes.recurring_id);
        if (partnerSpecificBatchesData !== null && partnerSpecificBatchesData !== undefined && partnerSpecificBatchesData.length > 0) {
          ...
          course_version === 'v1'
            ...
            : course_version === 'v2'
              ...
              : {
                ...
                exercise_v3: classUpdates.exercise_id,
              };
      ...
      const deleteClassesAndClearReg = await transaction(
        ...
        async (
          ...
          try {
            ...
            await MergedClassesModel.query(txn).delete().whereIn('class_id', filterIds).orWhereIn('merged_class_id', filterIds);
          } catch (err) {
            return errorHandler(err);
          }
  ...
  async getClassesForXXXTime(duration) {
    ...
    try {
      ...
      return [errorHandler(err), null]; 3
    }
  }

  // get classes for a week
  async getAllPartnerClasses(duration, scope, partner_id, space_id) {
    ...
    try {
      ...
      if (scope.indexOf('admin') > -1) {
        ...
        if (space_id !== null && space_id !== undefined) {
          classRecurringIds = await PartnerSpecificBatches.query().where('partner_id', partner_id).andWhere('space_id', space_id)
        } else {
          classRecurringIds = await PartnerSpecificBatches.query().where(
            'partner_id',
            partner_id
          );
        }
  ...
  async getIfStudentEnrolled(id, pathwayId) {
    ...
    try {
      ...
      if (classesRegisteredByUser != null) {
        const mergedClassId = await MergedClasses.query().whereIn('class_id', classesRegisteredByUser);
  ...
  async enrollStudentBySpaceId(classId, h) {
    ...
    try {
      ...
      if (classesDetails == null || classesDetails == undefined || Object.keys(classesDetails).length <= 0) {
        return [null, "classId is wrong"];
      }
      const registerStudentsData = await ClassRegistrations.query().where('class_id', classId)
      if (registerStudentsData !== null && registerStudentsData !== undefined && registerStudentsData.length > 0) {
        for (const registerStudent of registerStudentsData) {
          const [errUserDetail, userDetail] = await userService.findById(
            registerStudent.user_id
          );
          if (errUserDetail) {
            logger.error(
              // eslint-disable-next-line
      ...
      const recurringClassEntry = await PartnerSpecificBatches.query().where('recurring_id', classesDetails.recurring_id);
      if (recurringClassEntry !== null && recurringClassEntry !== undefined && recurringClassEntry.length > 0) {
        const usersDataForEnrollment = await User.query().where('space_id', recurringClassEntry[0].space_id);
        if (usersDataForEnrollment !== null && usersDataForEnrollment !== undefined && usersDataForEnrollment.length > 0) {
  ...
  async getClassesByPathwaysId(duration, pathwayId, scope, partner_id, space_id) {
    const { Classes, ClassesToCourses, PartnerSpecificBatches } = this.server.models();
    const dateIST = UTCToISTConverter(new Date());
    let classes;
    try {
      ...
      for (const key in recurringIds) {
        let recurring_classes = await Classes.query()
          ...
          .withGraphFetched({ registrations: true, facilitator: true, parent_class: true })
          .modifyGraph('registrations', (builder) => builder.select('user_id'));
        const last_class = await Classes.query().select()
          .where('recurring_id', recurringIds[key])
  ...
  async getEnrolledClasseByRecurringId(recurringId) {
    ...
    try {
      ...
      for (const c in classes) {
        ...
        if (mergeClasses !== null && mergeClasses !== undefined && Object.keys(mergeClasses).length > 0) {
    ...
    try {
      for (var i of enrolledClasses) {
        patchExerId = await ClassesToCourses.query()
          .update({ pathway_v3: i.pathway_id, course_v3: i.course_id, exercise_v3: i.exercise_id })
          .where('class_id', i.id);
      }
      return;
    } catch (err) {
    ...
    try {
      ...
      if (headers && classDetails.recurring_id) {
        ...
        if (errInFetchingRecurringClasses) {
          /* eslint-disable */
          logger.error(
            `id- ${userId}: ` + JSON.stringify(errInFetchingRecurringClasses)
          );
          return h
        ...
        for (const i in OldRecurringclasses) {
          if (OldRecurringclasses[i].sub_title !== null) {
            ...
            for (let i in revisionClasses) {
              const revision = await this.getFixedClasses(
                revisionClasses[i],
                userId
              );
      ...
      const [
        ...
        removedRegistration,
      ] = await this.removeRegistrationById(classIds, userId);
      ...
      if (removedRegistration.google_registration_status === true) {
        ...
        try {
          ...
          /* eslint-disable */
          logger.error(
            `id- ${userId}: Calendar event error- ` + JSON.stringify(err)
          );
        }
      }
  ...
  async mergedClasses(classId, mergeClassId, h) {
    const { ClassRegistrations, MergedClasses, Classes } = this.server.models();
    const { displayService, userService } = this.server.services();
    try {
      ...
      const classesExists = await Classes.query().where('id', classId)
      const classesMergeExists = await Classes.query().where('id', mergeClassId)
      let classesMerged = []
      if (classesMergeExists !== null && classesMergeExists !== undefined && classesMergeExists.length > 0
        && classesExists !== undefined && classesExists !== null && classesExists.length > 0) {
        const docs = {}
        docs.class_id = classId
        docs.merged_class_id = mergeClassId
        classesMerged = await MergedClasses.query().insert(docs);
      }

      return [null, classesMerged];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
  ...
  async getStudentsDataByRecurringId(reccuringId) {
    const { Classes, ClassRegistrations, User, UserRole } = this.server.models();
    try {
      ...
      if (classData.length > 0 && classData !== null) {
        ...
        let registerStudentsData = await ClassRegistrations.query().select('user_id').whereIn('class_id', classIds);
        if (registerStudentsData.length > 0 && registerStudentsData !== null) {
          ...
          const excludedRoles = ['admin', 'volunteer', 'partner'];
          const rolesData = await UserRole.query().whereIn('user_id', userIds)
          for (let urdata of rolesData) {
            if (excludedRoles.includes(urdata.role)) {
              const indexToDelete = userIds.indexOf(urdata.user_id);
              if (indexToDelete !== -1) {
                userIds.splice(indexToDelete, 1);
              }
            }
          }
          let usersData = await User.query().select('id', 'name', 'email').whereIn('id', userIds);
          return [null, usersData, userIds];
        }
        else {
          return [{
            status: true,
  ...
  async checkBatchTitle(group_id, title) {
    ...
    try {
      ...
      const batchData = await PartnerSpecificBatches.query().throwIfNotFound().where('group_id', group_id)
  ...
  async getAllPartnerBatch( page, _limit, pathwayId, type_) {
    const {Classes,Volunteer } = this.server.models();
    try {
      const dateIST = UTCToISTConverter(new Date());
  
      const recurringIds = await Classes.query()
        ...
        let count__ = 0
        if (true){
          const count_ = await Classes.query()
          .select('classes.recurring_id', 'partner_specific_batches.partner_id')
          .innerJoin('main.partner_specific_batches', 'classes.recurring_id', 'partner_specific_batches.recurring_id')
          .where('partner_specific_batches.pathway_id', pathwayId)
          .andWhere('classes.start_time', '>', dateIST)
      ...
      const totalData = await Promise.all(
        recurringIds.map(async ({ recurring_id }) => {  
          ...
          return { ...firstClass,volunteer:{name,email},barch_start: start_time, batch_end: lastClass.start_time };
        })
      );
  ...
  async mergeWarToday(){
    ...
    try {    
      ...
      const midnightDate = new Date(`${dateString}T23:59:59.999Z`);
    
      const classes = await Classes.query()
        .select('id','title','start_time','sub_title','recurring_id','type' )
        .where('start_time', '>=', currentDate)
        .andWhere('start_time', '<', new Date(midnightDate));
