...
module.exports = class classesService extends Schmervice.Service {
  ...
  async registerToClassById(registrationDetails) {
    ...
    let isClassValid, isAlreadyRegistered, successReg;
    try {
      isClassValid = await Classes.query().findById(class_id);
      isAlreadyRegistered = await ClassRegistrations.query().findOne({
        user_id,
        class_id,
      });
  ...
  async removeRegistrationById(classId, userId, txn) {
    ...
    let deletion;
    try {
      deletion = await ClassRegistrations.query(txn)
        .del()
