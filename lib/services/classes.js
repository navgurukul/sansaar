...
module.exports = class classesService extends Schmervice.Service {
  ...
  async getAllPartnerBatch( pages, pageSize) {
    const {PartnerSpecificBatches, Classes,Volunteer } = this.server.models();
    ...
    try {
      ...
      .select('recurring_id')
      .distinct('recurring_id')
      .where('start_time', '>', dateIST)
      .andWhere('type', 'batch')
      .orderBy('recurring_id', 'desc')
        .offset((pages - 1) * pageSize)  // Calculate the offset based on the page number
        .limit(pageSize); // Limit the number of returned rows
