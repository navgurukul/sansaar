...
module.exports = class classesService extends Schmervice.Service {
  ...
  async registerClasses(classId, headers, userDetails, h) {
    const { displayService, chatService, userService, calendarService } = this.server.services();
    const payload = [];
    try {
...
=======
  ...
  async getClassRegistrations(start_time, end_time) {
    const { ClassRegistrations } = this.server.models();
    try {
      const data = await ClassRegistrations.query()
        .select('user_id')
        .where('registered_at', '>=', start_time)
        .andWhere('registered_at', '<=', end_time)
        .withGraphFetched('ClassRegistrations_data');
      console.log(data, 'ertyui');
      return [null, data];
    } catch (err) {
      console.log(err, ':qwert');
      return [errorHandler(err), null];
    }
  }
>>>>>>> Stashed changes
