...
module.exports = class classesService extends Schmervice.Service {
  ...
  async mergeWarToday(query) {
    ...
    try {
      ...
      const midnightDate = new Date(date+'T23:59:59.999+05:30');
      
      logger.info(`currentDate: ${currentDate}`)
      logger.info(`midnightDate: ${midnightDate}`)
      const classes = await Classes.query()
      ...
      let filter_classes = classes.filter((item) => {
        if (item.PartnerSpecificBatches != null && item.PartnerSpecificBatches.pathway_id == pathway_id && !reccuringIds.includes(item.recurring_id)) {
