...
module.exports = class classesService extends Schmervice.Service {
  ...
  async getClassesTemp() {
    ...
    try {
      ...
      // query on classes table where course_id is not null
  ...
  async getAllClassesAfterDate(startDate, scope, partner_id) {
    ...
    let classes;
    try {
      // ----- getting all batches data -----
      const partnerDetails = await PartnerSpecificBatches.query();
      // console.log(partnerDetails, '++++++++');
      const recurringIds = [];
      const doubtClassIds = [];
      // ----- collecting all class_id and recurring_id in separate arrays -----
      ...
      // ----- getting classes all details from classes and classes_to_courses by starting from time given as start time ( for admin )  -----
      if (scope.indexOf('admin') > -1) {
        ...
        // ----- getting details of batches as per partner_id in ( classRecurringIds ) -----
        ...
        // ----- collecting class_id and recurring_id from ( classRecurringIds ) if ( not null ) -----
        ...
        // ----- getting classes all details from classes and classes_to_courses by starting from time given as start time and where recurring_id and doubt_class_ids matches -----
        classes = await Classes.query()
          .skipUndefined()
          .where('end_time', '>=', startDate)
          ...
          .withGraphFetched('classes_to_courses');
        if (classes.length <= 0) {
          // ----- getting classes and classes_to_courses details as per end_time >= start_time  -----
          ...
          // ----- getting  class details from classes and classes_to_courses where recurring_id is null and if id not present in doubtClassIds -----
        ...
        // ----- getting classes and classes_to_courses details as per given conditions -----
        const recurringClasses = await Classes.query()
          .skipUndefined()
          .where('end_time', '>=', startDate)
          .whereNotNull('recurring_id')
        ...
        // ----- getting doubt class details from classes and classes_to_courses details as per given conditions -----
        const doubtClasses = await Classes.query()
          .skipUndefined()
          .where('end_time', '>=', startDate)
          .whereNull('recurring_id')
        ...
        // ----- assigning recurringClasses and doubtClasses data to classes variable -----
        classes = [...recurringClasses, ...doubtClasses];
      }
      // eslint-disable-next-line
      classes.forEach((c) => {
        if (c.classes_to_courses) {
          // ----- updating course version, hathway version and exercise version as per course version -----
          if (c.course_version === 'v2') {
            c.course_v2 = c.classes_to_courses.course_v2;
            c.pathway_v2 = c.classes_to_courses.pathway_v2;
            c.exercise_v2 = c.classes_to_courses.exercise_v2;
        ...
        // ----- delete classes_to_courses if it's false -----
        delete c.classes_to_courses;
      });
      return [null, classes];
    } catch (err) {
  ...
  async getClassBySubtitle(id, sub_title, classRecurringId) {
    ...
    try {
      ...
      // all data from partner_specific_bataches table ---------------- -------- - --------- ----
      const partnerDetails = await PartnerSpecificBatches.query();
      const allRecurringIdInPartners = [];
      ...
      // takes only those data which is required and return this data.-------------
      classes.forEach((c) => {
        if (c.classes_to_courses) {
          if (c.course_version === 'v2') {
            c.course_v2 = c.classes_to_courses.course_v2;
