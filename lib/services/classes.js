...
module.exports = class classesService extends Schmervice.Service {
  ...
  async getStudentsDataByRecurringId(reccuringId) {
    ...
    try {
      let classData = await Classes.query().select('id').where('recurring_id', reccuringId).limit(1);
      if (classData.length > 0 && classData !== null) {
        let registerStudentsData = await ClassRegistrations.query().select('user_id').where('class_id', classData[0].id);
        if (registerStudentsData.length > 0 && registerStudentsData !== null) {
          let userIds = registerStudentsData.map(ids => ids.user_id);
