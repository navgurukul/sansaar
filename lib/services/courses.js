...
module.exports = class CoursesService extends Schmervice.Service {
  ...
  async createNewCourse(details, txn) {
    ...
    if (lang_available) {
      data = {
        ...data,
        lang_available: val(lang_available).asArray().castTo('text[]'),
      };
    }
    const createCourse = await Courses.query(txn).insert(data);
    return createCourse;
  }
  ...
  async patchCourse(details, name) {
    ...
    if (lang_available) {
      ...
      data = {
        ...
        lang_available:
          findLang.length > 0
            ? val(`${lang_available},${findLang}`).asArray().castTo('text[]')
