...
module.exports = class CourseService extends Schmervice.Service {
  ...
  async getExercises(courseId) {
    ...
    const exercises = await Courses.query().where('id', courseId).orderBy('sequence_num', 'asc');
    return exercises;
  }
  ...
  async deleteCourseById(courseId) {
    const { Courses, CourseEnrolment, Exercises, Submissions } = this.server.models();
    ...
    _.map(exercises, (exercise) => {
      promises.push(Submissions.query().delete().where('exercise_id', exercise.id));
    });
    ...
    return Courses.query().deleteById(courseId);
  }
  ...
  async findByCourseName(name, graphFetchRelation = '', txn) {
    const { Courses } = this.server.models();
    const course = await Courses.query(txn)
      .where('name', name)
