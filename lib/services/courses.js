...
module.exports = class CoursesService extends Schmervice.Service {
  ...
  async getAllCoursesV2(txn = null) {
  ...
  async getRecommendedCoursesV2() {
    ...
    try {
      randomCourses = await CoursesV2.query()
        ...
        .orderByRaw(`random()`)
        .limit(2);
      const CoursesV2 = [...randomCourses, ...typingGuru];
  ...
  async deleteCourseByIdV2(courseId) {
  ...
  async findByCourseNameV2(name) {
    const { CoursesV2 } = this.server.models();
    let course;
    try {
        course = await CoursesV2.query()
          .skipUndefined()
          .andWhere('name', name);
      return [null, course];
    } catch (err) {
  ...
  async createNewCourseV2(details, txn) {
    const { CoursesV2 } = this.server.models();
  ...
  async markCourseCompleteV2(userId, courseId) {
  ...
  async getIdForRemovalV2(userId, courseId) {
    const { CourseCompletionV2 } = this.server.models();
  ...
  async removeCourseCompleteV2(id, userId, courseId) {
    const { CourseCompletionV2 } = this.server.models();
  ...
  async getCourseCompleteV2(userId) {
    const { CourseCompletionV2 } = this.server.models();
    let completedCourse;
    try {
