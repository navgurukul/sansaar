...
const { errorHandler } = require('../errorHandling'); // Your custom error handler
const fs = require('fs');
const allProperties = {};
...
function loadPropertiesFiles() {
  glob('**/PROPERTIES_FILES/**/*.json', (err, propertiesFiles) => {
    if (err) {
      console.error('Error finding properties files:', err);
      throw err;
    }
    ...
    propertiesFiles.forEach((fileName) => {
      try {
        ...
        allProperties[fileName] = JSON.parse(fileContent);
      } catch (parseError) {
        console.error(`Error reading or parsing file ${fileName}:`, parseError);
        throw parseError; // Handle the error as needed
      }
    });
      ...
      _.map(newExercises, (exercise) => {
        ...
        if (modifiedFile.length > 1) {
          if (modifiedFile[0].includes('_json__')) {
            modifiedFile = `${modifiedFile[0].split('__')[0]}/PARSED_CONTENT/PROPERTIES_FILES/${modifiedFile[0].split('__')[1]
          ...
          if (modifiedFile[0].includes('_json__')) {
            modifiedFile = `${modifiedFile[0].split('__')[0]
              }/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_json_${modifiedFile[0].split('__')[1]
              }_${lang}`;
          } else {
            modifiedFile = `${modifiedFile[0].split('__')[0]
              }/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_${modifiedFile[0].split('__')[1]
              }_${lang}`;
          }
        }
