...
module.exports = class CourseEditorService extends Schmervice.Service {
  ...
  async addSingleExercise(courseFolderName, data, course_name) {
    try {
      if (fs.existsSync(`${courseFolderName}`)) {
        fs.writeFileSync(
          path.resolve(`${courseFolderName}/${data.name}.json`),
      ...
      const propertiesFilePath = `${courseFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${course_name}_${data.name}`;
      await this.fixedTranslation(propertiesFilePath);
