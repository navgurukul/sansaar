...
module.exports = class CourseEditorService extends Schmervice.Service {
      ...
      if (data.name !== undefined) {
        ...
        if (fs.existsSync(`${courseFolderName}`)) {
          ...
          for (const lang in language) {
            if (
              fs.existsSync(
                `${courseFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${partPath[0]}_${partPath[1]}_${exerciseName}_${language[lang]}.json`
              )
            ) {
              fs.renameSync(
                `${courseFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${partPath[0]}_${partPath[1]}_${exerciseName}_${language[lang]}.json`,
                `${courseFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${partPath[0]}_${partPath[1]}_${data.name}_${language[lang]}.json`
              );
            }
          }
          // eslint-disable-next-line
          ...
          if (data.content === undefined) {
            const propertiesFilePath = `${courseFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${partPath[0]}_${partPath[1]}_${data.name}`;
            await this.fixedTranslation(propertiesFilePath);
          }
        }
      }
      if (data.content !== undefined) {
  ...
  async parseTranslatedData(courseName, curriculumFilePath) {
    ...
    try {
      ...
      const hindiContent = globals.get('HiContent');
      const teContent = globals.get('TeContent');
      const mrContent = globals.get('MrContent');
      const taContent = globals.get('TaContent');
      ...
      for (const lang in language) {
        ...
        if (contentPath !== undefined && contentPath !== '' && contentPath !== null) {
          if (
            fs.existsSync(
              `curriculum_new/${courseName}/v1/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_v1_${contentPath}_${fileName}_en.json`
            )
          ) {
            ...
            if (!_.isEmpty(language[lang][0])) {
              _.forEach(parsedContent, (jsonContent) => {
                keyNumber += 1;
                keyProp = `${contentPath}_${fileName}_key${keyNumber}`;
                if (language[lang][0][jsonContent] !== undefined) {
                  keyPropMapping[keyProp] = `${language[lang][0][jsonContent]}`;
                }
              });
              fs.writeFileSync(
                path.resolve(
                  `curriculum_new/${courseName}/v1/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_v1_${contentPath}_${fileName}_${lang}.json`
                ),
          ...
          if (
            fs.existsSync(
              `curriculum/${folderName}/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_json_${fileName}_en.json`
            )
          ) {
            const content = fs.readFileSync(
              `curriculum/${folderName}/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_json_${fileName}_en.json`
            ...
            if (!_.isEmpty(language[lang][0])) {
              ...
              fs.writeFileSync(
                path.resolve(
                  `curriculum_new/${courseName}/v1/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_v1_${fileName}_${lang}.json`
                ),
                JSON.stringify(keyPropMapping, null, '\t')
              );
            }
  ...
  async fixedTranslation(folderPath) {
    const hindiContent = globals.get('HiContent');
    const teContent = globals.get('TeContent');
    const mrContent = globals.get('MrContent');
    const taContent = globals.get('TaContent');
        ...
        for (const lang in language) {
          ...
          for (const jsonContent in parsedContent) {
            if (language[lang][0][parsedContent[jsonContent]] !== undefined) {
              ...
              // eslint-disable-next-line
              const translatedData = await this.quickstart(parsedContent[jsonContent], lang);
    ...
    try {
      ...
      for (const course of courses) {
        ...
        if (fs.existsSync(`curriculum_new/${courseSingle.name}/`)) {
          ...
          let i = 1;
          while (i <= version) {
            if (fs.existsSync(`curriculum_new/${courseSingle.name}/v${i}`)) {
              if (
                fs.existsSync(
                  `curriculum_new/${courseSingle.name}/v${i}/PARSED_CONTENT/MODIFIED_FILES`
                )
              ) {
                fs.readdirSync(
                  `curriculum_new/${courseSingle.name}/v${i}/PARSED_CONTENT/MODIFIED_FILES`
                  ...
                  const propertiesFilePath = `curriculum_new/${
                    courseSingle.name
      ...
      globals.set('HiContent', [keyPropMappingHi]);
      globals.set('MrContent', [keyPropMappingMr]);
      globals.set('TeContent', [keyPropMappingTe]);
      globals.set('TaContent', [keyPropMappingTa]);
      return [null, courses];
    } catch (err) {
      return [errorHandler(err), null];
    }
