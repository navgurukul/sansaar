...
module.exports = class CourseEditorService extends Schmervice.Service {
  ...
  async updateCourseEditor(courseId, courseDetails) {
    ...
    try {
      if (courseDetails.name !== undefined) {
        ...
        const version = parseInt(availableVersion[0].version.slice(1))+1
  ...
  async deleteExerciseById(exerciseId) {
    const { CoursesV2, ExercisesV2, CourseProductionVersions, ExerciseAddedAndDeletedVersion } = this.server.models();
    let deletedDataRecord;
    try {
      if (exerciseId !== undefined) {
        let availableExercise = await ExercisesV2.query().where({ 'id': exerciseId })
        const courseDetails = await CoursesV2.query().where({ 'id': availableExercise[0].course_id })
        let folderName = `${courseDetails[0].name}_${courseDetails[0].id}`;
        const language = ['hi', 'en', 'te', 'mr', 'ta'];

        let ver = await CourseProductionVersions.query().select('version').where({ 'course_id': availableExercise[0].course_id })
        let currentVersion = ver[0].version.slice(0, 2)
        let versionOf_v = ver[0].version.slice(0, 1)
        let version = ver[0].version.slice(1);
        let versionIntoInt = (parseInt(version)) + 1;
        let numberToString = versionOf_v + (versionIntoInt.toString());

        if (fs.existsSync(`curriculum_v2/${folderName}`)) {
          if (fs.existsSync(`curriculum_v2/${folderName}/${numberToString}`)) {

            if (fs.existsSync(`curriculum_v2/${folderName}/${numberToString}/${availableExercise[0].name}.json`)) {
              fs.unlinkSync(`curriculum_v2/${folderName}/${numberToString}/${availableExercise[0].name}.json`);

              fs.unlinkSync(`curriculum_v2/${folderName}/${numberToString}/PARSED_CONTENT/MODIFIED_FILES/${folderName}_${availableExercise[0].name}.json`);

              for (const lang in language) {
                if (fs.existsSync(`curriculum_v2/${folderName}/${numberToString}/PARSED_CONTENT/PROPERTIES_FILES/${folderName}_${availableExercise[0].name}_${language[lang]}.json`)) {
                  fs.unlinkSync(`curriculum_v2/${folderName}/${numberToString}/PARSED_CONTENT/PROPERTIES_FILES/${folderName}_${availableExercise[0].name}_${language[lang]}.json`)
                }
              }
              deletedDataRecord = await ExerciseAddedAndDeletedVersion.query().insert({
                course_id: availableExercise[0].course_id,
                exercise_id: availableExercise[0].id,
                version: numberToString,
            ...
            if (!fs.existsSync(`curriculum_v2/${folderName}/${numberToString}`)) {
              fs.mkdirSync(`curriculum_v2/${folderName}/${numberToString}`);
            }
            const courses = fs.readdirSync(`curriculum_v2/${folderName}/${currentVersion}`);
            for (const course of courses) {
              fs.copySync(`curriculum_v2/${folderName}/${currentVersion}/${course}`, `curriculum_v2/${folderName}/${numberToString}/${course}`);
            }
            if (fs.existsSync(`curriculum_v2/${folderName}/${numberToString}/${availableExercise[0].name}.json`)) {
              fs.unlinkSync(`curriculum_v2/${folderName}/${numberToString}/${availableExercise[0].name}.json`);
              fs.unlinkSync(`curriculum_v2/${folderName}/${numberToString}/PARSED_CONTENT/MODIFIED_FILES/${folderName}_${availableExercise[0].name}.json`);
