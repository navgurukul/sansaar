...
module.exports = class CourseEditorService extends Schmervice.Service {
  ...
  async insertExerciseById(course_id, name, content, version) {
    const { CoursesV2, ExercisesV2, CourseProductionVersions, ExerciseAddedAndDeletedVersion } = this.server.models();
    try {
      let insertExerciseId = await ExercisesV2.query().insert({
        name: name,
        content: content,
        course_id: course_id
      }).where("course_id", course_id)
      let findExerciseId = await ExercisesV2.query().where("name", name)
      let insertitem = {
        course_id,
        version: `v${version}`,
        exercise_id: findExerciseId[0].id,
        addedOrRemoved: true
      }
      const data = await ExerciseAddedAndDeletedVersion.query().insert(insertitem)
      return data;
    } catch (error) {
      return [errorHandler(error), null];
    }
