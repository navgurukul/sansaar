...
module.exports = class CourseEditorService extends Schmervice.Service {
  async createCourses() {
    ...
    try {
      ...
      for (const course of courses) {
        ...
        const [, coursesDetails] = await coursesServiceV2.getCourseById(
  ...
  async getCourseExercise(course_id, version, lang = 'en') {
  ...
  async updateCourseEditor(courseId, courseDetails) {
    ...
    try {
      if (courseDetails.name !== undefined) {
        ...
        const availableVersion = await CourseProductionVersions.query()
          .select('version')
  ...
  async createNewCourseEditor(details, txn = null) {
    const { CoursesV2 } = this.server.models();
  ...
  async postcreateCourses(received) {
    const { ExerciseAddedAndDeletedVersion } = this.server.models();
    try {
      // const volunteers = await Volunteer.query();
      const data = await ExerciseAddedAndDeletedVersion.query().insert(received);
      return [null, data];
    } catch (err) {
      return [errorHandler(err), null];
    }
