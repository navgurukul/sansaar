...
module.exports = class CourseEditorService extends Schmervice.Service {
    ...
    try {
      ...
      for (const course of v1Courses) {
        ...
        if (!fs.existsSync(`curriculum_v2/${courseSingle.name}`)) {
          fs.mkdirSync(`curriculum_v2/${courseSingle.name}`);
        }
        if (!fs.existsSync(`curriculum_v2/${courseSingle.name}/v1`)) {
          fs.mkdirSync(`curriculum_v2/${courseSingle.name}/v1`);
        }
        // eslint-disable-next-line
  ...
  async addExercises(data) {
    ...
    try {
      const courseFolderName = `curriculum_v2/${data.course_name}/v1`;
    ...
    try {
      ...
      for (const lang in language) {
        ...
        if (contentPath !== undefined && contentPath !== '' && contentPath !== null) {
          if (
            fs.existsSync(
              `curriculum_v2/${courseName}/v1/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_v1_${contentPath}_${fileName}_en.json`
            )
          ) {
            ...
            if (!_.isEmpty(language[lang][0])) {
              ...
              fs.writeFileSync(
                path.resolve(
                  `curriculum_v2/${courseName}/v1/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_v1_${contentPath}_${fileName}_${lang}.json`
                ),
          ...
          if (
            ...
            if (!_.isEmpty(language[lang][0])) {
              ...
              fs.writeFileSync(
                path.resolve(
                  `curriculum_v2/${courseName}/v1/PARSED_CONTENT/PROPERTIES_FILES/${courseName}_v1_${fileName}_${lang}.json`
                ),
                JSON.stringify(keyPropMapping, null, '\t')
              );
            }
                ...
                if (fs.existsSync(`curriculum_v2/${courseSingle.name}/v${i}`)) {
                  if (
                    ...
                    fs.readdirSync(
                      ...
                      const propertiesFilePath = `curriculum_v2/${
                        courseSingle.name
