...
module.exports = class CourseEditorService extends Schmervice.Service {
  async createCourses(v1Courses) {
    const { CoursesV2, Exercises, CourseVersions } = this.server.models();
    try {
      ...
      for (const course of v1Courses) {
        ...
        if (!fs.existsSync(`curriculum_new/${courseSingle.name}/v2`)) {
          fs.mkdirSync(`curriculum_new/${courseSingle.name}/v2`);
        }
        // eslint-disable-next-line
  ...
  async addExercises(data) {
    ...
    try {
      ...
      if (courseDetails !== undefined) {
        const courseFolderName = `curriculum_new/${courseDetails.name}/v2`;
