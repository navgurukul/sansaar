...
module.exports = class CourseEditorService extends Schmervice.Service {
  ...
  async findDetailInCourseVersionsById(course_id, lang = 'en') {
    console.log(course_id, 'this is course_id..........');
    const { CourseProductionVersions, CoursesV2 } = this.server.models();
    try {
      const course_version = await CourseProductionVersions.query()
        .where('course_id', course_id)
        .andWhere('lang', lang);
      const C_Name = await CoursesV2.query().where('id', course_id);
      course_version[0].name = C_Name[0].name;
