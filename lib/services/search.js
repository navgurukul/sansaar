...
module.exports = class searchService extends Schmervice.Service {
  async showing_popular_searches(userID) {
    ...
    try {
      const popularSearchData = await UserPopularSearch.query().select().orderBy('count', 'desc');

      console.log(!userID == null, 'userID: ', userID);
      const recentSearchData = await UserSearch.query().where({ user_id: userID });
      let total_array = [...popularSearchData.slice(0, 2), ...recentSearchData];
      let user_search = [];
      let top_popular = [];
  ...
  async sending_data(payload_data) {
    ...
    try {
      ...
      const maxRows = 3; // maximum number of rows allowed in the table
