...
module.exports = class UserService extends Schmervice.Service {
  ...
  async getAllStudent(query) {
    ...
    const studentsId = [];
    const student_Id = await UserRole.query().select('user_id as id').where('role', 'student');
    _.map(student_Id, (n) => {
      studentsId.push(n.id);
    });
    try {
      ...
      const length = await User.query().whereIn('id', studentsId).count();
