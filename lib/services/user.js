...
module.exports = class UserService extends Schmervice.Service {
  ...
  async filterByNameOrEmail(searchQuery) {
    const { User } = this.server.models();
    try {
      let users;
      if (searchQuery.search) {
        users = await User.query()
          .where('name', 'ilike', `%${searchQuery.search}%`)
          .orWhere('email', 'ilike', `%${searchQuery.search}%`);

      }
      return [null, users];
    } catch (error) {
      console.error('Error in filterEmailName:', error);
      return [error, null];
    }
  }
  ...
  async exportUsersToCSVAndJsonFile() {
    try {
      const [error, users] = await this.getAllUsersWithContact();
      ...
      const jsonFilePath = path.join('userdata.json');

      const ws = fs2.createWriteStream(filePath);

      fastcsv
        .write(users, { headers: true })
        .on('finish', () => {
          return { success: true, message: 'Write to CSV successfully!' };
        })
        .pipe(ws);
      fs2.writeFileSync(jsonFilePath, JSON.stringify(users, 2), 'utf-8');

      await this.insertDataFromCSV(filePath);
      return { success: false, message: 'Error writing to CSV.', error: err.message };

    } catch (err) {
      return { success: false, message: 'Error exporting users to CSV.', error: err.message };

    }
  }
  
  async insertDataFromCSV(csvFilePath) {
    const users = [];
        ...
        fs2.createReadStream(csvFilePath)
            ...
            .on('end', async () => {
                try {
                    ...
                    const { UserStoreData } = this.server.models();
                    await UserStoreData.query().insert(users);
                    // await UserStoreData.query().insert(users);
                    const countResult = await UserStoreData.query().count();
                    ...
                    reject({ success: false, message: 'Insertion Error', error });
                }
            })
            .on('error', (error) => {
                console.error('CSV Read Error:', error);
                reject({ success: false, message: 'CSV Read Error', error });
            });
    });
}
