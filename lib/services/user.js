...
module.exports = class UserService extends Schmervice.Service {
  ...
  async updateById(id, payload) {
    const { User, UserTokens } = this.server.models();
    const { name, partner_id, lang_1, lang_2, email } = payload;
    let user;
    let checkEmailExists;
    try {
      if (email) {
        const emlDelete = await UserTokens.query().where('user_email', email).delete();
        checkEmailExists = await User.query().findOne({ email });
      }
      if (!checkEmailExists) {
        user = await User.query()
          .patchAndFetchById(id, { name, partner_id, lang_1, lang_2, email })
          .skipUndefined();
        return [null, user];
      }
      checkEmailExists['message'] = 'this user email allreadt exists'
      return [null, checkEmailExists];
    } catch (err) {
