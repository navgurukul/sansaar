...
module.exports = class UserService extends Schmervice.Service {
  ...
  async loginMerakiStudent(user_name, password) {
    const { MerakiStudent } = this.server.models();
    try {
      const meraki_student = await MerakiStudent.query().where('user_name', user_name).first();
      if(meraki_student){
        const compare = await bcrypt.compare(password, meraki_student.password);
        if(compare){
  ...
  createTokenV2 = (data) => {
    const JWTtoken = JWT.sign({student_id:data.id, username: data.user_name, flag: 'meraki_student' }, CONFIG.auth.jwt.secret, {
      algorithm: 'HS256',
      expiresIn: CONFIG.auth.jwt.expiresIn,
    });
