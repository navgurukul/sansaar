...
module.exports = class UserService extends Schmervice.Service {
  ...
  async loginWithGoogle(authDetails, txn) {
    ...
    if (!user) {
      ...
      const google_user_id_user = await User.query(txn).findOne({ google_user_id: userObj.google_user_id });
      if (google_user_id_user){
        return {
          user: google_user_id_user, first_time_login: false
        }
      }
  ...
  async getAllUsers(start_date,end_date){
    ...
    try {
      const allMerakiCertifacate = await Certificate.query().where('created_at', '>=', `${start_date}`).andWhere('created_at', '<', `${end_date}`)
      const alluserDetails = []
      for(let ind = 0; ind < allMerakiCertifacate.length; ind++){
        const userDetails = await User.query().where('id',allMerakiCertifacate[ind].user_id).select('id','name','email','contact')
  ...
  async findUserByNameAndEmail(name, email) {
    ...
    async createUsersJWT(data) {
      const token = jwt.sign(data, 'sdxfcghgjsdfghhe2345678909876543wsdxfcv6eewdxfcgvho7654erfgfghgf1!@#$%^&*kjhgfdcv');
      return token;
    }
    ...
    async verifyGoogleIdToken(idToken) {
      try {
        ...
        const decodedToken = await jwt.verify(idToken, 'sdxfcghgjsdfghhe2345678909876543wsdxfcv6eewdxfcgvho7654erfgfghgf1!@#$%^&*kjhgfdcv');
        const { email, id } = decodedToken;
        // Do something with the user information, such as create a session and set a cookie
        return { success: true, user: { email, id } };
      } catch (error) {
        // Handle the error, such as by returning an error response to the client
        console.error(error);
