...
module.exports = class UserService extends Schmervice.Service {
  ...
  async createUser() {
    ...
    const newChatUser = {
      ...
      id: userDetails.chat_id,
      password: userDetails.chat_password,
      threepids: [],
    };
    await chatService.createChatUser(newChatUser); // Create a matrix id for the user
    await chatService.createChatRoom(newChatUser.id); // Create a room for the user for initial conversation

    // await chatService.initializeMeraki(newChatUser.id);
  ...
  async loginWithGoogle(authDetails, txn) {
    ...
    if (!user) {
      ...
      if (user) {
        ...
        await chatService.createChatRoom(newUser.id);
        // await chatService.initializeMeraki(newUser.id);
      }
    }
    return user;
  }
