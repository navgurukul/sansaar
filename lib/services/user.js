...
module.exports = class UserService extends Schmervice.Service {
  ...
  async updateContactUserById(userId, contact) {
    const { User } = this.server.models();
    try {
      const user = await User.query().findById(userId).skipUndefined();
      user.contact = contact;
      const updateUser = await User.query().updateAndFetchById(userId, user);
