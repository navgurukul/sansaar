...
module.exports = class UserService extends Schmervice.Service {
  ...
  async createUser() {
    const userDetails = { name: 'Led Zeppelin', email: 'stairwayto@heaven.com' };
    const { User } = this.server.models();
  ...
  async loginWithGoogle(authDetails, txn) {
    ...
    if (!user) {
      ...
      await User.query(txn).patch({
        chat_id: userObj.chat_id,
        chat_password: userObj.chat_password,
      });
