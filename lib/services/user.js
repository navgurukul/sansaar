...
module.exports = class UserService extends Schmervice.Service {
  ...
  async loginMerakiStudent(login_id, password) {
    const { MerakiStudent,User } = this.server.models();
    try {
      const meraki_student = await MerakiStudent.query().where('login_id', login_id).andWhere('password', password).first();
      if (meraki_student) { 
  ...
  createTokenV2 = (data) => {
    const JWTtoken = JWT.sign({student_id:data.id, login_id: data.login_id, flag: 'meraki_student' }, CONFIG.auth.jwt.secret, {
      algorithm: 'HS256',
      expiresIn: CONFIG.auth.jwt.expiresIn,
    });
