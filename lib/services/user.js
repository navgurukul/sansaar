...
module.exports = class UserService extends Schmervice.Service {
  async findById(id, graphFetchRelation='', txn) {
    ...
    const user = User.query(txn).throwIfNotFound().findById(id).withGraphFetched(graphFetchRelation);
    return user;
  }
  ...
  async addRoles(id, rolesList, txn) {
    ...
    if (newRoles.length !== rolesList.length) {
      throw Error("Some roles given are already assigned to the user.");
    }
  ...
  async removeRoles(id, rolesList, txn) {
    ...
    _.forEach(rolesList, roleName => {
      ...
      if (!userRole) {
        throw Error("Some role(s) to delete are not assigned roles of the user.");
      }
    });
