...
module.exports = class UserService extends Schmervice.Service {
  ...
  async extractPayloadFromURL(url, payload) {
    try{
      ...
      if (typeof utm_content === 'string' && utm_content.includes('partner_id') || utm_content.includes('cluster_manager_id')) {
  ...
  async updateById(id, payload) {
    const { User, C4caRole ,TeacherCapacityBuilding, UserRole } = this.server.models();
    try {
      let userData;
      ...
      } else if(payload.cluster_manager_id){
        const rolesToCheck = ['clusterManager', 'careerTeacher', 'careerAdmin'];
        let presentRole = await UserRole.query()
          .andWhere('user_id', id)
          .whereIn('role', rolesToCheck);

        if(presentRole.length === 0){
            await UserRole.query().insert({user_id: id, role: "careerTeacher"})
            userData = await User.query().patchAndFetchById(id, payload).withGraphFetched('roles').skipUndefined();
            const roles = userData.roles.map(({ role }) => role);
  ...
  async getRandomUsersWithContact() {
    try {
      const query = `
        SELECT id, name, email, contact
        FROM users
      ...
      this.logger.error('Error fetching random users with contact:', error);
      throw Boom.internal('Failed to fetch random users with contact');
    }
  }
};
