...
module.exports = class UserRoleService extends Schmervice.Service {
  async getVolunteersCountAccordingToPathways(){
    ...
    try {
      ...
      let pythonVolunteers = await Volunteer.query().select('user_id as id').where('pathway_id',1);
    let spokenEnglishVolunteers = await Volunteer.query().select('user_id as id').where('pathway_id',2);
  ...
  async getAllVolunteers(createdTime, createTime2, volunteerStatus, name, lang, class_title,pathway) {
    ...
    try {
      ...
      if (class_title !== undefined){
        filtered_res = filtered_res.filter(
          (el) => el.classes[el.classes.length - 1].title.toLowerCase().includes(class_title.toLowerCase())
          );
      }
      // filtering data
