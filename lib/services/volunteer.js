...
module.exports = class UserRoleService extends Schmervice.Service {
  ...
  async CheckNumberRegisteredOrNot(userId) {
    try {
      const { Volunteer, User } = this.server.models();
      let VolunteerData;
      let checkPhoneNo;
      if(userId){
        VolunteerData = await Volunteer.query().where({ user_id: userId });
        if(VolunteerData.length > 0 && VolunteerData !== null && VolunteerData !== undefined){
          checkPhoneNo = await User.query().where({ id: VolunteerData[0].user_id})
          if(checkPhoneNo[0].contact.length > 0 && checkPhoneNo[0].contact !== null && checkPhoneNo[0].contact !== undefined){
            return [null, 'your mobile number is already registerd'];
          }
        }
      }
      return [null, 'Please registered your number'];
    } catch (err) {
      console.log(err);
      return [err, null];
    }
