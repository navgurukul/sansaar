...
<<<<<<< HEAD
  static calculateEngagement(d1, d2) {
    d1 = new Date(d1);
    d2 = new Date(d2);
    let months;
  ...
  async getAllVolunteers() {
    ...
    try {
      ...
      /**
       * You should use withGraphFetched function of Objection to leverage the relation mapping of UserRole model
       * with Users model further with classes model
       *
       * Something like this
       * const volunteers = await UserRole.query().where('role', 'volunteer').withGraphFetched('user');
       *
       * Just take care of one thing Classes -> User relation mapping is done on keys classes.user_id -> users.id, so this will give you the data
       * of students joining the class and not the facilitators
       *
       */

      volunteer_ids.forEach((v, index) => {
        volunteer_ids[index] = v.id;
      });
...
>>>>>>> f9de92b746a03e1ad9efdd96c0715d7338a44704
...
<<<<<<< HEAD
      ...
      volunteers.forEach((volunteer, i) => {
        ...
        if (classes.length > 0) {
          ...
          volunteer.engagement_in_months = UserRoleService.calculateEngagement(
      ...
      volunteers_data.count = volunteer_ids.length;
