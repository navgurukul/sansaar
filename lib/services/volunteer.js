...
module.exports = class UserRoleService extends Schmervice.Service {
  ...
  async createVolunteer(details) {
    ...
    try {
      const volunteer = await Volunteer.query()
        ...
        .andWhere('hours_per_week', details.hours_per_week)
        .andWhere('available_on_days', details.available_on_days)
        .andWhere('available_on_time', details.available_on_time);
  ...
  async volunteerWorking(data, user_id) {
    ...
    try {
      ...
      const available_time = data.available_on_time.join();
      await Volunteer.query().where({ user_id }).update({
        id,
        pathway_id,
        user_id,
        hours_per_week,
        available_on_days,
        available_on_time: available_time,
      });
      return 'updated';
    } catch (err) {
      return [errorHandler(err), null];
