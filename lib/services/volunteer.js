...
module.exports = class UserRoleService extends Schmervice.Service {
  ...
  async getAllVolunteers(
    ...
    try {
      ...
      const date_IST = UTCToISTConverter(new Date()); // .toString();
      const last_class_data = [];
      for (let i = 0; i < res.length; i++) {
        const newArray = [];
        const num = i;
        for (const j of res[i].classes) {
          if (new Date(j.end_time) < date_IST) {
            console.log('line number 204');
            if (!newArray.includes(j)) {
              newArray.push(j);
              // res[i].classes = newArray;
              res[i].last_class = newArray;
            }
          }
        }
        // res[i].classes = newArray[newArray.length - 1];
        res[i].last_class = newArray[newArray.length - 1];
        last_class_data.push(res[i]);
        // last_class_data.push(res[i].last_class);
      }

      return [null, last_class_data];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
