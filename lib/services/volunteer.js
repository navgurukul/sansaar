  ...
  async deleteVolunteer(volunteer_IDS) {
    try {
      ...
      for (let item = 0; item < volunteer_IDS.length; item++) {
        id_of_volunteer = await Volunteer.query().select('id').where({ user_id: volunteer_IDS[item] })
        await Classes.query().patch({volunteer_id: null}).where({volunteer_id: id_of_volunteer[0].id})
