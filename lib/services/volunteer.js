...
module.exports = class UserRoleService extends Schmervice.Service {
  ...
  async updatevolunteer(data, Payloadreceiver, user_id, paramsid) {
    console.log(Payloadreceiver, '//////')
    try {
      const { Volunteer } = this.server.models();
      const letscheck = await Volunteer.query().where('user_id', paramsid.userID);
      console.log(letscheck, 'letscheck');
      const pathway_Id = letscheck[0].pathway_id;
      const hours_perweek = letscheck[0].hours_per_week;
      const available_on_Days = letscheck[0].available_on_days;
      const available_on_Time = letscheck[0].available_on_time;
      console.log(available_on_Time, pathway_Id, hours_perweek, available_on_Days, '>>>>>');
      for (const i of data[1]) {
        const voldata = await Volunteer.query().where('user_id', i.id);
        const idIntoString = paramsid.userID.toString();
        if (i.id === idIntoString) {
          console.log(';;;;;;;;;');
          // await Volunteer.query()
          //   .patch({
          //     status: Payloadreceiver,
          //   })
          //   .where({ user_id: paramsid.userID });
          await Volunteer.query()
            .update({
              user_id: paramsid.userID,
              pathway_id: pathway_Id,
              hours_per_week: hours_perweek,
              available_on_days: available_on_Days,
              available_on_time: available_on_Time,
