...
module.exports = class UserRoleService extends Schmervice.Service {
  async getAllVolunteers(createdTime, createTime2, volunteerStatus, name, lang) {
    ...
    try {
      ...
      const desire_users = [];
      if (name !== undefined) {
        const str = 'DVET- REVISION CLASS-INTRODUCTION OF PYTHON PART 2';
        const str1 = 'Class 13 - Spoken English - Manthan Darunde';
        for (const i of volunteers) {
          const sort_Class = _.sortBy(i.classes, 'start_time');
          const class_title = sort_Class.slice(-1)[0].title.toLowerCase();
          // console.log(sort_Class.length, 'jai ho kr skta hu mei');
          // console.log(sort_Class.slice(-1)[0].title, 'titleeee');
          // console.log(sort_Class.length, 'sortclass 522222222222');
          if (sort_Class.length > 0) {
            if (class_title === name.toLowerCase()) {
              desire_users.push(i);
              console.log('aa gya');
              volunteers = desire_users;
              break;
            }
          }
          // console.log('62222222 if se bahar');
        }
        // console.log('loop se bahar 644444444444')
      }
      console.log(volunteers);
      console.log(desire_users, 'desire');
      ...
      let res = filtered_res.map((v) => {
        ...
        return {
          ...
          status: v.status,
          volunteer_form: v.volunteer_form,
