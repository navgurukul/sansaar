...
module.exports = class UserRoleService extends Schmervice.Service {
  async getAllVolunteers(createdTime, createTime2, volunteerStatus, lang) {
    ...
    try {
      ...
      let langData = [];
      volunteers.map((langUserData) => {
        if (langUserData.lang_1 === lang){
          langData.push(langUserData)
          volunteers = langData
        }
      })
  ...
  async deleteVolunteer(paramsid) {
    try {
      const { Volunteer, UserRole } = this.server.models();
      console.log(paramsid, 'services');
      const removeVolunteerRoles = await UserRole.query()
        .delete()
      ...
      if (l.includes(paramsid)) {
        await Volunteer.query().delete().where({ user_id: paramsid });
      }
