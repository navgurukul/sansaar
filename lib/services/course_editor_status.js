...
module.exports = class CourseEditorService extends Schmervice.Service {
  ...
  async courseEditor_get() {
    const { CourseEditor } = this.server.models();

    try {
      const data = await CourseEditor.query().select().where('course_id', 1); // mannyally takes course_id as 1
      return [null, data];
    } catch (err) {
      return [errorHandler(err), null];
    }
  ...
  async courseEditor_update() {
    ...
    try {
      const data = await CourseEditor.query() // mannualy takes data till now
        .update({
