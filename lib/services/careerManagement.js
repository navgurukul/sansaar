...
module.exports = class CareerManagementService extends Schmervice.Service {
  ...
  async careerTeacherInviteLink(admin_id, manager_id) {
    ...
    try {
      // Verify if the cluster manager exists
      const manager = await User.query().findById(admin_id).throwIfNotFound();
      if (!manager) {
        return [{ message: "Admin not found" }, null];
      }
  
      // Check if a web link already exists for the given admin_id
      ...
      if (existingWebLink && existingWebLink.web_link) {
        // If a web link already exists, return it directly
        return [null, {
      ...
      // Create tracking URL with cluster manager ID
      ...
      // Generate a Bitly short link
      const webShortLink = await axios.post(
      ...
      return [null, {
        short_url: webShortLink.data.link,
      }];
