...
module.exports = class CareerManagementService extends Schmervice.Service {
  ...
  async clusterManagerUpdateBy(id, data) {
    const { ClusterManager, User, UserRole } = this.server.models();
    try {
      let userID
      ...
      if (oldData.email !== data.email) {
        ...
        if (point_of_contact_oldData.cluster_manager_id != null) {
          ...
          await UserRole.query().where('user_id', oldUserID).andWhere('role','clusterManager').del();
      ...
      if (point_of_contact_data !== null && point_of_contact_data !== undefined) {
        ...
        let roleAdmin = await UserRole.query().where('user_id', userID).andWhere('role', 'careerAdmin');
        ...
        let rolePrt = await UserRole.query().where('user_id', userID).andWhere('role', 'clusterManager');
        if (rolePrt.length === 0) {
          await UserRole.query().insert({ role: 'clusterManager', user_id: userID });
      ...
      if (updateData){
        ...
        let rolePrt = await UserRole.query()
  ...
  async createClusterManager(data) {
    ...
    try {
      ...
      const existingManager = await ClusterManager.query().findOne('phone_number', data.phone_number);
      // Handle duplicate - perhaps return an error or update existing record
      if (existingManager) {
        return [{
          Error: true,
          code: 403,
          message: `Cluster manager with this phone number already exists`,
          },
          null
        ];
      }
      const existingManagerEmail = await ClusterManager.query().findOne('email', data.email);
      if (existingManagerEmail) {
        return [{
          Error: true,
          code: 403,
          message: `Cluster manager with this email already exists`,
          },
          null
        ];
      }
      ...
      return [errorHandler(err.message), null];
    }
  }
  ...
  async getAllClusterManagers(limit = 10, offset = 0) {
    ...
    try {
      ...
      const data = await ClusterManager.query()
        ...
        .limit(safeLimit)
        .offset(safeOffset)
        .withGraphFetched('career_teachers.teams_data');
  ...
  async careerTeacherInviteLink(admin_id, manager_id) {
    ...
    try {
      ...
      const baseUrl = process.env.code_folding_mode === "main"
        ? process.env.CAREER_EXP_URL || 'http://localhost:5000'
        : process.env.CAREER_EXP_URL_DEV || 'http://localhost:5000';

      // Create tracking URL with admin and manager IDs
      const utmParams = new URLSearchParams({
        utm_source: "whatsapp",
        utm_content: `cluster_manager_id:${manager_id}`,
      });
  ...
  // write assignCareerAdminRole
  async assignCareerAdminRole(email) {
    ...
    try {
      ...
      const role = await UserRole.query().where('user_id', user_id).andWhere('role', 'careerAdmin').first();
      if (role) {
        return [{ message: "User already has careerAdmin role" }, null];
      }
      await UserRole.query().insert({ user_id, role: 'careerAdmin' });
      return [null, { message: "Role assigned successfully" }];
    } catch (err) {
      console.error("Assign Career Admin Role Error:", err);
      return [errorHandler(err), null];
    }
  }
