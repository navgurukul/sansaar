  ...
  async markAssessmentCompleteBYSlug(detailsArray, txn) {
    ...
    try {
      ...
      for (const details of detailsArray) {
        ...
        if (correctCount > 0 && correctCount < correct_array.length && details.selected_option.length < correct_array.length) {
          details.status = 'Partially_Correct';
        } else if (correctCount === 0 && details.selected_option.length > 1) {
          details.status = 'Fail';
        } else if (wrongCount > 0 && correctCount > 0) {
          details.status = 'Partially_Incorrect';
        } else if (correct_array.length === details.selected_option.length && correct_array.every((val, idx) => val === details.selected_option[idx])) {
          details.status = 'Pass';
        ...
        details.attempt_count = assessments.length > 0 ? assessments[0].attempt_count + 1 : 1;
