...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async validateAnswerCurrectOrNot(assessment_id) {
    ...
    try {
      ...
      // console.log(data,'this is dattaaaaaaaaaaa......');
    ...
    try {
      ...
      if (result.length > 0 && result != null) {
        ...
        if (modules !== null && modules !== undefined && modules.data.length > 0) {
          details.module_id = moduleCourseDetails?.data?.attributes?.modules?.data[0]?.id
        }
      ...
      console.log(err,'this is errorr.......');
      return [errorHandler(err), null];
    }
  }
  ...
  async studentMultipleResult(details, txn) {
    ...
    try {
      ...
      console.log(correct_array, incorrect_array,flag,'this is pgsjigdkgfdlkgffkngkndkjfn');
      // if(flag){
      //   if(details.selected_multiple_option){
      //     return [null,{ message: 'This is single choice, Please pass data into selected_option field.' }];
      //   }
      // }else{
      //   if(details.selected_option){
      //     return [null,{ message: 'This is multiple choice, Please pass data into selected_multiple_option field.' }];
      //   }
      // }
      let count = 0;
      let wrong = 0;
  ...
  async findMultipleResultById(assessment_id, user_id) {
    ...
    try {
      ...
      if (assessment.length > 0 && assessment != null) {
        ...
        result.selected_option = assessment[0].selected_option;
        result.selected_multiple_option.push(result.selected_option)

        result.selected_multiple_option = `[${result.selected_multiple_option}]`
        delete result.selected_option
      } else {
        result.attempt_status = 'NOT_ATTEMPTED';
      ...
      console.log(err,'weeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee');
      return [errorHandler(err), null];
    }
  }
};
