...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async markAssessmentCompleteBYSlug(detailsArray, txn) {
    ...
    try {
      ...
      if (count === correct_array.length) {
        details.status = 'Pass';
      } else if (count > 0 && details.selected_option.length < correct_array.length) {
        details.status = 'Partially_Correct';
  ...
  //       let count = 0;
  //       let wrong = 0;
  //       for (let option of details.selected_option) {
  ...
  //         jsonDetails.course_id = details.course_id;
  //         jsonDetails.pathway_id = getPathwayId;
  //         jsonDetails.module_id = details.module_id || null;
