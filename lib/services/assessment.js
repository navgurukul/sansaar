...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async validateAnswerCurrectOrNot(slug_id) {
    try {
      const { data } = await strapi.findOne('slugs', slug_id, {
        populate: ['assessments', 'assessments.option'],
      });
      const assessmentsData = data.attributes.assessments.data;
      const assessment = assessmentConverterV2(assessmentsData[0].attributes);
      let correct_array;
      let incorrect_array;
      let flag = false;
  ...
  async studentResult(details, txn) {
    ...
    try {
      ...
      const [correct_array, incorrect_array, flag] = await this.validateAnswerCurrectOrNot(details.assessment_id);
  ...
  async studentMultipleResult(details, txn) {
    ...
    try {
      ...
      const [correct_array, incorrect_array, flag] = await this.validateAnswerCurrectOrNot(
        details.slug_id
      );
      let count = 0;
      let wrong = 0;
      for (let i = 0; i < details.selected_multiple_option.length; i++) {
        if (correct_array.includes(details.selected_multiple_option[i])) {
          count++;
        } else if (incorrect_array.includes(details.selected_multiple_option[i])) {
          wrong++;
        }
      }
      if (
      ...
      if (result.length > 0 && result != null) {
        result = result[0];
        const selected_multiple_option = JSON.parse(result.selected_multiple_option);
        result.selected_multiple_option = selected_multiple_option;
        const jsonDetails = {};
        const { data } = await strapi.findOne('slugs', details.slug_id, {
          populate: ['assessments', 'assessments.option'],
        });

        const assessment_id = data.attributes.assessments.data[0].id;
        let assData;
        let course_id;
        try {
          assData = await strapi.findOne('assessments', assessment_id, {
            populate: ['course'],
          });
          course_id = assData.data.attributes.course.data.id;
        } catch (error) {
