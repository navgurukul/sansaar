...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async validateAnswerCurrectOrNot(assessment_id) {
    try {
      ...
      let solustion = assessment[2].value;
      let checkSingle = assessment[2];
      let flag = false;
      if (checkSingle.type === 'single') {
        flag = true;
      }
      let solution_array = [];
      for (let solu = 0; solu < solustion.length; solu++) {
        solution_array.push(solustion[solu].value);
      }
      return [flag,solution_array];
    } catch (err) {
      logger.error(JSON.stringify(err));
      return [errorHandler(err), null];
    }
  ...
  async studentResult(details, txn) {
    ...
    try {
      ...
      let [flag,validateAnswerCurrectOrNot] = await this.validateAnswerCurrectOrNot(details.assessment_id);
      if(flag){
        if(details.selected_option.length > 1){
          return [null, 'Please select only one option'];
        }
      }else{
        if(details.selected_option.length < 2){
          return [null, 'Please select atleast two option'];
        }
      }
      const arraysAreEqual =
        details.selected_option.length === validateAnswerCurrectOrNot.length &&
        details.selected_option.every(
          (value, index) => value === validateAnswerCurrectOrNot[index]
        );
      if (arraysAreEqual) {
      ...
      if (assessment.length > 0 && assessment != null) {
        ...
        if (assessment[0].selected_option) {
          details.selected_option = details.selected_option[0];
          ...
          let multiple_choice = JSON.stringify(details.selected_option);
          delete details.selected_option;
          details.multiple_choice = multiple_choice;
        ...
        if (details.selected_option.length > 1) {
          let multipleOption = JSON.stringify(details.selected_option);
          delete details.selected_option;
          details.multiple_choice = multipleOption;
  ...
  async findResultById(assessment_id, user_id) {
    ...
    try {
      ...
      if (assessment.length > 0 && assessment != null) {
        ...
        let selOptions = assessment[0].selected_option;
        if(!selOptions){
          let opt = JSON.parse(assessment[0].multiple_choice); 
