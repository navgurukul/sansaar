...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async findAssessmentByCourseDetails(
    ...
    const res = await axios.get(
      `${merdBaseURL}/exercises/${exerciseDetail.id}?populate[0]=assessment`
    );
    const assesData = res.data.data;
    const asses = await assesData.attributes.assessment.data.map((ele) => {
      return {
        id: ele.id,
        course_id: courseId,
    ...
    if (asses.length > 0) {
      ...
      for (const file of asses) {
        ...
        newExercise.content = asses[0].content;
