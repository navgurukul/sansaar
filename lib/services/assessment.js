...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  // this method is for removing \n and \\ from the string from the assessments.
  async removeNewlinesAndBackslashes(str) {
    // Remove \n and \\ from the string
    return str.replace(/\\n/g, '').replace(/\\\\/g, '\\');
  }
  ...
  async findAssessmentByCourseDetails(
    ...
    const asses = await data.attributes.assessment.data.map((ele) => {
      // console.log(assessmentConverter(ele.attributes),"ele");
      return {
        id: ele.id,
        course_id: courseId,
        exercise_id: exerciseDetail.id,
        content: this.removeNewlinesAndBackslashes(assessmentConverter(ele.attributes)),
