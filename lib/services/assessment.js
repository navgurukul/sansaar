...
module.exports = class AssessmentService extends Schmervice.Service {
  async updateJsonAssessmentContent() {
    ...
    try {
      ...
      for (const data of assessments) {
        ...
        const courseFolderName = `assessment/${courseDetails.name}_${data.course_id}/${exercise.name}/${data.name}.json`;
        if (fs.existsSync(`${courseFolderName}`)) {
          const DataInString = JSON.parse(fs.readFileSync(courseFolderName));
          // eslint-disable-next-line
          for (const jsonData of DataInString) {
            if (jsonData.component === 'questionCode' && jsonData.value) {
              const token = marked.lexer(jsonData.value);
              if (token[0] !== undefined && !token[0].tokens[0].text.includes('=')) {
                let codeText = '';
                let splitData = '';
                // eslint-disable-next-line
                for (const tokenText of token[0].tokens) {
                  if (!tokenText.text.includes('=')) {
                    // eslint-disable-next-line
                    codeText += tokenText.text + ' ';
                    if (tokenText.text !== '<br>') {
                      splitData = tokenText.text;
                    }
                  } else {
                    break;
                  }
                ...
                if (splitData !== '') {
                  const leftText = jsonData.value.split(splitData);
                  // eslint-disable-next-line
                  jsonData.value = leftText[1];
                  const jsonContent = {};
                  jsonContent.component = 'questionExpression';
                  jsonContent.type = jsonData.type;
                  jsonContent.title = '';
                  jsonContent.value = `${codeText}`;
                  DataInString.unshift(jsonContent);
                  splitData = '';
                }
              }
            }
          }
          const path_name = `assessment/${courseDetails.name}_${data.course_id}/${exercise.name}`;
          const newContent = JSON.stringify(DataInString);
          if (fs.existsSync(`${path_name}`)) {
            fs.writeFileSync(
              path.resolve(`${path_name}/${data.name}.json`),
              JSON.stringify(JSON.parse(newContent), null, '\t')
            );
          }
          // eslint-disable-next-line
          ...
          // eslint-disable-next-line
          await Assessment.query().patch(data).where('id', data.id);
        }
      }
      return [null, 'updated'];
    } catch (err) {
