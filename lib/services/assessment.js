...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async markAssessmentCompleteBYSlug(detailsArray, txn) {
    ...
    try {
      ...
      for (const details of detailsArray) {
        ...
        const assessments = await assessmentsHistory.query().where('slug_id', details.slug_id).andWhere(user_team);
        ...
        const correctCount = details.selected_option.filter(option => correct_array.includes(option)).length;
        const wrongCount = details.selected_option.filter(option => incorrect_array.includes(option)).length;
  
        details.status = this.determineStatus(correctCount, wrongCount, correct_array, details.selected_option);
        details.attempt_count = assessments.length ? assessments[0].attempt_count + 1 : 1;
        details.selected_option = JSON.stringify(details.selected_option);
        
        if (assessments.length) {
          await assessmentsHistory
          .query(txn).patch(details)
        ...
        const result = await assessmentsHistory
        .query()
        .where('slug_id', details.slug_id)
        .andWhere(user_team).first();
        if (result) {
  ...
  determineStatus(correctCount, wrongCount, correct_array, selected_option) {
    if (correctCount > 0 && correctCount < correct_array.length && selected_option.length < correct_array.length) {
      return 'Partially_Correct';
    } else if (correctCount === 0 && selected_option.length > 1) {
      return 'Fail';
    } else if (wrongCount > 0 && correctCount > 0) {
      return 'Partially_Incorrect';
    } else if (correct_array.length === selected_option.length && correct_array.every((val, idx) => val === selected_option[idx])) {
      return 'Pass';
    } else if (selected_option.length > 0 && selected_option.every((val, idx) => val === correct_array[idx])) {
      console.log("ASDFGHJKL",_.every)
      return 'Pass';
    } else {
      return 'Fail';
    }
