...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async validateAnswerCurrectOrNot(assessment_id) {
    try {
      ...
      const assessment = JSON.parse(assessmentConverter(data.attributes));
      let correct_array;
      let incorrect_array;
      let flag = false;
  ...
  async studentResult(details, txn) {
    ...
    try {
      ...
      console.log(correct_array, incorrect_array, flag, 'correct_array,incorrect_array,flag');
      if (flag) {
        if (details.selected_option.length > 1) {
          return [null, 'Please select only one option'];
        }
      }
      let count = 0;
      let wrong = 0;
      ...
      const arraysAreEqual =
        ...
        details.selected_option.every(
          (value, index) => value === correct_array[index]
        );
      if (arraysAreEqual) {
      ...
      if (assessment.length > 0 && assessment != null) {
        ...
        let selected_option = JSON.stringify(details.selected_option);
        delete details.selected_option;
      ...
      if (result.length > 0 && result != null) {
        // eslint-disable-next-line
        result = result[0];
        let selected_option = JSON.parse(result.selected_option);
  ...
  async findResultById(assessment_id, user_id) {
    ...
    try {
      ...
      if (assessment.length > 0 && assessment != null) {
        ...
        const userSelectedOptions = JSON.parse(assessment[0].selected_option);
        let selected_option;
        const inArr = [];
        if (typeof userSelectedOptions === 'number') {
          inArr.push(userSelectedOptions);
          selected_option = inArr;
        } else {
          selected_option = userSelectedOptions;
        }
        result.selected_option = selected_option;
  ...
  async studentMultipleResult(details, txn) {
    ...
    try {
      ...
      result.selected_multiple_option = JSON.parse(result.selected_multiple_option);
      return [null, result];
    } catch (err) {
      return [errorHandler(err), null];
    }
  ...
  async findMultipleResultById(assessment_id, user_id) {
    const { assessmentOutcome } = this.server.models();
    const result = {};
    try {
      const assessment = await assessmentOutcome
      ...
      if (assessment.length > 0 && assessment != null) {
        if (assessment[0].status === 'Pass') {
          ...
          result['attempt_status'] = 'INCORRECT';
          console.log(result, 'after destroying result..');
        }
        if (assessment[0].status === 'Partially_Correct') {
          result.attempt_status = 'PARTIALLY_CORRECT';
        } else if (assessment[0].status === 'Partially_Incorrect') {
        ...
        const userSelectedOptions = JSON.parse(assessment[0].selected_multiple_option);
        let selected_multiple_option;
        const inArr = [];
        if (typeof userSelectedOptions === 'number') {
          inArr.push(userSelectedOptions);
          selected_multiple_option = inArr;
        } else {
          selected_multiple_option = userSelectedOptions;
        }
      ...
      console.log(err, 'thnjketlkekletlkrtlkr');
      return [errorHandler(err), null];
    }
  }
};
