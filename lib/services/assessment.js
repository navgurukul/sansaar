...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async postAssessmentsBySlug(detailsArray, txn) {
    const { assessmentsHistory } = this.server.models();
    const { coursesServiceV2, progressTrackingService, exercisesServiceV2 } = this.server.services();
    try {
      let results = [];
      for (let details of detailsArray) {
        const user_team = details.team_id ? { 'team_id': details.team_id } : { 'user_id': details.user_id };
        const assessment = await this.getAssessment(assessmentsHistory, details, user_team);
        const [correct_array, incorrect_array, flag] = await this.validateSlug(details.slug_id, details.course_id);
        if (typeof flag === 'string') {
          return [{ message: flag, code: 400 }, null];
        }
        const status = this.determineStatus(details, correct_array, incorrect_array);
        details.status = status;
        details.attempt_count = this.getAttemptCount(assessment);
        details.selected_option = JSON.stringify(details.selected_option);
        await this.saveAssessment(assessmentsHistory, assessment, details, user_team, txn);
        let result = await this.getResult(assessmentsHistory, details, user_team);
        if (result.length > 0 && result != null) {
          result = result[0];
          await this.updateProgressTracking(details, result, coursesServiceV2, progressTrackingService, exercisesServiceV2);
        }
  ...
  async getAssessment(assessmentsHistory, details, user_team) {
    return await assessmentsHistory.query().where('slug_id', details.slug_id).andWhere(user_team);
  }
  
  determineStatus(details, correct_array, incorrect_array) {
    let count = 0;
    let wrong = 0;
  
    details.selected_option.forEach(option => {
      if (correct_array.includes(option)) {
        count++;
      } else if (incorrect_array.includes(option)) {
        wrong++;
      }
    });
  
    if (count < correct_array.length && count > 0 && details.selected_option.length < correct_array.length) {
      return 'Partially_Correct';
    } else if (count === 0 && details.selected_option.length > 1) {
      return 'Fail';
    } else if (wrong > 0 && count > 0) {
      return 'Partially_Incorrect';
    } else if (this.arraysAreEqual(details.selected_option, correct_array)) {
      return 'Pass';
  ...
  getAttemptCount(assessment) {
    if (assessment.length > 0 && assessment != null) {
      return assessment[0].attempt_count + 1;
    }
    return 1;
  }
  
  async saveAssessment(assessmentsHistory, assessment, details, user_team, txn) {
    if (assessment.length > 0 && assessment != null) {
      await assessmentsHistory.query(txn).patch(details).where('slug_id', details.slug_id).andWhere(user_team);
    } else {
      await assessmentsHistory.query(txn).insert(details);
    }
  }
  
  async getResult(assessmentsHistory, details, user_team) {
    return await assessmentsHistory.query().where('slug_id', details.slug_id).andWhere(user_team);
  }
  
  async updateProgressTracking(details, result, coursesServiceV2, progressTrackingService, exercisesServiceV2) {
    const [errInPathwayId, getPathwayId] = await coursesServiceV2.getPathwayIdByCourseId(details.course_id);
    ...
    const jsonDetails = {
      ...
      pathway_id: getPathwayId,
      module_id: details.module_id ? details.module_id : null,
