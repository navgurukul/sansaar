    ...
    try {
      ...
      if (assessmentDetail !== undefined) {
        ...
        if (courseDetails !== undefined && exerciseDetails !== undefined) {
          ...
          if (fs.existsSync(`${courseFolderName}`)) {
            const AssmntJson = `${courseFolderName}/${assessmentDetail.name}.json`;
            if (fs.existsSync(AssmntJson)) {
              const propertiesAssmnt = `${courseFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${assessmentDetail.name}_en.json`;
              const modifiedAssmnt = `${courseFolderName}/PARSED_CONTENT/MODIFIED_FILES/${assessmentDetail.name}.json`;
              fs.unlinkSync(propertiesAssmnt);
              fs.unlinkSync(modifiedAssmnt);
            }
            fs.unlinkSync(AssmntJson);
          }
