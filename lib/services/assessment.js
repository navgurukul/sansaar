...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async getSlugsPreQuiz(prequiz) {
    try {
      const requiredLangData = [];
      requiredLangData.push(assessmentConverterV2(prequiz.attributes));

      const assessmentsObj = requiredLangData.flat();
      const solutionObject = assessmentsObj.find(item => item.component === 'solution');

      const assType = solutionObject?.type || null;
      const maxSelectionCount = solutionObject?.correct_options_value?.length || 0;

      const assessment = {
        content_type: 'prequiz',
        max_selection_count: maxSelectionCount,
        assessment_type: assType,
        content: assessmentsObj,
      };
  ...
  async DeleteAssessmentResultByUserOrTeam(user_team, txn) {
    ...
    return assessmentsHistory
      .query(txn)
      .delete()
      .where(user_team)
      .then(() => {
        return exercisessHistory
          .query(txn)
          .delete()
          .where(user_team);
      })
  ...
  async markAssessmentCompleteBYSlug(detailsArray, txn) {
    ...
    try {
      ...
      for (const details of detailsArray) {
        const userTeam = getUserTeam(details);
        const assessment = await getAssessmentHistory(assessmentsHistory, details, userTeam);
        const [correctArray, incorrectArray, validationFlag] = await this.validateSlug(details.slug_id, details.course_id);

        if (typeof validationFlag === 'string') {
          return [{ message: validationFlag, code: 400 }, null];
        }

        details.status = calculateAssessmentStatus(details.selected_option, correctArray, incorrectArray);
        details.attempt_count = getAttemptCount(assessment);
        details.selected_option = JSON.stringify(details.selected_option);

        if (assessment?.length) {
          await updateAssessmentHistory(assessmentsHistory, txn, details, userTeam);
        } else {
          await insertAssessmentHistory(assessmentsHistory, txn, details);
        }

        const updatedResult = await getUpdatedAssessmentHistory(assessmentsHistory, details, userTeam);
        if (updatedResult?.length) {
          const jsonDetails = await prepareProgressDetails(
            coursesServiceV2,
            details,
            updatedResult[0]
          );
          ...
          await exercisesServiceV2.courseModulePathwayPercentCalculator(
            details.user_id,
            jsonDetails.course_id,
            jsonDetails.team_id,
            jsonDetails.module_id,
            jsonDetails.pathway_id
          );
        }

        updatedResult[0].selected_option = JSON.parse(updatedResult[0].selected_option);
        results.push(updatedResult[0]);
      }
...
function getUserTeam(details) {
  return details.team_id ? { team_id: details.team_id } : { user_id: details.user_id };
}

async function getAssessmentHistory(assessmentsHistory, details, userTeam) {
  return assessmentsHistory.query().where('slug_id', details.slug_id).andWhere(userTeam);
}

function calculateAssessmentStatus(selectedOptions, correctArray, incorrectArray) {
  let count = 0;
  let wrong = 0;
  ...
  for (const option of selectedOptions) {
    if (correctArray.includes(option)) {
      count++;
    } else if (incorrectArray.includes(option)) {
      wrong++;
    }
  }

  if (count === correctArray.length) return 'Pass';
  if (count > 0 && selectedOptions.length < correctArray.length) return 'Partially_Correct';
  if (count === 0 && selectedOptions.length > 1) return 'Fail';
  if (wrong > 0) return 'Partially_Incorrect';
  return 'Fail';
}

function getAttemptCount(assessment) {
  return assessment?.[0]?.attempt_count ? assessment[0].attempt_count + 1 : 1;
}

async function updateAssessmentHistory(assessmentsHistory, txn, details, userTeam) {
...
async function insertAssessmentHistory(assessmentsHistory, txn, details) {
  await assessmentsHistory.query(txn).insert(details);
}

async function getUpdatedAssessmentHistory(assessmentsHistory, details, userTeam) {
  return assessmentsHistory.query().where('slug_id', details.slug_id).andWhere(userTeam);
}

async function prepareProgressDetails(coursesServiceV2, details, assessment) {
  const [errInPathwayId, pathwayId] = await coursesServiceV2.getPathwayIdByCourseId(details.course_id);
  if (errInPathwayId) return null;
  if (details.team_id) {
    const moduleCourseDetails = await strapi.findOne('courses', details.course_id, { populate: ['modules'] });
    const modules = moduleCourseDetails?.data?.attributes?.modules?.data;
    if (modules?.id) {
  ...
  return {
    user_id: details.user_id ? +details.user_id : null,
    team_id: details.team_id ? +details.team_id : null,
    slug_id: details.slug_id,
