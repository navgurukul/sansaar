    ...
    try {
      ...
      if (assessmentDetail) {
        if (assessmentDetail) {
          const courseDetails = await CoursesV2.query().findById(assessmentDetail.course_id);
          const exercise = await ExercisesV2.query().findById(assessmentDetail.exercise_id);
          const courseFolderName = `assessment/${courseDetails.name}_${assessmentDetail.course_id}/${exercise.name}`;
          console.log(courseFolderName);
          const AssmntJson = `${courseFolderName}/${assessmentDetail.name}.json`;
          if (fs.existsSync(`${courseFolderName}`)) {
            if (fs.existsSync(AssmntJson)){
              const assmnt = `${courseFolderName}/PARSED_CONTENT/MODIFIED_FILES/${assessmentDetail.name}_en.json`;
            }
            fs.removeSync(assmnt);
            fs.removeSync(AssmntJson);
          }
