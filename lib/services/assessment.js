...
module.exports = class AssessmentService extends Schmervice.Service {
  ...
  async checkMultipleOrSingle(assessment_id) {
    try {
      const { data } = await strapi.findOne('assessments', assessment_id, {
        populate: ['dynamic', 'explaination'],
      });
      ...
      for (let ind = 0; ind < assessment.length; ind++) {
        const element = assessment[ind];
        if(element.component === 'solution'){
  ...
  async studentResult(details, txn) {
    ...
    try {
      ...
      let checkMultipleOrSingle = await this.checkMultipleOrSingle(details.assessment_id);
      if(!checkMultipleOrSingle){
        if(details.selected_option.length === 1){
          return [null, 'Please select multiple options']
        }
      }
      if(checkMultipleOrSingle){
        if(details.selected_option.length > 1){
          return [null, 'Please select single option']
        }
      }
      if (assessment.length > 0 && assessment != null) {
        ...
        if(assessment[0].selected_option){
          ...
          let multiple_choise = JSON.stringify(details.selected_option)
          delete details.selected_option
          details.multiple_choise = multiple_choise;
        ...
        if(details.selected_option.length > 1){
          ...
          delete details.selected_option
          details.multiple_choise = multipleOption
