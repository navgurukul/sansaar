...
module.exports = class ChatService extends Schmervice.Service {
  async handleCommand(roomId, event) {
    ...
    if (body.toLowerCase().startsWith('hindi') || body.toLowerCase().startsWith('english')) {
      ...
      if (!userEmail[0].email.includes('@fake')) {
        ...
        const existingRooms = await botFunctions.getPublicRoomId(`general${langCode}`);
    ...
    if (body.toLowerCase().startsWith('join')) {
      let splitted = body.toLowerCase().split('-');
      if (splitted.length <= 1) {
        splitted = body.toLowerCase().split(' ');
      }

      const roomToJoin = splitted.pop();
      const roomIdToJoin = botHandling.getRoomId(roomToJoin);
  ...
  async sendNewClassNotification(newClass) {
    ...
    message.body = botHandling.newClassNotification.body.replace('---TIME---', time);
    if (facilitator_name) {
    ...
    const allUsers = User.query().select('chat_id').whereNotNull();
    allUsers.forEach((user) => {
      const chatId = `@${user.chat_id}:navgurukul.org`;
      const privateRoomId = botFunctions.getPrivateRoomId(chatId);
      // await client.sendMessage(privateRoomId, 'm.room.message', )
    });
  }
