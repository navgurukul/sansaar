...
module.exports = class ChatService extends Schmervice.Service {
  ...
  async sendInitialMessageToStudent(roomId, id) {
    try {
      const { server } = this;
      const { User } = this.server.models();
      const client = server.app.chatClient;
      ...
      if (user[0].lang_1 == null) {
        const message = await botHandling.initialMessageForStudent(user[0].name, 'en');
        await client.sendMessage(roomId, message);
      } else {
        const message = await botHandling.initialMessageForStudent(user[0].name, user[0].lang_1);
        await client.sendMessage(roomId, message);
      }
      return this.res.data;
    } catch {
  ...
  async sendInitialMessageToFacilitator(roomId, id) {
    try {
      ...
      const client = server.app.chatClient;
      const user = await User.query().where('id', id);
      const message = await botHandling.initialMessageForFacilitator(user[0].name);
      await client.sendMessage(roomId, message);
      return this.res.data;
    } catch {
      return {
