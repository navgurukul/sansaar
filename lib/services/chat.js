...
module.exports = class ChatService extends Schmervice.Service {
  ...
  async removeUsersToMerakiClass(roomId, userId) {
    this.res = null;
    try {
      this.res = await axiosAuth.post(`/_matrix/client/r0/rooms/${roomId}/kick`, {
        reason: 'user unregistered the class',
        user_id: userId,
      });
      return this.res.data;
    } catch (err) {
      return {
        error: true,
        message:
          'Cannot remove user to the specified room. Room id could be incorrect or the user is already remove to the room.',
      };
    }
  }
