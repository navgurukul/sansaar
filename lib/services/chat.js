...
module.exports = class ChatService extends Schmervice.Service {
  async handleCommand(roomId, event) {
    ...
    const privateRoomId = await botHandling.getPrivateRoomId(event.sender);
  ...
  async sendClassJoinConfirmation(meetLink, startTime, userId) {
    ...
    const privateRoomId = await botHandling.getPrivateRoomId(chatId);
  ...
  async userUpdateProfile(name, chatId, token) {
    this.name = name;
    this.token = token;
    this.chatId = chatId;
    await axiosUnAuth
