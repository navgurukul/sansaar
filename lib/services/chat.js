...
module.exports = class ChatService extends Schmervice.Service {
  ...
  async addUsersIntoCohortRoom(roomId, userId, id) {
    this.res = null;
    try {
      const { server } = this;
      const { User } = this.server.models();
      ...
      if (user[0].lang_1 == null) {
        await client.sendMessage(roomId, botHandling.CohortChatRoomForStudent.en);
      } else {
        await client.sendMessage(roomId, botHandling.CohortChatRoomForStudent[user[0].lang_1]);
      }
      return this.res.data;
    } catch {
      return {
  ...
  async sendFacilitatorMessage(roomId, userId) {
    this.res = null;
    try {
      const { server } = this;
      const client = server.app.chatClient;
      ...
      await client.sendMessage(this.roomId, botHandling.thankToFacilitator);
      return this.res.data;
    } catch {
      return {
        error: true,
