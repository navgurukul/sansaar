...
module.exports = class ChatService extends Schmervice.Service {
  async handleCommand(roomId, event) {
    ...
    if (body.toLowerCase().startsWith('hindi') || body.toLowerCase().startsWith('english')) {
      ...
      if (!userEmail[0].includes('@fake')) {
        console.log('*******************************');
        console.log('This is a GENUINE ID');
        console.log('*******************************');

        // Find existing agastya class rooms
      ...
      console.log('*******************************');
      console.log(await botFunctions.languageSelected(client, roomId));
      console.log(roomId);
      console.log('*******************************');
