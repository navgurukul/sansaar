...
module.exports = class ChatService extends Schmervice.Service {
  ...
  async getMerakiClassRooms() {
    return chatKnex('public.room_aliases')
  ...
  async addUsersToMerakiClass(roomId, userId) {
    try {
      const res = await axiosAuth.post(`/_synapse/admin/v1/join/${roomId}`, {
        user_id: userId,
      });
      ...
      return {
        error: true,
        message: 'Cannot add user to the specified room. Room id could be incorrect.',
      };
    }
  }
};
