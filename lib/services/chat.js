...
module.exports = class ChatService extends Schmervice.Service {
  async handleCommand(roomId, event) {
    ...
    const retrieveLastByMeraki = await bolKnex
      .select('*')
      .from('public.events')
    ...
    const privateRoomId = await bolKnex('public.users_who_share_private_rooms')
    ...
    if (!body.startsWith('xformat')) {
      if (msgtype !== ('org.matrix.buttons' || 'org.matrix.response')) return;
    }

    const content = botHandling.buttonFormat;

    await client.sendEvent(roomId, 'm.room.message', content, '');
    if (event.content.msgtype !== 'org.matrix.response') {
  ...
  async createChatRoom(chatUserId) {
    ...
    const initialMessage = botHandling.initialMessage;
