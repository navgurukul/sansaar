...
module.exports = class ChatService extends Schmervice.Service {
  async handleCommand(roomId, event) {
    ...
    if (body.toLowerCase().startsWith('join')) {
      ...
      if (roomToJoin.includes('hiclass' || 'enclass')) {
        roomIdToJoin = await botFunctions.getPublicRoomId(roomToJoin);
      }
  ...
  async sendClassJoinConfirmation(meetLink, startTime, userId) {
    ...
    let classTime = startTime.toISOString().split('T')[1].split('').slice(0, -8).join('');
    ...
    message.label = botHandling.classJoined.label.replace(/---TIME---/g, classTime);
    message.label = botHandling.classJoined.label.replace(/---DATE---/g, classDate);
  ...
  async sendScheduledMessage(privateRoomId, classDetails) {
    const client = this.server.app.chatClient;
    const { title, type, meet_link } = classDetails;
    const message = botHandling.classReminder;
    message.label = botHandling.classReminder.label.replace('--CLASS--', title);
    message.body = botHandling.classReminder.body.replace('^TYPE^', type);
    message.options[0].value = botHandling.classReminder.options[0].value.replace(
      '^MEET_LINK^',
      meet_link
    );
  ...
  async sendNewClassNotification(newClass) {
    const { server } = this;
    const client = server.app.chatClient;
    const classTypes = { doubt_class: 'doubt class', workshop: 'workshop' };

    const { title, type, facilitator_name, start_time } = newClass;

    let classTime = start_time.toISOString().split('T')[1].split('').slice(0, -8).join('');
    classTime = timeFormatter(classTime);
    const classDate = start_time.toISOString().split('T')[0];

    const message = botHandling.newClassNotification;
    message.body = botHandling.newClassNotification.body.replace('---TYPE---', classTypes[type]);
    message.body = botHandling.newClassNotification.body.replace('---TITLE---', title);
    message.body = botHandling.newClassNotification.body.replace('---TIME---', classTime);
    message.body = botHandling.newClassNotification.body.replace('---DATE---', classDate);
    if (facilitator_name) {
      message.body = botHandling.newClassNotification.body.replace(
        ' ---The class will be conducted by ---',
        `. The class will be conducted by ${facilitator_name}`
      );
    } else {
      message.body = botHandling.newClassNotification.body.replace(
        ' ---The class will be conducted by ---',
    ...
    const privateRoomIds = await botFunctions.getPrivateRoomId();

    privateRoomIds.forEach(async (room) => {
      await client.sendMessage(room.room_id, message);
    });
  }
