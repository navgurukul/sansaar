...
module.exports = class ChatService extends Schmervice.Service {
  ...
  async sendClassJoinConfirmation(meetLink, startTime, userId) {
    ...
    message.options[0].value = message.options[0].value.replace('^MEET_LINK^', meetLink);
  ...
  async sendNewClassNotification(newClass) {
    ...
    const message = _.cloneDeep(botHandling.newClassNotification);
    message.body = message.body.replace('---TYPE---', classTypes[type]);
    message.body = message.body.replace('---TITLE---', title);
    message.body = message.body.replace('---TIME---', classTime);
    message.body = message.body.replace('---DATE---', classDate);
    if (facilitator_name) {
      ...
      message.body = message.body.replace(' ---The class will be conducted by ---', ``);
    }
