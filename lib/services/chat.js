...
module.exports = class ChatService extends Schmervice.Service {
  async handleCommand(roomId, event) {
    ...
    if (body.toLowerCase().startsWith('hindi') || body.toLowerCase().startsWith('english')) {
      ...
      const existingRooms = await botFunctions.getPublicRoomId(langCode);
      console.log('**************************************************************');
      ...
      if (existingRooms.length > 0) {
        ...
        if (userList.length <= 3) {
          ...
          if (isAdded) {
            await client.sendMessage(privateRoomId, {
              msgtype: 'text',
              body: `Successfully added you to a class room`,
            });
          }
        }
      } else {
    ...
    if (body.toLowerCase().startsWith('join')) {
      ...
      await botFunctions.joinARoom(client, sender, roomIdToJoin, roomToJoin);
    }
  }
