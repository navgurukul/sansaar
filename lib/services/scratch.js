...
module.exports = class ScratchService extends Schmervice.Service {
  async uploadFile(fileObj, userId_scratch) {
    ...
    try {
      ...
      store.MerakiUrl = `https://www.merakilearn.org/scratch/project/project_id:${project_id}`;
      return [null, store];
    } catch (e) {
  ...
  async updateFile(project_id, file, userId_scratch) {
    ...
    try {
      ...
      if (project_id_from_table.length === 0) {
        ...
        MerakiUrl = `https://www.merakilearn.org/scratch/project/project_id:${projectId}`;
      } else {
