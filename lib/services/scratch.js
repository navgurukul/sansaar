...
module.exports = class ScratchService extends Schmervice.Service {
  ...
  async uploadFile(fileObj, userId_scratch, project_name) {
    ...
    const scratchProjects = await Scratch.query().select('project_name').where('project_name', 'LIKE', `%${project_name}%`);
    const allProjectsNames = scratchProjects.map(project => {
      if (project.project_name !== undefined && project.project_name !== null) {
        return project.project_name.replace(/\.sb3$/, '');
      }
    ...
    try {
      ...
      const store = await Scratch.query().insert({ project_id, url, userId_scratch, project_name:NewName+'.sb3' });
  ...
  async updateFile(project_id, file, userId_scratch, project_name) {
    try {
      ...
      const updatedProject = await Scratch.query().findById(project.id).patch({ url, project_name:project_name+".sb3" });      const scratch_url = `https://www.scratch.merakilearn.org/projects/${project_id}`;
