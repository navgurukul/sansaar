...
module.exports = class ScratchService extends Schmervice.Service {
  ...
  async uploadFile(fileObj, userId_scratch, project_name) {
    ...
    const scratchProjects = await Scratch.query().select('project_name')
    const c = scratchProjects.map(project => project.project_name);
    const b = (changedName) => {
      if (c.includes(changedName)) {
        return false;
      }
      return true;
    };
    ...
    try {
      ...
      const store = await Scratch.query().insert({ project_id, url, userId_scratch, project_name:NewName });
