...
module.exports = class ScratchService extends Schmervice.Service {
  ...
  async DeleteProject_By_userId_scratch(userId_scratch) {
    const { Scratch } = this.server.models();
    try {
      const project_dertais = await Scratch.query().where('userId_scratch', userId_scratch);
      if (project_dertais.length > 0) {
        // eslint-disable-next-line no-plusplus
        for (let ind = 0; ind < project_dertais.length; ind++) {
          const Key = project_dertais[ind].url.split('amazonaws.com/')[1];
          const deleteParams = { Bucket: 'chanakya-dev', Key };
