...
module.exports = class ScratchService extends Schmervice.Service {
  async uploadFile(fileObj, userId_scratch) {
    ...
    try {
      ...
      const store = await Scratch.query().insert({ project_id, url, userId_scratch });
      // https://www.merakilearn.org/scratch/project/project_id:06c78b35-b56d-4380-b557-bfac4dd23da4"
      // https://scratch.mit.edu/projects/782877788
      store.userId = userId_scratch;
      store.scratch_url = `https://scratch.mit.edu/projects/${project_id}`;
      return [null, store];
    } catch (e) {
  ...
  async updateFile(project_id, file, userId_scratch) {
    const { Scratch } = this.server.models();
    ...
    try {
      ...
      if (project_id_from_table.length === 0) {
        ...
        store = await Scratch.query().insert({ project_id, url, userId_scratch });
        scratch_url = `https://scratch.mit.edu/projects/${projectId}`;
      } else {
        ...
        scratch_url = `https://scratch.mit.edu/projects/${project_id}`;
      }
  ...
  async getScratchFile(userId_scratch) {
    ...
    try {
      ...
      const storeData = await Scratch.query().select('url').where('userId_scratch', userId_scratch);
