...
module.exports = class ScratchService extends Schmervice.Service {
  async uploadFile(fileObj, userId_scratch, project_name) {
    ...
    const imageBuffer = happy.base64.decode(fileObj);
    console.log(imageBuffer);
    console.log(key, 'BHUPENDRA DESHMUKH...........');
    try {
  ...
  async getScratchFile(userId_scratch) {
    ...
    try {
      let storeData;
      storeData = await Scratch.query().select().where('userId_scratch', userId_scratch);
      const desire_data = [];
