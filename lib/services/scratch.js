...
module.exports = class ScratchService extends Schmervice.Service {
  ...
  async uploadFile(fileObj, userId_scratch, project_name) {
    ...
    const allProjectsNames = scratchProjects.map(project => {
      const removeSb3 = project.project_name.replace(/\.sb3$/, '');
      const removeBrackets = removeSb3.replace(/\((.*?)\)/g, (match, number) => number);
  ...
  async updateFile(project_id, file, userId_scratch, project_name) {
    try {
      ...
      if (allProjectsNames.length != 1) {
        NewName = this.createNameValidate(project_name, allProjectsNames);
      }
      ...
      const updatedProject = await Scratch.query()
        .findById(project.id)
        .patch({ url, project_name: `${NewName}.sb3` });
  ...
  async DeleteFile(project_id) {
    ...
    try {
      ...
      if (projectFile.length > 0) {
        ...
        const deleteParams = { Bucket: CONSTANTS.auth.aws.scratchBucket, Key };
