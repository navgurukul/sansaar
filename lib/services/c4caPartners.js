...
module.exports = class C4caPartnerService extends Schmervice.Service {
    ...
    try {
      ...
      if (oldData.email !== data.email) {
        ...
        if (point_of_contact_oldData.c4ca_partner_id != null) {
          await User.query()
            .throwIfNotFound()
            .patchAndFetchById(oldUserID, { c4ca_partner_id: null });
      ...
      if (point_of_contact_data !== null && point_of_contact_data !== undefined) {
        ...
        const { c4ca_partner_id } = point_of_contact_data;
  ...
  async getAllC4caPartnerData(limit, offset) {
    const { C4caPartners } = this.server.models();
    try {
      const data = await C4caPartners.query()
        .page(offset, limit)
        .withGraphFetched('teachers.teams_data');
        ...
        if (teachers && teachers.length > 0) {
          for (const teacher of teachers) {
            if (teacher.teams_data && teacher.teams_data.length > 0) {
              for (const team of teacher.teams_data) {
  ...
  async createFacilitator(data) {
    ...
    try {
      ...
      if (point_of_contact_data !== null && point_of_contact_data !== undefined) {
        userID = parseInt(point_of_contact_data.id);
        const { c4ca_facilitator_id, c4ca_partner_id } = point_of_contact_data;
      ...
      if (res) {
        userID = parseInt(point_of_contact_data.id);
        const re = await User.query().throwIfNotFound().patchAndFetchById(userID, {
  ...
  async c4caTeacherInviteLink(partner_id, facilitator_id) {
    ...
    try {
      ...
      const data = await Facilitator.query().throwIfNotFound().patchAndFetchById(facilitator_id, {
