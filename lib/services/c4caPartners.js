...
module.exports = class C4caPartnerService extends Schmervice.Service {
  ...
  async c4caTeacherInviteLink(partner_id, facilitator_id) {
    const { urlShortnerService } = this.server.services();
    ...
    try {
      ...
      let webShortLink;
      try {
          const [err, shortUrlResponse] = await urlShortnerService.createUrlShortner(c4ca_web_link);
          if (err) {
        ...
        if(!shortUrlResponse.short_code){
          return [
            {
              Error: true,
              code: 500,
              message: `Error in creating short link`,
            },
          ];
        }

        webShortLink = process.env.BASE_URL + `${/s/}` + `${shortUrlResponse.short_code}`;

      } catch (err) {
        return [errorHandler(err), null];
      }
        let data = await Facilitator.query().throwIfNotFound().patchAndFetchById(facilitator_id, {
        web_link: webShortLink
      });
      return [null, data];  
    } catch (err) {
      return [errorHandler(err), null];
