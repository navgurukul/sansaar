...
module.exports = class C4caPartnerService extends Schmervice.Service {
  ...
  async getC4caPartnerBy(id) {
    try {
      const { C4caPartners, User } = this.server.models();
      const data = await C4caPartners.query().throwIfNotFound()
        .withGraphFetched('user')
        .where('id', id);
      var data1 = data[0]['user'];
      delete data[0]['user'];
      data[0]['email'] = data1[0]['email']
      data[0]['point_of_contact'] = data1[0]['name']
      return [null, data]
    } catch (err) {
      return [errorHandler(err), null];
    }
  ...
  async createC4caPartner(data) {
    ...
    try {
      ...
      data.user_id = parseInt(point_of_contact_data.id);
  ...
  async getAllC4caPartnerData(limit, offset) {
    const { C4caPartners, user } = this.server.models();
    try {
      let data = await C4caPartners.query()
        .page(offset, limit)
        .withGraphFetched('[teachers.[teams_data], user]');
      data.results.map((result) => {
        const { teachers, user } = result;
        let counts = 0;
        ...
        if (user) {
          ...
          result.point_of_contact = user[0].name;
        }
        
        var i = 0;
        while (i < data.results.length) {
          if (data['results'] && data['results'][i] && data['results'][i]['user']) {
            const currentUser = data['results'][i]['user'][0];
            delete data['results'][i]['user'];
        
            if (currentUser) {
              data['results'][i]['email'] = currentUser.email;
              data['results'][i]['point_of_contact'] = currentUser.name;
            }
          }
  ...
  async c4caTeacherInviteLink(partner_id, facilitator_id) {
    ...
    try {
      const c4ca_web_link = CONSTANTS.c4ca_teacher_link
