...
module.exports = class C4caPartnerService extends Schmervice.Service {
  ...
  async getAllC4caPartnerBatch(limit, offset) { 
    const { C4caPartners } = this.server.models();
    try {
      let data = await C4caPartners.query().page(offset, limit);
      return [null, data];
    }
    catch (err) {
      console.log(err, 'err')
  ...
  async createFacilitator(data) {
    const { Facilitator,User,UserRole } = this.server.models();
    ...
    try {
      ...
      if (point_of_contact_data !== null && point_of_contact_data !== undefined) {
        ...
        let rolePrt = await UserRole.query().where('user_id', userID).andWhere('role', 'facilitator');
        if (rolePrt.length === 0) {
          await UserRole.query().insert({ role: 'facilitator', user_id: userID });
      ...
      data.web_link = 'link'
      data.android_link = 'link'

      const res = await Facilitator.query().insert(data);
      if (res){
        userID = parseInt(point_of_contact_data.id)
        let re = await User.query().throwIfNotFound().patchAndFetchById(userID,{'c4ca_facilitator_id': res.id,"c4ca_partner_id":data.c4ca_partner_id})
        let rolePrt = await UserRole.query().where('user_id', userID).andWhere('role', 'facilitator').first();
        if (!rolePrt) {
          await UserRole.query().insert({ role: 'facilitator', user_id: userID });
          logger.info('Giving the facilitator role to the user');
        }
      }
      return [null, {status:true, message:'createFacilitator created successfully!',data:res}]
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
