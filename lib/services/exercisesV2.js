...
module.exports = class ExercisesServiceV2 extends Schmervice.Service {
  ...
  async markExerciseComplete(userId, exerciseId) {
    ...
    const completedExercise = { user_id: userId, exercise_id: exerciseId, complete_at: dateIST };
    try {
      const data = await ExerciseCompletionV2.query()
        .context({ user_id: userId, exercise_id: exerciseId, complete_at: dateIST })
  ...
  async getExerciseByExerciseId(id, lang = 'en') {
    ...
    try {
      ...
      return [errorHandler(err), `this exrcise is not exists`];
    }
  }
