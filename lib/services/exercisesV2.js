...
module.exports = class ExercisesServiceV2 extends Schmervice.Service {
  ...
  async parseIntoModifiedContent(filePath, jsonContent, beginKeyOn) {
    const exerciseName = filePath.split('/').pop().split('.')[1];
  ...
  async updateSingleExercises(exerciseId, data) {
    ...
    try {
      if (data.name !== undefined || data.content !== undefined) {
        ...
        if (courseDetails !== undefined) {
          const courseFolderName = `curriculum_new/${courseDetails.name}_${courseDetails.id}`; // curriculum_new/If-else_10
          console.log(courseFolderName,'Ha path check krne ke liye......');
          const partPath = courseFolderName.split('curriculum_new/')[1]; // If-else_10
          if (data.name !== undefined) {
            if (fs.existsSync(`${courseFolderName}`)) {
              const courseData = fs.readFileSync(
                `${courseFolderName}/${exerciseDetails.name}.json` // curriculum_new/If-else_10/if-statement-flowcharts_if-statement-intro.json
              );
              const allJsonData = courseData.toString().trim();
              fs.renameSync(
                `${courseFolderName}/${exerciseDetails.name}.json`, // curriculum_new/If-else_10/if-statement-flowcharts_if-statement-intro.json
                `${courseFolderName}/${data.name}.json` // curriculum_new/If-else_10/if-intro.json
              );
              fs.renameSync(
                `${courseFolderName}/PARSED_CONTENT/MODIFIED_FILES/${partPath}_${exerciseDetails.name}.json`, // curriculum_new/If-else_10/PARSED_CONTENT/MODIFIED_FILES/If-else_10_if-statement-flowcharts_if-statement-intro
                `${courseFolderName}/PARSED_CONTENT/MODIFIED_FILES/${partPath}_${data.name}.json` // curriculum_new/If-else_10/PARSED_CONTENT/MODIFIED_FILES/If-else_10_if-intro
              );
              // eslint-disable-next-line
              for (const lang in language) {
                if (
                  fs.existsSync(
                    ...
                    // curriculum_new/If-else_10/PARSED_CONTENT/PROPERTIES_FILES/If-else_10_if-statement-flowcharts_if-statement-intro_en.json
                  )
                ) {
                  fs.renameSync(
                    `${courseFolderName}/PARSED_CONTENT/PROPERTIES_FILES/${partPath}_${exerciseDetails.name}_${language[lang]}.json`,
                    ...
                    // curriculum_new/If-else_10/PARSED_CONTENT/PROPERTIES_FILES/If-else_10_if-statement-flowcharts_if-statement-_en.json
                  );
                }
              }
