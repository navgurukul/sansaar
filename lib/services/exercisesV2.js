...
module.exports = class ExercisesServiceV2 extends Schmervice.Service {
  ...
  async getExerciseByExerciseId(id) {
    const { ExercisesV2,CoursesV2, } = this.server.models();
    let exerciseData
    try {
      const exerciseDetails = await ExercisesV2.query().findById(id);
      const courseDetails = await CoursesV2.query().findById(exerciseDetails.course_id);
      const currentAssessmentFolderName = `curriculum_new/tesla/${courseDetails.name}_${courseDetails.id}/${exerciseDetails.name}.json`;
      let readData = fs.readFileSync(currentAssessmentFolderName, 'utf-8')
      exerciseData = readData
      return [null, exerciseData];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
