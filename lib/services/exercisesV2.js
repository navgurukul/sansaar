...
module.exports = class ExercisesServiceV2 extends Schmervice.Service {
      ...
      if (course_id !== null && course_id !== undefined) {
        ...
        if (module_id) {
          ...
          if (moduleCoursePercentDetails !== null && moduleCoursePercentDetails !== undefined) {
            ...
            if (pathway_id !== null && pathway_id !== undefined) {
              ...
              if (pathwayModule !== null && pathwayModule !== undefined && pathwayModule.length > 0) {
                ...
                let completedPathway;
                if (team_id) {
                  completedPathway = { pathway_id, complete_at: dateIST, percentage: moduleAvg };
                  existspathway = await PathwayCompletionV2.query().where(
                    "team_id", team_id).andWhere(
                      "pathway_id", pathway_id
                    );
          ...
          if (pathway_id !== null && pathway_id !== undefined) {
            ...
            const pathwayCourses = data.attributes.courses.data
            // const [err, pathway] = await pathwayServiceV2.findById(pathway_id, true);
  ...
  async exerciseComplete(user_id = null, slug_id, lang = 'en', team_id = null) {

    const { exercisessHistory } = this.server.models();
    const { progressTrackingService } = this.server.services();
    try {
      const url = `${process.env.STRAPI_URL}/slugs/${slug_id}?populate[course][populate][pathway][populate]=modules`
      ...
      const course = data && data.data && data.data.attributes && data.data.attributes.course.data  // "course --> line 630")
      const pathway = course ? course && course.attributes && course.attributes.pathway && course.attributes.pathway.data : null  // "module --> line 604")
      const module_id = pathway ? pathway && pathway.attributes && pathway.attributes.modules && pathway.attributes.modules.data.id : null  //   "module --> line 604")

      const jsonDetails = {}
      jsonDetails.user_id = parseInt(user_id) || null;
      jsonDetails.team_id = parseInt(team_id) || null;
      jsonDetails.course_id = course.id;
      jsonDetails.pathway_id = pathway ? pathway.id : null;
      jsonDetails.slug_id = slug_id;
      jsonDetails.type = 'exercise';
      ...
      const existsExercise = await (team_id
        ? exercisessHistory.query().where("team_id", team_id).andWhere("slug_id", slug_id)
        : exercisessHistory.query().where("user_id", user_id).andWhere("slug_id", slug_id));

      if (existsExercise == null || existsExercise == undefined || existsExercise.length <= 0) {
        await exercisessHistory.query().insert(completedExercise);
      }

      await this.courseModulePathwayPercentCalculator(user_id, course.id, team_id, module_id, pathway.id)
