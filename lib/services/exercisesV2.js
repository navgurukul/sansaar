...
module.exports = class ExercisesServiceV2 extends Schmervice.Service {
  ...
  async exerciseComplete(user_id = null, slug_id, type = 'exercise', lang = 'en', team_id = null) {
    ...
    try {
      const url = `http://3.110.213.190/api/slugs/${slug_id}?populate[course][populate][modules][populate]=pathway`
      const { data } = await axios.get(url)

      const course = data && data.data && data.data.attributes && data.data.attributes.course.data  // "course --> line 630")
      const module = course && course.attributes && course.attributes.modules && course.attributes.modules.data  //   "module --> line 604")
      const pathway_id = module && module.attributes && module.attributes.pathway && module.attributes.pathway.data.id   // "module --> line 604")
      ...
      jsonDetails.module_id = module.id || null;
      jsonDetails.project_topic_id = null;
      jsonDetails.project_solution_id = null;
      ...
      // await this.calculateCourseModulePathwayPercent(user_id, course.id, module.id, pathway_id, team_id)
