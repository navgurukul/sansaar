...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
  ...
  async find(withCourse = false) {
    ...
    try {
      const res = await strapi.find(
        'pathways',
        withCourse ? { populate: ['courses'], sort: 'createdAt:asc' } : { sort: 'createdAt:asc' }
      );
      const d = res.data;
      pathway = d.map((path) => {
        const data = {
          ...
          logo: path.attributes.logo,
          description: path.attributes.description,
        };
        if (withCourse) {
          data.courses = path.attributes.courses.data.map((course) => ({
            id: course.id,
  ...
  async findById(id, withCourse = false) {
    // const { PathwaysV2 } = this.server.models();
    try {
      const res = await strapi.findOne('pathways', id, withCourse ? { populate: ['courses'] } : {});
      const d = res.data;
      const pathway = {
        id: d.id,
        code: d.attributes.code,
        name: d.attributes.name,
        logo: d.attributes.logo,
        description: d.attributes.description,
      };
      if (withCourse) {
        pathway.courses = d.attributes.courses.data.map((course) => ({
          id: course.id,
          name: course.attributes.name,
          logo: course.attributes.logo,
          short_description: course.attributes.short_description,
  ...
  async upComingDoubtClasses(pathwayId, partnerID) {
    ...
    try {
      ...
      if (idsInArray.length > 0 && idsInArray != null && idsInArray != undefined) {
        ...
        if (ClassidsInArrayOfPathway.length > 0 && ClassidsInArrayOfPathway != null && ClassidsInArrayOfPathway != undefined) {
          let classesDetails = await Classes.query().whereIn('id', ClassidsInArrayOfPathway);
          return [null, classesDetails];
        }
      }
