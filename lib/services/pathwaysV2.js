...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
  ...
  async gettingUpcomingClasses(pathwayId, spaceID) {
    const { PathwayCoursesV2, Classes, ClassesToCourses, PartnerSpecificBatches } = this.server.models();
    let classDetails;
    try {
      let date = new Date();
      const classIds = await ClassesToCourses.query().where('pathway_v2', pathwayId).pluck('class_id');
      const exitSpaceId = await PartnerSpecificBatches.query().where('space_id', spaceID);
      if (exitSpaceId.length > 0) {
        let classesData = await Classes.query().where('start_time', '>=', date).whereIn('id', classIds);
        classDetails = {
          batch: [],
          doubtClass: []
        }
        for (let classes of classesData) {
          if (classes.recurring_id != null && classes.recurring_id != undefined) {
            classDetails.batch.push(classes);
          } else {
            classDetails.doubtClass.push(classes);
          }
        }
      }
