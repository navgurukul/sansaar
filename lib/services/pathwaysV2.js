...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
  ...
  /**
   * This is an asynchronous function that retrieves data from a Strapi API endpoint and returns an
   * array of pathway objects with optional course data.
   * @param [withCourse=false] - A boolean parameter that determines whether or not to populate the
   * "courses" field of the "pathways" collection. If set to true, the "courses" field will be
   * populated with additional data about each course. If set to false, only the "logo" field will be
   * populated.
   * @returns an array with two elements. The first element is either an error object or null,
   * depending on whether an error occurred during the execution of the function. The second element is
   * an array of pathway objects, which are retrieved from the Strapi API and transformed into a
   * specific format using the map function.
   */
  async find(withCourse = false) {
    ...
    try {
      const { data } = await strapi.find(
        'pathways',
        withCourse
          ? { populate: ['courses', 'logo'], sort: 'createdAt:asc' }
  ...
  /**
   * This is an asynchronous function that finds a pathway by its ID and optionally populates its
   * courses and logo.
   * @param id - The ID of the pathway to be retrieved from the database.
   * @param [withCourse=false] - A boolean parameter that determines whether or not to populate the
   * "courses" field of the "pathways" model. If set to true, the "courses" field will be populated
   * along with the "logo" field. If set to false, only the "logo" field will be populated.
   * @returns an array with two elements. The first element is either an error object or null,
   * depending on whether an error occurred during the execution of the function. The second element is
   * either an object representing a pathway or null, depending on whether the pathway was found in the
   * database.
   */
  async findById(id, withCourse = false) {
    ...
    try {
      const { data } = await strapi.findOne(
        'pathways',
        id,
        withCourse ? { populate: ['courses', 'logo'] } : { populate: ['logo'] }
      );
      pathway = {
        id: data.id,
        ...data.attributes,
  ...
  //   let pathway;
  //   try {
  //     const { data } = await strapi.find('pathways', withCourse ? { populate: ['courses', 'logo'] } : { populate: ['logo'] });
  //     data.forEach((obj) => {
  //       if (code === obj.attributes.code) {
  //         pathway = {
  //           id: obj.id,
  //           ...obj.attributes,
  //         };
  //         if (obj.attributes.summary) {
  //           const formattedSummary = strapiToMerakiConverter(
  //             JSON.parse(obj.attributes.summary).blocks
  //           );
  //           pathway.summary = formattedSummary;
  //         } else {
  //           pathway.summary = [];
  //         }
  //         if (withCourse) {
  //           pathway.courses = obj.attributes.courses.data.map((course) => ({
  //             id: course.id,
  //             name: course.attributes.name,
  //             logo: course.attributes.logo,
  //             short_description: course.attributes.short_description,
  ...
  //           }));
  //         }
  //         pathway.logo = obj.attributes.logo.data ? obj.attributes.logo.data.attributes.url : null;
  //       }
  //     });
  //     console.log(pathway, "sddjsd");
  //     return [null, pathway];
  //   } catch (err) {
  //     return [errorHandler(err), null];
  //   }
    ...
    try {
      ...
      data.map((obj) => {
        if (code === obj.attributes.code) {
          ...
          if (withCourse) {
            pathway.courses = obj.attributes.courses.map((course) => ({
              id: course.id,
              name: course.name,
              logo: course.logo.data ? course.logo.data.attributes.url : null,
  ...
  async pathwaysNames() {
    try {
      ...
      data.map((pathway) =>
        names.push({ 'id': pathway.id, "name": pathway.attributes.name })
      )
      return [null, names]
    } catch (err) {
      return [errorHandler(err), null];
