...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
  ...
  async findById(id, withCourse = false) {
    ...
    try {
      ...
      const formattedSummary = strapiToMerakiConverter(JSON.parse(data.attributes.summary).blocks);
      pathway = {
        id: data.id,
        ...data.attributes,
      };
  ...
  async upComingDoubtClasses(pathwayId, partnerID, userId) {
    ...
    try {
      ...
      return [null, `there is no doubt classes present with pathwayId ${partnerID}`];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
