...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
  ...
  async find(txn) {
    ...
    let pathway;
    try {
      const res = await axios.get(`${merdBaseURL}/pathways?populate=courses`);
      // console.log(res.data.data, '!!!!!!!!');
      pathway = res.data.data.map((item) => {
        return {
          id: item.id,
          ...
          courses: item.attributes.courses.data.map((course) => {
            return {
              id: course.id,
  ...
  async findById(id) {
    ...
    try {
      ...
      const res = await axios.get(`${merdBaseURL}/pathways/${id}?populate[0]=courses`);
      ...
      pathway = {
        ...
        courses: d.attributes.courses.data.map((course) => {
          return {
            id: course.id,
            name: course.attributes.name,
            logo: course.attributes.logo,
