  ...
  async resultScorePathwayCourses(user_id, pathwayCourses) {
    try {
      ...
      const getResultsData = async (assessmentIds) => {
        const rawData = await assessmentOutcome.query()
          .whereIn('assessment_id', assessmentIds)
          .andWhere('user_id', user_id);

        const resultsData = new Set(rawData.map(result => result.assessment_id));
        return Array.from(resultsData).map(assessment_id => {
          return rawData.find(result => result.assessment_id === assessment_id);
        });
      };
      const getExercisesData = async (exerciseIds) => {
        const rawData = await ExerciseCompletionV2.query()
        .whereIn('exercise_id', exerciseIds)
        .andWhere('user_id', user_id);
    
        const exercisesData = new Set(rawData.map(exercise => exercise.exercise_id));    
        return Array.from(exercisesData).map(exercise_id => {
          return rawData.find(exercise => exercise.exercise_id === exercise_id);
        });
      };
