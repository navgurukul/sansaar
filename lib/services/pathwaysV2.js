  ...
  async findByCode(code, withCourse = false) {
    ...
    try {
      const { data } = await strapi.find(
        'pathways',
        withCourse
          ? { populate: ['courses', 'courses.logo', 'logo'], sort: 'createdAt:asc' }
          : { populate: ['logo'], sort: 'createdAt:asc' }
      );
      ...
      if (pathwayData) {
        ...
        if (withCourse && pathwayData.attributes.courses) {
          pathway.courses = pathwayData.attributes.courses.map((course) => ({
            id: course.id,
            name: course.name,
