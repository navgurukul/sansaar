...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
  ...
  async findById(id, withCourse = false) {
    ...
    try {
      const { data } = await strapi.findOne(
        'pathways',
        id,
        withCourse ? { populate: ['courses.logo', 'logo','courses.android_logo','android_logo'] } : { populate: ['logo','android_logo'] }
      );
      pathway = {
        id: data.id,
        ...data.attributes,
  ...
  async findByCode(code, withCourse = false) {
    ...
    try {
      ...
      const pathwayData = data.find((obj) => obj.attributes.code.toLowerCase() === code.toLowerCase());
      if (pathwayData) {
        pathway = {
          id: pathwayData.id,
