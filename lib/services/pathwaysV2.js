...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
  ...
  transform = async (input, user_id) => {
    const { ModuleCompletionV2, CourseCompletionV3 } = this.server.models();
      ...
      await Promise.all(
        input.attributes.modules.data.map(async (module) => {
          const percentage = await ModuleCompletionV2.query()
            ...
            .andWhere('user_id', user_id);
          const module_percentage = percentage.length > 0 ? percentage[0].percentage : 0;
          ...
          await Promise.all(
            module.attributes.courses.data.map(async (course) => {
              const logo = course.attributes.logo.data?.attributes.url || null;
              const c_percentage = await CourseCompletionV3.query()
                .select('percentage')
                .where('course_id', course.id)
                .andWhere('user_id', user_id);
              const course_percentage = c_percentage.length > 0 ? c_percentage[0].percentage : 0;
  ...
  async getC4CAPathway (user_id) {
    ...
    try {
      ...
      pathway = this.transform(data, user_id);
