...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
  ...
  async resultScorePathwayCourses(user_id, pathwayCourses) {
    try {
      ...
      await Promise.all(pathwayCourses.map(async (course) => {
        ...
        const ExercisesData = await ExerciseCompletionV2.query()
          .whereIn('exercise_id', course.exercisesIds)
          .andWhere('user_id', user_id);

        let attemtedProgress = ExercisesData.length + resultsData.length 
        let totalCourse = course.assessmentIds.length + course.exercisesIds.length

        let progressBar = (attemtedProgress/ totalCourse) * 100
        // pathWayProgressBar.progress += progressBar
        ...
        course.courseProgressBar = progressBar
        course.mcqs = assessment_result;
        delete course.assessmentIds;
        delete course.exercisesIds;
      }));
      let total = 0
      await pathwayCourses.map(({courseProgressBar}) =>{
        total += parseInt(courseProgressBar)
      })
      pathwayCourses.push((pathwayCourses.length/total)*100);
      return [null, pathwayCourses];
    } catch (err) {
      return [errorHandler(err), null];
    }
  ...
  async resultScorePathwayCoursesUsers(user_ids, pathwayCourses, studentsData) {
    try {
      ...
      const assessmentIDS = await pathwayCourses.flatMap(course => course.assessmentIds)
      // Fetch all results for the given user_ids and course assessmentIds in a single query
      const resultsData = await assessmentOutcome.query()
      ...
      let allUsersProgress = await studentsData.map(( user)=>{
        let progress = 0;
        if (userResultsMap.hasOwnProperty(user.id)){
          progress = (userResultsMap[user.id].pass / assessmentIDS.length) * 100
        }
        if (progress > 100) {
          progress =  100 
        }
        user.completed_percent = progress
        return user
      })
