...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
  ...
  async resultScorePathwayCoursesUsers(user_ids, pathwayCourses, studentsData) {
    try {
      ...
      resultsData.map((res) => {
        const { user_id, status } = res;
        const strUSERID = user_id;
        const statusLowC = status.toLowerCase();
        if (userResultsMap[strUSERID]) {
          userResultsMap[strUSERID][statusLowC] += 1;
        } else {
          userResultsMap[strUSERID] = { pass: 0, fail: 0 };
          // eslint-disable-next-line no-undef
          userResultsMap[strUSERID][statusLowC] += 1;
        }
      });
      let returnData = await studentsData.map(( user)=>{
        // eslint-disable-next-line radix
        let progress = parseInt((userResultsMap[user.id].pass / assessmentIDS.length) * 100)
        if (progress > 100) {
          progress =  100 
        }
