...
module.exports = class PathwayServiceV2 extends Schmervice.Service {
    ...
    try {
      ...
      await Promise.all(
        input.attributes.modules.data.map(async (module) => {
          ...
          moduleObj && moduleObj.courses && moduleObj.courses.sort((a, b) => a.id - b.id);
          if(moduleObj && moduleObj.courses && moduleObj.courses.length > 0){

            moduleObj.courses[0].course_lock = false; 
      
            for (let i = 1; i < moduleObj.courses.length; i++) {
              if (moduleObj.courses[i - 1].completed_portion >= 100) {
                moduleObj.courses[i].course_lock = false; // Uncourse_lock the module
              } else {
                moduleObj.courses[i].course_lock = true; // Lock the module
              }
            }
          }
      ...
      output && output.modules && output.modules.sort((a, b) => a.id - b.id);
      if(output && output.modules && output.modules.length > 0){

        output.modules[0].module_lock = false; // The first module is unmodule_locked by default
  
        // Loop through the remaining modules and update their module_lock status
        for (let i = 1; i < output.modules.length; i++) {
          if (output.modules[i - 1].completed_portion >= 100) {
            output.modules[i].module_lock = false; // Unmodule_lock the module
          } else {
            output.modules[i].module_lock = true; // Lock the module
          }
        }
      }
      return output;
  ...
  async getModuleC4CA(team_id, user_id) {
    ...
    try {
      ...
      // let previousModulePercentage = 100;
      for (let i = 0; i < data.attributes.modules.data.length; i++) {
        ...
        // let isUnlocked = previousModulePercentage ===100;
        // previousModulePercentage = module_percentage;
        let moduleObj = {
          ...
          logo: data.attributes.modules.data[i].attributes.logo.data ?
            data.attributes.modules.data[i].attributes.logo.data.attributes.url : null,
          color: data.attributes.modules.data[i].attributes.color,
          description: data.attributes.modules.data[i].attributes.description,
          completed_portion: module_percentage,
          borderColor: data.attributes.modules.data[i].attributes.borderColor,
          button_type: data.attributes.modules.data[i].attributes.button_type,
        };
        AllModulesData.modules.push(moduleObj);
      }
      AllModulesData && AllModulesData.modules && AllModulesData.modules.sort((a, b) => a.id - b.id);
      if(AllModulesData && AllModulesData.modules && AllModulesData.modules.length > 0){

        AllModulesData.modules[0].module_lock = false; // The first module is unmodule_locked by default
  
        // Loop through the remaining modules and update their module_lock status
        for (let i = 1; i < AllModulesData.modules.length; i++) {
          if (AllModulesData.modules[i - 1].completed_portion >= 100) {
            AllModulesData.modules[i].module_lock = false; // Unmodule_lock the module
          } else {
            AllModulesData.modules[i].module_lock = true; // Lock the module
          }
        }
      }
