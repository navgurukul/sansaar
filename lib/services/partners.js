...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getPartnerThrowUserID(id) {
    ...
    try {
      const partner = await Partner.query()
        .select('name', 'website_link', 'description', 'logo')
        .where('id', id);
      return [null, partner];
    } catch (err) {
      return [errorHandler(err), null];
  ...
  async createPartner(partner_data) {
    try {
      partner_data['created_at'] = new Date();
      partner_data['notes'] = 'None';
      partner_data['slug'] = partner_data.name;
  ...
  async createPartnerNewSpace(NewSpace_data) {
    try {
      ...
      let existParterSpace = await ParterSpace.query().where({
        space_name: NewSpace_data.space_name,
      });
  ...
  async getAllParterSpace() {
    try {
      const { ParterSpace } = this.server.models();
      const Data = await ParterSpace.query().select();
      return [null, Data];
    } catch (error) {
  ...
  async deleteParterSpace(id) {
    try {
      console.log('running deleteParterSpace');
      const { ParterSpace } = this.server.models();
      // Update the ParterSpace data
      const delParterSpace = await ParterSpace.query().delete().where('id', id);
