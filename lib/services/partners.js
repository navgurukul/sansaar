...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getPartnerUserInside(studentData,existGroup) {
    let { email, name, group_id } = studentData;
    const { User, Partner, UserRole } = this.server.models();
      ...
      if (partnerUser.length > 0) {
        ...
        if (userRole.length <= 0 || userRole == null || userRole == undefined) {
          if (partnerUser[0].group_id == null) {
            ...
            return {
              Error: `true`,
              message: `This Student is already associated with this space group_id:${partnerUser[0].group_id}.`,code:403,
  ...
  async groupSpecificLinkAuto(platform,spaceID){
    ...
    async updateUsersPartnerSpaceGroupIds(query, email) {
      const { User, Partner, SpaceGroup, PartnerSpace } = this.server.models();
      const { partner_id, space_id, group_id } = query;
      let checkUserExit;
      try {
        checkUserExit = await User.query().where('email', email);
        if (checkUserExit.length > 0 && checkUserExit !== null) {
          if (checkUserExit[0].group_id === null && checkUserExit[0].space_id === null && checkUserExit[0].partner_id === null) {
            ...
            console.log('PartnerId, SpaceId, GroupId updated successfully')
