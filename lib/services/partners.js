      ...
      fs.createReadStream('/home/andy/navgurukul/sansaar/partnerGroupsData/' + filename)
        ...
        .on('end', async () => {
          ...
          for (const item of partnerGroupData) {
            // we are not using these fields in partner_group table.

            // item.Partners = item.Partners.split(';');
            // item['Sub groups'] = item['Sub groups'].split(';');
            // item['emails (partner view access role)'] = item[
            //   'emails (partner view access role)'
            // ].split(';');

            // if (item.Partners) {
            //   item.Partners = val(item.Partners).asArray().castTo('text[]');
            // }
            // if (item['Sub groups']) {
            //   item['Sub groups'] = val(item['Sub groups']).asArray().castTo('text[]');
            // }
            // if (item['emails (partner view access role)']) {
            //   item['emails (partner view access role)'] = val(
            //     item['emails (partner view access role)']
            //   )
            //     .asArray()
            //     .castTo('text[]');
            // }
            ...
            partnergroup = await PartnerGroup.query().insert({
              ...
              // subgroups: item['Sub groups'],
              // emails: item['emails (partner view access role)'],
            });
            // console.log(partnergroup, 'partnergroup\n');
          }
        });
      return [null, true];
    } catch (err) {
