...
module.exports = class PartnerService extends Schmervice.Service {
      ...
      if (partnerUser.length > 0) {
        ...
        if (userRole.length <= 0 || userRole == null || userRole == undefined) {
          if (partnerUser[0].group_id == null) {
            ...
            return {
              Error: `true`,
              message: `This Student is already associated with this space group_id:${existGroup[0].id}.`,
            };
          }
        } else {
          return {
  ...
  async createGroup(space_id,group_data){
    try {
      ...
      const gettingData = await SpaceGroup.query().select().where({'group_name' : group_data.group_name })
      if (gettingData.length){
        return [null, { status: 'succesfully created the space group', data: gettingData }];
      }
    } catch (error) {
      return [errorHandler(error), null];
    }
