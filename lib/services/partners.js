...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getAllPartners() {
    try {
      const { Partner } = this.server.models();
      const Data = await Partner.query().select('id',"name","point_of_contact_name","email","state","platform",'created_at')
      return [null,Data]
    } catch (err)  {
      return [errorHandler(err),  null];
    }
  ...
  async createPartnerNewSpace(NewSpace_data){
    try{
        ...
        let existParterSpace = await ParterSpace.query().where({"space_name":NewSpace_data.space_name})
  ...
  async updateParterSpace(id,updateData){
    try{
      ...
      const updatedParterSpace = await ParterSpace.query()
        .patchAndFetchById(id, updateData);
