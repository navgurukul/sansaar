...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async csvDataUpload(FilePath) {
    try {
      ...
      if (splitList[1] == 'xlsx') {
        ...
        return [null, await this.insideCode(string_data)];
      } else {
        return [{ Error: 'this file is invallid', code: 403 }, null];
      }
    } catch (err) {
  ...
  async insideCode(string_data){
    try{
      // total_res = []
      for (let data of string_data){
        const { User, Partner } = this.server.models();
        let existPartner = await Partner.query().skipUndefined().where("id", data.partnerId);
        if (!existPartner.length) {
          console.log({'error':'partner id is not exist',"existPartner":existPartner})
        } else {
          let user_data = await User.query().skipUndefined().where('email', data.email);      
          console.log('user_data',user_data,);
          console.log('user_data.length',user_data.length);
          if (user_data.length >0 && user_data.partner_id == undefined || user_data.partner_id == null ) {
          ...
          console.log({"status":'update the user partner data'})
          } else if(user_data.partner_id){
            console.log( {            status: `presend`,
            message: `allready add this student.` })
          }else {
            ...
            console.log({
                status: `success`,  
                message: `user added successfully to partner id}. `,
              })
            }
        }
      }
