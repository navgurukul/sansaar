...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async updatePartner(id, updateData) {
    try {
      ...
      let userRoleData  = await UserRole.query().where('user_id', userData[0].id).andWhere({role: 'partner'});
      if(userRoleData !== null || userRoleData !== undefined){
        await UserRole.query().delete().where('user_id', userData[0].id).andWhere({role: 'partner'});
      }
