...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getPartnerUsersDetails(partnerId, query) {
    ...
    try {
      ...
      for (let data of students) {
        let checkingRole = await UserRole.query().where('user_id', data.id);
  ...
  async getPartnerUser(email, partnerId) {
    ...
    try {
      ...
      if (partnerUser.length > 0) {
        ...
        if (userRole.length <= 0 || userRole == null || userRole == undefined) { 
          if (partnerUser[0].partner_id == null) {
            ...
            return {
              error: `true`,
              message: `This Student is already associated with ${parterName[0].name}.`,code:403
            };
          }
        }
        else {
          return { error: `true`, message: `Admins/Partners/Volunteers cannot be added as students`, code:403 }
        }
        // eslint-disable-next-line
      } else {
      ...
      return { error: true, message: 'Please enter a valid email. For eg., literalstudent@gmail.com',code:403 };
    }
  }

  async arrayCall(arrayOfData,partner_id, space_id, group_id) {
    ...
    else if (!arrayOfData[0].hasOwnProperty('name') ||!arrayOfData[0].hasOwnProperty('email') ){
      return [{
        error:true,
        message:'Need headers name and email mush in the file.',code:403
      }];
    }
    const promises = arrayOfData.map((data) => {
      if (
        ...
        return {
          Error: `true`,
          message:'There is no name or email. And undefined or null data present there in the Object value ',
          student_data: data,
        };
      } else {
  ...
  async getPartnerUserInside(studentData,existGroup) {
    let { email, name, group_id, partner_id, space_id } = studentData;
    const { User, Partner, UserRole, SpaceGroup, PartnerSpace } = this.server.models();
    let userRole;
    let checkUserExit;
    try {
      checkUserExit = await User.query().where('email', email);
      if (checkUserExit.length > 0) {
        //If users don't have any role then, In that case only a space_id will be asign.
        userRole = await UserRole.query().where('user_id', checkUserExit[0].id);
        if (userRole.length <= 0 || userRole == null || userRole == undefined) {
          if (checkUserExit[0].group_id === null && checkUserExit[0].space_id === null && checkUserExit[0].partner_id === null) {
            await User.query().where('email', email).patch({
              partner_id,
              space_id,
              group_id
            })
            return ['PartnerId, SpaceId, GroupId updated successfully'];
            // eslint-disable-next-line
          }
          else {
            if (checkUserExit[0].group_id) {
              let checkGroupData = await SpaceGroup.query().where('id', checkUserExit[0].group_id);
              return [`This student is already associated with ${checkGroupData[0].group_name} group`]
            }
            if (checkUserExit[0].space_id) {
              let checkPartnerSpace = await PartnerSpace.query().where('id', checkUserExit[0].space_id)
              return [`This student is already associated with ${checkPartnerSpace[0].space_name} space`]
            }
            if (checkUserExit[0].partner_id) {
              let checkPartnersData = await Partner.query().where('id', checkUserExit[0].partner_id);
              return [`This student is already associated with ${checkPartnersData[0].name} partner`]
            }
          }
        } else {
          ...
          return {
            Error: `true`,
            message: `Admins/Partners/Volunteers cannot be added as students`,code:403,
            student_data: studentData
          };
        }
        // eslint-disable-next-line
      } else {
        let newUserInserted = await User.query().insert({ name, email, group_id, partner_id, space_id });
  ...
  async getStudentsByGroupID(group_id) {
    try {
      let { User, SpaceGroup, PartnerSpecificBatches, Classes } = this.server.models();

      // let existSpaceId = await PartnerSpecificBatches.query().where('space_id', space_id);
      // if (existSpaceId.length > 0 && existSpaceId !== null && existSpaceId !== undefined) {
      //   const recurringIds = existSpaceId.map(batch => batch.recurring_id);
      //   let batchesData = await Classes.query()
      //       .select('title', 'recurring_id')
      //       .whereIn('recurring_id', recurringIds)
      //       .orderBy('timestamp_column', 'desc') // Replace 'timestamp_column' with the actual column name
  ...
  async removeTheStudentFromGroup(email) {
    try {
      ...
      else if (existData[0].group_id == null) {
      ...
      return [null, { status: 'student successfully removed from the space group,', update_data: data }];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
  ...
  async createGroup(space_id,group_data){
    try {
      ...
      const Data = await SpaceGroup.query().select().where({'group_name' : group_data.group_name }).andWhere('space_id',space_id);
      ...
      let data2 = await SpaceGroup.query().insert(group_data)
  ...
  async updateGroup(id, updateData){
    try {
      ...
      let updateName = await SpaceGroup.query()
      ...
      let data = await SpaceGroup.query()
        .where({ id: id });
  ...
  async getGroupByspaceID(space_id){
    try{
      let {PartnerSpace, SpaceGroup} = this.server.models()
      let existData = await PartnerSpace.query().where({ id: space_id })

      if (!existData.length) {
        return [{ status: 'Space id is not exist.', code: 403 }, null];
      }
      let totalSpaceGroup = await SpaceGroup.query().where('space_id',space_id);
  ...
  async getGroupByGroupID(group_id){
    const { SpaceGroup } = this.server.models();
    let totalSpaceGroup = await SpaceGroup.query().where('id',group_id);

    if (!totalSpaceGroup.length ){
      return [{ Error: `Group_id is not exist, group_id:${group_id}.`, code: 403 }];
    }
    return [null,totalSpaceGroup];
  };
  ...
  async deleteGroup(group_id){
    try {
      ...
      if (!deletedPartnerGroup) {
        return [{ Error: `group_id is not exist, ${group_id}.`, code: 403 }];
      };
      return [
        null,
        {
  ...
  async getPartnerTotalData(id){
    ...
    let totalData;
    try{
        totalData = await Partner.query().select().where("id",id).withGraphFetched('spaces_data.space_groups.students');      
  ...
  async extractDataFromCsv(filePath,partner_id, space_id, group_id) {
    try{
      ...
      const results = [];
       await new Promise((resolve, reject) => {
        ...
        return await this.arrayCall(results, partner_id, space_id, group_id)
      }catch(err){
        return [errorHandler(err),null]
      }
    }

    async extractDataFromXlsx(filePath,partner_id, space_id, group_id){
      try{
        let students_data = []
        ...
        return await this.arrayCall(students_data,partner_id, space_id, group_id);
      }catch(err){
        return [errorHandler(err),null]
      };
    };
   ...
   async studentsSearchEmail(email) {
    ...
    try {
      let exist_partner,name,status;

      let exist_user = await User.query().select().where('email',email)
    ...
    return [{ error: true, message: status,"name": name, code:403}, null ];
    } catch (error) {
      console.log(error);
      return [errorHandler(error), null];
    }
  ...
  async getClsByGroupId(group_id) {
    ...
    try {
      const ClassTitle = await Classes.query()
        .withGraphJoined('partner_specific_batches')
        .where('partner_specific_batches.group_id', group_id)
        .distinctOn('classes.recurring_id')
        .orderBy('classes.recurring_id')
        .select('classes.title');
      return [null, ClassTitle.map(item => item.title)];
    } catch (error) {
      return [errorHandler(error), null];
    }
  }
  async groupSpecificLinkAuto(platform,spaceID){
    const { SpaceGroup } = this.server.models();
    let group_ = await SpaceGroup.query().where('id',spaceID)
    if (group_ == null || group_ == undefined || group_.length == 0) {
      return { error: true, message: 'group_ does not exist' };
    }
    const groupName = encodeURIComponent(group_[0].group_name).replace(/%20/g, '+');
    let merakiLink;
    if (platform === 'web') {
      merakiLink = `${CONSTANTS.web_link_url}${spaceID}`;
    } else if (platform === 'android') {
      merakiLink = `${CONSTANTS.meraki_link_url}${spaceID}`;
    }// else if (platform == 'crca'){
    //   merakiLink = `${CONSTANTS.meraki_link_url}${spaceID}`;
    // }
    merakiLink = merakiLink.replace('partner_name', groupName);
    merakiLink = merakiLink.replace('partner_id', "group_id");
    // eslint-disable-next-line no-undef
    let merakiShortLink;
    try {
      merakiShortLink = await axios.post(
    ...
    if (platform === 'web') {
      return SpaceGroup.query()
        .throwIfNotFound()
        .patchAndFetchById(spaceID, { web_link: merakiShortLink.data.link });
    }
    // else if (platform == 'crca'){
    //   return SpaceGroup.query()
    //     .throwIfNotFound()
    //     .patchAndFetchById(spaceID, { crca_link: merakiShortLink.data.link });
    // }
    else if(platform == 'android'){
      return SpaceGroup.query()
        .throwIfNotFound()
        .patchAndFetchById(spaceID, { android_link: merakiShortLink.data.link });
      }
    }

    async returnCsvFile(errorData) {
      try{
        const errorColumns = ['Error', 'message', 'code', 'name', 'email'];
        const csvErrorData = [errorColumns.join(','), ...errorData.map(row => row.join(','))].join('\n');
        const csvFileName = 'output.csv';
        await fs.promises.writeFile(csvFileName, csvErrorData);
        const csvFilePath = path.resolve(csvFileName);
        // logger.error(JSON.stringify(data[0]));
        return [null,[csvFilePath, { mode: 'attachment', filename: csvFileName }]]
      } catch (error) {
        return [errorHandler(error), null];
      }
    }
