      ...
      for (let data of array_data){
        ...
        if (!existPartner.length) {
          ...
          if (user_data.length >0 && user_data.partner_id == undefined || user_data.partner_id == null ) {
            ...
            logger.info(`update the Email ${data.email}, partneId ${data.partnerId}, spaceID ${data.spaceID} `);
          // console.log({"status":'update the user partner data'})
            ...
            if (data1) {
              logger.info(`Created the Email ${data.email}, partneId ${data.partnerId}, spaceID ${data.spaceID} `);
            }
        }
      }
    }
