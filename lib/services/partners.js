...
module.exports = class PartnerService extends Schmervice.Service {
    ...
    try {
      ...
      if (email) {
        ...
        if (M_student.length == 0) {
          ...
          await MerakiStudent.query().insert({ user_name: user_name, name: name, partner_id: partnerId, password: hashedPassword });
          return { status: `success`, message: `user added successfully to ${partnerId} partner.` };
        } else {
          return {
            error: true,
  ...
  async updatePartnerUser(userId, student_id, payload_data) {
    const { User, MerakiStudent } = this.server.models();
    try {
      if (!student_id) {
        ...
        const updatepartneruser = await MerakiStudent.query()
          .update({
            user_name: partnerUser.user_name,
            name: payload_data.name,
