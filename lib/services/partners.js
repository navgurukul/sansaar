...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async groupSpecificLinkAuto(platform,partnerId,spaceID,groupID){
    const { SpaceGroup,PartnerSpace,Partner } = this.server.models();
    let [err,cond] = await this.relationChickerPartnerSpaceGroup(partnerId,spaceID,groupID)
    if (err) {
      return err;
    }
    const partner = await Partner.query().where('id', partnerId);
    ...
    const partnerSpace = await PartnerSpace.query().where('id', spaceID);

    if (partnerSpace == null || partnerSpace == undefined || partnerSpace.length == 0) {
      return { error: true, message: 'space_id does not exist' };
    }
    let group_ = await SpaceGroup.query().where('id',groupID)
    ...
    merakiLink += `%3D${spaceName}%26utm_content%3Dspace_id%253A${spaceID}`
    merakiLink += `%3D${groupName}%26utm_content%3Dgroup_id%253A${groupID}`
    ...
    if (platform === 'web') {
      return SpaceGroup.query()
        .throwIfNotFound()
        .patchAndFetchById(groupID, { web_link: merakiShortLink.data.link });
    }
    // else if (platform == 'crca'){
    //   return SpaceGroup.query()
    //     .throwIfNotFound()
    //     .patchAndFetchById(groupID, { crca_link: merakiShortLink.data.link });
    // }
    else if(platform == 'android'){
      return SpaceGroup.query()
        .throwIfNotFound()
        .patchAndFetchById(groupID, { android_link: merakiShortLink.data.link });
      }
    }
