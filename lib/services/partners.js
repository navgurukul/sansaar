...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async deletePartner(id) {
    try {
      ...
      const deletedPartnerSpace = await PartnerSpace.query().where('partner_id', id).del();
      if (!deletedPartner.length){
        return [null,{status:`Partner is delete succesfully, = ${id} There is no any space exist.`}];
      }
      return [null,{status:`Partner with id ${id} and its corresponding partner_space deleted.`}];
      // });
      // return result;
    } catch (err) {
      return [errorHandler(err), null];
  ...
  async createPartnerNewSpace(partner_id,NewSpace_data) {
    try {
      ...
      if (! existPartner.length){
        return [{status:"Not found",message: "partner_id is not exist in the partner table"},null]
      }
  ...
  async getAllPartnerSpace(partner_id) {
    try {
      ...
      return [null, { status: 'PartnerSpace ID is not exist. Try another ID!' }];
    } catch (error) {
...
// removing the partner space by the space_id 
  async deletePartnerSpace(space_id) {
    try {
      ...
      if (delPartnerSpace) {
        return [null, {status:`this space_id is delete ${space_id}`}];
      }
