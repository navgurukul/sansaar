...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getPartnerThrowPartnerID(id) {
    const { Partner } = this.server.models();
    try {
      const updateData = await Partner.query().select().where('id', id);
      ...
      return [null, {
        ...
        platform: updateData[0].platform,
        status: updateData[0].status,
        create_at: updateData[0].create_at,
        updateAt: updateData[0].updateAt,
      }];
    } catch (err) {
      return [errorHandler(err), null];
    }
      ...
      return [
        null,
        {
          ...
          'Update data': {
            ...
            create_at: updateData.create_at,
