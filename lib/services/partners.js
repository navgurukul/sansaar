...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  // this functions will get the titles of the batches by space_id
  async getBatchDetailsBySpaceId(spaceId) {
    try {
      ...
      if (existSpaceId.length > 0 && existSpaceId !== null && existSpaceId !== undefined) {
        ...
        let batchesData = await Classes.query().select('title', 'recurring_id').whereIn('recurring_id', recurringIds);
        const uniqueClassesMap = new Map();

        for (const cls of batchesData) {
          const { title, recurring_id } = cls;
          const key = `${title}_${recurring_id}`;
          if (!uniqueClassesMap.has(key)) {
            uniqueClassesMap.set(key, { title, recurring_id });
          }
        }
        const uniqueClassesArray = Array.from(uniqueClassesMap.values());
        return [null, uniqueClassesArray];
      }
