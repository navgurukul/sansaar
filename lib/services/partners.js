...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  //     if (userRole.length !== 0 && userRole.role !==student ) {
  //       const Roledata = await UserRole.query().update({role:null}).where('user_id', partnerUser.id);
  ...
  async  removeGrpPrtSpc(userId) {
    ...
    try {
      const partnerUser = await User.query().findById(userId);
      const userRoles = await UserRole.query().where('user_id', partnerUser.id);
      // Check if userRoles array is not empty and does not contain the role "student"
      const isStudentRoleExist = userRoles.some((role) => role.role === 'student');
  
      if (userRoles.length !== 0 && !isStudentRoleExist) {
        // Update the role of the user to null except the "student" role
        await UserRole.query().where('user_id', userId).update({ role: null });
        
  
        // Update the space_id, group_id, and partner_id to null for the user
        const removeAnPartnerUser = await User.query()
          .findById(userId)
          .patch({
            space_id: null,
  ...
  async getBatchesDataByRecurringId(recurring_id) {
    const { Classes, PartnerSpecificBatches, PathwaysV2 } = this.server.models();
    try {
      const classDetails = await Classes.query()
        .throwIfNotFound()
        .where('recurring_id', recurring_id)
      ...
      const partner_Specific = await PartnerSpecificBatches.query().where('recurring_id', recurring_id)   
      const id = partner_Specific[0].pathway_id
      const pathway_data = await PathwaysV2.query().findById(id)
      const response = {}
      ...
      response.start_time = classDetails[0].start_time;
      response.end_time = classDetails[0].end_time
      response.pathway_id = partner_Specific[0].pathway_id
      response.name = pathway_data.name
