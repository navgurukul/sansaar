...
module.exports = class PartnerService extends Schmervice.Service {
    ...
    const promises = arrayOfData.map((data) => {
      if (
        ...
        if (!emailRegex.test(email)) {
          return {
            Error: true,
            message: "The email address is in an invalid format", code: 403,
            student_data: data
          };
        }
  ...
  async getPartnerUserInside(studentData, existGroup) {
    ...
    try {
      ...
      if (partnerUser.length > 0) {
        ...
        await User.query().insert({ name: name, email: email, group_id: group_id, partner_id: partner_id });
        return {
          status: true,
  ...
  async createPartner(partner_data) {
    try {
      ...
      if (point_of_contact_data.length > 0) {
        ...
        if (partner_id !== null) {
          return [{Error: true,code:403,message:`This point of contact is already associated with another partner!`}]
        }
        let roleAdmin = await UserRole.query().where('user_id',userID).andWhere('role','admin');
        if (roleAdmin.length > 0 ){
          return [{Error: true,code:403,message:`This user has Admin access, You can't make this user into point of contact to this partner!`}]
        }
        ...
        return [{Error: true,code:403,message:`To create a partner, the ${partner_data.email} email must log in once to Meraki.`}];
      }
  ...
  async updatePartner(id, updateData) {
    try {
      ...
      if (updateData.name && !updateData.email && !updateData.point_of_contact_name) {
        const updateNameOfPartners = await Partner.query().where({ name: updateData.name }).whereNot({ id });
        if (updateNameOfPartners.length) {
          return [{ Error: true, message: 'Partner name is already exist. Try another name!', code: 403 }, null];
        }
        else{
          await Partner.query().update({ name: updateData.name }).where('id', id);
          return [
            null,
            {
              status: 'Update the data succesfully',
              'Update data': await Partner.query().where('id', id)
            }
          ];
        }
      }
      ...
      if(partnerData.length == 0){
        return [{ Error: true, message: `This partner_id is not exist!`, Partner_id: id, code: 403 }, null];
      }
      ...
      if (checkEmailExist.length) {
        return [{ Error: true, message: `${updateData.email} is already associated with partner ${checkEmailExist[0].name}. Try another email!`, code: 403 }, null];
      }

      let userData = await User.query().where('email', updateData.email);
      if(userData.length == 0){
        return [{ Error: true, message: `${updateData.email} is not exist in Meraki!`, Partner_id: id, code: 403 }, null];
      }

      let roleAdmin = await UserRole.query().where('user_id',userData[0].id).andWhere('role','admin');
      if (roleAdmin.length > 0 ){
        return [{Error: true,code:403,message:`This email ${updateData.email} has Admin access, We can't update this email address! for the partner. Please try with another email address!`}]
      }

      await User.query().update({partner_id: null}).where('email', partnerData[0].email);
      let deletePartnerRole = await User.query().where('email', partnerData[0].email);
      let userRoleData  = await UserRole.query().where('user_id', deletePartnerRole[0].id).andWhere({role: 'partner'});
      if(userRoleData !== null || userRoleData !== undefined){
        await UserRole.query().delete().where('user_id', deletePartnerRole[0].id).andWhere({role: 'partner'});
      }
      ...
      let checkUserRole = await UserRole.query().where('user_id', users[0].id).andWhere({role: 'partner'});
  ...
  async partnerIdsGetter(days,status = 'Inactive'){
    ...
    try {
      ...
      let ClassesData = await PartnerSpecificBatches.query().select().whereNotNull('group_id').withGraphFetched('batch');
      let partnersIDs = []
      await ClassesData.filter(async (cls) => {
        ...
        if (status == 'Inactive'){
          ...
          let updateCount = await Partner.query().where('status','Newly Onboarded').andWhere('created_at','<',days30).update({'status':'Inactive',"updated_at": new Date()})
          logger.info( `Successfully update the Inactive status of the ${updateCount} partners data.`);
  ...
  async PartnerStatusArchivedChange(){
    ...
    try{
      const days30 = new Date();
      days30.setDate(days30.getDate() - 29);
      let updateCount = await Partner.query().where('status','Inactive').andWhere('updated_at','<',days30).update({'status':'Archived',"updated_at": new Date()})
  ...
  async PartnerStatusActiveChange(partner_id){
    try {
      ...
      var value = await Partner.query()
      .patchAndFetchById(partner_id,{'status': 'Active'});
  ...
  // // This function will change the partner status to 'Re Onboarded'
  // async PartnerStatusReOnboardedChange(partner_id){
  //   try {
  //     let { Partner } = this.server.models();
  //     var value = await Partner.query()
  //       .patchAndFetchById(partner_id,{'status': 'Re Onboarded'});
