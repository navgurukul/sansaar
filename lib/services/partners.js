  ...
  async getPartnerUsersDetails(partnerId, query) {
    ...
    try {
      ...
      for (const user of students) {
        if (user.password) {
          const plainTextPassword = decrypt({ iv: user.pass_iv, encryptedData: user.password });
    ...
    try {
      ...
      if (email) {
        ...
        if (partnerUser.length == 0) {
          ...
          await User.query().insert({ user_name: user_name, name: name, partner_id: partnerId, password: hashedPassword, pass_iv: pass_iv });
          return { status: `success`, message: `user added successfully to ${partnerId} partner.` };
        } else {
          return {
            error: true,
