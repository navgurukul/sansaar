...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getPartnerGroupData(partnerGroupId) {
    ...
    try {
      ...
      const res = partnerSubGroups.map((g) => {
        return {
          partner_group_id: g.partner_group.id,
          partner_group_name: g.partner_group.name,
      ...
      res.forEach((pg) => {
        total_no_of_schools += pg.partners.length;
        pg.partners.forEach((p) => {
          total_no_of_students += p.users_count;
        });
      });
  ...
  async partnerGroup(filename) {
    ...
    try {
      ...
      // eslint-disable-next-line
      fs.createReadStream(
        '/home/cosmicblisss/Documents/navgurukul/sansaar/partnerGroupsData/' + filename
      )
        .pipe(csv())
