...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getPartnerUser(studentData, spaceID, existSpaceId) {
    ...
    try {
      ...
      if (partnerUser.length > 0) {
        ...
        return {
          status: `success`,
          message: `user added successfully to partner space partner_ID:${existSpaceId[0].partner_id}. `,
          space_id: spaceID,
        };
      }
    } catch (err) {
  ...
  async updatePartner(id, updateData) {
    try {
      ...
      if (updateData.hasOwnProperty('name')) {
        ...
        let updateName = await Partner.query().where({ name:updateData['name'] }).whereNot({ id: id });
      ...
      let partner_date = await Partner.query().select('id',
      'name',
      'point_of_contact_name',
      'email',
      'platform',
    ...
    try {
      ...
      if (!existPartner.length) {
        return [
          {
            Error: 'Not found',
            message: 'partner_id is not exist in the partner table',
            code: 403,
          },
          null,
        ];
  ...
  async getPartnerSpaceByPartnerID(partner_id) {
    try {
      ...
      let existPartner = await Partner.query().where('id', partner_id);
      if (!existPartner.length) {
        return [{ Error: 'Not found', message: 'partner_id is not exist.', code: 403 }, null];
      }
  ...
  async updatePartnerSpace(id, updateData) {
    try {
      ...
      let updateName = await PartnerSpace.query().where({ space_name: updateData.space_name }).whereNot({"id":id});
      if (updateName.length) {
  ...
  async getBatchDetailsBySpaceId(spaceId) {
    try {
      let { PartnerSpecificBatches, Classes } = this.server.models();
      let existSpaceId = await PartnerSpecificBatches.query().where('space_id', spaceId);
      if (existSpaceId.length > 0 && existSpaceId !== null && existSpaceId !== undefined) {
        ...
        let batchesData = await Classes.query().select('title').whereIn('recurring_id', recurringIds);
        const titleOfBatches = new Set(batchesData.map(cls => cls.title));
        return [null, [...titleOfBatches]];
      }
      return [null, 'SpaceId is not exist'];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
