...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  // partner dashboard    verson 2 changes
  async insertNewPartnerDataIntoUsersTable(name,email,partner_id){
    const { PartnerSpace, Partner,User , UserRole} = this.server.models();
    try{
      let existGamil = await User.query().where('email',email) ;
      if (existGamil.length !== 0 ){
        console.log('old user: ', existGamil);
        let roleExist = await UserRole.query().where({"user_id": existGamil[0].id, "role": 'admin'})
        if (roleExist.length !== 0) {
          let updateD = await User.query().patchAndFetchById(existGamil.id,{name,email,partner_id})
          console.log('update', updateD);
        }
      } else {
        console.log('else condtion!!!!!!!!!!!!!!!');
        let newUser =  await User.query().insert({name,email,partner_id})
        console.log(newUser);
        await UserRole.query().insert({"user_id":newUser.id, "role":"partner"})
  ...
  async createPartner(partner_data) {
    try {
      ...
      if (!existPartner.length) {
        let inData = await Partner.query().insert(partner_data);
        console.log('>><', inData);

        let {id, point_of_contact_name, email,status,created_at,name,platform,updated_at} = inData
        console.log(id, point_of_contact_name, email,'>>>>>>>>>>>>>>');
        await this.insertNewPartnerDataIntoUsersTable(point_of_contact_name,email, id)

        return [null, { status: 'Data sent to the database succesfully', data: {id, point_of_contact_name, email,status,created_at,name,platform,updated_at}}];
      }
