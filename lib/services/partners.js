...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async arrayCall(arrayOfData,group_id,space_id,partner_id) {
    let { SpaceGroup, PartnerSpace, Partner } = this.server.models();
    let existGroup = await SpaceGroup.query().where("id", group_id);
    let existSpace = await PartnerSpace.query().where("id", space_id)
    ...
    else if(!existSpace.length){
      return [{
        error: true,
        message: `Space id is not exist`,
        code:404
      }]
    }
    else if(!existPartner.length){
      return [{
        error: true,
  ...
  async getPartnerUserInside(studentData,existGroup) {
    let { email, name, group_id, space_id, partner_id } = studentData;
    ...
    try {
      ...
      if (partnerUser.length > 0) {
        ...
        userRole = await UserRole.query().where('user_id', partnerUser[0].id);
        if (userRole.length <= 0 || userRole == null || userRole == undefined) {
          if (partnerUser[0].group_id == null && partnerUser[0].space_id === null && partnerUser[0].partner_id === null) {
            ...
            if (partnerUser[0].group_id) {
              let checkGroupData = await SpaceGroup.query().where('id', partnerUser[0].group_id);
              return [{
                Error: true,
                message: `This student is already associated with ${checkGroupData[0].group_name} group`,code:403,
                student_data:studentData
              }];
            }
            else if (partnerUser[0].space_id) {
              let checkPartnerSpace = await PartnerSpace.query().where('id', partnerUser[0].space_id)
              return [{
                Error: true,
                message: `This student is already associated with ${checkPartnerSpace[0].group_name} space`,code:403,
                student_data:studentData
              }];
            }
            else {
              ...
              return [{
                Error: true,
                message: `This student is already associated with ${checkPartnersData[0].group_name} partner`,code:403,
                student_data:studentData
              }];
            }
          }
        ...
        await User.query().insert({ name: name, email: email, group_id: group_id, space_id: space_id, partner_id });
        return {
          status: true,
