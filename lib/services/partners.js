...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async deletePartner(id) {
    try {
      ...
      if (delPartner) {
        return [null, `this partner_id is delete${id}`];
      }
  ...
  async updateParterSpace(id, updateData) {
    try {
      ...
      if (updatedParterSpace == undefined) {
  ...
  async getParterSpaceBySpaceid(space_id) {
    try {
      const { ParterSpace } = this.server.models();
      const parterSpaceData = await ParterSpace.query().select().where('id', space_id);
      if (parterSpaceData.length) {
        return [null, parterSpaceData];
      }
      return [null, `This space_id is not exist!,space_id:${space_id}`];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
  ...
  async deleteParterSpace(space_id) {
    try {
      ...
      if (delParterSpace) {
        return [null, `this space_id is delete${space_id}`];
      }
      return [null, `This space_id is not exist ,space_id:${space_id}!`];
    } catch (err) {
      return [errorHandler(err), null];
