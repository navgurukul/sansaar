...
module.exports = class PartnerService extends Schmervice.Service {
  async getAllPartner(query) {
    ...
    try {
      ...
      console.log(length,"length of count ")
      const data = { partners: partner, count: length[0].count };
      console.log(data,"data of parterns")
    ...
    try {
      ...
      if (email) {
        ...
        if(!user_name || !password){
          return {
            error: `true`,
            message: `Please provides User_name or Password.`,
            code: 200,
          };
        }
        ...
        partnerUser = await User.query().where('user_name', user_name);
        if (partnerUser.length == 0) {
          ...
          await User.query().insert({ user_name: user_name, name: name, partner_id: partnerId, password: hashedPassword, pass_iv: pass_iv, auth_tag: auth_tag });
          return { status: `success`, message: `user added successfully to ${partnerId} partner.` };
        } else {
          return {
            error: true,
