  ...
  async getStudentsByGroupID(group_id) {
    try {
      ...
      for (let ind = 0; ind < result.length; ind++) {
        let rolesData = await UserRole.query().where('user_id', result[ind].id).select('user_id');
        uniqueUserIds.push(...new Set(rolesData.map(role => parseInt(role.user_id))));
      }
      for (let ind = 0; ind < result.length; ind++) {
        if(uniqueUserIds.includes(parseInt(result[ind].id))){
          result.splice(ind,1)
        }
      }
      return [null, result];
