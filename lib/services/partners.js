...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  // This function will change the partner status to 'Archived'. we are using in node-cron in server/index folder #Automation
  async PartnerArchivedChangeAutomation() {
    try {
      ...
      await Partner.query()
        ...
        .where(function() {
          this.whereNull('status')
            .orWhereIn('status', ['Newly Onboarded', 'Active', 'Inactive', 'Re Onboarded']);
  ...
  // This function will change the partner status to 'Inactive'. we are using in node-cron in server/index folder
  async PartnerStatusInactiveChange(){
    try {
      let { Partner } = this.server.models();
      const oneDaysAgo = new Date()
      oneDaysAgo.setDate(oneDaysAgo.getDate() - 1);
      console.log('oneDaysAgo: ',oneDaysAgo);
      var partner_id = await Partner.query()
        .select('id')
        .where('updated_at', '<=', oneDaysAgo)
        .where(function(){
          this.whereNull('status')
          .orWhereIn('status', ['Newly Onboarded', 'Active', 'Re Onboarded']);
        })
