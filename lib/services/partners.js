...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async removeTheStudentFromGroup(email) {
    try {
      ...
      let data = await User.query().patchAndFetchById(existData[0].id, { space_id: null }); // changing the space_id into null

      return [null, { status: 'successfully removed the Space_id', update_data: data }];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
