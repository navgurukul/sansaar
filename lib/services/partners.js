    ...
    try {
      ...
      if (email) {
        ...
        if (partnerUser.length == 0) {
          ...
          await User.query().insert({ user_name: user_name, name: name, partner_id: partnerId, password: hashedPassword });
          return { status: `success`, message: `user added successfully to ${partnerId} partner.` };
        } else {
          return {
            error: true,
  ...
  async updatePartnerUser(userId, payload_data) {
    ...
    try {
      ...
      if (payload_data.password) {
        ...
        const hashedPassword = await bcrypt.hash(payload_data.password, 10);
