...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getPartnerUsersDetails(partnerId, query) {
    ...
    try {
      ...
      for (const data of students) {
        const checkingRole = await UserRole.query().where('user_id', data.id);
  ...
  async createPartner(partner_data) {
    try {
      ...
      if (!existPartner.length) {
        const d = await Partner.query().insert(partner_data);
  ...
  async spaceIdExist(space_id) {
    const { User, PartnerSpace } = this.server.models();
    const existData = await PartnerSpace.query().select().where('id', space_id);
    if (!existData.length) {
  ...
  async getStudentsByGroupID(group_id) {
    try {
      const { User, SpaceGroup, PartnerSpecificBatches, Classes } = this.server.models();
      ...
      const user_id = await PartnerSpecificBatches.query().where('space_id', space_id);
