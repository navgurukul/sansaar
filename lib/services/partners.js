...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getClsByGroupId(group_id) {
    ...
    try {
      const partnerSpecificBatchData = await PartnerSpecificBatches.query().where('group_id', group_id);
      ...
      for (const recurring_id of recurringIds) {
        ...
        if (classDetails.length > 0) {
          clssTitle.push(classDetails[0].title);
        }
      }
      return [null, clssTitle];
    } catch (error) {
      return [errorHandler(error), null];
    }
  }
  async relationChickerPartnerSpaceGroup(partnerID, spaceID, groupID) {
    ...
    let totalData = await Partner.query()
      .select()
      .where("id", partnerID)

    if (totalData.length != 0) {
      // Handle error: spaceID does not exist
      let totalDataSpace = await PartnerSpace.query()
        .select()
        .where({ "id": spaceID, "partner_id": partnerID })
      if (totalDataSpace != 0) {
        let totalDataGroup = await SpaceGroup.query()
          .select()
          ...
          if (totalDataGroup!= 0) {
            ...
            return [ {error: true, "message": `group_id is not exist, ${groupID}.`, code: 403}, null];
          }
      } else {
        return [{error: true, "message": "SpaceID does not exist.", code: 403}, null];
      }
    } else {
      return [{error: true, "message":'Partner does not exist',code: 403}, null];
    }
  }

  async groupSpecificLinkAuto( partnerId, spaceID, groupID) {
    const { SpaceGroup } = this.server.models();
    let [err, conditions] = await this.relationChickerPartnerSpaceGroup(partnerId, spaceID, groupID);
    if (err) {
      return [err, null];
    }
    let Group = await SpaceGroup.query().select().where({ id: groupID });
  
    let webKey = 'web_link';
    let androidKey = 'android_link';
    ...
    let [partner, partnerSpace, group_] = conditions;
    ...
    let webLink = `${CONSTANTS.web_link_url}${partnerId}`;
    ...
    let webShortLink, androidShortLink;
    try {
    ...
    let _data_ = await SpaceGroup.query()
      .throwIfNotFound()
      .patchAndFetchById(groupID, dict);
