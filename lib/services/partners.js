...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  // new partner   ------------------------------------------ NEW VERSON CODE by (PREM)
  async createPartner(partner_data) {
    try {
      ...
      if (!existPartner.length) {
        ...
        return [null, { Status: 'Data sent to the database!!', data: partner_data }];
      }
  ...
  async updatePartner(id, updateData) {
    try {
      ...
      if (updatedPartner.hasOwnProperty('id')) {
        return [null, { Status: 'Update the data', 'Update data': updatedPartner }];
      }
  ...
  async deletePartner(id) {
    try {
      ...
      const { Partner } = this.server.models();
      // Update the partner data
      const delPartner = await Partner.query().delete().where('id', id);
      console.log('deleted: ',delPartner);
      if (delPartner) {
        return [null, delPartner];
      }
      return [null, 'This id is not exist!'];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
  ...
  // new space for the partner DB >>>>>>>>>>>>>>
  async createPartnerNewSpace(partner_id,NewSpace_data) {
    try {
      NewSpace_data['partner_id'] = partner_id
      const { ParterSpace, Partner } = this.server.models();

      console.log('data', NewSpace_data);
      let existParter = await Partner.query().where('id',partner_id)
      if (! existParter.length){
        return [{"Status":"Error : partner_id is not exist in the partner table"},null]
      }
      let existParterSpace = await ParterSpace.query().where("space_name", NewSpace_data.space_name).where('partner_id',partner_id);
      // let partner = await Partner.query().where({"id":NewSpace_data.partner_id})
      // && partner.hasOwnProperty('partner_id')
      if (!existParterSpace.length) {
        let { partner_id } = await ParterSpace.query().insert(NewSpace_data);
        // await ParterSpace.query().where({partner_id}).withGraphFetched('partner')
        return [null, { Status: 'Data sent to the database!!', data: NewSpace_data }];
      }
      return [null, { Status: 'ParterSpace name is already exist. Try another name!' }];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }

  async getAllParterSpace(partner_id) {
    try {
      const { ParterSpace } = this.server.models();
      console.log('parter_id: ',partner_id);
      const Data = await ParterSpace.query().select().where({'partner_id':partner_id});
      return [null, Data];
    } catch (error) {
  ...
  async updateParterSpace(id, updateData) {
    try {
      console.log('>>>', id, updateData);
      const { ParterSpace } = this.server.models();
      // Update the ParterSpace data
      const updatedParterSpace = await ParterSpace.query().patchAndFetchById(id, updateData);

      if (updatedParterSpace.hasOwnProperty('id')) {
        return [null, { Status: 'Update the data', 'Update data': updatedParterSpace }];
      }
      return [null, { Status: 'This id is not exist!' }];
    } catch (err) {
      return [errorHandler(err), null];
  ...
  async deleteParterSpace(space_id) {
    try {
      console.log('running deleteParterSpace');
      const { ParterSpace } = this.server.models();
      // Update the ParterSpace data
      const delParterSpace = await ParterSpace.query().delete().where('id', space_id);
      // console.log('deleted: ',delParterSpace);
      if (delParterSpace) {
        return [null, delParterSpace];
      }
      return [null, 'This space_id is not exist!'];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }

  async readTheFile(FilePath,partner_id) {
    try {
      ...
      if (splitList[1] != 'csv') {
        ...
        console.log(JSON.stringify(string_data, null, 2));
        await new Promise((resolve, reject) => {
          stream.on('end', () => {
            const output = JSON.stringify(string_data, null, 2);
            resolve(output);
          });
          stream.on('error', (err) => {
            reject(err);
      ...
      //let insertData = await knex('students').where('partner_id',partner_id).insert(string_data)
      // console.log('insertData: ',insertData);

      return [null, string_data];
    } catch (err) {
      return [err, null];
    }
  }
