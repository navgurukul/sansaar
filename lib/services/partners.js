...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async createPartner(partner_data) {
    try {
      ...
      if (point_of_contact_data.length > 0) {
        ...
        return [{Error: true,code:403,message:`Please check the provided email or login in Meraki with provided email. Then try!`}];
      }
  ...
  async updatePartnerSpace(id, updateData) {
    try {
      updateData.space_name = updateData.space_name
  ...
  async partnerIdsGetter(days,status = 'Inactive'){
    try {
      let { PartnerSpecificBatches } = this.server.models();
