...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async updatePartner(id, updateData) {
    try {
      ...
      let userData = await User.query().where('email', partnerData[0].email);
      if(userData.length == 0){
        return [{ Error: true, message: `${partnerData[0].email} is not exist in Meraki!`, Partner_id: id, code: 403 }, null];
      }

      let roleAdmin = await UserRole.query().where('user_id',userData[0].id).andWhere('role','admin');
      if (roleAdmin.length > 0 ){
        return [{Error: true,code:403,message:`This email ${updateData.email} has Admin access, You can't make this user into point of contact to this partner!`}]
      }
