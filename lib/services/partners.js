...
const CONSTANTS_IND = require('../config/index');
const S3Bucket = new AWS.S3({
  accessKeyId: CONSTANTS_IND.auth.merakiCertificate.s3SecretKeyId,
  secretAccessKey: CONSTANTS_IND.auth.merakiCertificate.s3SecretAccessKey,
  Bucket: CONSTANTS_IND.auth.merakiCertificate.s3Bucket,
});
  ...
  async arrayCall(arrayOfData, group_id) {
    ...
    if (!existGroup.length) {
      ...
      return [
        {
          error: true,
          message: 'Need headers name and email mush in the file.',
          code: 403,
        },
      ];
    }
  ...
  async returnCsvFile(data) {
    try {
      const errorData = data
        .filter((obj) => obj.Error === 'true' && !('status' in obj))
        .map(({ Error, message, code, student_data }) => [
          Error,
          message,
          code,
          student_data.name,
      ...
      if (errorData.length != 0) {
        ...
        const uploadParams = {
          Bucket: CONSTANTS_IND.auth.aws.scratchBucket,
          Key: csvFileName,
          Body: fs.readFileSync(csvFileName),
        };
        const uploadResult = await S3Bucket.upload(uploadParams).promise();
        const csvFileUrl = uploadResult.Location;
        return [null, [{ csvUrl: csvFileUrl }, { Succes: data.length - errorData.length, Failed: errorData.length }]];
      }
      return [null, data]
    } catch (error) {
      return [errorHandler(error), null];
