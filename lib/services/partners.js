...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async createPartner(partner_data) {
    try {
      ...
      // Check if the provided email already exists in the User table
      const existingUser = await User.query().where('email', partner_data.email);
      let newUserid;
      if (!existingUser.length) {
         newUserid= await User.query().insert({ name: partner_data.point_of_contact_name, email: partner_data.email});
      }else{
        return [{ Error: 'Email is already in use. Please try another email!', code: 403 }, null];
          //  console.log("user created")
      }

  
      // Check if the partner name already exists in the Partner table
      ...
      if (existPartner.length) {
        return [{ Error: 'Partner name already exists. Please try another name!', code: 403 }, null];
      }
  
      // If the email and partner name are unique, insert the new partner data into the Partner table
      ...
      const dataPartner = await Partner.query()
        ...
        // Create a new UserRole entry with 'partner' role for the newly created partner
        const role = await UserRole.query().insert({user_id:newUserid.id , role: 'partner' });
  
      return [null, { status: 'Data sent to the database successfully', data: dataPartner }];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
