...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async createPartner(partner_data) {
    try {
      ...
      if (!existPartner.length) {
        ...
        return [null, { status: 'Data sent to the database succesfully', data: partner_data }];
      }
  ...
  async deletePartner(id) {
    try {
      ...
      console.log('running!!, 583>>>>>>>>>>>>>>>>',deletedPartner);

      if (!deletedPartner){
        return [null,{status:`Partner_ID is not exist, ${id}.`}];
      }
      // Find the corresponding partner_space and delete it
      // const data_total = await PartnerSpace.query().where('partner_id', id);
      // console.log('running!!, 583>>>>>>>>>>>>>>>>',data_total);
      // let ids = data_total.map((x)=>{return x.id})
      // console.log(ids,'>>>>>>>>>>>>>>>>');
      // let deletedPartnerspace = await PartnerSpace.query().whereIn('id', ids).del()
      // console.log('deletedPartnerspace: ',deletedPartnerspace);
      // if (!deletedPartner && !deletedPartnerspace){
      //   return [null,{status:`Partner is delete succesfully.`}];
      // }
      return [null,{status:'succesfully',message:`Partner with id ${id} and its corresponding partner_space deleted.`}];
      // });
      // return result;
    } catch (err) {
      console.log('err: ',err);
  ...
  async createPartnerNewSpace(partner_id,NewSpace_data) {
    try {
      ...
      if (! existPartner.length){
        return [null,{status:"Not found",message: "partner_id is not exist in the partner table"}]
      }
      let existPartnerSpace = await PartnerSpace.query().where("space_name", NewSpace_data.space_name).where('partner_id',partner_id);
  ...
  async getPartnerSpaceBySpaceid(space_id) {
    try {
      ...
      if (partnerSpaceData.length) {
        return [null,{ status: 'Data fetch from the database succesfully!!', data: partnerSpaceData } ];
      }
...
// removing the partner space by the space_id 
  ...
  async readTheFile(FilePath,IDs) {
    try {
      let {partner,PartnerSpace} = this.server.models();
      var string_data = [];
