...
const { encrypt, decrypt } = require('../helpers/middleware/index')
...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getPartnerUsersDetails(partnerId, query) {
    ...
    try {
      ...
      for (const user of students) {
        if (user.password) {
          const plainTextPassword = decrypt({ iv: user.pass_iv, encryptedData: user.password, tag: user.auth_tag });
          user.password = plainTextPassword;
          delete user.pass_iv;
          delete user.auth_tag;
        }
  ...
  async getPartnerUser(payload_data, partnerId) {
    ...
    const handleEmailUserConflict = (email, user_name, password) => {
      if (email && user_name && password) {
        return {
          error: true,
          message: "Please provide either Email or User_name and Password, not both.",
          code: 200,
          errorCode: 2002,
        };
      }
    ...
    const handlePartnerAssociation = async (partnerUser, partnerId) => {
      const userRole = await UserRole.query().where('user_id', partnerUser.id);
      ...
      if (!userRole.length) {
        if (!partnerUser.partner_id) {
          ...
          const partnerName = await Partner.query()
            .select('name')
            .where('id', partnerUser.partner_id);

          return {
            error: true,
            message: `This student is already associated with ${partnerName[0]?.name}.`,
            code: 403,
          };
        }
      }
    ...
    const validatePassword = (password) => {
      ...
      if (!passwordRegex.test(password)) {
        return {
          error: true,
          message: "Password must be at least 8 characters long, contain at least one number, one letter, and one special character.",
          code: 200,
          errorCode: 2003,
        };
      }
    ...
    try {
      ...
      const conflictError = handleEmailUserConflict(email, user_name, password);
      if (conflictError) return conflictError;

      if (email) {
        const partnerUser = await User.query().where('email', email).first();
        if (partnerUser) {
          return handlePartnerAssociation(partnerUser, partnerId);
        } else {
          await User.query().insert({ name, email, partner_id: partnerId });
        ...
        if (!user_name || !password) {
          return {
            error: true,
            message: "Please provide User_name and Password.",
            code: 200,
          };
        }
        ...
        const pass_data = encrypt(password);
        const hashedPassword = pass_data.encryptedData;
        const { iv: pass_iv, tag: auth_tag } = pass_data;

        const partnerUser = await User.query().where('user_name', user_name).first();
        if (!partnerUser) {
  ...
  async updatePartnerUser(userId, payload_data) {
    const { User } = this.server.models();
    try {
      const partnerUser = await User.query().findById(userId);
      if (partnerUser === null || partnerUser === undefined) {
        return [{ status: 'success', message: 'user id not found', code: 404 }, null];
      }
      if (payload_data.password) {
        const passwordRegex = /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[!@#$%^&*(),.?":{}|<>])[A-Za-z\d!@#$%^&*(),.?":{}|<>]{8,}$/;
        if (!passwordRegex.test(payload_data.password)) {
          return [{
            error: true,
            message: `Password must be at least 8 characters long, contain at least one number, one letter, and one special character.`,
            code: 200,
        ...
        const pass_data = encrypt(payload_data.password);
        const hashedPassword = pass_data.encryptedData;
        const pass_iv = pass_data.iv;
        const auth_tag = pass_data.tag;
        payload_data.password = hashedPassword;
        payload_data.pass_iv = pass_iv;
        payload_data.auth_tag = auth_tag;
      }
      if (payload_data.name) {
        const nameRegex = /^[a-zA-Z\s'-]+$/;
        const name = payload_data.name;
        const flag = nameRegex.test(name);
        if (!flag) {
          return [{
            error: true,
            message: `Name can only contain letters, spaces, hyphens, and apostrophes.`,
  ...
  async removeAndUserFromPartner(userId) {
    ...
    try {
      ...
      if (!userRole.length) {
        ...
        return [null, { status: 'success', message: 'Yaay! User removed successfully!' }];
      } else {
