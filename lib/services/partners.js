...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async getPartnerThrowUserID(id) {
    ...
    try {
      ...
      if (!partner.length) {
        return [{ Error: 'partner id is not exist' ,code:404},null];
      }
      return [null, partner];
    } catch (err) {
      return [errorHandler(err), null];
  ...
  async createPartner(partner_data) {
    try {
      ...
      return [ { Error: 'Partner name is already exist. Try another name!' ,code:404},null];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
  ...
  async updatePartner(id, updateData) {
    try {
      ...
      if (updatedPartner == undefined) {
        return [{ Error: `This partner_id is not exist!`, Partner_id: id,code:404 },null];
      }
  ...
  async deletePartner(id) {
    try {
      ...
      if (!deletedPartner) {
        return [{ Error: `Partner_ID is not exist, ${id}.`,code:404 }];
      }
  ...
  async createPartnerNewSpace(partner_id, NewSpace_data) {
    try {
      ...
      if (!existPartner.length) {
        return [
          { Error: 'Not found', message: 'partner_id is not exist in the partner table',code:404 },null
        ];
      }
      ...
      return [{ Error: 'PartnerSpace name is already exist. Try another name!' ,code:404 },null];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
  ...
  async getPartnerSpaceByPartnerID(partner_id) {
    try {
      ...
      let existPartner = await Partner.query().where('id', partner_id);
      if (!existPartner.length) {
        return [{ Error: 'Not found', message: 'partner_id is not exist.',code:404 },null];
      }
      ...
      return [{ Error: "Partner haven't created space." ,code:404 },null];
    } catch (error) {
  ...
  async updatePartnerSpace(id, updateData) {
    try {
      ...
      if (updateData.hasOwnProperty('space_name')) {
        ...
        if (updateName.length) {
          return [{ status: 'Space name is already exist.' ,code:404 },null];
        }
      }
  ...
  async getPartnerSpaceBySpaceid(space_id) {
    try {
      ...
      return [{ Error: `This space_id is not found!`, space_id: space_id ,code:404 },null];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
  ...
  async deletePartnerSpace(space_id) {
    try {
      ...
      return [{ Error: `This space_id is not exist`, space_id: space_id ,code:404 },null];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
  ...
  async readTheFile(FilePath, space_ID) {
    try {
      ...
      if (splitList[1] == 'xlsx') {
        ...
        return  [{Error: 'this file is invallid',code:404 },null]
      }
  ...
  async getStudentsBySpaceid(space_id){
    try{
      let {User,PartnerSpace} = this.server.models()
      let existData = await PartnerSpace.query().select().where('id',space_id)
      if (!existData.length){
        return [{'error':'space id is not found',code:404}]
      }
      let data = await User.query().select('id','email','name','space_id').where('space_id',space_id)
      console.log(data);
      return [null,data]
    }catch(err){
      return [errorHandler(err), null];
  ...
  //this function will only remove the student from the partner space with the email
  async removeTheStudentFromSpace(email){
    try{
      let {User} = this.server.models()   // import the user model
      let existData = await User.query().select('name','email','id',"space_id").where('email',email)

      if (existData[0].space_id == null){
        return [{'error':'this student not associate with any partner space',code:404},null]
      }
      else if (!existData.length){
        return [{'error':'Student email is not found',code:404},null]
      }
