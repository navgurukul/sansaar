  ...
  async csvDataUpload(FilePath) {
    try {
      ...
      if (splitList[1] == 'xlsx') {
        ...
        return [null, await this.insideCode(array_data)];
      } else {
        return [{ Error: 'this file is invallid', code: 403 }, null];
      }
    } catch (err) {
  ...
  async insideCode(array_data){
    try{
      ...
      for (let data of array_data){
        ...
        if (!existPartner.length) {
          logger.info({'error':'partner id is not exist',"existPartner":existPartner})
        } else {
          ...
          if (user_data.length >0 && user_data.partner_id == undefined || user_data.partner_id == null ) {
            ...
            logger.info({"status":'update the user partner data'});
