  ...
  async updatePartner(id, updateData) {
    try {
      ...
      if (checkEmailExist.length) {
        return [{ Error: `${updateData.email} is already associated with partner ${checkEmailExist[0].name}. Try another email!`, code: 403 }, null];
      }

      let userData = await User.query().where('email', partnerData[0].email);
      if(userData.length == 0){
        return [{ Error: `${partnerData[0].email} is not exist in Meraki!`, Partner_id: id, code: 403 }, null];
      }
