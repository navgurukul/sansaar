...
module.exports = class PartnerService extends Schmervice.Service {
    ...
    try {
      ...
      await ClassesData.filter(async (cls) => {
        ...
        if (status == 'Archived') {
          ...
          } else if (lastClass < daysAgo && partnersIDs.includes(cls.partner_id)) {
            partnersIDs.splice(partnersIDs.indexOf(cls.partner_id), 1);
          }
        }else if (status == 'Inactive'){
          // make the newly Onboarded status into inactive after 30 days.
          const days30 = new Date();
          days30.setDate(days30.getDate() - 29);
          await Partner.query().where('status','Newly Onboarded').andWhere('created_at','<',days30).update({'status':'Inactive'})
          if (lastClass < daysAgo && !partnersIDs.includes(cls.partner_id)) {
            partnersIDs.push(cls.partner_id)
          }
        }
  ...
  // This function will change the partner status to 'Archived'. we are using in node-cron in server/index folder
  async PartnerStatusArchivedChange(){
    try {
      let { Partner } = this.server.models();
      // If all batches are done after 30 days we will make that partner archived status
      let [errPartnersIDs,partnersIDs ]= await this.partnerIdsGetter(29,'Archived') 
      if (errPartnersIDs){
        return [errPartnersIDs,null]
      }
      //update the select partnersIDs to Archived partner
      let[err, partner ] =  await Partner.query()
      .whereIn('id', partnersIDs)
      .andWhere('status', 'Inactive')
      .update({'status':'Archived'})
      .then((v)=>{
        let info_value = `Successfully update the Archived status of the ${v} partners data `;
        logger.info(info_value);
        return [null,info_value]
      }).catch((err)=>{
        return [err, null]
