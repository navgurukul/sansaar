...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async c4caTeacherInviteLink(query) {
    const { Facilitator } = this.server.models();
    try {
      const { partner_id, parter_name, facilitator_id } = query;
      const c4ca_web_link = CONSTANTS.c4ca_teacher_link
        .replace('partner_name', parter_name)
        .replace('partner_id_val', partner_id)
        .replace('facilitator_id_val', facilitator_id);

      const c4ca_meraki_link = CONSTANTS.c4ca_meraki_url
        .replace('partner_name', parter_name)
        .replace('partner_id_val', partner_id)
        .replace('facilitator_id_val', facilitator_id);
      console.log(c4ca_web_link,'<<<<<<<<<', c4ca_meraki_link);
      let webShortLink;
      let androidShortLink;
      ...
      let data = await Facilitator.query().throwIfNotFound().patchAndFetchById(facilitator_id, {
        ...
        android_link: androidShortLink.data.link,
      });
      return [null, data];  
    } catch (err) {
      return [errorHandler(err), null];
