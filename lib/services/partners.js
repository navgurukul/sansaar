  ...
  async createPartner(partner_data) {
    try {
      ...
      if (!existPartner.length) {
        ...
        let dataPartner = await Partner.query().select('id',
        'name',
        ...
        'created_at',
        "updatedAt",
        'status').where({ name: partner_data.name });
  ...
  async createPartnerNewSpace(partner_id,NewSpace_data) {
    try {
      ...
      if (!existPartnerSpace.length) {
        ...
        let dataSpace = await PartnerSpace.query().select().where("space_name", NewSpace_data.space_name).where('partner_id',partner_id)
  ...
  async readTheFile(FilePath,IDs) {
    try {
      ...
      if (splitList[1] != 'csv') {
        ...
        if (!string_data.hasOwnProperty('name') && !string_data.hasOwnProperty('email')){
          return [null,{status:'name or email Not found',masssage:'Name column and Email column must needed!'}]
        }
        ...
        await new Promise((resolve, reject) => {
          stream.on('end', () => {
            var string_data = JSON.stringify(string_data, null, 2);
