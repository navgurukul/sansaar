...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async createPartner(partner_data) {
    try {
      ...
      return [null, { Status: 'Partner name is alread exist. Try another name!' }];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
  ...
  async createPartnerNewSpace(partner_id,NewSpace_data) {
    try {
      console.log('running createPartnerNewSpace');
      NewSpace_data['partner_id'] = partner_id
      const { ParterSpace, Partner } = this.server.models();

      console.log('data', NewSpace_data);
      let existParterSpace = await ParterSpace.query().where({
        space_name: NewSpace_data.space_name,'partner_id':partner_id
      });
      ...
      return [null, { Status: 'ParterSpace name is alread exist. Try another name!' }];
    } catch (err) {
      return [errorHandler(err), null];
    }
  }
