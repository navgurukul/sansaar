...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async arrayCall(arrayOfData, group_id) {
    ...
    if (arrayOfData.length === 0) {
      return [
        {
          error: true,
          message: 'An empty file cannot be uploaded. Please upload the file with valid and proper student details.',
          code: 403,
        }
      ]
    }
  ...
  async GetUserProgress(user_ids,pathway_id) {
    const { pathwayServiceV2, progressTrackingService } = this.server.services();
    const { User } = this.server.models();

    try {
      const users_progress  = []; // all courses id as course_id with or withour completed_portion
      for(let user_id of user_ids){
        const users = await User.query().findById(user_id)
        if(users != undefined && users != null){
          const [err, pathway] = await pathwayServiceV2.findById(pathway_id, true); // true = get pathway with it's courses\\
          if(pathway != null && pathway != undefined){
            ...
            let count = 0;
            // eslint-disable-next-line    
            const progres = {}; // single course id as course_id with completed_portion
            for (const course of pathway.courses) {
              const [
                errInPercentage,
                percentage,
                // eslint-disable-next-line
              ] = await progressTrackingService.getPartnerUsersPercentage(
                user_id,
                course.id
              );
              sum += percentage;
            ...
            progres.pathway_id = pathway_id;
            progres.completed_portion = avg;
            progres.pathwayName = pathway.name;
            progres.user_id = user_id
            users_progress.push(progres);
          }
        }
      }
      // logger.info('Get a single pathway');
      return [null, { users_progress: users_progress}];
