...
module.exports = class PartnerService extends Schmervice.Service {
  ...
  async arrayCall(arrayOfData,group_id) {
    ...
    if (!existGroup.length) {
      ...
      return [{
        Error:'Need headers name and email mush in the file. ',code:403
      }];
    }
      ...
      if (partnerUser.length > 0) {
        ...
        if (userRole.length <= 0 || userRole == null || userRole == undefined) {
          if (partnerUser[0].group_id == null) {
            ...
            return {
              status: `success`,
              message: `update the group_id student data successfully.`,
              group_id: group_id,
        ...
        return {
          status: `success`,
          message: `user added successfully to space group id:${existGroup[0].id}.`,
          group_id: group_id,
          student_data: studentData
  ...
  //geting the groups data bye the space_id
  ...
  //geting the group data bye the group_id
  ...
  //  the total partner data including space, group and students data
  async getPartnerTotalData(id){
   ...
   //uploading the csv file data in to the database
  async extractDataFromCsv(filePath,group_id) {
    ...
    //uploading the xl file data in to the database
  ...
  async groupSpecificLinkAuto(platform,spaceID){
    ...
    console.log('group_[0].group_name',group_[0].group_name);
    ...
    console.log('merakiLink: ',merakiLink);
    // eslint-disable-next-line no-undef
    let merakiShortLink;
    try {
      merakiShortLink = await axios.post(
