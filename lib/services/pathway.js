...
module.exports = class PathwayService extends Schmervice.Service {
  ...
  async updateCoursesByPathwayId(pathwayId, pathwayCourseId) {
    const { PathwayCourses } = this.server.models();

    const allPathwayCourses = await PathwayCourses.query().where('pathway_id', pathwayId);
    if (allPathwayCourses) {
      allPathwayCourses.push(pathwayCourseId);
      await PathwayCourses.query().where('pathway_id', pathwayId).del();
      return PathwayCourses.query().insertAndFetch(allPathwayCourses);
    }
    throw Boom.badRequest(`Pathway for Pathway Id : ${pathwayId} does not exist`);
  }
};
