...
module.exports = class PathwayService extends Schmervice.Service {
  ...
  async findCoursesByPathwayId(pathwayId) {
    ...
    const pathwayCourses = await PathwayCourses.query().where('pathway_id', pathwayId);
    if (pathwayCourses) {
  ...
  async upsertPathwayCourses(pathwayId, courseIds) {
    const { PathwayCourses } = this.server.models();
    const courseList = [];
    const filteredCourseIds = _.uniq(courseIds);
    _.map(filteredCourseIds, (courseId) => {
    ...
    const insertedCourses = PathwayCourses.query().insert(courseList);
    if (insertedCourses) {
      return insertedCourses;
    }
    throw Boom.badRequest(
      'Cannot process. You might want to check if one of the course id already exist.'
    );
  }
};
