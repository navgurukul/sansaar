...
module.exports = class PathwayService extends Schmervice.Service {
  ...
  async deleteById(id) {
    const { Pathways } = this.server.models();
    const deleted = await Pathways.query().delete().throwIfNotFound().where('id', id);
    if (deleted) {
      return {
        success: true,
      };
  ...
  async getDefaultPathway() {
    const { Pathways } = this.server.models();
    return Pathways.query().where({ code: 'TYPPAT' }); // For testing typing app only we are returning this as default pathway
  }
  ...
  async markPathwayComplete(userId, pathwayId) {
    ...
    const success = await PathwayCompletion.query().insert(completedPathway);
    if (success) return { success: true };
    throw Boom.badRequest('Pathway might have already been marked completed');
  }

  async removePathwayComplete(userId, pathwayId) {
    const { PathwayCompletion } = this.server.models();
    const success = await PathwayCompletion.query()
      .del()
      .where('user_id', userId)
      .andWhere('pathway_id', pathwayId);
    if (success) return { success: true };
    throw Boom.badRequest('Pathway might have already been marked incompleted');
  }

  async getPathwayComplete(userId) {
    const { PathwayCompletion } = this.server.models();
    const completedPathway = PathwayCompletion.query().where('user_id', userId);
    if (completedPathway) return completedPathway;
