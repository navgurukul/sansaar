...
module.exports = class PathwayService extends Schmervice.Service {
  ...
  async getOldPathwayIdByCode(code) {
    ...
    let pathway;
    try {
      pathway = await Pathways.query().throwIfNotFound().where('code', code);
      return [null, pathway];
    } catch (err) {
      const error = errorHandler(err);
      return [error, null];
  ...
  async insertPathwayCourses(pathwayId, courseIds, txn) {
    ...
    try {
      ...
      for (let courseID in uniqueCourseIds) {
        const courses = await PathwayCourses.query().where('course_id', uniqueCourseIds[courseID]).andWhere('pathway_id', pathwayId);
