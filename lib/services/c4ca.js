...
module.exports = class C4caService extends Schmervice.Service {
  ...
  async getFacilitatorByPartnerId(partner_id) {
    const { Facilitator,Partner} = this.server.models();
    try {
      const facilitator = await Facilitator.query().throwIfNotFound().where('partner_id', partner_id);
      const partnerData=await Partner.query().where('id',partner_id)
      let partner_name = partnerData[0].name
      let newarray=[]
      
      for (let i of facilitator){
        i.partner_name=partner_name
        newarray.push(i)
