...
module.exports = class C4caService extends Schmervice.Service {
  ...
  async deleteTeam(team_id) {
    ...
    try {
      ...
      const student = await C4caStudents.query().where('team_id', team_id);
      if(!team){
        return [{ message: 'Team not found' }, null];
      }
      const deleteStudent = await C4caStudents.query().where('team_id', team_id).del();
      const deleteTeam = await C4caTeams.query().where('id', team_id).del();
      const resp = {
        deleteStudent,
        deleteTeam,
      }
  ...
  async getTeamsByTeacherId(teachers_id, pathway_id = 18) {
    const { C4caTeams, PathwayCompletionV2, C4caPartners, C4caTeachers, Facilitator } = this.server.models();
    try {
      let combinedData;
      const teams = await C4caTeams.query().throwIfNotFound().where('teacher_id', teachers_id);
      ...
      if (teachersDetails.length > 0) {
        ...
        if(facilitatorDetails.length > 0){
          ...
          if (partnerDetails.length > 0) {
            ...
            combinedData =  {...partner_name, ...facilitator_name, ...teacher_name, teamsData: teams}
  ...
  async getFacilitatorByPartnerId(partner_id) {
    ...
    try {
      ...
      let partner_name = {'partner_name': partnerDetails[0].name}
      let combinedData =  {...partner_name, teachersData: totalFacilitatorsDetails}
      return [null, combinedData];
    } catch (error) {
      logger.error(JSON.stringify(error));
      return [errorHandler(error), null];
  ...
  async getTeachersByFacilitator(facilitator_id, page, limit = 10) {
    const { C4caTeachers, C4caTeams, C4caStudents, C4caPartners, Facilitator } = this.server.models();
    try {
      const teachers = await C4caTeachers.query()
        .throwIfNotFound()
        .where('facilitator_id', facilitator_id)
      ...
      let combinedData =  {...partner_name, ...facilitator_name, teachersData: totalTeachersDetails}
      return [null, combinedData];
    } catch (error) {
      logger.error(JSON.stringify(error));
      return [errorHandler(error), null];
