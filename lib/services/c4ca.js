...
module.exports = class C4caService extends Schmervice.Service {
  async setTeacherProfile(profile_obj, data, user_id, partner_id) {
    const { C4caTeachers, UserRole } = this.server.models();
    try {
      ...
      let profile_url = `https://${CONSTANTS.auth.c4ca.c4caS3Bucket}.s3.ap-south-1.amazonaws.com/${key}`;
      ...
      const role = await UserRole.query().insert({
        user_id,
        role: 'c4caTeacher',
      });
  ...
  async updateTeacherProfile(data, teacher_id, user_id) {
    ...
    try {
      ...
      if (profile_url != undefined && profile_url != null) {
        ...
        const uploader = S3Bucket.upload(uploadParams);
        const uploaded = await uploader.promise();
        let p_url = `https://${CONSTANTS.auth.c4ca.c4caS3Bucket}.s3.ap-south-1.amazonaws.com/${key}`;
  ...
  async getTeamByTeamId(team_id) {
    ...
    try {
      ...
      const member = await C4caStudents.query().where('team_id', team.team_id);
      member.reduce((result, item, index) => {
        result[index] = item;
        return result;
      }, {});
      return [null, team];
