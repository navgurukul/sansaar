...
module.exports = class C4caService extends Schmervice.Service {
  async setTeacherProfile(profile_obj, data, user_id, c4ca_partner_id) {
    ...
    try {
      ...
      await C4caRole.query().insert({
        role: 'c4caTeacher',
        teacher_id: C4caTeacher.id,
        user_id,
      });
  ...
  async checkIfTeacher(user_id, txn = false) {
    const { C4caTeachers } = this.server.models();
    try {
      let boolResponse = false;
      const teacher = await C4caTeachers.query().where({ user_id });

      if (teacher !== null && teacher !== undefined && teacher.length > 0 && !txn) {
        boolResponse = true;
      } else {
        boolResponse = teacher.id;
      }
  ...
  async getAllAdminPageData(partner_id) {
    ...
    try {
      ...
      if (partner_id) {
        ...
        const facilitatorDetails = await Facilitator.query()
          .throwIfNotFound()
          .where('partner_id', partner_id);
