...
module.exports = class C4caService extends Schmervice.Service {
  ...
  try {
    ...
    if (teacherRole) {
      ...
      if (teacher) {
        ...
        return [null, 'Teacher data not found for the user with c4caTeacher role'];
      }
  ...
  async uploadProjectTopic(file, team_id, project_title, project_summary, is_submitted, module_id, projectTopic_file_name) {
    ...
    try {
      ...
      const trx = await C4caTeamProjectTopic.startTransaction();
  
      try {
        // Check if the team exists
        const team = await C4caTeams.query(trx).findById(team_id);
        if (!team) {
          // Rollback the transaction if the team is not found
          await trx.rollback();
          return [{ message: 'Team not found' }];
        }
  
        // Initialize URL for the project topic
        let url = '';
  
        // If a file is provided, perform S3 upload
        if (file) {
          ...
          // Perform S3 upload and handle errors
          try {
            const uploader = S3Bucket.upload(uploadParams);
            const uploaded = await uploader.promise();
        ...
        // Check if the project exists for the team
        const existingProject = await C4caTeamProjectTopic.query(trx).findOne({ team_id });
  
        if (existingProject) {
          // Update the existing project
          const updatedProject = await C4caTeamProjectTopic.query(trx)
            .patchAndFetchById(existingProject.id, {
              project_topic_url: url,
              team_name: team.team_name,
              project_title,
          ...
          // Additional logic when the project is submitted
          if (updatedProject.is_submitted) {
            const modulePathwayDetails = await strapi.findOne('modules', updatedProject.module_id, {
              populate: ['pathway'],
            });
          ...
          // Log and return the updated project
          return [null, updatedProject];
        } else {
          // Create a new project
          const newProject = await C4caTeamProjectTopic.query(trx).insertGraphAndFetch({
            project_topic_url: url,
            team_name: team.team_name,
            project_title,
            project_summary,
