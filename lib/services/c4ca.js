...
module.exports = class C4caService extends Schmervice.Service {
  ...
  async getTeamByTeamId(team_id, pathway_id = C4CA_PATHWAY_ID) {
    ...
    try {
      ...
      const modulePathwayDetails = await strapi.findOne('modules', moduleCompletion.module_id);
      const courseCompletion = await CourseCompletionV3.query().where('team_id', team_id).orderBy('complete_at', 'desc').first();
      const CoursePathwayDetails = await strapi.findOne('courses', courseCompletion.course_id);
      ...
      const responese = {
        ...
        projectLink,
        projectFileUrl,
        module_name: modulePathwayDetails.data.attributes.name,
        course_name: CoursePathwayDetails.data.attributes.name,
      };
