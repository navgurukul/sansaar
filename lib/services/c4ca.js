...
module.exports = class C4caService extends Schmervice.Service {
    ...
    try {
      ...
      if (teams !== null && teams !== undefined && teams.length > 0) {
        for (const team of teams) {
          ...
          let allModulePer = 0;
          for (const module of AllmoduleCompletion) {
            allModulePer += module.percentage;
          }
          const newPathwayPercent = Math.floor(allModulePer / allPathwayModule.data.length);
  ...
  async getTeamByTeamId(team_id, pathway_id = C4CA_PATHWAY_ID) {
    ...
    try {
      ...
      const moduleCompletion = await ModuleCompletionV2.query()
        .where('team_id', team_id)
        .orderBy('complete_at', 'desc')
        .first();
      const modulePathwayDetails = moduleCompletion
        ? await strapi.findOne('modules', moduleCompletion.module_id)
        : null;
      const courseCompletion = await CourseCompletionV3.query()
        .where('team_id', team_id)
        .orderBy('complete_at', 'desc')
        .first();
      const CoursePathwayDetails = courseCompletion
        ? await strapi.findOne('courses', courseCompletion.course_id)
        : null;
      const projectStatus = await C4caTeamProjectSubmitSolution.query()
        .where('team_id', team_id)
        .first();
  ...
  async uploadProjectTopic(
    ...
    try {
      ...
      if (existingProject) {
        ...
        const updatedProject = await C4caTeamProjectTopic.query().patchAndFetchById(
          existingProject.id,
          resp
        );
        if (updatedProject.is_submitted) {
  ...
  async uploadProjectSubmit(
    ...
    try {
      ...
      if (existingProject) {
        ...
        const updatedProject = await C4caTeamProjectSubmitSolution.query().patchAndFetchById(
          existingProject.id,
          {
      ...
      if (newProject && newProject.is_submitted) {
        ...
        await exercisesServiceV2.calculateModulePercentageByModuleId(null, module_id, team_id, 100);
  ...
  async getFacilitatorByPartnerId(partner_id) {
    const { Facilitator, C4caTeachers, C4caTeams, C4caStudents, C4caPartners } =
      this.server.models();
    try {
  ...
  async getTeamsByDistrictOrSchool(district, school, pathway_id = C4CA_PATHWAY_ID) {
    const { C4caTeams, C4caTeachers, PathwayCompletionV2, ModuleCompletionV2 } =
      this.server.models();
    try {
  ...
  async getTeachersByFacilitator(facilitator_id, page, limit = 10) {
    const { C4caTeachers, C4caTeams, C4caStudents, Facilitator, C4caPartners } =
      this.server.models();
    try {
      const teachers = await C4caTeachers.query()
        .where('facilitator_id', facilitator_id)
  ...
  async getProjectSubmitByTeamId(team_id, module_id) {
    ...
    try {
      ...
      const projects = await C4caTeamProjectSubmitSolution.query().where('team_id', team_id);
      // .andWhere('module_id', module_id)
      // .orderBy('created_at', 'desc') // Assuming there's a timestamp column for submission time
      // .limit(1);

      // Retrieve the last project topic
      const projectTopic = await C4caTeamProjectTopic.query().whereIn('team_id', [team_id]);
      // .andWhere('module_id', module_id)
  ...
  async getAllAdminPageData(partner_id) {
    const { C4caTeams, C4caTeachers, C4caTeamProjectSubmitSolution, C4caStudents, Facilitator } =
      this.server.models();
    try {
      if (partner_id === undefined || partner_id === null) {
