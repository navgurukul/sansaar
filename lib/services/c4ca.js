...
module.exports = class C4caService extends Schmervice.Service {
  ...
  async getProjectTopicByTeamId(team_id) {
    const { C4caTeamProjectTopic } = this.server.models();
    try {
      const totalSubmitTopic = await C4caTeamProjectTopic.query().where('team_id', team_id).resultSize();
      const project = await C4caTeamProjectTopic.query()
        .throwIfNotFound()
        .where('team_id', team_id);
        const responce ={
          totalSubmitTopic,
  ...
  async getProjectSubmitByTeamId(team_id) {
    const { C4caTeamProjectSubmitSolution } = this.server.models();
    try {
      const totalSubmitProject = await C4caTeamProjectSubmitSolution.query()
        .where('team_id', team_id)
        .resultSize();
      const project = await C4caTeamProjectSubmitSolution.query()
        .throwIfNotFound()
        .where('team_id', team_id);
        const responce ={
          totalSubmitProject,
