...
module.exports = class C4caService extends Schmervice.Service {
  ...
  async getProjectTopicCount(){
    const { C4caTeamProjectTopic } = this.server.models();
    try {
      const count = await C4caTeamProjectTopic.query().resultSize();
      return [null, count];
    } catch (error) {
      logger.error(JSON.stringify(error));
      return [errorHandler(error), null];
  ...
  async getProjectSubmitCount(){
    const { C4caTeamProjectSubmitSolution } = this.server.models();
    try {
      const count = await C4caTeamProjectSubmitSolution.query().resultSize();
      return [null, count];
    } catch (error) {
      logger.error(JSON.stringify(error));
      return [errorHandler(error), null];
