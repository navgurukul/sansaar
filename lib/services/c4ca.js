...
module.exports = class C4caService extends Schmervice.Service {
  ...
  async getTeachersByFacilitator(facilitator_id, page, limit = 10) {
    ...
    try {
      ...
      const partnerDetails = await C4caPartners.query().where('id', facilitatorDetails.c4ca_partner_id);
      let partner_id = {'partner_id': partnerDetails[0].id}
      let partner_name = { 'partner_name': partnerDetails[0].name}
      let facilitator_details = { 'facilitator_name': facilitatorDetails.name,  'teacher_link': facilitatorDetails.web_link }
      ...
      let combinedData = {...partner_name, ...partner_id, ...facilitator_details,  teachersDetails: totalTeachersDetails}
      return [null, combinedData];
    } catch (error) {
      logger.error(JSON.stringify(error));
      return [errorHandler(error), null];
