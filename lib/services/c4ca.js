...
module.exports = class C4caService extends Schmervice.Service {
  ...
  async getTeamsByDistrict(district, school, page, limit) {
    const { C4caTeams, C4caTeachers } = this.server.models();
    try {
      let Teachers;
      if (district !== undefined && school !== undefined) {
        Teachers = await C4caTeachers.query()
          .throwIfNotFound()
      ...
      for (const teacher of Teachers) {
        const c4caTeams = await C4caTeams.query().where('teacher_id', teacher.id).withGraphFetched({
          pathway_completion_relationship: true,
        });
        teacher.teams = c4caTeams;
        alldATA.push(teacher);
      }
      alldATA.sort((a, b) => {
        const maxPercentageA = Math.max(
          ...a.teams.map((team) =>
            team.pathway_completion_relationship.length > 0
              ? team.pathway_completion_relationship[0].percentage
              : 0
          )
        );
        const maxPercentageB = Math.max(
          ...b.teams.map((team) =>
            team.pathway_completion_relationship.length > 0
              ? team.pathway_completion_relationship[0].percentage
              : 0
      ...
      const output = alldATA.map((item) => {
        const team = item.teams[0];
        const complate_portions = team ? team.pathway_completion_relationship[0]?.percentage : null;
        if (team) {
          delete team.pathway_completion_relationship;
          team.complate_portions = complate_portions;
        }
