...
module.exports = class C4caService extends Schmervice.Service {
  async setTeacherProfile(profile_obj, data, user_id, c4ca_partner_id) {
    const { C4caTeachers, User, C4caRole } = this.server.models();
    try {
      ...
      await C4caRole.query().insert({ role: 'c4caTeacher', user_id });
  ...
  async getTeamsWithTopicByTeacherId(teachers_id, pathway_id = 18) {
    const {
      pathwayServiceV2
    } = this.server.services();


    const { C4caTeams, PathwayCompletionV2, C4caTeamProjectTopic, C4caTeamProjectSubmitSolution, C4caStudents } = this.server.models();
    try {
      const teams = await C4caTeams.query().where('teacher_id', teachers_id);
      ...
      if (teams !== null && teams !== undefined && teams.length > 0) {
        for (const team of teams) {
          ...
          if (pathway !== null && pathway !== undefined && pathway.length > 0) {
            ...
            const Project_topic_submission = await C4caTeamProjectTopic.query()
              .where('team_id', team.id);

            if (Project_topic_submission !== null && Project_topic_submission !== undefined && Project_topic_submission.length > 0) {
              team.Project_topic_submission = Project_topic_submission
            }


            const projectSolution = await C4caTeamProjectSubmitSolution.query()
              .where('team_id', team.id);


            if (projectSolution !== null && projectSolution !== undefined && projectSolution.length > 0) {
              team.Project_solution_submission = projectSolution
            }

            completed_portion = pathway[0].percentage;

            const [err, currentTopic] = await pathwayServiceV2.findPathwaysOngoingTopicByPathwayId(null, team.id, pathway_id);
  ...
  async updateTeamById(data, team_id) {
    ...
    try {
      ...
      const deleteStudent = await C4caStudents.query().where('team_id', team_id).del();
      ...
      return [null, { msg: 'Data Updated Successfully', data: resp }];
    } catch (error) {
      logger.error(JSON.stringify(error));
      return [errorHandler(error), null];
    }
  ...
  async uploadProjectTopic(file, team_id, project_title, project_summary, is_submitted, module_id) {
    ...
    try {
      const existingProject = await C4caTeamProjectTopic.query().where('team_id', team_id).andWhere('module_id', module_id).first();
      ...
      if (store !== null && store !== undefined && store.length > 0) {
        ...
        const pathway_id = modulePathwayDetails?.data?.attributes?.pathway?.data.id
        let jsonDetails = {}
  ...
  async uploadProjectSubmit(file, team_id, project_link, is_submitted, module_id) {
    ...
    try {
      const existingProject = await C4caTeamProjectSubmitSolution.query()
        .where('team_id', team_id).andWhere('module_id', module_id)
      ...
      const store = await C4caTeamProjectSubmitSolution.query().where('team_id', team_id).andWhere('module_id', module_id).update({
  ...
  async updateFacilitatorById(data, id) {
    ...
    try {
      ...
      if (oldData.email !== data.email) {
        ...
        if (point_of_contact_oldData.c4ca_partner_id != null && point_of_contact_oldData.c4ca_facilitator_id != null) {
          await User.query().throwIfNotFound().patchAndFetchById(oldUserID, {'c4ca_partner_id':null, "c4ca_facilitator_id": null});
      ...
      if (point_of_contact_data !== null && point_of_contact_data !== undefined) {
        ...
        let { c4ca_partner_id, c4ca_facilitator_id } = point_of_contact_data;
        ...
        let rolePrt = await C4caRole.query().where('user_id', userID).andWhere('role', 'facilitator');
        if (rolePrt.length === 0) {
  ...
  async getFacilitatorByPartnerId(partner_id) {
    const { Facilitator, C4caTeachers, C4caTeams, C4caStudents, C4caPartners } = this.server.models();
    try {
      ...
      let partner_name = {'partner_name': partnerDetails[0].name}
      ...
      let combinedData = {...partner_name, facilitatorsDetails: totalFacilitatorsDetails}
  ...
  async getTeamsByDistrictOrSchool(district, school, pathway_id = 18) {
  ...
  async getTeachersByFacilitator(facilitator_id, page, limit = 10) {
    const { C4caTeachers, C4caTeams, C4caStudents, Facilitator, C4caPartners } = this.server.models();
    try {
      const teachers = await C4caTeachers.query()
        .throwIfNotFound()
        .where('facilitator_id', facilitator_id)
      ...
      const facilitatorDetails = await Facilitator.query().where('id', facilitator_id);
      const partnerDetails = await C4caPartners.query().where('id', facilitatorDetails[0].c4ca_partner_id);
      let partner_id = {'partner_id': partnerDetails[0].id}
      let partner_name = { 'partner_name': partnerDetails[0].name }
      let facilitator_name = { 'facilitator_name': facilitatorDetails[0].name }
      ...
      let combinedData = {...partner_name, ...partner_id, ...facilitator_name, teachersDetails: totalTeachersDetails}
      return [null, combinedData];
    } catch (error) {
      logger.error(JSON.stringify(error));
      return [errorHandler(error), null];
  ...
  async getProjectTopicByTeamId(team_id, module_id) {
    ...
    try {
      ...
      let project = await C4caTeamProjectTopic.query()
        .whereIn('team_id', arr).andWhere("module_id", module_id);
  ...
  async shareProjectTopicTime(team_id, module_id) {
    ...
    try {
      ...
      const existingRecord = await C4caTeamProjectTopic.query().where('team_id', team_id).andWhere("module_id", module_id).first();
  ...
  async getAllAdminPageData(partner_id) {
    ...
    try {
      ...
      if (partner_id) {
        ...
        let dataNotFound = { totalNoOfStundents, totalNoOfTeams, totalProjectsSubmitByTeams }
        const facilitatorDetails = await Facilitator.query().throwIfNotFound().where('c4ca_partner_id', partner_id);
  ...
  async getTeacherById(teachers_id) {
    ...
    try {
      let combinedData = {};
      let teachersDetails = await C4caTeachers.query().where('id', teachers_id);
      if (teachersDetails.length > 0) {
        let facilitatorDetails = await Facilitator.query().where('id', teachersDetails[0].facilitator_id);
        if (facilitatorDetails.length > 0) {
          let partnerDetails = await C4caPartners.query().where('id', facilitatorDetails[0].c4ca_partner_id);
          if (partnerDetails.length > 0) {
            let partner_name = { 'partner_name': partnerDetails[0].name }
            let facilitator_name = { 'facilitator_name': facilitatorDetails[0].name }
            let teacher_name = { 'teacher_name': teachersDetails[0].name }
            combinedData = { ...partner_name, ...facilitator_name, ...teacher_name }
            return [null, combinedData];
          }
          else {
            let partner_name = { 'partner_name': "Partner not found" }
            let facilitator_name = { 'facilitator_name': facilitatorDetails[0].name }
            let teacher_name = { 'teacher_name': teachersDetails[0].name }
            combinedData = { ...partner_name, ...facilitator_name, ...teacher_name }
            return [null, combinedData];
          ...
          let partner_name = { 'partner_name': "Partner not found" }
          let facilitator_name = { 'facilitator_name': "Facilitator not found" }
          let teacher_name = { 'teacher_name': teachersDetails[0].name }
          combinedData = { ...partner_name, ...facilitator_name, ...teacher_name }
          return [null, combinedData];
        }
        ...
        let partner_name = { 'partner_name': "Partner not found" }
        let facilitator_name = { 'facilitator_name': "Facilitator not found" }
        let teacher_name = { 'teacher_name': "Teacher not found" }
        combinedData = { ...partner_name, ...facilitator_name, ...teacher_name }
        return [null, combinedData];
      }
    } catch (error) {
