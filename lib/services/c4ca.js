...
module.exports = class C4caService extends Schmervice.Service {
  ...
  async getTeamsByDistrict(district) {
    const { C4caTeams, C4caTeachers } = this.server.models();
    try {
      let Teachers = await C4caTeachers.query().throwIfNotFound()
      .where('district', district)
      let alldATA = []
      for (const teacher of Teachers) {
        const c4caTeams = await C4caTeams.query()
        .where('teacher_id', teacher.id)
        .withGraphFetched({
          'module_completion_relationship': true,
          });
        teacher.teams = c4caTeams;
        alldATA.push(teacher);
      }
      alldATA.sort((a, b) => {
        const maxPercentageA = Math.max(...a.teams.map((team) => (team.module_completion_relationship.length > 0 ? team.module_completion_relationship[0].percentage : 0)));
        const maxPercentageB = Math.max(...b.teams.map((team) => (team.module_completion_relationship.length > 0 ? team.module_completion_relationship[0].percentage : 0)));
