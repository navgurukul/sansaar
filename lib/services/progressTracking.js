...
module.exports = class ProgressTrackingService extends Schmervice.Service {
  ...
  async getProgressTrackPercentage(userId, courseId) {
    const { LearningTrackStatusOutcome, assessmentOutcome, ExerciseCompletionV2 } =
      this.server.models();
  ...
  async getProgressInPercent(userId, courseId) {
    ...
    try {
      ...
      const attemptedAssess = await assessmentsHistory
        .query()
        .where('user_id', userId)
        .andWhere('course_id', courseId);
      const attemptedExer = await exercisessHistory
        .query()
        .where('user_id', userId)
        .andWhere('course_id', courseId);
