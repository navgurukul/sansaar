...
module.exports = class ProgressTrackingService extends Schmervice.Service {
  ...
  async createLearningTrackStatus(details) {
    const { LearningTrackStatus, ExercisesV2 } = this.server.models();
    try {
      const data = await ExercisesV2.query().where('id', details.exercise_id).andWhere('course_id', details.course_id);
  ...
  async getLearningTrackStatus(userId, courseId) {
    ...
    try {
      ...
      for (const assessmentData of assessment) {
        // eslint-disable-next-line
        const completeAssessment = await assessmentResult
          .query()
          .where('assessment_id', assessmentData.id)
          .andWhere('user_id', userId);        
        if (
          completeAssessment.length > 0 &&
          completeAssessment !== null &&
          completeAssessment !== undefined
        ) {
          if (completeAssessment[0].status == 'Pass'){
            assessmentContent.push(assessmentData.id);
          }
          else if(completeAssessment[0].attempt_count == 2){
            assessmentContent.push(assessmentData.id);
          }
        }
      }
  ...
  async removeDublicateDataInLearningTrackStatus() {
    const { LearningTrackStatus, ExercisesV2 } = this.server.models();
    try {
      const lrningTrkData = await LearningTrackStatus.query();
      for (let ind = 0; ind < lrningTrkData.length; ind++) {
        ...
        if (ExerciseData.length <= 0 || ExerciseData === null || ExerciseData === undefined) {
          const deleteData = await LearningTrackStatus.query()
            .delete()
