...
module.exports = class ProgressTrackingService extends Schmervice.Service {
  ...
  async addLearningTrack(details) {
    const { LearningTrackStatusOutcome } = this.server.models();
    const { exercisesServiceV2 } = this.server.services();
    try {
      const user_team = details.team_id
  ...
  // new
  async getLearningTrack(userId, courseId, teamId) {
    ...
    try {
      ...
      const url = `${process.env.STRAPI_URL}/courses/${courseId}?populate=pathway&populate=slugs.exercises&populate=slugs.assessments`;
      ...
      const exerciseSlugs = [];
      const assessmentSlugs = await data.attributes.slugs.data.map((ele) => {
        if (data.attributes.type === 'assessment') {
          return {
            id: ele.id,
          };
        }
      ...
      console.log('assessmentSlugs', assessmentSlugs);
      console.log('exerciseSlugs', exerciseSlugs);

      // const exerciseSlugs = await data.attributes.exercises.data.map((ele) => {
      //   return {
      //     id: ele.id,
      //   };
      // });
      ...
      for (const slug of exerciseSlugs) {
        ...
        const learningTrackStatus =
          teamId !== null && teamId !== undefined
            ? await exercisessHistory.query().where('slug_id', slug.id).andWhere('team_id', teamId)
            : await exercisessHistory.query().where('slug_id', slug.id).andWhere('user_id', userId);
      ...
      for (const slug of assessmentSlugs) {
        ...
        const learningTrackStatus =
          teamId !== null && teamId !== undefined
            ? await assessmentsHistory.query().where('slug_id', slug.id).andWhere('team_id', teamId)
