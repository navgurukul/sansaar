...
module.exports = class ProgressTrackingService extends Schmervice.Service {
  ...
  async getProgressTrackPercentage(userId, courseId) {
    ...
    try {
      ...
      const slugs = data.attributes.slugs.data;
      const exercises = [];
      const assessment = [];
      // eslint-disable-next-line
      for (const slug of slugs) {
        // eslint-disable-next-line
        if (slug.attributes.type === 'assessment') {
          const assessmentlearningTrackStatus = await assessmentsHistory
            .query().where('slug_id', slug.id)
            .andWhere('user_id', userId);
          console.log(assessmentlearningTrackStatus);
          attemptedData.push(...assessmentlearningTrackStatus);
        }
        if (slug.attributes.type === 'exercise') {
          const exercisesDatalearningTrackStatus = await exercisessHistory
            .query()
