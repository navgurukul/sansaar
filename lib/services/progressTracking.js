...
module.exports = class ProgressTrackingService extends Schmervice.Service {
  ...
  async getLearningTrack(userId, courseId, teamId) {
    ...
    try {
      ...
      const assessmentSlugs = await data.data.attributes.slugs.data.map((ele) => {
        if (ele.attributes.type === 'assessment') {
          return {
            slug: ele,
          };
        }
      ...
      for (const slug of exerciseSlugs) {
        ...
        if (slug !== undefined && slug !== null) {
          const learningTrackStatus = teamId
            ? await exercisessHistory
                .query()
                .where('slug_id', slug.slug.id)
                .skipUndefined()
                .andWhere('team_id', teamId)
            : await exercisessHistory
                .query()
          ...
          if (
            ...
            exerciseContent.push(learningTrackStatus[0].slug_id);
          }
        }
      }
      ...
      for (const slug of assessmentSlugs) {
        ...
        if (slug !== undefined && slug !== null) {
          ...
          if (
            ...
            if (learningTrackStatus[0].status === 'Pass') {
              assessmentContent.push(learningTrackStatus[0].slug_id);
            } else if (learningTrackStatus[0].attempt_count === 2) {
              assessmentContent.push(learningTrackStatus[0].slug_id);
            }
          }
