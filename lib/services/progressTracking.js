...
module.exports = class ProgressTrackingService extends Schmervice.Service {
  ...
  const { PathwayFormStructure } = this.server.models();
    const formStructure = await PathwayFormStructure.query(txn).where({ pathway_id: pathwayId }, txn);

    const existingQuestionIds = _.fromPairs(formStructure.map(s => [s.question_id, s]).filter(s => s[0]));
    const existingParamIds = _.fromPairs(formStructure.map(s => [s.parameter_id, s]).filter(s => s[0]));
