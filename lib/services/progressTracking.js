...
module.exports = class ProgressTrackingService extends Schmervice.Service {
  ...
  async getProgressTrackPercentage(userId, courseId) {
    ...
    try {
      ...
      // const exercises = await  strapi.findOne('courses', course_id, {
      //   populate: ['exercises'],
      //   pagination: {
      //     start: 0,
      //     limit: -1,
      //   },
      // })
      // console.log(exercises,"exercisesd");
      // const assessment = await Assessment.query().where('course_id', courseId);
      const { data } = await strapi.find('assessments', {
        sort: 'createdAt:asc',
        pagination: { start, limit },
      });
      let assessment = data.forEach((dic)=>{
        console.log({id :dic.id ,...dic.attributes});
        return {id :dic.id ,...dic.attributes}
      })
